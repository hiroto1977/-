...
2. **Browser standalone** (`npm run build:web` → `dist/standalone.html`) — a single self-contained HTML
   file (実測 11.21 MiB full / 3.02 MiB `build:web:lite` mobile variant — 2026-09-11 計測: 11,752,227 B / 3,164,973 B。天井は CI が両方に掛けている: 16 MB / 4 MB、85% で警告。前回 2026-09-07 は 11,635,770 B / 3,048,518 B で、4 日で両方 +約 116 KB) that runs in any browser with no Node/Electron. See `docs/BROWSER_REDESIGN.md`.
