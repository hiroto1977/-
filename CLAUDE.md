...
npm run e2e              # Playwright 実機 E2E (desktop/phone/tablet)。e2e:lite で LITE 版を検証
                         #   32 suite に**suite ごとの床** (実測の 85%) と全 suite 時の合計の床 376 (パス 303。
                         #   パス 322 で shell suite 27 件を足し 350 → 370、パス 323 で kessanTax +7 → 376 = 実測 443 の 85%)。
