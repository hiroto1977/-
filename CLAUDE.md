...
A verified academic knowledge base lives in `src/renderer/data/academicKnowledge.ts`
(`VERIFIED_CONCEPTS`, ≥2 sources / ≥1 authoritative each). It is the single source of truth for an
**Obsidian knowledge vault** (`knowledge-vault/`, generated by `npm run vault:build`) and is injected
as context into the AI-orchestration runtime per executive role (`orchestration/knowledge-map.json`,
`npm run orchestrate:context` / dispatch). `vault:check` (in `verify:all`/CI) enforces vault sync and
forbids duplicate ids. See `docs/KNOWLEDGE_VAULT.md`.
