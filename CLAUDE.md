...
2. **Browser standalone** (`npm run build:web` → `dist/standalone.html`) — a single self-contained HTML
   file (実測 11.22 MiB full / 3.03 MiB `build:web:lite` mobile variant — 2026-09-12 パス 183 後の計測: 11,775,895 B / 3,188,643 B。天井は CI が両方に掛けている: 16 MB / 4 MB、85% で警告。同日パス 182 後は 11,774,254 B / 3,187,002 B・パス 181 後は 11,765,766 B / 3,178,512 B・前日 2026-09-11 は 11,752,227 B / 3,164,973 B。LITE は 85% の警告線 3.4 MB まで残り約 211 KB) that runs in any browser with no Node/Electron. See `docs/BROWSER_REDESIGN.md`.
