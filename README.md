...
| **`/v19/ui/`** | v19 ダッシュボード — **8 ルート 統合** UI (overview / integrations / **integration-claude** / **orchestrate** / **governance** / **journal** (業務) / audit / settings)。3 AI プロバイダ + ローカル専用モード + 感情適応モード + ブラウザ audit + ストレージメーター + 業務 引継ぎ Free 可視化 |
| **`/cowork/`** | ローカル AI と Chat する最短ルート（CLI + 自動起動スクリプト） |
| **`/governance/`** | ローカルファースト業務運用のガバナンス文書群 16 本（法制度・データ分類・運用・士業別ルール・**統合システム設計図 (37 反復)**・**チーム オーケストレーション (4 × 4)**・**ブートストラップ知識**・**感情倫理ガード** 他） |
