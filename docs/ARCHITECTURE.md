...
> 自己検証: `npm run verify:arch` で 649 個の `file:line` 参照 + 41 個のライブメトリクスが
...
| ユニットテスト | **14771** | `npm test` (静的 `it(` 数; `it.each` / テンプレート for ループ展開で実行時はさらに増える) |
...
Stryker の対象 (`stryker.config.json` の `mutate`) は **294 ファイル**。
