...
| 軸 | 値 | 出典 |
|---|---:|---|
| サービス数 | 18 | `src/shared/serviceId.ts:9-29` |
| IPC ハンドラ数 | 9 | `src/main/main.ts:99-224` |
| client モジュール (fetcher + actions) | 18 | `src/main/clients/index.ts:21-69` |
| OAuth 対応サービス | 3 (drive / calendar / gmail) | `src/main/oauth.ts:54-85` |
| 外部接続先ホスト | 12 + ローカル 1 | §4.3 |
| ユニットテスト | **913** | `npm test` (静的 `it(` 数; `it.each(seeds)` の 5×5 展開で実行時は 938) |
...
### 3.1 18 services の認証スタイル

`src/shared/serviceId.ts:9-29` の `SERVICE_IDS` が **single source of truth**。
