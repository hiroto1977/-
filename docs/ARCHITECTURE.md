...
| ユニットテスト | **671** | `npm test` (静的 `it(` 数; `it.each(seeds)` の 5×5 展開で実行時は 696) |
| Mutation score (total) | **100.00%** | `docs/QUALITY.md` |
| Mutation score (covered) | **100.00%** | `docs/QUALITY.md` |
| Stryker break threshold | **99.8%** (CI fails below — every mutant killed across all 11 files including 5 stocks actions) | `stryker.config.json` |
...
| `stocks` | Stocks (local mock) | Bearer (Anthropic, advisor のみ) | ✅ | | `register-ticker`, `backtest`, `compare-strategies`, `advise`, `export-dashboard` (Phase 7 で broker 接続) |
