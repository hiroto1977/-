...
| ユニットテスト | **10574** | `npm test` (静的 `it(` 数; `it.each` / テンプレート for ループ展開で実行時はさらに増える) |
...
Stryker の対象 (`stryker.config.json` の `mutate`) は **257 ファイル**。
