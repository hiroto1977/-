...
| ユニットテスト | **14291** | `npm test` (静的 `it(` 数; `it.each` / テンプレート for ループ展開で実行時はさらに増える) |
...
`ok:false` の `message` は **必ず** `safeErrorMessage()` (`src/shared/redact.ts:445-447`。
...
| microsoft-365 | `send-mail` | `{ to, subject, body? }` | **共有台帳 `MS365_MAIL_FIELDS`** を共有の `checkMail` が読む (型・長さ) + Graph message envelope も共有 (`graphMailRequest`)。宣言は `SendMailPayload`、欄の名前のずれは検査が留める | `microsoft-365.ts:17-269` |
...
Stryker の対象 (`stryker.config.json` の `mutate`) は **287 ファイル**。
