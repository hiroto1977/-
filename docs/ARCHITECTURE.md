...
ダッシュボード。59 のサービス (Home / 事業ダッシュボード / チームレーダー /
...
| 軸 | 値 | 出典 |
|---|---:|---|
| サービス数 | 59 | `src/shared/serviceId.ts:9-43` |
| IPC ハンドラ数 | 11 | `src/main/main.ts:99-251` |
| client モジュール (fetcher + actions) | 59 | `src/main/clients/index.ts:44-83` |
| OAuth 対応サービス | 3 (drive / calendar / gmail) | `src/main/oauth.ts:54-85` |
| 外部接続先ホスト | 12 + ローカル 1 | §4.3 |
| ユニットテスト | **1428** | `npm test` (静的 `it(` 数; `it.each(seeds)` の 5×5 展開で実行時は 1477) |
...
flowchart LR
  ...
  subgraph ELE["Electron app (single OS process tree)"]
    ...
    subgraph RND["Renderer (sandboxed, contextIsolated, CSP)"]
      PAGES[59 React pages<br/>+ useServiceData hook]
    end
    ...
    subgraph MN["Main (Node, full privileges)"]
      ...
      CLIENTS[59 clients<br/>fetcher + ActionMap]
