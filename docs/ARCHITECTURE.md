...
| ユニットテスト | **9628** | `npm test` (静的 `it(` 数; `it.each` / テンプレート for ループ展開で実行時はさらに増える) |
...
| 陰性対照つきゲート | 23 / 30 (残る 7 件は外部ツール 2 (`typecheck` / eslint) と、知識コーパス系 4 + `lint:repo-size`) | `package.json` |
...
ランタイムソース **≥ 400 ファイル**を **31 個の禁止パターン** で scan し、
1 件でも検出すれば fail。
