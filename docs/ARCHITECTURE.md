...
| ユニットテスト | **9753** | `npm test` (静的 `it(` 数; `it.each` / テンプレート for ループ展開で実行時はさらに増える) |
...
| 陰性対照つきゲート | 28 / 33 (残る 5 件は外部ツール 2 (`typecheck` / eslint) と、知識コーパス系 3。後者 3 つは 2026-08-25 に実物へ違反を植えて鳴ることを確認済み —— `lint:repo-size` だけは実データで失敗経路が一度も走らず、守りを外しても ✅ を返していたので陰性対照を付けた) | `package.json` |
...
`ok:false` の `message` は **必ず** `safeErrorMessage()` (`src/shared/redact.ts:202-204`。
