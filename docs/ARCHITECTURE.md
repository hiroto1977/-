...
| ユニットテスト | **5471** | `npm test` (静的 `it(` 数; `it.each` / テンプレート for ループ展開で実行時は 5556) |
| Mutation score (total) | **100.00%** | `docs/QUALITY.md` |
| Mutation score (covered) | **100.00%** | `docs/QUALITY.md` |
| Stryker break threshold | **99.8%** (CI fails below — every mutant killed across all 11 files including 6 stocks actions + equity curve + Markdown export) | `stryker.config.json` |
| `npm audit` (prod) | 0 vulnerabilities | `package-lock.json` |
| 不変条件 (CI で fail-on-violation) | 15 | §8.1 |
| `file:line` 参照数 | 179 | 自己検証 |
...
| `assistant` | AI アシスタント — Claude を頭脳に確証済みナレッジ + 全サービスを統合した専用チャット | Bearer (Anthropic, chat のみ) | ✅ | | `chat` (RAG 文脈は renderer の `data/assistantContext.ts` で構築。確証済み 5 コレクション=学術/コンプラ/補助金/相談窓口/経済史 を `data/knowledgeIndex.ts` の転置インデックスで横断検索。表/成果物は `data/assistantMarkdown.ts` で描画。未設定時は `data/chatbot.ts` の決定論エンジンへフォールバックし、オフラインでも出典つきナレッジを回答) |
