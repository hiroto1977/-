...
| ユニットテスト | **9732** | `npm test` (静的 `it(` 数; `it.each` / テンプレート for ループ展開で実行時はさらに増える) |
...
| 陰性対照つきゲート | 27 / 33 (残る 6 件は外部ツール 2 (`typecheck` / eslint) と、知識コーパス系 3 + `lint:repo-size`) | `package.json` |
