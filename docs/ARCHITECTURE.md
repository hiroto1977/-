...
| ユニットテスト | **14066** | `npm test` (静的 `it(` 数; `it.each` / テンプレート for ループ展開で実行時はさらに増える) |
...
`ok:false` の `message` は **必ず** `safeErrorMessage()` (`src/shared/redact.ts:352-354`。
...
classDiagram
  ...
  class SecretsStore~secrets.ts~ {
    ...
    +getToken(id) : secrets.ts:300
    +clearToken(id) : secrets.ts:305
    +listConfiguredServices() : secrets.ts:327
    +setOAuthTokens(id, ts) : secrets.ts:412
    +getOAuthTokens(id) : secrets.ts:416
