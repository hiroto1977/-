...
| ユニットテスト | **9266** | `npm test` (静的 `it(` 数; `it.each` / テンプレート for ループ展開で実行時はさらに増える) |
...
`ok:false` の `message` は **必ず** `safeErrorMessage()` (`src/shared/redact.ts:167-169`。
...
| skills, emotions | `api.anthropic.com` | `POST /v1/messages` | `x-api-key` | `skills.ts:262`, `emotions.ts:209` |
...
| 5 | 外部 URL を OS へ渡す扉は 2 つ (`app:openExternal` と新窓ハンドラ)、**どちらも同じ関門**を通る — http(s) 限定 | `EXTERNAL_URL_SCHEMES` `src/main/externalUrlGate.ts:50` + `src/main/__tests__/externalUrlGate.test.ts` 37 件 (扉の数を数える検査を含む) |
