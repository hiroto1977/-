...
> 自己検証: `npm run verify:arch` で 650 個の `file:line` 参照 + 42 個のライブメトリクスが
...
| ユニットテスト | **16046** | `npm test` (静的 `it(` 数; `it.each` / テンプレート for ループ展開で実行時はさらに増える) |
...
Stryker の対象 (`stryker.config.json` の `mutate`) は **298 ファイル**。
