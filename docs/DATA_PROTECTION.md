...
### この在庫**全体**に掛かる前提 — 生成元は 1 つしかない（2026-08-28 に判断）

上の 30 件は媒体こそ違うが、**すべて生成元 (origin) 単位で持たれる。パス単位ではない。**
GitHub Pages のユーザーリポジトリは `https://hiroto1977.github.io/<repo>/` へ出るので、
**このアカウントが公開する Pages サイトはすべて同じ生成元 `https://hiroto1977.github.io`
を共有する。** これは構成の都合ではなく、ブラウザの保存領域が生成元で区切られる
という仕様である。

したがって、同じ生成元に別のページが 1 枚でも増えると、そのページの JS は:

- `business-hub-vault` の**暗号文を読める**（鍵は導出されず端末にも無いので
  復号はできないが、PBKDF2 60 万回に対する総当たりの標的にはなる）
- 同じ保存領域を**上書きできる**。`meta.salt` を固定値へ差し替える形は
  本 PR の #69 で塞いだ判定の対象そのものである
- localStorage 22 キーを平文で読める
- **偽の解錠画面を出してマスターパスワードを取れる。** 生成元が同じなら
  利用者にも同一生成元ポリシーにも区別できない

**ページ側で打てる緩和策は無い。** 攻撃者が定義上「同一生成元」だからである。

**判断: 生成元の分離（独自ドメイン）は「やる」。** ただし実行できるのは
持ち主だけで、ドメインの取得と DNS 設定が要る（このセッションの
ネットワーク方針は `hiroto1977.github.io` への接続自体を拒むので、
現在ほかの Pages サイトが在るかどうかも当方からは確認できていない）。

**それまでの運用上の約束（1 行で言える）:
`hiroto1977.github.io` に、この repo 以外の Pages サイトを増やさない。**
増やした瞬間に上の 4 つが有効になり、しかも**何も鳴らない**。
この repo が同一生成元へ出す 6 枚（app / lite / landing / デモ 3 本）は
同じ供給網から出る第一者なので、この境界の内側にある。
...
### localStorage（22 キー・すべて平文・すべて立ち退きの対象・バックアップに入らない）
...
残りは配色 (アシスタントの `assistant-theme`・画面全体の `servicehub.theme` = ライト / ダーク / OS に合わせる · パス 317)・お気に入り・最近開いたサービス・プラン・招待コードの引き換え状態・
...
IndexedDB 4 つ（業務レコード / 保管庫 / ライブラリ / preferences）・Cache Storage・localStorage 22 鍵・
...
### 配る単一 HTML が残す物（アプリとは別の生成元 · 2026-09-21 · パス 364）

**上の在庫はアプリ (`app.html` / `lite.html` / デスクトップ版) の生成元の話である。**
このリポジトリはそれとは別に、**利用者がダウンロードして自分で開く単一 HTML** を配っている。
そのうち **3 本が入力を `localStorage` へ自動保存する** —— 保存先はその文書自身の生成元
(`file://` で開けば file 由来の領域) なので、**アプリの「すべてのデータを削除」はここへ届かない。**

| 配る物 | キー | 中身 | 消す口 |
|---|---|---|---|
| `電子定款メーカー.html` | `teikan-maker-values-v1` | 商号・本店所在地・**発起人の氏名と住所**・引受株数/出資額・事業目的 | 画面の「🗑 入力を消す」(`btn-clear`) |
| `就業規則メーカー.html` | `shugyokisoku-maker-values-v1` | 事業場の名称・所在地・代表者名など就業規則の差込値 | 画面の「🗑 入力を消す」(`btn-clear`) |
| `経営書類スタジオ.html` | `docs-studio-values-v1` | 12 種の書類すべての差込値（会社名・氏名・住所を含む） | 画面の「🗑 入力を消す」(`btn-clear`) |

`業務自動化ダッシュボード.html` は保存しない（0 件）。

**2026-09-21 まで、この 3 本はどの台帳にも在庫にも載っておらず、消す口も無かった。**
`lint:storage` の走査範囲は `src/renderer` だけなので 3 鍵とも母集団の外で、
`removeItem` は 3 本とも **0 件** —— 利用者はブラウザのサイトデータ設定を知らないかぎり、
**自分の氏名と住所を入れたまま**にするほか無かった。入れたのは「個人情報を扱う画面」ではなく
「書類を作る道具」なので、残っている自覚が持ちにくい側である。

自動保存そのものは残す（タブを閉じても書きかけが消えないための機能で、
docblock にも最初から「入力値は localStorage に自動保存」と書いてある）。
足したのは**消す口**と、ここに載せることである。消す口は確認文で**何が消えるかを名指しする**
（「よろしいですか」だけの確認は何も言っていないのと同じ · `replaceRestoreConfirmMessage` と同じ考え方）。

母集団と 3 本の要求は `src/shared/__tests__/distributedArtifactStorage.test.ts` が**両方向**に留める
（保存する builder が増えたら落ち、台帳から消えても落ちる。消す口を外しても落ちる）。
**この表の行もその検査が読む** —— 在庫と実物が別々に古びない。
