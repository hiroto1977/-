...
| 直近 16 往復の会話 | **利用者が選んだプロバイダ**（合議は設定済みの全部・Ollama は端末内） | アシスタント | ✅ |
...
| `~/.local/business-hub/talent.json` | 人材育成の入力（部署名・個人名） | 平文 0600 |
| `~/.local/business-hub/team-radar.json` | チームレーダー（氏名・軸ごとの評価・付箋） | 平文 0600 |
| `~/.local/business-hub/state.json` | 株式ダッシュボードの状態 | 平文 0600 |

2026-09-09 まで、この在庫は文書に無かった —— 同じ端末に置く 5 つのうち封緘していたのは `secrets.json`
だけで、健康に関わる記録が平文だった。talent / team-radar / state は平文のまま（人事の評価と氏名を含む —— **残る物**。
封緘するなら同じ `atRest.ts` を通す）。
