...
実装規模: **1225 静的 it() / 1274 実行時 tests · 45 client modules · 11 IPC handlers ·
...
| 7 | ユニットテスト | `vitest run` | 1274 件のうち 1 つでも fail |
...
**現状値**: 1225 静的 `it()` / 1274 実行時 tests passing · Stryker mutation **100.00%** ·
...
| **PR #9 + utils (現在)** | **45** | **45+** | **1225 / 1274** | **100.00%** | **99.8** | **ServiceActionPanel utils (XSS / locale)** |
...
PR #9 完了時点で:
- 45 services / 1274 tests / 100% mutation / 171 refs / 4 cross-doc facts が CI で
  自動検証
