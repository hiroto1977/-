...
実装規模: **1213 静的 it() / 1262 実行時 tests · 45 client modules · 11 IPC handlers ·
...
| 7 | ユニットテスト | `vitest run` | 1262 件のうち 1 つでも fail |
...
**現状値**: 1213 静的 `it()` / 1262 実行時 tests passing · Stryker mutation **100.00%** ·
...
| **PR #9 + utils (現在)** | **45** | **45+** | **1213 / 1262** | **100.00%** | **99.8** | **ServiceActionPanel utils (XSS / locale)** |
...
PR #9 完了時点で:
- 45 services / 1262 tests / 100% mutation / 171 refs / 4 cross-doc facts が CI で
  自動検証
