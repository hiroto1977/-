...
知識ベース（学術 3,417 / 法令実務 393 / 補助金 140 / 経済史 86 / 相談窓口 3 = 4,039 項目）を
...
| `missingAsOf` | 項目別 asOf を**持たない / 時点として成り立たない**コレクション（経済史 86 件は 2026-07 に一括付与済み → 現状 0）。**未来の月・暦に無い月もここに数える** (2026-09-06) —— `monthsSince` が負の月数を返していた頃は、`asOf: '2027-06'` が「読めない」にも「期限超過」にも当たらず**再確証キューから静かに外れて**いた。書く側 (`build-knowledge-vault.cjs` の `assertAsOfIsPast`) も未来を拒むので、通常は push の時点で気付く | データ側に asOf を一括付与する 1 回のタスク／未来の値は打ち間違いなので直す |
...
| `dedupeId` 重複疑い（id） | id の記号を落とし独語ウムラウトの二重母音を畳んだ正規化が一致するペア（人名の翻字ゆれで id が割れた重複）。裁定済みは除外 | 同上 |
| `dedupeSource` 重複疑い（第一出典） | **同一コレクション内**で第一出典の DOI が同じペア（出版社ページの URL に埋め込まれた DOI も拾う。法令・SEP・Wikipedia など DOI でない URL は多くの別概念が共有するので鍵にしない）。裁定済みは除外。2026-09 の初回走査で 179 ペア＝上の 3 系列をすり抜けていた多重重複（stakeholder-salience ×4・成人愛着 ×5 など） | 塊ごとに全文を読み、最良の 1 件へ出典と固有の記述を移して残りを削除（統合パス）。DISTINCT なら台帳へ |
| `dedupeShared` 重複疑い（出典共有） | **同一コレクション内**で任意の位置の出典 DOI を共有し、かつ（共有 DOI が 2 件以上、または id の語彙＝分野接頭辞と 2 文字以下を除いた '-' 区切りの Jaccard ≥ 0.5）のペア。第一出典が違うだけの同一概念（時間的非整合性 ×2・センスメイキング ×4・快楽適応 ×2 など）は `dedupeSource` を素通りしていた。共有 DOI 1 件で id が別物のペア（別概念が同じ古典を引いているだけ）は鳴らさない。裁定済みは除外。2026-09-05 の初回走査で 22 ペア | 同上（塊ごとに統合、DISTINCT なら台帳へ） |
| `missingKeyFigures` 提唱者・初出の欠落 | 学術項目で `keyFigures`（提唱者・初出）が無い／空の物。欠落すると概念表 docs/ACADEMIC_KNOWLEDGE.md の「提唱者・初出」列が黙って空になるのに、増強キューは本文の長さしか見ない。2026-09-05 の初回走査は **0 件**（生ファイルの正規表現で 210 件と誤認したのが導入の動機。正規化後に数えると欠落は無かった） | 本文に書かれている人名・年から起こす（新しい事実は足さない） |
...
npm run knowledge:auto -- --links=400         # 出典 URL 死活も（CI 想定 — 生ネットワークが必要）
...
- 実行内容: `knowledge:auto --ci --links=400` → **派生成果物ドリフトなし証明**（再生成後に
  `git diff --exit-code` — 決定論が壊れたら fail）→ キューをアーティファクト保存（90 日）→
  常設 Issue「📚 知識オートパイロット報告（自動更新）」を作成/更新（作業ゼロなら自動クローズ）。
- 出典 URL 死活は**週替わりシャード**（通し週番号 × 400 件のステートレス・ローテーション）で、
  出典 12,229 件を **31 週で一巡**する。
  **2026-08 まで壊れていた**: 起点に ISO 週番号（1..53）を使っていたため毎年 1 に戻り、
  同じ 53 個の窓が永久に繰り返されていた。実測すると検査対象になり得るのは
  **12,229 件中 5,300 件（43%）だけで、6,929 件は一度も検査されない**。
  「リンク切れ 0 件」はその大半について**偽の安全信号**だった。
  固定起点からの通し週に変えて毎週前進するようにし、`src/shared/__tests__/linkShard.test.ts`
  で「一巡すれば全件を覆う」ことと「1..53 で循環させると 43% しか覆えない」ことの両方を固定した。
