...
| **CVE-2025-66960** | GGUF v1 の文字列長で panic (`readGGUFV1String`)。0.12.10 で報告 | DoS | **修正版未公表** (GHSA-jr3x-q8gx-4gw3・2026-01-21) |
| **CVE-2026-7482** ("Bleeding Llama") | `/api/create` に細工した GGUF → 量子化でヒープ範囲外読み取り → プロセスメモリ (鍵・会話) の漏洩 | 情報漏洩 (CVSS 8.8〜9.1) | **0.17.1** 以降 (PR #14406・2026-02-25 merge。GHSA-x8qc-fggm-mpqg・2026-05-04 公表) |
| **CVE-2026-86289** | GGUF の文字列長の整数オーバーフロー (`readGGUFV1String`)。0.31.1 まで | 低 | **0.31.2** 以降 (GHSA-c2q9-58w2-gjg4・2026-09-07) |

→ **Ollama 0.31.2 以降 (推奨: 最新安定版)** を使ってください。
台帳 (`src/shared/ollama.ts` の `OLLAMA_ADVISORIES`・8 件) は **2026-09-09 時点** (再照合期限 2027-03-09 ——
`lint:rate-freshness` が 60 日前から警告し、過ぎたら CI が落ちる)。Service Hub はバージョンを取得し、**当てはまる CVE を名指しして**警告します
(床と照合日は `lint:docs` がこの文書とコードで照合する —— 2026-09-09 まではこの文書に 0.1.46 が 4 か所手で
写されていた)。
...
**2026-09-09 の訂正 (パス 139)**: この節と画面の固定文は 2026-05-12 から「ベンダー報告済み・公式パッチ未公開」と
書いていた。実際にはこの脆弱性は **CVE-2026-7482** (GHSA-x8qc-fggm-mpqg) として 2026-05-04 に公表され、修正
(PR #14406 "ggml: ensure tensor size is valid") は **2026-02-25 に merge され 0.17.1 に入っていた** —— 固定文を
書いた時点で既に「未パッチ」ではなかった。日付の無い主張は、正しかった日から黙って嘘になる。いまは台帳
(`OLLAMA_ADVISORIES`) が照合日と再照合期限を持ち、画面の注意はその日付を刷る。

**モデル / エンジンファイル形状の不備による Out-of-Bounds Read** (当時の記述・以下は歴史):

- **状態**: (2026-05-12 時点の記述) ベンダー (Ollama) へ報告済み、公式パッチ未公開 → **誤り。0.17.1 で修正済み**
...
**Service Hub の実装はこの (当時「未パッチ」と信じていた) 脆弱性への対応として、危険な書き込みエンドポイントを
1 つも呼ばない設計**になっています (修正済みと分かった今も、GGUF 経由の攻撃面を持ち込まない設計は変えない)。アプリ内からモデルダウンロードはできず、ユーザが CLI
...
さらに、Ollama が起動して接続できる毎リクエストで台帳の注意 (`advisoryLedgerNotice` —— 照合日・再照合期限・
修正版未公表の件数・当てはまる CVE の名指し) を snapshot の `warnings[]` に追加し、UI のステータスバーで継続的に
ユーザへ注意喚起します (運用上 CLI 経由でモデルを取得する際に「検証済みソースのみ」を選ぶよう誘導)。
2026-09-09 までここは日付の無い固定文 `UNPATCHED_OOB_NOTICE` だった。
...
### 配信元 CSP による遮断 (claude.ai アーティファクト等)

**ホスト側の CSP が `connect-src 'self'` の場合、ローカルへの接続は原理的に不可能**。
claude.ai のアーティファクトとして配信した場合がこれに当たる (実測: シェル側の
CSP は `connect-src 'self' https://<id>.frame.claudeusercontent.com` のみ)。
ブラウザがリクエストを送る前に落とすため、**Ollama 側を何度設定しても解決しない**。

厄介なのは、この失敗が「Ollama が起動していない」ときと **区別がつかない** こと ——
通常 fetch も no-cors フォールバックも同じように失敗する。素直に判定すると
`not-running` と誤診し、利用者を「Ollama を入れて起動する」という絶対に終わらない
作業へ送り込む。そこで `securitypolicyviolation` イベントを監視し、違反が観測された
場合は `csp-blocked` として原因と**代替経路**を出す (`createCspWatcher`)。

罠: 違反イベントは **タスク** として配送されるため、fetch の reject 直後
(マイクロタスク連鎖の中) に読むとまだ届いていない。実ブラウザでこの誤診を踏んだので、
一度タスクキューへ譲ってから読む (`settledCspHit` / `CSP_SETTLE_MS`)。

代替経路は 3 つ。**いちばん簡単なのは 2 番目** —— Ollama は既定で
`http://localhost:*` / `http://127.0.0.1:*` からの読み取りを許可しているため、
アプリ自身を localhost から配信すれば `OLLAMA_ORIGINS` の設定すら要らない。

1. `npm run ollama` / `npm run ollama:setup` (ブラウザを通らない)
2. アプリを localhost から配信して開く (**許可設定も不要**)
3. Electron 版 (main プロセスから直叩き)

**案内は段階に応じて出し分ける**。「つながらない」原因は 1 つではなく、①未導入
②未起動 ③モデルが無い ④読み取り未許可 —— のどれか。診断で分かっている段を飛ばして
長い手順を読ませると、どこを直せばいいのか分からなくなる。そこで
`setupCommands()` (導入からの通し手順) と `originsSetupSteps()` (許可設定だけ) を
持ち、probe が `cors-blocked` を返したときだけ後者に絞る。

`npm run ollama:setup` (`scripts/ollama-setup.sh`) は同じ判定をターミナル側で行い、
**足りない段だけを埋める**。最後に必ず `/api/chat` を 1 往復させる —— バージョンが
読めるだけでは「使える」と言えない (モデルが無ければ生成は失敗する) ため。
`--check` は何も変更せず現状だけ報告し、`--origin <URL>` はブラウザ版から使う場合の
永続化手順を OS 別に出す (実行は利用者に委ねる — systemd / launchctl への書き込みを
黙って行わない)。
...
2. 台帳 (`OLLAMA_ADVISORIES`) の各修正版と比較し、当てはまる CVE を名指しする (床は `0.31.2`)
3. 床未満なら **警告バッジ「Outdated — known CVEs」** をステータスバーに表示。台帳の注意 (照合日・再照合期限) は常に表示
...
更新後 `ollama --version` で 0.31.2 以上を確認してください。
