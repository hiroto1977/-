...
分母の範囲: `stryker.config.json` の `mutate` が名指しする **295 本**。`src/` の `.ts` (検査と `.d.ts` を除く) は **413 本**で、**118 本は分母の外**に在る (学術コーパスなどの定数表を含む。範囲の台帳は `src/shared/__tests__/mutateScopeCensus.test.ts`)
