...
| `managementReport.ts` | 7 | **0/7** —— このファイルの検査には `vi.resetModules()` が **1 つも無く**、静的変異体を観測できる検査が構造的に存在しなかった |
...
**測れていないもの (正直に書く)**: Stryker は 1 度も回していないので、
**スコアは主張しない**。主張できるのは (a) 母集団の数え上げ、(b) 各変異を当てたとき
狙った検査が鳴ること、(c) 変異が当たったのに検査が鳴らない状態が無いこと、の 3 つである。
静的変異体が「届いていなかった」ことの根拠は**構造** (読み直しの経路がファイルに 1 つも無い)
であって計測ではない。
