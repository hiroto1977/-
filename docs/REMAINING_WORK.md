...
## 変異検査で測っていない範囲 (2026-08-18 実測・724 行)

`npm run lint:mutation-scope` の台帳 `KNOWN_BROAD` に載っている **4 ファイル
/ 6 箇所 / 724 行**（着手前は 36 ファイル / 46 箇所 / 5,189 行）。これは「許した」ではなく「まだ測っていないと
...
### 残り 3 ファイルの実測値 (2026-08-18・無効化を外して測った)
...
残っているのは **3 つだけ** — `library` 217 / `business` 199 / `stocks` 169
(ほかに `templates` の座標算術 139 行)。**次は `library.ts` (68.4%)** が低い。
