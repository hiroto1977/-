...
- **ハードリセットの「復旧不可な形で消去されます」は、消えなくても同じ顔をする**
  (2026-09-07 実測・**直していない** → 次のパスの持ち物)。施錠の伝播を入れる
  ...
  直すときの要点: (1) `wipeAndReset` は成否を**返す** (今は成功も失敗も
  `resolve()`)、(2) 呼び出し側は失敗を画面に出し、**reload しない**
  (reload すると診断も消える)、(3) 施錠の伝播 (`lockEverywhere`) を先に流せば
  他のタブは鍵を落とすが、**接続を掴んでいる可能性は残る** ——
  この repo の `openDb()` は操作ごとに開いて `finally` で閉じるので常時は
  掴まないが、書き込み中に重なれば `onblocked` は起こりうる。
