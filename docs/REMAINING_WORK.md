...
## 変異検査で測っていない範囲 (2026-08-19 実測・308 行)

`npm run lint:mutation-scope` の台帳 `KNOWN_BROAD` に載っている **2 ファイル
/ 2 箇所 / 308 行**（着手前は 36 ファイル / 46 箇所 / 5,189 行 = **94% 消化**）。これは「許した」ではなく「まだ測っていないと
