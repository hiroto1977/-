...
**上限方針が無いのが本題。** `perf` ゲートは起動時の巨大 JSON.parse
(`bigParses`) を見ているだけで、**バンドルサイズには上限が無い**。
知識データは今後も増えるので、LITE がモバイルで開けなくなる方向へ
単調に育つ。閾値を決めて `perf` かCI に上限チェックを入れるべきだが、
「いくつなら許容か」は実機での体感が要るので未着手。
