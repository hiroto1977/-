...
## パス 209 の候補 (2026-09-13 · 測定済み・未修正) — **⛔ の「マイナスの値」から判定が出る。しかも 4 件は「より good な方向」へ動く**

パス 206〜208 で閉じたのは `max`（上限）の家系である。`guardNumber` が `fatal` を
返すもう一つの道は **`negativeIsFatal`**（「マイナスの値（−9999）は指定できません」）
なので、全画面の ⛔ になる欄に **−9999** を入れて Stat タイルの前後を差分で取った。

### 測った事実

**`team` (5 欄) と `overview` (12 欄) はタイルが 1 つも動かない** ——
パス 208 で「この 2 画面は対象外」と書いた根拠を、上限だけでなく負値でも確かめた。
`real-estate` の寸法 11 欄・`tax` の税率 4 欄も動かない（パス 206 / 208 で閉じた）。

**動くのは以下。とくに ★ は「より good な方向」へ動く** ——
10^163 のような明らかに変な値ではなく、**より安心させる答え**が出る:

| 画面 | ⛔ の欄 | 出る物 |
| --- | --- | --- |
| real-estate | 月額賃料 | `NOI (年) ¥-600,000`・**`DSCR -0.40`**・`CCR -21.00%`・`実質利回り -1.43%` |
| real-estate | ★ 年間経費 | `実質利回り 3.37% → **4.80%**`・**`DSCR 0.88 → 1.28`**（危険水域から目安超えへ）・`損益分岐入居率 104% → 74.4%`・`返済後CF −84,000 → **+516,000**` |
...
| real-estate | 耐用年数 (年) | `償却年数` のタイルに**文面が入る**（`1〜100 年で入力してください`）—— 数の欄に案内文 |
| mutual-funds | ★ 目標額 (円) | `現行積立での到達見込み ¥4,183,440 (不足 ¥5,816,560)` → **`¥4,183,440 (達成)`**・`実質価値 ¥-8,203` |
| mutual-funds | ★ 取得時レート | `為替損益 ¥200,000 → **¥1,500,000**`（1,500,000 円の利益） |
| mutual-funds | 現在レート | `損益率 15.4% → **-7791.5%**` |
| mutual-funds | 達成年数 / 現在の積立額 / 毎月の生活費 / 手元資金 / 外貨額 / 為替手数料 | 0 に倒れる（`nonNeg` の契約どおり） |
| tax | 商品代金・運賃・保険料 (輸入 / 輸出 各 3 欄) | 課税価格が 0 として再計算される（`¥535,000 → ¥35,000` など）。金額は減る側だが、**何を計算したかを言わない** |

### 何が defect で、何が契約どおりか

**0 に倒れるのは契約どおり**（`nonNeg`: 「負は無意味なので 0」）。画面も ⛔ で欄を
名指ししている。これは「未入力です。0 円として計算されています」の家系で、
severity は低い —— ただし**文面が「何を計算したか」を言わない**点はパス 206 と同じ形
なので、直すなら注記を足す方向。

**defect は「負値が clamp されずに流れて判定を作る」7 件** で、うち ★ の 4 件は
**より good な方向**である（パス 204 の家系 ——「読めない値から最も都合のよい答え」）。
`DSCR 1.28`（金融機関に渡る指標が危険水域から目安超えへ）と `到達見込み (達成)` は
とくに重い。

### 直し方の見通し（次のパスで決める）

パス 206 の道具が**同じファイルに既に在る** —— `ZONING_READS` / `ZoningRefusal` は
「段ごとに ⛔ の欄を名指しして算定しない」形で、レバレッジ試算・精緻化指標・
NPV/IRR・減価償却の段にそのまま当てられる。`mutual-funds` は目標額・レートの段。
**`nonNeg` を足すだけでは足りない** —— 0 に倒すと「経費 0 円の物件」「取得レート 0」
という別の判定になる（パス 205 の「天井を床にした」の裏返し）。

`耐用年数` のタイルに案内文が入る件は別の形（**数を刷る枠に文を入れている**）で、
こちらは `Stat` の値として妥当かを先に決める。
