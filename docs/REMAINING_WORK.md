...
しきい値で判定していたらここも間違えていた。**なぜ ¥2,245,834 でそれだけの段が立つのか**
(等級表の段か、課税所得の 1,000 円未満切捨てか、給与所得控除の段か) は測っていない。
1 円の昇給で手取りが ¥6,695 減るなら**役員報酬の設計で意味を持つ**ので、次のパスの候補。
二分探索が解を見つけ損なう形もありうるが、**その取りこぼしは `reached` が拾う**
(検算するので、誤った額面を「解けた」として返すことはない)。
