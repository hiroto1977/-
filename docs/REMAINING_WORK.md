...
残りの内訳: StringLiteral 354 / ConditionalExpression 255 / BlockStatement 131 /
EqualityOperator 95 / LogicalOperator 55。密度は **L1200-1599 (invoke の if 連鎖)**
に集中している (991 件中 429 件)。

### 据え置きの理由は 1 つに減った

**「載せられない」ではない。** 今の理由は生存 652 のまま `mutate` へ入れると
`thresholds.break = 99.8` を割って**週次 CI が恒常的に赤になる**、の 1 点だけ。
「赤が当たり前」は測っていないのと変わらないので、載せるなら先に生存を潰す。
取り掛かるなら L1200-1599 の if 連鎖から (残りの 43% がそこに在る)。
