...
## パス 206 の候補 (2026-09-13 · 測定済み・未修正) — **画面が ⛔ で断っている寸法から、都市計画上の判定を作っている**
...
「300 m 以下で入力してください（現在 999999）」を出す。

**しかし `RealEstatePage` には `guardAll` も fatal の関門も 1 つも無い** (実測: この
ファイルに `guardAll` / `guardCounts` の呼び出しは 0 件)。都市計画の試算は

    const height = reNum(zpHeightStr);          // ← 断られた値をそのまま読む
    const slope = planRoadSlope({ plannedHeightM: height, … });
    const shadow = planShadowRegulation({ plannedHeightM: height, … });
    const tradeoff = planSetbackTradeoff({ plannedHeightM: height, … });

と進むので、**999,999 m の高さから道路斜線・日影規制・後退のトレードオフの判定が
出る。** しかも `max` の文面は「0 m として計算されています」と違って**何を計算したか
を言わない** —— 利用者は ⛔ と一緒に、その値で作られた判定を読む。

対象の宣言 8 件 (`RealEstatePage.tsx`):

| 欄 | 宣言 | 計算側 |
| --- | --- | --- |
| 敷地の奥行 / 間口 | `max: 2000` ×2 | `reNumOrNull` → 判定に入る |
| 計画する最高高さ / 日影規制の対象高さ | `max: 300` ×2 | `reNum` → 判定に入る |
| 建ぺい率 / 前面道路幅員 / 後退 / 想定入居率 | `max: 100` ×4 | 同上 |

### なぜこれが欠陥か (このファイル自身の判例)

**パス 77 が同じファイルで同じ形を直している** —— 「空欄の敷地寸法から『この敷地には
0 ㎡しか建てられない』という都市計画上の判定を作る」。そのとき入れた道が
`reNumOrNull` と「寸法が未入力 (null) なら**描かない**」の関門で、コメントにも
「未入力から作った図である」と書かれている。

**未入力を断るのに、宣言した上限を超えた値は断っていない。** 同じ「画面が受け付けて
いない値」から同じ「都市計画上の判定」を作っている。

### なぜ今すぐ直さなかったか

直し方に設計の判断が要る。候補は 3 つで、どれも上限の数字が**今は画面にしか無い**
ことに触る:

1. 画面に fatal の関門を置き、判定の代わりに理由を出す (パス 77 と同じ形)。
2. 上限を `shared` の台帳へ移し、`GuardedNumber` の spec と planner の両方が
   同じ数字を読む (パス 93 / 110 の「数字を 2 か所に書かない」)。
3. planner 側が物理・法令の範囲を自分で持ち、範囲外は判定を返さない。

**2 が本筋**だと思うが、上限が 8 欄で 3 通り (2000 / 300 / 100) あり、どれが法令の
値でどれが「もっともらしさの上限」かを先に決める必要がある (法令の値なら
`parameters.ts` の台帳に載せる対象で、安全上限なら載せない —— CLAUDE.md の規約)。
**測る前に決めると外す** (パス 198 の教訓) ので、次のパスで欄ごとに確かめてから触る。
