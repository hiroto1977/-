...
| `src/renderer/data/charts.ts` | 72.01% | 75 | 未着手 |
...
残りの 4 件は数字を記録して次の対象として残す (`ollamaWeb.ts` を 59.25% で
記録して後から 100% にしたのと同じ進め方)。**`villageLayout.ts` の 13.30% が
最も低い**が、AI の村の配置計算なので実害は視覚的なものに留まる。
`charts.ts` の 72.01% は**全画面のグラフ描画の土台**なので、次はここが筋。
