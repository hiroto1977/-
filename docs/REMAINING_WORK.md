...
- 村の `if (res.ok && res.data.text)` の **else が無い**のはそのまま (天井以外の失敗 ——
  鍵未設定・通信断 —— は今も黙って端末内の応答に落ちる)。天井の断りは上で出したが、
  一般の失敗を言うかは「オフライン応答のまま出すほうが親切」という設計判断と衝突するので
  このパスでは触っていない。直すなら `AssistantPage` の「（AI 応答を利用できないため
  簡易モードで回答します: …）」と同じ形になる。
