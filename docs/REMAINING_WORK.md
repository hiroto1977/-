...
**未着手の 3 ファイル** (`bankSubmission.ts` 17 / `managementHighlights.ts` 7 /
`managementReport.ts` 7) は**測っていない**。上と同じ手順 (キャッシュを消して subset 実行 →
生存を 1 件ずつ手で当てる) が要るが、この箱では subset 実行 1 回が約 7 分掛かる。
**kpiActuals の実測が示すとおり、記録された件数は古い可能性がある** —— 触る前に測り直すこと。
