...
### 書類スタジオ: 開き直しで戻るのは「書類の群れ」までで、定款の会社形態は戻らない (2026-09-06 観察)

計算書類 4 点を一覧の独立したエントリへ分けたとき、`kessanSheet` の保存だけでは開き直しに
戻れなくなったので `collection` も対で保存した (`data/docstudioStore.ts`)。同じ理屈は
電子定款の**会社形態** (`teikanType` = 株式会社 / 合同会社) にも当たるが、そちらは保存して
いないため、合同会社を書いていても開き直すと株式会社に戻る。壊れはしない (入力値は
`store.teikan.gk` / `.kk` に別々に残る) が、「同じ書類から続けられる」は半分しか成立して
いない。直すなら `collection` と同じ形 (union の型ガード + sanitize + 対で保存)。
現状は `pages/__tests__/docstudioImport.test.ts` が「開き直すと株式会社に戻る」を
そのまま留めているので、直したらその検査も一緒に変えること。
