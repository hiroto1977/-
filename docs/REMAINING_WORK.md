...
**ブラウザ側の呼び出し口はまだ測っていない。** `src/renderer/network/ollamaWeb.ts` を
同時に実測したところ **454 変異体 59.25%**（生存 113 / 未到達 72）。59% のまま
`mutate` へ載せると全体スコアが break 閾値 (99.8) を割って週次の変異 CI が
落ちるため、**この数字を明記したうえで次の対象として残している**。生存の多くは
OS 別セットアップ手順の文字列で、これは利用者が貼り付けるコマンドそのものなので
除外ではなく golden で固定するのが筋である。
