...
## 変異検査で測っていない範囲 (2026-08-18 実測・2,157 行)

`npm run lint:mutation-scope` の台帳 `KNOWN_BROAD` に載っている **17 ファイル
/ 22 箇所 / 2,157 行**（着手前は 36 ファイル / 46 箇所 / 5,189 行）。これは「許した」ではなく「まだ測っていないと
