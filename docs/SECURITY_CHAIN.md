...
`src/` のファイルには外側の証人が居る —— 検査は別ファイル (`__tests__/`) に在り、
どちらも変異検査に載っているので、**守りだけを消せば必ず鳴る**。

利用者の機械で強い権限で走るシェルスクリプトには、それが無い。挙動の検査は
`--self-test` として**スクリプト自身の中**に在り、`lint:shell` がそれを走らせる
だけである。つまり**証人と対象が同じ紙に書かれている**。

2026-08-26 に実測した。`make-live-usb.sh` のシステムディスク判定を `false` にし、
自己テスト節を丸ごと `ok "self-test 全件一致"; exit 0` へ差し替える ——
**ガードは無効、検査は自称合格、そして `self-test` / `lint:shell` /
`chain:verify` / `lint:forbidden` がすべて緑**になった。

外側の錨は鎖しかない。そこで次の 3 本を保護対象に足した。基準は
`httpLimits.ts` を昇格させたときと同じ ——
**「1 行書き換えるだけで、実装を触らずに守りが消えるか」**:

| ファイル | 1 行で消える守り |
| --- | --- |
| `scripts/make-live-usb.sh` | システムディスク判定。`dd` の宛先が稼働中のディスクでも通る |
| `scripts/make-autoinstall.sh` | ホスト名の検証。**インストール時に root で走る**設定へ `early-commands` を差し込める |
| `scripts/migrate.sh` | `-pass env:` → `pass:`。パスフレーズが `/proc/<pid>/cmdline` (444) に載る |

同じ改竄を鎖に載せた後で再実行すると、`chain:verify` だけが鳴る（前後の対照つき）。

上の 3 本 (`setup-linux.sh` / `security-audit.sh` / `setup-obsidian-docker.sh`) が
保護対象なのは**別の理由** —— §2 の「7 本の柱」で検証装置そのものを構成しているため。
...
#### 閉包検査が見ていた言語は 1 つだけだった（2026-08-27）

上の約束（材料の方も守らないと素通し）は、**当時の保護対象 55 件のうち 39 件に
しか効いていなかった**。`collectClosureProblems` の先頭に
`if (!/\.(ts|tsx)$/.test(rel)) continue;` があり、`.cjs` 3 件・`.sh` 6 件・
workflow 3 件・`assets/sw.js` ほか計 16 件を**丸ごと飛ばしていた**。

飛ばされていた側が読むリポジトリ内のファイルを実際に走査したところ、
`inject-pwa.cjs → inline-html.cjs` の 1 本だけで、それは既に保護対象だった。
**実害は無かったが、偶然そうだっただけである。**

しかも `resolveRelativeImport` は拡張子を補う側しか試しておらず、
`require('./inline-html.cjs')` のように**綴りが既に拡張子を持つ**場合は解決に
失敗して `null` を返し、静かに飛ばされていた。つまり `.cjs` を走査対象に
入れるだけでも足りず、二重に見えなくなっていた。

直したのは 2 点 —— 依存の綴りを言語ごとに読む（ESM の `from '…'`／CommonJS の
`require(…)`／`importScripts(…)`）ことと、拡張子つきの綴りを解決すること
（ディレクトリを掴まないよう、ファイルであることまで確かめる）。

前後の対照: `inline-html.cjs` を保護対象から外すと、直した後は
「`inject-pwa.cjs` が読んでいるのに保護対象でも除外台帳でもない」と鳴り、
直す前は**何も鳴らない**。

#### 追っていたのは辺の**種類**が足りていなかった（2026-08-28）

`#73` で広げたのは**言語**（`.cjs` / `.sh` / workflow を走査対象へ）であって、
**辺の種類**ではなかった。閉包が数えていたのは `import` / `require` の辺だけで、
workflow が `run:` で直接起こす本は 1 本も辺になっていなかった。

その結果こうなっていた —— `release.yml` は保護対象である（理由は「利用者が
ダウンロードするインストーラを公開する」）のに、**そのワークフローが
「公開してよいか」を決めるために呼ぶ本は 1 つも守られていない。**

実測（鎖を tip まで採掘したうえで、それぞれの `main()` 冒頭に `return 0;`）:

| 骨抜きにした本 | 直す前の `chain:verify` |
|---|---|
| `lint-sample-data.cjs`（**個人データが出荷物に載るのを止める門**） | ✅ 緑のまま |
| `verify-release-artifacts.cjs` | ✅ 緑のまま |
| `checksum-release.cjs` | ✅ 緑のまま |

> 最初の対照は**未採掘の変更に汚染されて空振りした**（`release.yml` を編集した
> 状態で回したので、鎖はそちらの理由で赤になり、問いが隠れた）。採掘してから
> やり直している。5 本目も `function main(` を持たないファイルに置換を当てて
> **ファイルが変わっていなかった** —— 「緑のまま」は対照の結果ではなく、
> 対照が着地していない報せだった。

直したのは `dependencySpecs` に `'workflow'` の種別を足し、
`node|bash|sh|python3? scripts/…` を辺として数えること。出てきた 5 本
（上の 3 つ + `lint-artifact-csp.cjs` + `smoke-app.cjs`）を保護対象へ入れた
（56 → **61**）。

**`npm run X` は辿らない。** package.json 越しに追えなくはないが、
`verify:all` から 34 本が芋づるで入り、鎖が「編集のたびに採掘し直す」だけの
装置になる。そして**直接呼びを `npm run` へ書き換えるには保護対象の workflow
自身を触ることになり、そちらが鳴る** —— つまり逃げ道にはならない。
この判断は self-test で挙動として留めてある（字面の約束にしない）。

対照 4 本: 走査を空にする → self-test 2 件 / 種別の判定から workflow を外す
→ 1 件 / 5 本を保護から外す → 閉包 8 件 / 各本を 1 バイト変える → 5 本とも鳴る。

#### 保管庫への書き込みの関門（2026-08-27）

`scripts/safe-vault-write.cjs` を保護対象へ足した（55 → **56**）。7,543 個の
ノートを書き出すとき、書き込み先が保管庫の根の外へ出ないことを決めている
唯一の関門で、symlink による逃げ道を同日に塞いだ。書き出し先の封じ込めという
役どころは `src/main/clients/exportPaths.ts`（既に保護対象）と同じで、
`vault:check` として CI でも走る。守る基準が同じなら、扱いも同じにする。
