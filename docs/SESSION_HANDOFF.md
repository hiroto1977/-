...
### 🟢 税額計算の残論点 (並列監査で整理。優先度順)
精度向上の機能追加候補 (いずれも要設計判断・UI 拡張):
- 住宅ローン控除の **居住年×住宅性能区分** 別パラメータ (控除率/期間/残高上限が令和2/3/4
  以降や ZEH/省エネ基準/非適合で異なる)。現状は 0.7%・残高上限3,000万の既定固定。
- **住民税の調整控除** (所得税と住民税の人的控除差を埋める税額控除)・**非課税限度額**。
  低〜中所得帯で住民税が実際より高めに出る。
- **配当控除の投信区分** (証券投資信託・REIT 等で率が異なる)。現状は国内株式配当の標準率のみ。
- **退職所得・一時所得・譲渡所得** (給与+配当に特化中。別所得は未対応 = スコープ外)。
- 生命保険料控除の **旧制度** (平成23年以前契約、上限5万/2.5万)。実用性は低い。
- 社会保険料の標準報酬月額テーブル化 (現状は額面比例15%概算。セクション③は実額入力で回避済)。
- ✅ 復興特別所得税の適用順序バグは d179dfe で修正済。配偶者特別控除の本人所得段階調整は
  factor で実装済 (監査の誤検出)。
