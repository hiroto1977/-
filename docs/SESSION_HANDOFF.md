...
| **直近完了作業** | 🦙✅ **Ollama を「実際に使える」状態にする** — 「オラマが使えるか調べて使えない場合実装しテストを繰り返して」への回答。まず調査: **本物の Ollama はこのコンテナに入れられない** (ollama.com の install.sh も GitHub releases もプロキシが 403)。そこで方針を「スタブと実物の差を潰す」に切り替え、実運用で最初に踏むエラーを実装した。**発見した実欠陥**: ブラウザ版は `invoke('ollama','chat')` が**未実装**で、画面にチャット欄はあるのに送信だけ無反応だった (web-shim が not_implemented を返していた) → `chatOllama()` を新設し web-shim に配線。**エラー翻訳**: Ollama は失敗時に `{"error": "…"}` を返すが、全経路で `grep -c '"error"'` が 0 = どこも封筒を開けていなかった → `extractOllamaError` / `classifyOllamaError` / `describeOllamaError` / `suggestInstalledModel` / `adviseFromBody` を `src/shared/ollama.ts` に追加し、未取得モデル・メモリ不足・推論プロセス異常・origin 拒否・エンドポイント不在の 5 種を日本語の説明＋手順に変換 (未取得モデルのときだけ `/api/tags` を引いて実在モデルと `llama3.2`→`llama3.2:latest` の近似提案を添える)。**古い Ollama 対応**: `/api/version` が 404 でも `/api/tags` が読めれば接続成功扱い (0.1.14 未満には該当エンドポイントが無い)。HTTP 403 は cors-blocked に寄せる (直し方が同じため)。**E2E にチャット往復を追加** — スタブに `OPTIONS`/`POST /api/chat` を実装 (POST+JSON は CORS プリフライトを起こすので、これが無いとチャットだけブラウザに落とされる)。`npm run e2e:ollama` は 6 状態 green。テスト 5,921 静的 / **6,006** 実行時・全11ゲート green・`clients/ollama.ts` の mutation は 122 mutant で **100.00%** 維持 |
| **前回の作業** | 💻🦙 **Ollama CLI 新設 (`npm run ollama`)** — 「Claude Code 内で出来るようにして」への回答。**CORS も mixed content も『間にブラウザがいる』ことに由来する制約**なので、Node から直接叩けば一つも発生しない。`scripts/ollama-cli.cjs`: 状態確認 (バージョン+モデル一覧・整列表示)、`-- chat <model> "..."` (--system / --json 対応)、--port、--help。終了コードは 0=成功 / 1=エラー / 2=到達不能。判定ロジックは `src/shared/ollama.ts` を esbuild で変換して共有するため制約は UI 版と同一。**罠2つ**: ①`new Function` は lint:forbidden の invariant #9 に触れる → 一時ファイルへ書いて普通に require (vm も使わない) ②CLI のコメント/‑‑help が禁止エンドポイント名を含むため lint:forbidden の allowFile に完全パスで追加 |
