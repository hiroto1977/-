...
| **直近完了作業** | 🚫➡️✅ **アーティファクトでは Ollama に接続できないと判明 → 誤診を止めて代替経路へ誘導** — 利用者がアーティファクト URL を貼ってきたので配信元の CSP を実測したところ `connect-src 'self' https://<id>.frame.claudeusercontent.com` のみ。**ブラウザがリクエストを送る前に落とすため、Ollama 側を何度設定しても解決しない**。ここまでの案内はこの経路を前提にしていたので、利用者を絶対に終わらない作業へ送り込んでいた。**厄介な点**: この失敗は「未起動」と区別がつかない (通常 fetch も no-cors も同じように失敗する) → `securitypolicyviolation` を監視して `csp-blocked` を新設し、原因と代替経路 3 つ (①CLI ②localhost 配信 ③Electron 版) を出す。**②が最も簡単** — Ollama は既定で `http://localhost:*` を許可しているので `OLLAMA_ORIGINS` の設定すら不要。**踏んだ罠**: 違反イベントは**タスク**として配送されるため fetch の reject 直後 (マイクロタスク連鎖内) に読むとまだ届いておらず、実ブラウザで not-running と誤診した → `settledCspHit` で一度タスクキューへ譲る (`CSP_SETTLE_MS=60`)。E2E に**ヘッダで `connect-src 'self'` を送るケースを追加**して実機再現 (7 状態 green)。テスト 5,949 静的 / **6,034** 実行時・全11ゲート green |
