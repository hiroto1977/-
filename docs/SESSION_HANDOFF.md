...
| **直近完了作業** | 💻🦙 **Ollama CLI 新設 (`npm run ollama`)** — 「Claude Code 内で出来るようにして」への回答。**CORS も mixed content も『間にブラウザがいる』ことに由来する制約**なので、Node から直接叩けば 一つも発生しない。`scripts/ollama-cli.cjs`: 状態確認 (バージョン+モデル一覧・整列表示)、`-- chat <model> "..."` (--system / --json 対応)、--port、--help。終了コードは 0=成功 / 1=エラー / 2=到達不能 (「同じマシンで実行する必要がある」旨を案内)。判定ロジックは `src/shared/ollama.ts` を esbuild で変換して共有するため制約は UI 版と同一 (ループバック限定・読み取り3エンドポイント限定・モデル名検証)。**罠2つ**: ①`new Function` は lint:forbidden の invariant #9 に触れる → 一時ファイルへ書いて普通に require する形に変更 (vm も使わない) ②CLI のコメント/‑‑help が禁止エンドポイント名を含むため lint:forbidden の allowFile に完全パスで追加。統合テスト `src/shared/__tests__/ollamaCli.test.ts` 11 件 — スタブ Ollama を立てて**子プロセスとして CLI を起動**し、出力・終了コード・叩いた URL (読み取り3本のみ) まで検証。OllamaPage の案内にも「いちばん確実: ターミナルから使う」を最上段に追加。テスト 5,869 静的 / 5,954 実行時・全11ゲート green |
