...
| **直近完了作業** | 🖥️ **デスクトップ版リリース着手 (利用者 = Linux PC・「やり方がわからない」への回答)** — アーティファクトの CSP 遮断が確定したため、進め方を AskUserQuestion で確認 → 「デスクトップ版を作ってもらう」を選択。**PR #721**: CSP 誤診防止 (`csp-blocked` 新設・`securitypolicyviolation` 監視・違反イベントはタスク配送なので `settledCspHit` で待つ) + electron-builder linux target を `[AppImage, deb]` に拡張 (deb は Ubuntu 系ダブルクリック導入) + `desktopSetupCommands()` (Electron 版は CORS 不在なので OLLAMA_ORIGINS/sudo/launchctl を出さない。判定は診断結果から: Electron 版は未起動でも errorMessage が無い) → マージ済。**PR #722**: タグ push がセッションの git プロキシに拒否される (4 回リトライで実測) ため release.yml に workflow_dispatch (tag_name 入力) を追加 — softprops がタグ未存在なら起動 SHA に自動作成 → マージ済。**release run 30247824701 を tag_name=v0.1.0 で起動済み** (3 OS 並列・各ジョブ 6,038 テスト + electron-builder)。cron fde11897 が 08:09 UTC に完了確認 → 成功なら Linux 導入手順を報告 / 失敗なら修正ループ。テスト 5,953 静的 / 6,038 実行時・全11ゲート green |
| **前回の作業** | 🧭 **段階に応じた案内と `npm run ollama:setup` (PR #720 マージ済)** — 案内が「Ollama は既に入っている」前提だったため、①未導入で止まっている人が④の手順をやっても動かなかった。`setupCommands(origin)` で導入→モデル取得→許可→再起動→確認を OS ごとに 1 ブロック化し、OllamaPage は診断結果で出し分け (`cors-blocked` なら許可設定だけ)。`scripts/ollama-setup.sh` は同じ判定をターミナルで行い足りない段だけ埋め、最後に必ず `/api/chat` を 1 往復。既定モデルは `llama3.2:1b` (最初の1回が確実に載ることを優先)。**罠**: `curl -f` は HTTP 500 で本文を捨てるため Ollama のエラー封筒が握り潰され「応答なし」と誤診していた → chat だけ `-f` を外す |
