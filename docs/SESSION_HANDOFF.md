...
| **直近完了作業** | 🔑✅ **認可の送り先を決める表が測られていなかった + 測定の前提を 1 つ訂正した** — `oauth.ts` の 55 行の無効化を外して実測すると **394 変異体 70.05%**、生存 117 のうち **103 件が `OAUTH_CONFIGS`** (9 サービスの認可 URL / トークン URL / スコープ)。既存検査は `toMatchObject` の部分一致で、触れていないサービス・フィールドが丸ごと素通りしていた。全サービス完全一致の golden にした。**★ 訂正: 「static 変異体は構造的に殺せない」は不正確だった** — `ignoreStatic` が無視するのは**未到達の** static だけで、覆われているものは実行され『生存』と報告される (providers.ts は未到達 static だったので消えたが、oauth.ts の表は覆われていたので 103 件残った)。**覆われた static は `vi.resetModules()` + 動的 `await import()` でモジュールを読み直せば殺せる** (これだけで 70.05% → 92.13%)。定数表は「測れない」のではなく**読み直せば測れる**。**★ 結び先も固定** — `server.listen(0, '127.0.0.1')` の host が消えると 0.0.0.0 で待ち受け、同一ネットワークの別ホストから OAuth コールバック口が見える。`listen` に渡した引数を直接観測する形にした (最初に書いた検査は**無条件に通る空検査**で、陰性対照を取るまで気付かなかった)。**379 変異体 100%**。**★ 台帳は 2,590 → 2,535 行** (着手前 5,189 行)。次は `calendar.ts` (43.3%) / `templates.ts` (47.5%)。|
