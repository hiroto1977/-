...
| **直近完了作業** | 🔬 **出典の誤りを 13 件確定・修正し、同種の混入を止めるゲート `lint:citations` を新設** — MERGE のブロッカーだった DOI 照合を、`WebSearch` が使えることに気づいて解決 (`api.crossref.org`・`doi.org`・JSTOR は egress ポリシーで CONNECT 403 だが、WebSearch は通る)。4 並列で一次照合したところ **当初報告の 4 件を超えて 13 件の誤り** が確定した。**DOI が別論文を指していた 9 件**: Weitzel&Jonsson 1989 は 258302/258079 とも誤り (258xxx は AMR 帯で ASQ 論文にはありえない) → 正 `10.2307/2392987`／Bourgeois 1981 も 2392586/2392580 とも誤りで**両方が ASQ 23(4) 1978 内の別項目** (無題書評と Gephart 1978) → 正 `10.5465/amr.1981.4287985`／Keynes 1929 の 2223852 は **EJ 42(166) 1932** の別項目 (当初仮説『オリーンの反論』は否定)／Samuelson 1952 の 2296547 は **Phelps&Pollak 1968 RES**／Nohria&Gulati 1996 は SICI の AID が 1997 Smith et al. のもので**内部矛盾する無効 DOI**。**書籍に他誌論文の DOI 2 件**: `10.1177/0306312706056047` の実体は **Gillespie 2006 Social Studies of Science** で Bowker とも Edwards とも無関係 → Bowker 1994 は DOI 未付与なので MIT Press URL+ISBN へ、『Edwards et al. (2007) JAIS』は 2007 のタイトルと 2009 の掲載誌の**合成ラベル**だったので JAIS 2009 版へ。**人名 2 件**: 「ヌーリアとサージェント」(マクロ経済学者 Thomas Sargent の混入)・「ノリアとグルク」→ ともに「ノーリアとグラティ」。**★恒久対策**: 確証ゲートは *出典の件数と種別しか見ておらず書誌の正しさを見ていない* という穴があった → `scripts/lint-citations.cjs` を新設し **1 DOI = 1 著作 = 1 出版年** を検査 (`verify:all` は 11→**12 ゲート**)。既存 18 件は `knowledge-citation-baseline.json` に明示列挙し、**新規は即ブロック / 直したら台帳から消さないと落ちる双方向設計**で腐敗を防ぐ。精度調整の記録: 「同一著者+年に複数 DOI」は同一著者同年の別論文を拾うため不採用 (March 1991 に 5 件)、年抽出はページ範囲やタイトル内の年を除外 (`Neuropsychologia, 47(10), 2015–2028` の誤検出を実測) |
| **前回の作業** | 🔍 **重複疑い 52 件を全文読解で裁定 → キュー 52→20 (PR #728 マージ済)** — 4 並列が 104 エントリの全文を読み **DISTINCT 32 / MERGE 20**。DISTINCT 32 件を台帳へ記録 (84→116)。MERGE 20 件は ①同一論文に食い違う DOI が 4 組見つかった ②感情調整 4 件/ストレスコーピング 3 件/代替の弾力性 3 件が収束クラスタでペア単位実行すると survivor が二重に残る、の 2 理由で保留し `knowledge-merge-plan.json` に計画化。**別途** `npm run smoke` が main.ts の 12 IPC ハンドラ中 6 個しかスタブせず 'No handler registered' を出しながら exit 0 していた (本物の登録漏れと見分けがつかない) → main.ts からチャンネル一覧を抽出して突き合わせるガードを追加 |
...
| **次のアクション** | ①**MERGE の実行** — `knowledge-merge-plan.json` の `ready` **15 件** (DOI 照合済みで解禁済。keep 側へ migrate を取り込んでから drop を削除) → `clusters` 3 件をまとめて統合。実行後 vault:build → verify:all ②`knowledge-citation-baseline.json` の **残 18 件**を WebSearch で照合して台帳から消す (併せて `citationCheckResolved.stillUnverified` の Gillespie 3 件と Wikipedia 2 URL の存否も) ③増強バックログ 509 件のトランシェ消化 ④新規概念 Batch 724 |
