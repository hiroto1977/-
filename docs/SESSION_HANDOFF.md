...
| **直近完了作業** | 📱🦙 Ollama を**別端末 (スマホ) からも使える**ように接続先ポリシーを拡張: `127.0.0.1` は各端末にとって自分自身なので、スマホのブラウザからは PC の Ollama に原理的に届かない。`isAllowedOllamaBase(base, pageHostname)` で許可を **3 通り**に限定しつつ拡張 — ①ループバック ②**ページ自身と同じホスト名への http** (PC で配信したページをスマホから開く構成。既にそのホストからアプリを読み込んでいるので新たな到達先を与えていない = 内部探索に使えない) ③**任意の https** (cloudflared / Tailscale Serve のトンネル。https なので mixed content で弾かれず、相手が CORS で許可しない限り読めないためホスト名を絞らなくても安全)。**平文 http で別ホストは不許可** (ブラウザも mixed content で遮断するうえプロンプトが平文で流れる)。`parseOllamaEndpoint` で `11434` / `192.168.1.10:11434` / `https://…` を正規化し、UI はポート欄から「接続先」欄に変更 + `<details>` に経路 A (同じ Wi-Fi: `OLLAMA_HOST=0.0.0.0` + `python3 -m http.server` で**アプリ自身も PC から配信**するのが要点) と経路 B (https トンネル) の手順を掲載。localStorage キーは `servicehub.ollama.endpoint` (旧 `…port` も読む後方互換)。e2e:ollama を 5 ケースに拡張 (未起動 / CORS未許可 / 接続成功 / 別端末経路 / 許可外拒否)。テスト 5,858 静的 / 5,943 実行時・全11ゲート green |
