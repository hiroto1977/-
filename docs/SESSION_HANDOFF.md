...
| **次のアクション** | ①`knowledge-citation-baseline.json` の**残 17 件**を WebSearch で照合して台帳から消す（**照合手順は確立済み** → 「DOI 接尾辞の完全一致検索で実体を先に確定 → 主張者を個別照合」。api.crossref.org / api.openalex.org / doi.org は egress 403 なので WebSearch のみ。到達できず実体を特定できない場合は**台帳に残す**のが正しい＝直したことにしない）②`citationCheckResolved.stillUnverified` の Gillespie 3 件と Wikipedia 2 URL の存否 ③増強バックログ 504 件のトランシェ消化 ④新規概念 Batch 724 ⑤重複疑いキューは 0 件到達済み — 新規追加後にオートパイロットが再検出したら同じ手順（全文読解＋事前スナップショットで減少ゼロを証明）で処理 |
