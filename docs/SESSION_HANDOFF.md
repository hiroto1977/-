...
| **直近完了作業** | 🛡️🔧 **監査 第3ラウンド — 未対処項目の総ざらい (並列4系統 + Electron 更新)**: 第2ラウンドで「受容」「上流責務」「別タスク」として残した項目を実際に潰した。①**プロキシ上流の SSRF 対策を実装**: 配布用リファレンス Worker は「解決後IPを再検査する」と文書化しながら実装していなかった (アローリストと既定のリダイレクト追従のみ) → DoH で A/AAAA を引いて private/reserved 判定 (IPv6 は 8 グループ展開の数値比較で 64:ff9b::a9fe:a9fe や任意サフィックス 6to4 まで捕捉)・redirect:manual + ホップ上限3で各 Location を再検査・ホスト跨ぎで Authorization/Cookie を破棄・DoH 失敗は fail-closed・共有シークレットを定数時間比較。残存 (TTLレース/DoH 依存/網固有 NAT64) は §3.1 に明記 ②**CSP を 'unsafe-inline' → sha256 ハッシュ固定**: バンドルのバイト列を固定し、inject-pwa は SW スニペットのハッシュを冪等追記 (ハッシュが1つでもあると unsafe-inline は無視されるため追記必須)。仕上がり文書から再導出して未ピン留めならビルド失敗。実 chromium で 10MB/2.2MB 両方の正常描画と**注入 script の遮断**を確認 ③**生体認証の見せかけ検証を fail-closed 化**: `rawId.byteLength > 0` だけで true を返していた `verifyBiometric` を認証器呼び出し前に throw へ。「生体解錠のためにマスターパスワード/派生鍵を保存しない」不変条件を明記し、実態と食い違っていた SECURITY_CHAIN.md §3 を訂正 ④**保護状態の可視化**: keychain 不在時の `plain:` 警告は console のみ = GUI 利用者に届いていなかった → `secrets:protection` IPC (秘密は返さない) + 設定画面に暗号化可否/未暗号化件数/保存先/keyring 導入手順を表示。リカバリーキー .txt も平文であることを明示 ⑤**画像URLのスキーム検証** (https?/data:image のみ・javascript: では src 自体を出さない) ⑥**Electron 33→43 / electron-builder 25→26**: ランタイム依存で唯一残っていた既知CVE を解消。E34〜43 の breaking-changes を精査し使用 API に影響なしを確認 (frameless 未使用/renderer は electron の clipboard 不使用) ⑦**mutation 3モジュールを 100% 化して config へ登録** (zoningPlanner 97.06→100 / shigyoDirectory 78.5→100 / investments 73.7→100)。等価変異は JS セマンティクスで確認して pragma + 理由コメント。**罠**: stryker は incremental キャッシュが効くため、測定前に `.stryker-incremental.json` を消さないと古い survived が残って誤判定する (実際に一度誤った) |
| **前回の作業** | 🛡️ **セキュリティ監査 第2ラウンド (13件修正)**: 5並列エージェント (Electron main / OAuth+PKCE / プロキシSSRF / WebCryptoVault / XSS・ビルド時injection) + 手動 (CI/秘密混入/SW/CSP) で全攻撃面を診断し、**再現検証できた指摘のみ**修正 → 詳細表は [`docs/SECURITY_AUDIT.md`](SECURITY_AUDIT.md) 冒頭。要点: ①`app:openPath` が $HOME 配下の任意ファイルを OS 既定アプリで開けた (Windows は .exe 実行プリミティブ) → `~/.local/business-hub` + 非実行拡張子 + realpath に限定 ②4クライアントの `customPath` も同様に $HOME 全域 → 新設 `clients/exportPaths.ts` に一元化 (どのUIも path を送っていないため機能影響ゼロ) ③`dataCrypto` の PBKDF2 が 210k (SHA-512 の行を SHA-256 と誤記) → **600k** ④マスターパスワード 8→**12文字** ⑤SSRF ガードの**末尾ドット回避** (`localhost.` 等・実測再現) と**生URL転送によるパーサ差分**を修正 (+CGNAT 100.64/10) ⑥SW が全GETを平文キャッシュ → 同一オリジン限定 ⑦CSP `worker-src` 欠落で **SW が一度も登録できていなかった** → 追加し実ブラウザで登録確認 ⑧書類メーカー3種の `<` 未エスケープ (2026-07-24 事故と同型) → `scripts/lib/json-for-script.cjs` 新設。**受容/上流責務**: ホスト名のみ検査の限界 (DNS リバインドはプロキシ側で解決後IP再検査)・Electron ^33 の既知CVE (43 へのメジャー更新は別タスク)。テスト 5,695→**5,709** (静的) / 5,794 (実行時)・全11ゲート green |
