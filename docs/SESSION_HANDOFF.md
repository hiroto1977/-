...
| **直近完了作業** | 🔍 **重複疑い 52 件を全文読解で裁定 → キュー 52→20・smoke の穴を塞ぐ** — 引継ぎの「次のアクション①」を消化。4 並列エージェントが 52 ペア **104 エントリの全文** (title/statement/keyFigures/authority/sources) を読み、**DISTINCT 32 / MERGE 20** を裁定。DISTINCT 32 件を `knowledge-distinct-pairs.json` へ記録 (台帳 84→116) し、重複疑いキューは **52→20** へ。**DISTINCT の中身**: compliance 側は「制度本体 vs 特定年改正」(子の看護等休暇 vs 2025年4月改正パッケージ)・「原則 vs 特例」(源泉徴収 vs 納期の特例、時間外の原則限度 vs 特別条項)・「原則 vs 例外」(国家行為理論 vs ヒッケンルーパー修正、pacta sunt servanda vs ハードシップ条項) が典型。academic 側は後続文献の系列が互いに排他的なもの (吸収能力の Zahra 批判史 vs Volberda マルチレベル拡張)。**MERGE 20 件は実行を保留し `orchestration/knowledge-merge-plan.json` に計画として記録** — 理由は 2 つ: ①裁定の副産物として **同一論文に食い違う DOI/掲載誌** が 4 組見つかった (Weitzel&Jonsson 1989 が AMR と ASQ、Keynes 1929 が 2 通り、Bourgeois 1981 が 2 通り、infosoc 2 件が同一 DOI を別文献ラベルで引用)。統合すると誤った方を正として固定するため、一次資料で確認するまで実行しない (**推測で直さない** — このコーパスの価値は出典の正確さ) ②感情調整 4 件 / ストレス・コーピング 3 件 / 代替の弾力性 3 件が **3 件以上の収束クラスタ**で、ペア単位に実行すると survivor が二重に残る。計画には ready 13 件それぞれの keep/drop/移送すべき情報を明記済み。**別途**: `npm run smoke` が main.ts の 12 IPC ハンドラ中 6 個しかスタブしておらず 'No handler registered' を出しながら exit 0 していた → **main.ts からチャンネル一覧を抽出してスタブ集合と突き合わせ、ズレたら落とす**ガードを追加 (本物の登録漏れと見分けがつかない状態だった)。デスクトップ実機 22 ページの起動も確認済み |
| **前回の作業** | ✅🖥️ **デスクトップ版 v0.1.0 を全 OS でリリース完了** — https://github.com/hiroto1977/-/releases/tag/v0.1.0 に **exe / dmg / deb / AppImage** の 7 アセット。Ollama は **設定ゼロ**で使える (Electron は main プロセス直叩き = CORS も CSP も原理的に発生しない)。**3 ラン要した。失敗はすべてテスト側の可搬性で、プロダクトコードの不具合はゼロ**: Windows のパス流儀 (`D:\home\user` vs `/home/user`) 決め打ち 3 種、`.deb` の maintainer 欠落 (author 未設定)、electron-builder の暗黙 publish、`--connect-timeout` 無しで起動待ちループが 40 秒級に膨張。教訓: **リリース workflow は 3 OS で `npm test` を回すため、ubuntu 単独の通常 CI では露見しない可搬性バグがここで初めて出る** (PR #723/#726/#727) |
...
| **次のアクション** | ①**MERGE 20 件の実行** — まず `knowledge-merge-plan.json` の `blockedOnCitationCheck` 4 組の DOI を一次資料で確認 → クラスタ 3 件をまとめて統合 → ready 13 件 (keep 側へ migrate を取り込んでから drop を削除)。実行後は vault:build → vault:check → verify:knowledge → verify:graph ②増強バックログ 509 件のトランシェ消化 ③新規概念 Batch 724 ④柱 B Phase 4 |
