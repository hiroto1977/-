...
| 項目 | 値 |
|---|---|
| **現在の概念総数** | **3,583**（`grep -c "    id: '" src/renderer/data/academicKnowledge.ts`。重複統合 7 パス+オートパイロット消化で 4,350→3,583・**−767**。パス4/5/7 は全文読解で裁定し、統合先の出典・記述を一切減らさずに 25 件を統合。**重複疑いキューは 3 系列すべて 0 件** — タイトルコア一致 / グラフ term-overlap / **id 正規化 (パス7 で新設)**） |
| **直近完了作業** | 📐✅ **図面で詰まった箇所をアプリに実装 — 高さ制限（道路斜線・日影規制）を `zoningPlanner` に追加** — 飯塚市の敷地で植物工場の二案（12.1m / 10.0m）を作図したところ、**アプリ側が高さ制限を一切持っていない**ことが判明した。建蔽率・容積率・作業場 150 ㎡ までは計算できるのに、道路斜線と日影規制は手計算していた。そこを実装した。**① `planRoadSlope`** — 限度 =（幅員 + 後退×2）× 勾配（住居系 1.25 / その他 1.5・法56条1項1号）。**必要最小後退を逆算**する（a =（高さ ÷ 勾配 − 幅員）÷ 2）ので「この高さを通すには何 m 下がればよいか」が出る。**② `planShadowRegulation`** — 対象要件は「対象区域内」かつ「10m を超える」の 2 つ。**前者は自治体の条例指定で機械判定できないので `regulated` に `null`（不明）を返す** — 対象でないと誤って断定しない設計。飯塚市の区域指定が確認できなかった経験をそのまま型に落とした。「10m を超える」が要件なのでちょうど 10.0m は対象外になる。**③ `planSetbackTradeoff`** — 高さを下げると後退が減り奥行が伸びる関係を数値化。実測で案 A（12.1m）は後退 1.04m・奥行 18.46m、案 B（10.0m）は 0.34m・19.16m で**建築面積が +8.4 ㎡**。どちらに縛られているか（`coverage` / `geometry`）も返す。**★ 推測を型で防いだ**: 別表第三の適用距離は全表を確定できなかったので**ハードコードせず入力**にし、両方揃ったときだけ判定して `applicationDistanceChecked` で明示する。日影規制の閾値も入力（低層住専の軒高 7m 用）。**mutation で 2 種類の穴が出た**: (a) 適用距離の `&&` を片側だけ渡すケースが未テスト → 2 本追加 (b) `needed > 0 ? ceil2(needed) : 0` は `>= 0` と**結果が同じ等価変異** → `Math.max(0, ceil2(needed))` に書き換えて分岐そのものを消した。テストを足しても殺せない変異は、コードを単純にして消すのが正しい。結果 **zoningPlanner.ts は 173 変異体すべて撃墜で 100.00%**（閾値 99.8）。UI は不動産ページに「高さ制限」「後退と建築面積のトレードオフ」の 2 ブロックを追加し、対象区域が未確認のときは**黄色で「自治体に照会」と出す**。**成果**: テスト 6,064→**6,091**（静的 5,979→6,006）、zoningPlanner mutation 100 %、全 13 ゲート green |
| **前回の作業** | 🔍🛠 **「残作業は無いか徹底的に解析して」→ 全体監査で 3 つの実害を発見して修正** — ドキュメントの記述を信じず実測した。**★ 最大の発見: `verify:all` の 13 ゲートのうち 3 つが `ci.yml` に無く、PR で一度も走っていなかった** — `lint:citations` / `lint:knowledge-refs` / **`verify:knowledge`（確証ゲート＝出典2件以上・権威1件以上。CLAUDE.md 自身が「このコーパスの価値の土台」と書いている）**。CLAUDE.md には「typecheck + all verify/lint」と書いてあったので、記述を読むかぎり気づけない。**ゲートが存在するのに何も守っていない**状態だった。3 つを ci.yml に追加し、**恒久対策として `lint:docs` に「`verify:all` の全ゲートが ci.yml で実行されていること」の検査を新設**（負のコントロール 2 本: ci.yml から 1 つ抜く／verify:all にだけ足す、どちらも exit 1 を確認）。**② 検出器が両系列ですり抜けていた重複 1 ペア**: `infosoc-datafication-mayer-schoenberger` ⇔ `-schonberger`（ö の翻字ゆれ）。titleCore は「データ化」vs「データフィケーション」で不一致、term-overlap も閾値未満。統合し（本文 434→621 字・減少ゼロ）、恒久対策に `idDedupeSuspects`（id 正規化で人名の翻字ゆれを検出）を autopilot へ新設＝重複疑いキューが 3 系列になった。**③ 実装済みなのに「Phase 6 で予定」と言っていた記述 2 件**（`shigyoTypes.ts` / `snapshot.ts`）を是正 — Phase 6 の士業 CRM は `useCollection` で IndexedDB 永続化済み。**副産物の誤 DOI 1 件**: `10.2307/j.ctt1hj9w84` の実体は Schonfeld, Bowen & Varian『JSTOR: A History』(Princeton UP) で Mayer-Schönberger & Cukier ではない。**1 回しか引かれていない DOI は `lint:citations` では原理的に検出できない**（年の矛盾が生じない）ため、統合時に移送しないという判断で回避した。**測れなかったこと**: 今回追加した 26 URL の死活はこの環境からは検査不能（proxy が全ホストを遮断し `000ERR`）。`knowledge-auto.yml` が週次で `--links=100` を回すので、そこで順次検査される。コミット済みの `knowledge-queue.json` は `linksChecked: 0` なので、**その「リンク切れ 0 件」は偽の安全信号**である点に注意 |
| **サービス数** | **72**（cpa 追加 — 士業連携カテゴリ 8 士業＋担当領域ナビ） |
| **Knowledge Vault** | 7,671 files（知識 4,208＋人物・出典・年表・パス・教育・MOC/組織。統合パス4/5/7 で 25 概念分のノートが統合先へ吸収。パス6 の出典差し替えで出典ノートが再編） |
