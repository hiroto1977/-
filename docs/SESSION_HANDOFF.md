...
| **次のアクション** | ①**増強バックログ 504 件**のトランシェ消化 ②**新規概念 Batch 724** ③`knowledge-merge-plan.json` の `citationCheckResolved.stillUnverified` に残る **Gillespie 3 件**（同一論文にコーパス内で別 DOI が併存。`lint:citations` は「同一著作が別 DOI で引かれている」を**意図的に検査しない**設計なので人が照合する残作業）と、実体を特定できなかった JSTOR 8 件（出典自体は差し替え済みで、真の所有者が不明なだけ） ④重複疑いキューと出典ベースラインはともに **0 件到達済み** — 新規追加でオートパイロットが再検出したら確立済みの手順で処理（重複=全文読解＋事前スナップショットで減少ゼロを証明／出典=DOI 接尾辞の完全一致検索で実体を先に確定。確定できなければベースラインに残す＝直したことにしない） |
