...
| **直近完了作業** | ✅🖥️ **デスクトップ版 v0.1.0 を全 OS でリリース完了** — https://github.com/hiroto1977/-/releases/tag/v0.1.0 に **exe / dmg / deb / AppImage** の 7 アセットが揃った。これで Ollama は **設定ゼロ**で使える (Electron は main プロセスから直叩き = CORS も CSP も原理的に発生しない)。**リリースは 3 ラン要した。失敗はすべてテスト側の可搬性で、プロダクトコードの不具合はゼロ**: **ラン#1** ①Windows: `path.resolve('/home/user')` がドライブレター付き (`D:\home\user`) になり POSIX リテラル比較が落ちる (exportPaths) / `toContain('a/SKILL.md')` が `\` 区切りに不一致 (skills) / 「未導入」再現の `PATH='/usr/bin:/bin'` が Git Bash 自体を見失いスクリプトが無出力死 (ollamaSetup) ②Linux: `package.json` に author が無く .deb の maintainer を組めない (electron-builder FpmTarget) ③electron-builder が CI 検知で暗黙 publish を開始し softprops と二重アップロード寸前 → `--publish never` を明示 (**PR #723**)。**ラン#2** Windows 残 1 件は中身が `Test timed out in 30000ms`。`server_up()` の curl に `--connect-timeout` が無く、閉じたポートへの接続が `--max-time 3` いっぱい待つ環境では起動待ちループ (最大10周) が 40 秒級に膨張 → `--connect-timeout 1` 追加 + 当該テストに 120 秒付与 (**PR #726**)。**ラン#3 で 3 OS 全 success**。教訓: **リリース workflow は 3 OS で `npm test` を回すため、ubuntu 単独の通常 CI では露見しない可搬性バグがここで初めて出る**。テスト 5,953 静的 / 6,038 実行時・全11ゲート green |
| **前回の作業** | 🖥️ **デスクトップ版リリース着手** — アーティファクトの CSP 遮断が確定 (`connect-src 'self' https://<id>.frame.claudeusercontent.com` を実測) したため方針転換。**PR #721**: CSP 誤診防止 (`csp-blocked` 新設・`securitypolicyviolation` 監視・違反イベントはタスク配送なので `settledCspHit` で待つ) + electron-builder linux target を `[AppImage, deb]` に拡張 + `desktopSetupCommands()` (Electron 版は CORS 不在なので OLLAMA_ORIGINS/sudo/launchctl を出さない)。**PR #722**: タグ push がセッションの git プロキシに拒否されるため release.yml に `workflow_dispatch` (tag_name 入力) を追加 — softprops がタグ未存在なら起動 SHA に自動作成 |
