...
| **直近完了作業** | 🦙 Ollama 連携をブラウザ版で成立させた: AI プロバイダとしては元から対応済みだったが、Ollama **ページ**は snapshot 固定でデモ表示のみ = 起動しているかも入っているモデルも分からなかった。①`shared/ollama.ts` 新設 — 判定ロジック (ループバック限定・読み取り3エンドポイント限定・バージョン安全性・モデル名検証・tags 正規化) を main/renderer で共有 (main 側は再 export で後方互換)。**自作テストが実バグ2件を検出**: `Number('0x2b')===43` で 16 進ポートが黙って通っていた / `isSafeModelName('http://x/y')` が通っていた → 10進限定と連続スラッシュ拒否で修正 ②`renderer/network/ollamaWeb.ts` 新設 — **`mode:'no-cors'` フォールバックで「未起動」と 「起動しているが OLLAMA_ORIGINS 未設定」を切り分ける** (通常 fetch 失敗 + no-cors 成功 = CORS 拒否)。利用者にはどちらも『つながらない』に見えるが後者は環境変数1つで直るため、区別が本質 ③web-shim の fetchSnapshot('ollama') を実接続に接続 (code は `ollama_*`。'not_configured' は使わない — errorKind='auth' に分類され認証問題と誤表示されるため) ④OllamaPage に接続設定 (ポート・接続テスト・OS別 OLLAMA_ORIGINS 手順+コピー) を追加。**案内の出し分けはビルド判定でなく診断結果で行う** (両ブリッジが同じメソッドを持つため isBrowser 推測は脆い) ⑤`useServiceData` に `autoFetch` オプション (既定 false で従来動作) — 認証不要のローカルサービスは開いた時点で接続を試す ⑥`npm run e2e:ollama` 常設 (scripts/e2e/ollama.cjs) — スタブ Ollama で 3 状態を再現し**実際に出る文言**まで検証。テスト 5,847 静的 / 5,932 実行時 (+44)・全11ゲート green。**罠**: 「分析・ツール」カテゴリは既定で折りたたみ → E2E は `.sidebar-search-input` で絞ってから `.sidebar-item[data-service-id=...]` を押す。lint:forbidden は禁止エンドポイント名の記述自体を弾くため shared/ollama.ts を明示的に除外 (allowlist を定義する当事者。prefix ではなく完全パスで指定) |
