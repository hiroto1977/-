...
**品質メトリクス (2026-06-11 更新):** 5253 静的 / 5335 実行時 tests passing · typecheck / ESLint clean · verify:all green (175 file:line refs + 5 metrics; service count = 64) · standalone HTML ~806 KB

### 直近セッションの作業 (research/counseling エンジン)
- **他害衝動 (harm-other) ペルソナを研究シミュレータに追加** (`counselingResearch.ts`)。エンジンが持つ 8 トーンのうち唯一 research ペルソナ未カバーだった `harm-other` を、怒り→破壊衝動→危機の間の「切迫した他害」アーク (患者役: りく) として補完。`crisis: false` を維持し**希死念慮危機との区別** (commit e738f2a の方針) を保持。専用テストで「他害検知 → 窓口 (110/119) 照会必須」「全ターン適合」を不変条件化。research デモは 5→6 セッションに自動拡張 (デモ builder は件数を動的算出)。
