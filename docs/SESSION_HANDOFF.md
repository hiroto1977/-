...
| **直近完了作業** | ⚡ 起動性能の最適化: `services.ts` が全ページを静的 import する構造のため、`assistantContext.ts` の `export const KNOWLEDGE_CORPUS = buildCorpus()` が**アプリ起動時に必ず**評価され、学術コーパス 8MB の JSON.parse + 4,000 件超のマッピングをクリティカルパスで実行していた (誰も学術データを見ていない段階で)。①`knowledgeCorpus()` へ遅延化 (初回呼び出しでキャッシュ・既定引数は呼び出し時評価なので消費側は無変更) ②`data/lazyJsonArray.ts` 新設 — Proxy で「最初に触るまで JSON.parse しない」読み取り専用配列ビュー。vite の academic-json-parse がこれを経由して emit するので**ソースと node スクリプト群は無変更** (12 テストで反復/length/添字/find/filter/スプレッド/Array.isArray/in/Object.keys/JSON.stringify/読み取り専用/遅延/キャッシュを固定) ③`npm run perf` 新設 (scripts/perf/startup.cjs) — 実 chromium・スマホ幅で DCL/heap を測り、**起動時に 1MB 超の JSON.parse が走ったら FAIL**。遅延を意図的に壊す反証テストで検出できることを確認済み。**実測 DOMContentLoaded 637ms → 364ms / JS ヒープ 42.6MB → 33.8MB** (LITE は 112ms / 7.9MB)。E2E 18 チェック・全11ゲート green。CI には入れない — playwright は devDependencies に無く、CI で入れると無料枠を大きく消費するため `npm run e2e` と同じくローカル/手動ゲート運用 |
