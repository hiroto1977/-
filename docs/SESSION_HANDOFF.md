...
| **次のアクション** | ①**増強バックログ 504 件**のトランシェ消化 ②**新規概念 Batch 724** ③`knowledge-merge-plan.json` の `citationCheckResolved.stillUnverified` に残るのは**実体を特定できなかった JSTOR 8 件のみ**（出典自体はパス4で差し替え済みで、真の所有者が不明なだけ。ゲートには抵触しない） ④重複疑いキュー・出典ベースライン・Gillespie 照合はすべて **完了 (0 件)** — 新規追加でオートパイロットが再検出したら確立済みの手順で処理（重複=全文読解＋事前スナップショットで減少ゼロを証明／出典=DOI 接尾辞の完全一致検索で実体を先に確定し、確定できなければベースラインに残す＝直したことにしない。出版社接頭辞と誌名の不整合は実体不明でも確定できる） |
