...
| **直近完了作業** | 🧭 **「使えるようになるまで」の代行 — 段階に応じた案内と `npm run ollama:setup`** — 前回で実装は通ったが、利用者側が最初の 1 回に到達できていなかった。原因は案内が**「Ollama は既に入っている」前提**だったこと。実際に詰まるのは ①未導入 ②未起動 ③モデルが 1 つも無い ④読み取り未許可 のどれかで、済んでいる段を読ませると「手順どおりにやったのに動かない」になる。**対応**: (1) `setupCommands(origin)` を新設 — 導入→モデル取得→許可→再起動→確認 を OS ごとに 1 ブロックへまとめ、貼れば終わる形にした (macOS は公式 install.sh が Linux 向けなのでアプリ導入を案内、Linux/Windows は `command -v` / `Get-Command` で既導入なら何もしない)。(2) OllamaPage は診断結果で出し分け — `cors-blocked` なら許可設定だけ、それ以外は通し手順。(3) `scripts/ollama-setup.sh` (`npm run ollama:setup`) — 同じ判定をターミナルで行い**足りない段だけ埋める**。最後に必ず `/api/chat` を 1 往復させる (バージョンが読めるだけでは「使える」と言えない)。`--check` は無変更で現状報告、`--origin` は永続化手順を出すだけで systemd/launchctl へ黙って書かない。**罠**: `curl -f` は HTTP 500 で本文を捨てるため、Ollama のエラー封筒が握り潰されて「応答なし」と誤診していた → chat 呼び出しだけ `-f` を外す (self-test で検出)。既定モデルは `llama3.2:1b` (約1.3GB — 最初の1回を確実に成功させることを優先) で、shared の `DEFAULT_SETUP_MODEL` と .sh の一致をテストで固定。テスト 5,941 静的 / **6,026** 実行時・全11ゲート green・E2E 6状態 green |
| **前回の作業** | 🦙✅ **Ollama を「実際に使える」状態にする (PR #719 マージ済)** — 本物の Ollama はこのコンテナに導入できない (ollama.com・GitHub releases とも 403) ため「スタブと実物の差」を潰す方針に切替。**実欠陥2つ**: ①ブラウザ版は `invoke('ollama','chat')` が未実装で、画面にチャット欄はあるのに送信だけ無反応だった → `chatOllama()` 新設 + web-shim 配線 ②`grep -c '"error"'` が全経路 0 = Ollama のエラー封筒をどこも開けていなかった → `extractOllamaError`/`classifyOllamaError`/`describeOllamaError`/`suggestInstalledModel`/`adviseFromBody` を追加し 5 種を日本語の手順へ変換。`/api/version` が 404 でも `/api/tags` が読めれば接続成功扱い (0.1.14 未満対応)。E2E にチャット往復を追加 (POST+JSON はプリフライトを起こすためスタブに OPTIONS 実装)。`clients/ollama.ts` の mutation は 122 mutant で 100.00% 維持 |
