...
### 1.2 目的
- 61 サービスを **1 つのカテゴリ別サイドバー UI** で一元操作する。
...
| ID | 要件 | 状態 |
|---|---|---|
| FR-CORE-01 | 61 サービスをカテゴリ (おすすめ/分析・ツール/外部サービス連携) 別サイドバーで一覧・遷移できる | ✅ |
...
| FR-TAX-09 | 一時所得・譲渡所得 (申告分離) の計算 | ⬜ (将来・要方針確認) |
...
| NFR-QUAL-01 | 品質 | ユニットテスト 1577 (静的) / 1626 (実行時) が全 pass | `npm test` (CI) |
...
2. 全 61 サービスに対応するページ・client・テストが存在する (`lint:test-coverage`)。
