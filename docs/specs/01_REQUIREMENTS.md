...
### 1.2 目的
- 63 サービスを **1 つのカテゴリ別サイドバー UI** で一元操作する。
...
| ID | 要件 | 状態 |
|---|---|---|
| FR-CORE-01 | 63 サービスをカテゴリ (おすすめ/分析・ツール/外部サービス連携) 別サイドバーで一覧・遷移できる | ✅ |
...
| NFR-QUAL-01 | 品質 | ユニットテスト 1825 (静的) / 1874 (実行時) が全 pass | `npm test` (CI) |
...
2. 全 63 サービスに対応するページ・client・テストが存在する (`lint:test-coverage`)。
