---
collection: academic
id: bizlaw-attractive-nuisance-doctrine
category: "business-law"
category_ja: "ビジネス法務"
title: "魅力的危険物法理"
as_of: "2026-06"
source_count: 4
authoritative: true
tags:
  - collection/academic
  - academic/business-law
  - knowledge/verified
aliases:
  - "魅力的危険物法理"
---

# 魅力的危険物法理

> [!info] コレクション: [[学術概念]] ・ 区分: ビジネス法務 ・ asOf: 2026-06 ・ 出典: 4件（うち権威ある出典 ✓）

## 概要
土地占有者は児童を引き寄せる蓋然性のある人工的危険状態に対し、不法侵入児童にも合理的注意義務を負う。Stout判決（1873）が礎判例でRestatement §339が要件を体系化。

## 提唱者・初出
William Strong ／ Francis H. Bohlen ／ William L. Prosser

## 出典
- [Cornell LII Wex — Attractive Nuisance Doctrine](https://www.law.cornell.edu/wex/attractive_nuisance_doctrine) `リファレンス`
- [Railroad Company v. Stout, 84 U.S. 657 (1873) — Cornell LII](https://www.law.cornell.edu/supremecourt/text/84/657) `リファレンス`
- [Indiana Law Journal Vol. 32 — The Attractive Nuisance Doctrine (1956)](https://www.repository.law.indiana.edu/cgi/viewcontent.cgi?article=2759&context=ilj) `学術`
- [Marquette University Law Review — Elements of Attractive Nuisance](https://scholarship.law.marquette.edu/cgi/viewcontent.cgi?article=3290&context=mulr) `学術`

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
