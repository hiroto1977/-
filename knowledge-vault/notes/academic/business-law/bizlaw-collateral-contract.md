---
collection: academic
id: bizlaw-collateral-contract
category: "business-law"
category_ja: "ビジネス法務"
title: "付随的契約——主契約に併存する補助的な合意"
as_of: "2026-06-27"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/business-law
  - knowledge/verified
aliases:
  - "付随的契約——主契約に併存する補助的な合意"
---

# 付随的契約——主契約に併存する補助的な合意

> [!info] コレクション: [[学術概念]] ・ 区分: ビジネス法務 ・ asOf: 2026-06-27 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
付随的契約（collateral contract、副次的契約）は、英米契約法における概念で、当事者が「主契約（main contract）」を締結することを誘引し、あるいはそれに付随して、主契約とは別個に成立する補助的な契約をいう。典型的には、一方の当事者が口頭で行った保証や言明が、後に締結される書面の主契約には盛り込まれなかった場合に問題となる。付随的契約の理論は、こうした主契約の外にある約束に、独立した契約としての拘束力を与える法的構成である。古典的な例として、英国の判例シャンクリン埠頭事件（Shanklin Pier v Detel Products, 1951）がある。塗料メーカーが「自社の塗料は埠頭に7年もつ」と埠頭所有者に保証し、それを信じた所有者が、自らの請負業者にその塗料を使うよう指示させた。所有者と塗料メーカーの間には主契約（塗料の売買契約は請負業者とメーカーの間にあった）がなかったが、裁判所は、メーカーの保証を「付随的契約」として認め、所有者が請負業者に当該塗料を使わせるという行為を約因（consideration、対価）として、メーカーは保証に法的責任を負うと判断した。付随的契約が法理として重要なのは、二つの障壁を乗り越える機能を持つからである。第一に、書面契約の内容を口頭の証拠で覆すことを禁じる「口頭証拠排除法則（parol evidence rule）」の例外として、書面に書かれなかった口頭の保証を、別個の付随的契約として救済しうる。第二に、契約当事者でない者は契約上の権利を主張できないとする「契約の相対効（privity of contract）」を、当事者間の付随的契約を観念することで回避しうる。ただし、付随的契約が認められるには、その言明が単なる意見や売り込み（puff）ではなく、契約的拘束の意図をもって行われた約束（promissory）であること、そして主契約の締結など独立した約因が存在することが必要とされ、裁判所はこれを慎重に判断する（Heilbut, Symons & Co v Buckleton, 1913）。付随的契約は、書面契約の硬直性を補い、契約交渉の過程でなされた重要な保証に拘束力を与える、契約法上の柔軟な救済法理である。

## 提唱者・初出
ロード・モールトン ／ パトリック・アティヤ ／ ガンター・トライテル ／ フレデリック・ポロック

## 出典
- [Treitel, G. H. The Law of Contract — Sweet & Maxwell（付随的契約の体系的解説）](https://www.worldcat.org/oclc/872617915) `リファレンス`
- [Wikipedia: Collateral contract — parol evidence, privity, Shanklin Pier v Detel](https://en.wikipedia.org/wiki/Collateral_contract) `リファレンス`

## 関連概念
- [[bizlaw-adequate-consideration|契約法における約因の法理と比較法的考察]] — 同じ思想家
- [[bizlaw-nuisance|ニューサンス（生活妨害）——土地の使用収益に対する不当な干渉の不法行為]] — 同じ思想家
- [[bizlaw-quasi-contract|準契約——合意なき法定の債務による不当利得の回復]] — 同じ思想家
- [[bizlaw-trespass-to-land|土地不法侵入——占有地への直接的な侵入を損害の証明なしに保護する不法行為]] — 同じ思想家
- [[bizlaw-vicarious-liability|代位責任（使用者責任）——被用者の不法行為について使用者が負う責任]] — 同じ思想家
- [[bizlaw-contractual-estoppel|契約上の禁反言——合意された前提を後から否定できない]] — 同じ思想家
- [[bizlaw-letter-of-comfort|念書（レター・オブ・コンフォート）——保証に至らない安心の供与]] — 同じ思想家
- [[bizlaw-no-oral-modification-rock|口頭変更禁止条項——書面によらない契約変更を封じる約定]] — 同じ思想家
- [[bizlaw-non-est-factum|ノン・エスト・ファクトゥム——署名した文書が本質的に別物だったときの抗弁]] — 同じ思想家
- [[bizlaw-rectification-equity|更正（レクティフィケーション）——合意と食い違う契約書を裁判所が訂正する]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
