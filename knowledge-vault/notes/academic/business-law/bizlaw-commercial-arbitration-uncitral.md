---
collection: academic
id: bizlaw-commercial-arbitration-uncitral
category: "business-law"
category_ja: "ビジネス法務"
title: "国際商事仲裁とUNCITRALモデル法・ニューヨーク条約"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/business-law
  - knowledge/verified
aliases:
  - "国際商事仲裁とUNCITRALモデル法・ニューヨーク条約"
---

# 国際商事仲裁とUNCITRALモデル法・ニューヨーク条約

> [!info] コレクション: [[学術概念]] ・ 区分: ビジネス法務 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
国際商事仲裁（international commercial arbitration）は、国境をまたぐビジネス紛争を国家裁判所によらず私的な仲裁廷で解決する制度であり、1958年のニューヨーク条約（外国仲裁判断の承認及び執行に関する条約）が166か国以上に署名されたことで国際的実効性を確立した。UNCITRALモデル仲裁法（1985年採択、2006年改正）は、仲裁合意の有効性・仲裁廷の構成・手続の公正性・判断の取消事由を統一的に規律し、多くの国の国内立法の基礎となっている。ICC・LCIA・SIAC・JCAAなど主要機関がそれぞれの規則を定めるほか、ICSDが投資仲裁を担う。「手続の効率性と公正性の確保」「第三者資金提供（third-party funding）の透明性」「機密保持（confidentiality）とAmicus curiae参加」「仲裁地選択とその影響」が現代的論点として議論されている。

## 提唱者・初出
ゲーリー・ボーン ／ ルシア・バーチ ／ ヴァン・ホーテン ／ 小山田孝子

## 出典
- [Born, G. (2021). International Commercial Arbitration — Kluwer Law International](https://doi.org/10.1017/9781108659055) `学術`
- [Wikipedia: UNCITRAL Model Law — international arbitration](https://en.wikipedia.org/wiki/UNCITRAL_Model_Law_on_International_Commercial_Arbitration) `リファレンス`

## 関連概念
- [[bizlaw-international-commercial-arbitration|国際商事仲裁とニューヨーク条約の執行規範]] — 語彙が近い
- [[bizlaw-international-arbitration-uncitral|国際商事仲裁——UNCITRAL規則とニューヨーク条約による紛争解決の法的枠組み]] — 語彙が近い
- [[bizlaw-arbitration-clause|仲裁条項——紛争を裁判ではなく仲裁で解決する合意]] — 同じ思想家
- [[bizlaw-arbitration-adr|仲裁・代替的紛争解決（ADR）――私的裁定・調停・交渉による司法外紛争処理の理論と国際制度]] — 同じ思想家
- [[bizlaw-commercial-arbitration-icc|ICC国際商事仲裁——仲裁条項の設計と仲裁判断の執行]] — 同じ思想家
- [[bizlaw-international-commercial-arbitration-born|国際商事仲裁——ニューヨーク条約体制と仲裁手続法]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
