---
collection: academic
id: bizlaw-commercial-impracticability-ucc
category: "business-law"
category_ja: "ビジネス法務"
title: "商事上の履行困難性——UCC第2-615条と予見不能なリスクの配分"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/business-law
  - knowledge/verified
aliases:
  - "商事上の履行困難性——UCC第2-615条と予見不能なリスクの配分"
---

# 商事上の履行困難性——UCC第2-615条と予見不能なリスクの配分

> [!info] コレクション: [[学術概念]] ・ 区分: ビジネス法務 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
商事上の履行困難性（commercial impracticability）は、米国統一商事法典（UCC）第2-615条に規定された契約上の免責事由であり、予見不能な偶発事象の発生により履行が「実行不可能」（impracticable）となった場合に、売主の引渡義務を免除する法理である。英国法の契約目的の達成不能（frustration）と類似するが、UCC第2-615条は履行の物理的不能に限らず、費用の著しい増大による経済的困難性をも射程に含む点でより広い適用範囲を持つ。リステイトメント（第二次）契約法261条も同様の法理を規定する。1970年代の石油危機に伴う一連の判例（アルコア対エセックス・グループ事件等）で商事上の履行困難性の射程が争われ、単なる費用の増大ではなく契約の基本的前提の崩壊が要求されることが判例上確立された。ポスナーとローゼンフィールドはこの法理を効率的リスク配分の観点から経済分析し、より低いコストでリスクを負担できる当事者に損失を配分すべきであると論じた。

## 提唱者・初出
リチャード・ポスナー ／ アンドリュー・ローゼンフィールド ／ アラン・ファンズワース ／ ジョン・ホワイト

## 出典
- [Posner, R. A. & Rosenfield, A. M. (1977) Impossibility and Related Doctrines in Contract Law — Journal of Legal Studies](https://doi.org/10.1086/467163) `学術`
- [UCC Section 2-615: Excuse by Failure of Presupposed Conditions — Cornell Law Institute](https://www.law.cornell.edu/ucc/2/2-615) `公的`
- [Wikipedia: Impracticability — commercial excuse doctrine in contract law overview](https://en.wikipedia.org/wiki/Impracticability) `リファレンス`

## 関連概念
- [[bizlaw-commercial-impracticability|商業的実行不能の法理]] — 語彙が近い
- [[bizlaw-anticipatory-breach-hochster|履行期前の契約違反——ホクスター対デ・ラ・トゥール事件と予防的救済]] — 同じ思想家
- [[bizlaw-dragnet-clause-doctrine|包括担保条項の法理——将来債務を含む担保権の有効性と限界]] — 同じ思想家
- [[bizlaw-penalty-clause-liquidated-damages|違約金条項と損害賠償額の予定——ペナルティ法理による司法的統制]] — 同分野の近傍
- [[bizlaw-commercial-frustration|契約の挫折法理（履行不能による契約解消）]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
