---
collection: academic
id: bizlaw-corporate-social-responsibility-law
category: "business-law"
category_ja: "ビジネス法務"
title: "CSRの法的規制——企業の社会的責任の自主規制から強制開示・デューデリジェンス義務への展開"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/business-law
  - knowledge/verified
aliases:
  - "CSRの法的規制——企業の社会的責任の自主規制から強制開示・デューデリジェンス義務への展開"
---

# CSRの法的規制——企業の社会的責任の自主規制から強制開示・デューデリジェンス義務への展開

> [!info] コレクション: [[学術概念]] ・ 区分: ビジネス法務 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
企業の社会的責任（Corporate Social Responsibility: CSR）は，かつては企業の自主的取り組みとして位置づけられていたが，2010年代以降，各国で法的義務化・強制開示化が加速している。英国現代奴隷法（2015年）・仏デューデリジェンス法（2017年）・EU企業持続可能性デューデリジェンス指令（CSDDD 2024年）は，大企業にサプライチェーン全体の人権・環境リスクの特定・防止・対処を義務付ける「義務的デューデリジェンス」の枠組みを構築した。日本でも責任あるサプライチェーン等における人権尊重のためのガイドライン（2022年）が策定されたが，法的義務化には至っていない。EU CSRD（企業持続可能性報告指令）は大規模企業にESG情報の詳細開示を義務付け，EUタクソノミー規制と連動する。「グリーンウォッシング防止規制」もCSR開示の信頼性担保として立法化が進む。CSR規制の強化はコンプライアンスコストを増大させる一方，投資家・消費者への情報対称性を高める。

## 提唱者・初出
EU欧州委員会 ／ 国連グローバル・コンパクト ／ ジョン・ラギー ／ 外務省（日本）

## 出典
- [EU Commission (2022) Corporate Sustainability Due Diligence Directive — CSDDD](https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:52022PC0071) `公的`
- [Wikipedia: Corporate social responsibility — law and regulation](https://en.wikipedia.org/wiki/Corporate_social_responsibility) `リファレンス`

## 関連概念
- [[bizlaw-international-human-rights-due-diligence|国際人権デューデリジェンス（UNGPs）の法的義務化と企業責任]] — 語彙が近い
- [[bizlaw-corporate-social-responsibility-directive|企業持続可能性デューディリジェンス指令——サプライチェーンにおける人権・環境配慮義務]] — 語彙が近い
- [[bizlaw-ungps-business-human-rights|ビジネスと人権に関する指導原則（UNGPs）]] — 同じ思想家
- [[bizlaw-environmental-law-regulation|環境法と持続可能性規制の法律体系——ESG義務化と企業責任]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
