---
collection: academic
id: bizlaw-estoppel
category: "business-law"
category_ja: "ビジネス法務"
title: "禁反言（エストッペル）——先行の言動と矛盾する主張を信頼保護のため封じる法理"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/business-law
  - knowledge/verified
aliases:
  - "禁反言（エストッペル）——先行の言動と矛盾する主張を信頼保護のため封じる法理"
---

# 禁反言（エストッペル）——先行の言動と矛盾する主張を信頼保護のため封じる法理

> [!info] コレクション: [[学術概念]] ・ 区分: ビジネス法務 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
禁反言（estoppel、エストッペル）は、英米法の基本的な準則で、ある者が自らの先行する言明・表示・行為、あるいはとった立場と矛盾する主張をすることを、相手方がそれを信頼して行動した場合に、衡平と信義誠実の見地から禁じる（estop する）法理である。その目的は、人の言動の一貫性に対する正当な信頼を保護し、矛盾した態度の変更によって相手に不当な損害を与えることを防ぐことにある。禁反言には複数の類型がある。過去または現在の事実についての表示を信頼させた者が後にその事実を否認することを封じる「表示による禁反言（estoppel by representation）」、約因を欠く約束であっても被約束者がそれを信頼して不利益を被った場合に約束を強制しうる「約束的禁反言（promissory estoppel）」、土地等の権利に関する期待を生じさせ信頼させた場合の「所有権的禁反言（proprietary estoppel）」、当事者の行動から生じる「行為による禁反言」などである。とりわけ約束的禁反言は、英国のセントラル・ロンドン・プロパティ対ハイ・トリーズ・ハウス事件でデニング卿が展開し、厳格な約因法理を信頼保護の観点から修正する重要な役割を果たした。禁反言は、形式的な権利主張が信頼を裏切り不正義をもたらす場面で、衡平が介入して結果を是正する柔軟な装置として機能する。

## 提唱者・初出
デニング卿 ／ エドワード・コーク ／ サミュエル・ウィリストン ／ アーサー・コービン

## 出典
- [Cornell LII Wex: Estoppel — reliance, representation, promissory/proprietary estoppel](https://www.law.cornell.edu/wex/estoppel) `リファレンス`
- [Wikipedia: Estoppel — High Trees, promissory estoppel, reliance, good faith, types](https://en.wikipedia.org/wiki/Estoppel) `リファレンス`

## 関連概念
- [[bizlaw-estoppel-by-representation|表示による禁反言——信頼の保護と矛盾行為の禁止]] — 語彙が近い
- [[bizlaw-consideration-doctrine|約因の法理——約束が拘束力を持つために要する交換的対価]] — 語彙が近い
- [[bizlaw-promissory-estoppel|約束的禁反言（プロミッサリー・エストッペル）]] — 語彙が近い
- [[bizlaw-illegality-contract|違法な契約——法令や公序に反する契約の執行不能性]] — 同じ思想家
- [[bizlaw-mistake-contract|契約の錯誤——合意の基礎をなす事実の誤認による契約の無効・取消]] — 同じ思想家
- [[bizlaw-penalty-clause|違約罰条項——損害填補を超えた懲罰的支払条項の執行不能性]] — 同じ思想家
- [[bizlaw-restraint-of-trade|取引制限の法理——営業の自由を制約する条項の合理性審査]] — 同じ思想家
- [[bizlaw-capacity-contract|契約締結能力——拘束力ある契約を結ぶための法的な資格]] — 同じ思想家
- [[bizlaw-caveat-emptor|買主注意の原則——売買における危険の買主負担とその現代的後退]] — 同じ思想家
- [[bizlaw-condition-precedent|停止条件（先行条件）——履行義務の発生を将来の事実にかからしめる契約条件]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
