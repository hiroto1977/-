---
collection: academic
id: bizlaw-learned-hand-formula
category: "business-law"
category_ja: "ビジネス法務"
title: "ラーニッド・ハンドの定式——過失を費用と確率で測る"
as_of: "2026-06-27"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/business-law
  - knowledge/verified
aliases:
  - "ラーニッド・ハンドの定式——過失を費用と確率で測る"
---

# ラーニッド・ハンドの定式——過失を費用と確率で測る

> [!info] コレクション: [[学術概念]] ・ 区分: ビジネス法務 ・ asOf: 2026-06-27 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
ラーニッド・ハンドの定式（Learned Hand formula、過失算定式）は、アメリカの裁判官ラーニッド・ハンドが1947年の合衆国対キャロル・タウイング社事件（United States v. Carroll Towing Co.）で示した、過失（negligence）の有無を判断する基準で、不法行為法と「法と経済学」の礎石となった。ハンドは、ある予防措置を怠ったことが過失にあたるかを、三つの量の比較で表した。予防にかかる負担・費用を B（burden）、その措置を怠ったときに損害が生じる確率を P（probability）、生じる損害の大きさを L（loss）とすると、B が P×L より小さいとき、すなわち予防の費用が、防げたはずの「期待損害」より小さいときに、措置を怠ることは過失となる（B＜P×L）。逆に、予防の費用が期待損害を上回るなら、措置を取らないことは合理的で過失ではない。この定式は、不法行為法の「合理的な注意」という曖昧な基準を、費用と便益の比較として捉え直した。グイド・カラブレイジの『事故の費用』やリチャード・ポズナーの過失の経済理論を通じて、事故法の経済分析の中核概念となった——効率的な注意水準とは、事故による損害と、事故を避けるための費用との合計を最小にする水準である、という考えである。批判もある。現実には P や L を数量化するのは難しく、裁判所が実際に計算することはまれで、分配的正義や匡正的正義の観点を軽視する、といった点である。それでもハンドの定式は、過失基準の意味と、法と経済学を理解するための試金石であり続けている。

## 提唱者・初出
ラーニッド・ハンド判事 ／ リチャード・ポズナー ／ グイド・カラブレイジ ／ ロナルド・コース

## 出典
- [Wikipedia: Calculus of negligence (Hand formula) — B < P·L, United States v. Carroll Towing Co. (1947), economic analysis of tort](https://en.wikipedia.org/wiki/Calculus_of_negligence) `リファレンス`
- [Cornell LII (Wex): Negligence — reasonable care / breach standard and the Learned Hand cost-benefit formulation of duty](https://www.law.cornell.edu/wex/negligence) `リファレンス`

## 関連概念
- [[mgmt-transaction-cost-economics|取引コスト理論（取引費用経済学）]] — 同じ思想家
- [[bizlaw-economic-analysis-of-law|法の経済分析（Economic Analysis of Law）：ポズナーの法と経済学派]] — 同じ思想家
- [[bizlaw-tort-law|不法行為法：過失・厳格責任・経済分析による民事損害賠償の理論体系]] — 同じ思想家
- [[econ-coase-theorem|コースの定理（Coase theorem）]] — 同じ思想家
- [[bizlaw-competition-law|競争法・独占禁止法：市場支配力の規制と反トラスト法の経済学的分析]] — 同じ思想家
- [[bizlaw-contract-theory|契約理論：情報の非対称性とインセンティブ設計の経済・法学的分析]] — 同じ思想家
- [[econ-environmental-economics|環境経済学：外部性の内部化と持続可能な経済成長の理論的枠組み]] — 同じ思想家
- [[econ-cap-and-trade-dales|排出量取引——総量規制と取引可能な排出権による費用最小の汚染削減]] — 同じ思想家
- [[econ-coase-conjecture|コースの推測（耐久財独占）]] — 同じ思想家
- [[econ-durable-goods-monopoly|耐久財独占とコースの推測——独占者が自らの将来の値下げと競争する]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
