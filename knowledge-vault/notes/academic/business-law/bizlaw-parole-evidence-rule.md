---
collection: academic
id: bizlaw-parole-evidence-rule
category: "business-law"
category_ja: "ビジネス法務"
title: "口頭証拠排除法則"
as_of: "2026-06"
source_count: 4
authoritative: true
tags:
  - collection/academic
  - academic/business-law
  - knowledge/verified
aliases:
  - "口頭証拠排除法則"
---

# 口頭証拠排除法則

> [!info] コレクション: [[学術概念]] ・ 区分: ビジネス法務 ・ asOf: 2026-06 ・ 出典: 4件（うち権威ある出典 ✓）

## 概要
口頭証拠排除法則（Parol Evidence Rule）とは、当事者が最終的な書面契約を締結した場合、その契約内容を変更・矛盾・補完するような契約締結前または締結時の口頭合意は証拠として許容されないとするコモンロー上の原則である。ただし、詐欺・錯誤・強迫といった契約の効力を争う事由の立証、契約条件の曖昧さを解消するための解釈、または書面が「部分的統合」（partial integration）にとどまる場合には、例外的に口頭証拠の提出が認められる。米国統一商事法典（UCC）§ 2-202は動産売買契約に同法則を成文化したものであり、書面が最終的かつ完全な統合契約でない限り、書面と矛盾しない追加的条件を口頭証拠によって証明することを許容している。同法則の解釈をめぐっては、サミュエル・ウィリストンが提唱した文書の四隅のみに立脚する厳格な形式主義的アプローチと、アーサー・リントン・コービンが主張した契約締結の文脈・目的・慣行を広く考慮する目的論的アプローチとの間に学説上の対立が存在した。今日においても同法則は契約法の基本原則として機能しているが、例外の拡大と合理的期待保護の観点から実質的な制約を受けており、商事実務における交渉経緯や業界慣行の参照可能性がとりわけ拡大している。

## 提唱者・初出
Samuel Williston ／ Arthur Linton Corbin ／ Karl Nickerson Llewellyn

## 出典
- [Cornell Law School LII — Parol Evidence Rule](https://www.law.cornell.edu/wex/parol_evidence_rule) `リファレンス`
- [UCC § 2-202: Parol or Extrinsic Evidence (Cornell LII)](https://www.law.cornell.edu/ucc/2/2-202) `リファレンス`
- [American Law Institute — Restatement (Second) of Contracts](https://www.ali.org/publications/show/contracts/) `リファレンス`
- [Uniform Law Commission — Uniform Commercial Code](https://www.uniformlaws.org/acts/ucc) `公的`

## 関連概念
- [[bizlaw-statute-of-frauds|詐欺防止法]] — 同じ思想家
- [[bizlaw-mailbox-rule|発信主義（郵便箱の法則）]] — 同じ思想家
- [[bizlaw-mailbox-rule-acceptance|郵便箱ルール（発信主義）]] — 同じ思想家
- [[bizlaw-battle-of-the-forms|書式の戦い]] — 同じ思想家
- [[bizlaw-commercial-impracticability|商業的実行不能の法理]] — 同じ思想家
- [[bizlaw-contractual-unconscionability|契約の非良心性法理]] — 同じ思想家
- [[bizlaw-election-of-remedies|救済手段の選択法理（エレクション・オブ・レメディーズ）]] — 同じ思想家
- [[bizlaw-force-majeure-doctrine|不可抗力の法理]] — 同じ思想家
- [[bizlaw-holder-in-due-course-doctrine|正当所持人の法理]] — 同じ思想家
- [[bizlaw-implied-warranty-of-merchantability|商品性の黙示保証]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
