---
collection: academic
id: bizlaw-penalty-clause-comparative
category: "business-law"
category_ja: "ビジネス法務"
title: "違約金条項・損害賠償予定と比較契約法"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/business-law
  - knowledge/verified
aliases:
  - "違約金条項・損害賠償予定と比較契約法"
---

# 違約金条項・損害賠償予定と比較契約法

> [!info] コレクション: [[学術概念]] ・ 区分: ビジネス法務 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
違約金条項（penalty clause / liquidated damages clause）は、契約違反が生じた場合に支払うべき金額を予め定める条項であり、英米法と大陸法で伝統的に異なるアプローチをとってきた。英国判例法は長年「ペナルティ条項（懲罰的）」を無効とし「損害賠償予定（genuine pre-estimate）」のみを有効とする区別を維持したが、2015年のCavendish/ParkingEye連邦最高裁判決は「正当な利益（legitimate interest）の保護」という新基準を導入し実質的に運用を緩和した。フランス民法（2016年改正）・ドイツ民法・日本民法（420条）はそれぞれ異なるアプローチで過大な違約金を減額または無効とする権限を裁判所に付与する。UNIDROITの国際商事契約原則（PICC）7.4.13条はアメリカン・ロー・インスティテュートの提案とともに過大ペナルティの合理的減額を認めており、国際仲裁でも参照される。

## 提唱者・初出
ポール・クレイグ ／ ユルゲン・バセドウ ／ ロジャー・クラーク ／ 内田貴

## 出典
- [Treitel, G.H. (2015). The Law of Contract — Sweet & Maxwell](https://doi.org/10.1093/law/9780198747109.001.0001) `学術`
- [Wikipedia: Penalty clause — contract law comparison](https://en.wikipedia.org/wiki/Penalty_clause) `リファレンス`

## 関連概念
- [[bizlaw-liquidated-damages-clause|損害賠償額の予定条項（清算損害賠償条項）]] — 語彙が近い
- [[bizlaw-penalty-clause-liquidated-damages|違約金条項と損害賠償額の予定——ペナルティ法理による司法的統制]] — 語彙が近い
- [[infosoc-dataveillance|データヴェイランス（データ監視）]] — 同じ思想家
- [[bizlaw-de-facto-corporation|事実上の会社——設立手続きに瑕疵があっても会社として扱う法理]] — 同じ思想家
- [[bizlaw-derivative-action-shareholder|株主代表訴訟（代表訴訟制度）]] — 同じ思想家
- [[bizlaw-shareholders-derivative-suit|株主代表訴訟——会社に代わる取締役責任追及の法的仕組み]] — 同じ思想家
- [[bizlaw-standstill-agreement-ma|スタンドスティル協定（M&A）]] — 同じ思想家
- [[infosoc-dataveillance-clarke|データ監視（デイタヴェイランス）——個人データを通じた体系的な監視]] — 同じ思想家
- [[bizlaw-williams-v-roffey|ウィリアムズ対ロフィー・ブラザーズ——「実際的利益」と約因の修正]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
