---
collection: academic
id: bizlaw-privacy-law-gdpr-compliance
category: "business-law"
category_ja: "ビジネス法務"
title: "プライバシー法とGDPRコンプライアンス——個人データ保護の国際法制"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/business-law
  - knowledge/verified
aliases:
  - "プライバシー法とGDPRコンプライアンス——個人データ保護の国際法制"
---

# プライバシー法とGDPRコンプライアンス——個人データ保護の国際法制

> [!info] コレクション: [[学術概念]] ・ 区分: ビジネス法務 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
プライバシー法（privacy law）は個人の情報自己決定権と個人データの収集・処理・移転に関する権利を規律する法律分野であり、EU一般データ保護規則（GDPR、2018年5月施行）が世界のデータ保護規制の事実上の標準（de facto standard）として機能する。GDPRは目的制限・データ最小化・正確性・保存制限・完全性・機密性の6原則のもと、データ主体の同意・正当利益・契約履行等の適法処理根拠を要求し、違反に対して全世界年間売上高の4%または2,000万ユーロのいずれか高い方を上限とする制裁金を定める。移転規制（第5章）では標準契約条項（SCC）・拘束的企業準則（BCR）・適切性認定（adequacy decision）が第三国へのデータ移転の法的根拠となり、2023年のEU＝米国データプライバシーフレームワーク（DPF）がシュレムス判決後の法的空白を埋めた。日本は個人情報保護法（改正2022年施行）がGDPRと実質的に同等として一般的適切性評価を受け、日EU相互認定体制が構築されている。

## 提唱者・初出
ヴォルフガング・ホフマン＝リーム ／ ポール・シュワルツ ／ マックス・シュレムス ／ 宍戸常寿

## 出典
- [European Union (2018) General Data Protection Regulation (GDPR) — full text with recitals](https://gdpr-info.eu/) `公的`
- [Wikipedia: GDPR — data protection principles, rights, SCCs, adequacy, DPF, Schrems](https://en.wikipedia.org/wiki/General_Data_Protection_Regulation) `リファレンス`

## 関連概念
- [[bizlaw-data-protection-gdpr-adequacy|GDPRと十分性認定——個人データの越境移転とプライバシー保護の国際的枠組み]] — 語彙が近い
- [[bizlaw-data-protection-gdpr-principles|データ保護法とGDPR原則——EU一般データ保護規則の七原則と越境データ流通]] — 同じ思想家
- [[infosoc-gdpr-right-erasure|GDPRと忘れられる権利（消去権）]] — 出典を共有
- [[infosoc-privacy-by-design|プライバシー・バイ・デザイン]] — 出典を共有
- [[infosoc-data-portability|データポータビリティの権利（GDPR20条）]] — 出典を共有
- [[infosoc-digital-sovereignty-theory|デジタル主権論]] — 出典を共有

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
