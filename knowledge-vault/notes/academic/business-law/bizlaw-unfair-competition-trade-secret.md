---
collection: academic
id: bizlaw-unfair-competition-trade-secret
category: "business-law"
category_ja: "ビジネス法務"
title: "不正競争防止と営業秘密保護の比較法制"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/business-law
  - knowledge/verified
aliases:
  - "不正競争防止と営業秘密保護の比較法制"
---

# 不正競争防止と営業秘密保護の比較法制

> [!info] コレクション: [[学術概念]] ・ 区分: ビジネス法務 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
営業秘密（trade secret）は、非公開の情報であって競争上の価値を有し、秘密として保持するための合理的措置が講じられているものを指す。米国では2016年のDTSA（Defend Trade Secrets Act）が初の連邦民事訴訟制度を創設し、EUは2016年の営業秘密指令（2016/943）で欧州全域に統一保護を導入した。日本では不正競争防止法（2015年改正以降）が刑事罰強化・海外侵害への適用拡大・転得者の責任を整備した。「不正取得・不正使用・不正開示」の三類型に対する差止・損害賠償・刑事罰が主要法的手段である。サプライチェーンの国際化・元従業員による海外持ち出し・サイバー産業スパイ・AIが学習したデータの営業秘密該当性など、デジタル環境下での新たな保護課題が増大している。

## 提唱者・初出
マーク・レミュー ／ クリストフ・ロスノット ／ リンダ・トルマン ／ 茶園成樹

## 出典
- [Almeling, D.S. (2012). Trade Secret Law — Comparative Perspectives — Edward Elgar](https://doi.org/10.4337/9781848445376) `学術`
- [Wikipedia: Trade secret — unfair competition law](https://en.wikipedia.org/wiki/Trade_secret) `リファレンス`

## 関連概念
- [[bizlaw-trade-secret-misappropriation|営業秘密の不正取得（トレード・シークレット法）]] — 語彙が近い
- [[bizlaw-trade-secret-protection-trips|営業秘密保護法——不正競争防止と国際的調和の法的枠組み]] — 語彙が近い
- [[bizlaw-trade-secret-directive-eu|EU営業秘密指令——営業秘密保護の域内調和]] — 同分野の近傍
- [[bizlaw-trade-secret-protection-utsa|営業秘密保護法制——不正競争防止法とUTSAによる企業秘密の法的保護]] — 同分野の近傍
- [[bizlaw-data-governance-data-act|データガバナンス法制——EU Data Act・データ利活用促進法とデータ共有の権利・義務]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
