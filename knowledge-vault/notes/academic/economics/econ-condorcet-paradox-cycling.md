---
collection: academic
id: econ-condorcet-paradox-cycling
category: "economics"
category_ja: "経済学"
title: "コンドルセのパラドックス——多数決における選好の循環と社会的選択の不安定性"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/economics
  - knowledge/verified
aliases:
  - "コンドルセのパラドックス——多数決における選好の循環と社会的選択の不安定性"
---

# コンドルセのパラドックス——多数決における選好の循環と社会的選択の不安定性

> [!info] コレクション: [[学術概念]] ・ 区分: 経済学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
コンドルセのパラドックス（Condorcet paradox / voting paradox）は、18世紀フランスの数学者マルキ・ド・コンドルセが1785年に発見した集合的意思決定の根本的問題で、個々の投票者が合理的で推移的な選好順序を持っていても、多数決ルールによる集団的選好が推移性を満たさず循環を生じうることを示す。三人の投票者A・B・Cが三つの選択肢x・y・zについて、Aがx>y>z、Bがy>z>x、Cがz>x>yの選好を持つ場合、一対比較では xがyに、yがzに、zがxに多数決で勝利し、集団的選好はx>y>z>x…と循環する。この結果、明確な「多数の意思」が存在しなくなり、議事日程の操作や戦略的投票が結果を左右する余地が生じる。ケネス・アローは1951年にこの問題を一般化し、合理性・パレート原理・独立性・非独裁制の四条件を同時に満たす社会的厚生関数は存在しないという「不可能性定理」を証明し、社会選択理論の基礎を確立した。

## 提唱者・初出
マルキ・ド・コンドルセ ／ ケネス・アロー ／ ダンカン・ブラック ／ アマルティア・セン

## 出典
- [Arrow, K.J. (1950) A Difficulty in the Concept of Social Welfare — Journal of Political Economy 58(4)](https://doi.org/10.2307/1907187) `学術`
- [Wikipedia: Condorcet Paradox — voting cycle and intransitivity of majority rule](https://en.wikipedia.org/wiki/Condorcet_paradox) `リファレンス`
- [Sen, A.K. (1970) Collective Choice and Social Welfare — Holden-Day（Holden-Day, 1970, ISBN 978-0-8162-7765-0）](https://search.worldcat.org/title/140528) `リファレンス`

## 関連概念
- [[econ-arrow-impossibility|アローの不可能性定理]] — 語彙が近い
- [[econ-condorcet-jury-theorem|コンドルセの陪審定理（Condorcet Jury Theorem）]] — 語彙が近い
- [[econ-sen-paradox|センのリベラル・パラドックス（パレート派リベラルの不可能性）]] — 同じ思想家
- [[econ-welfare-theorems|厚生経済学の基本定理]] — 同じ思想家
- [[econ-welfare-economics|厚生経済学：社会的厚生と資源配分効率性の理論的分析]] — 同じ思想家
- [[econ-harsanyi-utilitarianism|ハルサニの功利主義（集計定理）——不偏な観察者と期待効用の和]] — 同じ思想家
- [[econ-median-voter|中位投票者定理]] — 同じ思想家
- [[econ-welfare-economics-bergson-samuelson|厚生経済学の基本定理——バーグソン＝サミュエルソン社会厚生関数とパレート効率性]] — 同じ思想家
- [[econ-capability-approach|潜在能力アプローチ（ケイパビリティ・アプローチ）]] — 同じ思想家
- [[econ-experience-curve-henderson|経験曲線——累積生産量の倍増ごとに単位費用が一定率で低下する]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
