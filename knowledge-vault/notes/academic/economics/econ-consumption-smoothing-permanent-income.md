---
collection: academic
id: econ-consumption-smoothing-permanent-income
category: "economics"
category_ja: "経済学"
title: "消費の平滑化——恒常所得仮説とライフサイクル仮説の統合的枠組み"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/economics
  - knowledge/verified
aliases:
  - "消費の平滑化——恒常所得仮説とライフサイクル仮説の統合的枠組み"
---

# 消費の平滑化——恒常所得仮説とライフサイクル仮説の統合的枠組み

> [!info] コレクション: [[学術概念]] ・ 区分: 経済学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
消費の平滑化（Consumption Smoothing）は、合理的な経済主体が一時的な所得変動に対して消費を安定的に維持しようとする行動パターンであり、フリードマン（1957）の恒常所得仮説とモディリアーニとブランバーグ（1954）のライフサイクル仮説が理論的基礎を構成する。恒常所得仮説は、消費が現在の所得ではなく生涯にわたる恒常所得（permanent income）に基づいて決定されると主張し、一時的な所得変動は貯蓄・借入で吸収される。ライフサイクル仮説は、個人が生涯の効用を最大化するように、勤労期に貯蓄し退職後に取り崩す消費パターンを予測する。ホール（1978）は合理的期待の下で消費がランダムウォークに従うことを示した。しかしキャンベルとマンキュー（1989）は、かなりの割合の消費者が流動性制約に直面し現在所得に依存して消費する「ルール・オブ・サム消費者」の存在を実証した。デートンの逆説（1991）は、消費の変動が理論予測より大きい（過剰感応性）ことを指摘し、予備的貯蓄動機やバッファストック行動の研究を促進した。

## 提唱者・初出
ミルトン・フリードマン ／ フランコ・モディリアーニ ／ ロバート・ホール ／ アンガス・ディートン ／ ジョン・キャンベル

## 出典
- [Hall (1978) Stochastic Implications of the Life Cycle–Permanent Income Hypothesis — JPE](https://doi.org/10.1086/260790) `学術`
- [Deaton (1991) Saving and Liquidity Constraints — Econometrica 59(5), 1221–1248](https://doi.org/10.2307/2938366) `学術`
- [Wikipedia: Consumption smoothing — 恒常所得仮説とライフサイクル仮説の概説](https://en.wikipedia.org/wiki/Consumption_smoothing) `リファレンス`

## 関連概念
- [[econ-precautionary-saving|予備的貯蓄]] — 語彙が近い
- [[econ-deaton-paradox|ディートンのパラドックス（消費の過剰平滑性）]] — 語彙が近い
- [[econ-permanent-income-hypothesis-friedman|恒常所得仮説——フリードマンの消費理論と生涯所得に基づく消費決定メカニズム]] — 語彙が近い
- [[mgmt-csr|企業の社会的責任（CSR）]] — 同じ思想家
- [[econ-friedman-rule|フリードマン・ルール（最適貨幣量）]] — 同じ思想家
- [[econ-helicopter-money|ヘリコプターマネー]] — 同じ思想家
- [[econ-life-cycle-hypothesis-modigliani|ライフサイクル仮説——生涯を通じた消費平準化と貯蓄の動学]] — 同じ思想家
- [[econ-modigliani-cohn-inflation-illusion|モディリアーニ＝コーンのインフレ錯覚——株式市場はインフレで株を過小評価する]] — 同じ思想家
- [[econ-natural-rate-friedman-phelps|自然失業率仮説——インフレと失業の長期トレードオフの不在]] — 同じ思想家
- [[econ-natural-rate-unemployment|自然失業率（NAIRU）]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
