---
collection: academic
id: econ-credit-cycle-minsky-financial-fragility
category: "economics"
category_ja: "経済学"
title: "ミンスキーの金融不安定性仮説——信用サイクルと「ミンスキー・モーメント」の経済学"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/economics
  - knowledge/verified
aliases:
  - "ミンスキーの金融不安定性仮説——信用サイクルと「ミンスキー・モーメント」の経済学"
---

# ミンスキーの金融不安定性仮説——信用サイクルと「ミンスキー・モーメント」の経済学

> [!info] コレクション: [[学術概念]] ・ 区分: 経済学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
ハイマン・ミンスキーが提唱した金融不安定性仮説は、資本主義経済においては長期安定期ほど金融システムが脆弱化するという逆説的ダイナミクスを説明する。好況期に企業・家計・金融機関がリスク許容度を高め、「ヘッジ金融」（キャッシュフローで元利を賄える状態）→「投機的金融」（利払いのみ賄える）→「ポンツィ金融」（借換えでしか存続できない）の順に積み上がる。この過程で信用膨張・資産価格高騰が進み、やがてごく小さなショックで急激な資産売却・信用収縮が連鎖する「ミンスキー・モーメント」を迎える。リーマン・ショック（2008年）を機に「ミンスキー」は一般にも普及した用語となり、マクロプルーデンス政策やシステミックリスク監視の理論的根拠として再評価されている。

## 提唱者・初出
ハイマン・ミンスキー ／ チャールズ・キンドルバーガー ／ ポール・マッカリー ／ スティーブ・キーン

## 出典
- [Minsky (1986) Stabilizing an Unstable Economy — Yale University Press](https://doi.org/10.1057/9780230391598) `学術`
- [Kindleberger (1978) Manias, Panics, and Crashes — Basic Books](https://doi.org/10.2307/2077948) `学術`
- [Wikipedia: Minsky moment — ミンスキー・モーメントと金融危機](https://en.wikipedia.org/wiki/Minsky_moment) `リファレンス`

## 関連概念
- [[econ-asset-price-bubbles-kindleberger-minsky|資産価格バブル——キンドルバーガーとミンスキーの信用サイクルと崩壊のモデル]] — 同じ思想家
- [[econ-speculative-bubbles-kindleberger|投機的バブルの解剖——キンドルバーガーのマニア＝パニック＝崩壊モデル]] — 同じ思想家
- [[econ-corridor-of-stability-leijonhufvud|安定性の回廊——レイヨンフーヴッドのマクロ経済不均衡分析]] — 同じ思想家
- [[econ-corridor-of-stability-theory|安定の回廊理論]] — 同じ思想家
- [[econ-debt-deflation-fisher|負債デフレーション理論——過剰債務と物価下落の悪循環メカニズム]] — 同じ思想家
- [[econ-debt-deflation-fisher-irving|債務デフレーション論——フィッシャーの過剰債務螺旋と大恐慌の金融的説明]] — 同じ思想家
- [[econ-endogenous-money-post-keynesian-moore|内生的貨幣論——ムーアの信用貨幣観と中央銀行の利子率コントロール]] — 同じ思想家
- [[econ-endogenous-money-supply-moore|内生的貨幣供給論——ポストケインズ派の信用創造メカニズムと中央銀行の受動的役割]] — 同じ思想家
- [[econ-hot-money-flows-kindleberger|ホットマネーと国際資本移動——投機的資本フローの不安定化効果]] — 同じ思想家
- [[econ-minsky-moment|ミンスキー・モーメント（金融不安定性仮説）]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
