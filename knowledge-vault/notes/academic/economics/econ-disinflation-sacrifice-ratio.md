---
collection: academic
id: econ-disinflation-sacrifice-ratio
category: "economics"
category_ja: "経済学"
title: "犠牲率——ディスインフレーション政策における産出量コストの測定"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/economics
  - knowledge/verified
aliases:
  - "犠牲率——ディスインフレーション政策における産出量コストの測定"
---

# 犠牲率——ディスインフレーション政策における産出量コストの測定

> [!info] コレクション: [[学術概念]] ・ 区分: 経済学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
犠牲率（sacrifice ratio）は、インフレーションを1パーセントポイント低下させるために必要な実質GDP損失の累積値を測定する概念であり、金融引締め政策のコストを評価する中心的指標である。ローレンス・ボールが1994年に28カ国のディスインフレーション・エピソードを分析し、犠牲率は概ねGDPの1〜3パーセントの範囲にあること、急速なディスインフレーションの方が漸進的なそれよりも犠牲率が低い傾向にあること（「コールド・ターキー」仮説の支持）を示した。新古典派経済学では、合理的期待と完全な信認の下でディスインフレーションは無コストで達成可能とされるが、ニューケインジアンのフィリップス曲線モデルでは、名目価格・賃金の硬直性により短期的な産出量コストが生じることが予測される。犠牲率の大きさは、賃金・価格の硬直度、中央銀行の信認、ディスインフレーションの速度、為替相場制度、初期インフレ率の水準に依存する。

## 提唱者・初出
ローレンス・ボール ／ アーサー・オクン ／ トーマス・サージェント ／ N・グレゴリー・マンキュー

## 出典
- [Ball, L. (1994) What Determines the Sacrifice Ratio? — NBER Macroeconomics Annual 1993](https://doi.org/10.3386/w4306) `学術`
- [Wikipedia: Sacrifice Ratio — output cost of reducing inflation](https://en.wikipedia.org/wiki/Sacrifice_ratio) `リファレンス`
- [Ball, L. & Mankiw, N.G. (2002) The NAIRU in Theory and Practice — Journal of Economic Perspectives 16(4)](https://doi.org/10.1257/jep.16.4.157) `学術`

## 関連概念
- [[econ-near-rationality-akerlof-yellen|近似合理性とメニューコスト——僅かな最適化の怠りが大きな景気変動を生む]] — 同じ思想家
- [[econ-fiscal-dominance|財政支配（Fiscal Dominance）]] — 同じ思想家
- [[econ-hysteresis-macroeconomics|マクロ経済学的ヒステリシス（Macroeconomic Hysteresis）]] — 同じ思想家
- [[econ-policy-ineffectiveness|政策無効命題]] — 同じ思想家
- [[econ-sticky-information|粘着情報モデル]] — 同じ思想家
- [[econ-goodharts-law-lucas-critique|グッドハートの法則とルーカス批判——政策目標化された統計的関係の崩壊]] — 同じ思想家
- [[econ-hyperinflation-cagan|カガンのハイパーインフレーション・モデル——貨幣需要と期待が織りなす物価爆発]] — 同じ思想家
- [[econ-hysteresis-blanchard|履歴効果（ヒステリシス）——景気循環が潜在産出量に与える永続的影響]] — 同じ思想家
- [[econ-hysteresis-unemployment|失業のヒステリシス（Hysteresis in Unemployment）]] — 同じ思想家
- [[econ-new-keynesian|ニュー・ケインジアン経済学]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
