---
collection: academic
id: econ-dollar-auction
category: "economics"
category_ja: "経済学"
title: "ドル・オークション：エスカレーション・トラップのゲーム理論的モデル"
as_of: "2026-06-25"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/economics
  - knowledge/verified
aliases:
  - "ドル・オークション：エスカレーション・トラップのゲーム理論的モデル"
---

# ドル・オークション：エスカレーション・トラップのゲーム理論的モデル

> [!info] コレクション: [[学術概念]] ・ 区分: 経済学 ・ asOf: 2026-06-25 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
ドル・オークションは、マーティン・シュービック（1971）がJournal of Conflict Resolutionに発表したゲーム理論モデルで、合理的な個人が漸進的な意思決定を通じて非合理的なエスカレーションに陥る構造を示した。1ドル紙幣を最高額入札者に売却するが、第二位入札者も自身の入札額を支払わねばならないという規則により、参加者は損失回避のために入札額を釣り上げ続け、最終的な支払総額が賞品の価値を大幅に超過する。この「エスカレーション・トラップ」は、バリー・スタウ（1976）が組織行動論で「コミットメントのエスカレーション」として発展させ、失敗プロジェクトへの追加投資継続（サンクコスト効果との相互作用）を説明した。さらにブレゲ&スターン（1979）は国際紛争のエスカレーション分析に応用し、軍拡競争・貿易戦争・価格競争など多様な社会的ジレンマの構造的類似性を明らかにした。

## 提唱者・初出
マーティン・シュービック ／ バリー・スタウ ／ アラン・テガー

## 出典
- [Shubik, M. (1971) The Dollar Auction Game: A Paradox in Noncooperative Behavior and Escalation — Journal of Conflict Resolution](https://doi.org/10.1177/002200277101500111) `学術`
- [Staw, B.M. (1976) Knee-Deep in the Big Muddy: A Study of Escalating Commitment to a Chosen Course of Action — Organizational Behavior and Human Performance](https://doi.org/10.1016/0030-5073(76)90005-2) `学術`
- [Teger, A.I. (1980) Too Much Invested to Quit — Pergamon Press](https://shop.elsevier.com/books/too-much-invested-to-quit/teger/978-0-08-022995-9) `学術`

## 関連概念
- [[mgmt-escalation-of-commitment|コミットメントのエスカレーション]] — 同じ思想家
- [[econ-behavioral-game-theory|行動ゲーム理論：心理学的洞察と実験経済学を統合した戦略的相互作用における公平性・互恵性・限定合理性の研究]] — 同じ思想家
- [[mgmt-threat-rigidity-staw|脅威硬直効果——危機的状況が組織の情報処理と行動を制約するメカニズム]] — 同じ思想家
- [[econ-auction-theory|オークション理論]] — 同分野の近傍
- [[econ-keynesian-beauty-contest|ケインズの美人投票]] — 同分野の近傍
- [[econ-disinflation-sacrifice-ratio|犠牲率——ディスインフレーション政策における産出量コストの測定]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
