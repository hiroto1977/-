---
collection: academic
id: econ-dorfman-steiner-theorem
category: "economics"
category_ja: "経済学"
title: "ドーフマン＝スタイナーの定理——最適な広告は弾力性で決まる"
as_of: "2026-06-27"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/economics
  - knowledge/verified
aliases:
  - "ドーフマン＝スタイナーの定理——最適な広告は弾力性で決まる"
---

# ドーフマン＝スタイナーの定理——最適な広告は弾力性で決まる

> [!info] コレクション: [[学術概念]] ・ 区分: 経済学 ・ asOf: 2026-06-27 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
ドーフマン＝スタイナーの定理（Dorfman–Steiner theorem）は、経済学者ロバート・ドーフマンとピーター・スタイナーが1954年の論文「最適広告と最適品質」（American Economic Review）で示した、企業が利潤を最大化するときに広告にいくら使うべきかを定める命題である。企業は、価格を下げて売上を伸ばすこともできれば、広告で需要そのものを押し上げることもできる。両者を同時に選ぶとき、最適な広告水準はどう決まるか。定理が示す条件はこうである。「広告費の売上高に対する比率」が、「需要の広告弾力性」を「需要の価格弾力性（の絶対値）」で割った値に等しくなるように広告を設定せよ——同じことだが、広告費／売上＝価格・費用マージン×需要の広告弾力性、とも表せる。直観は明快である。広告の価値は、第一に需要が広告にどれだけ敏感か（広告弾力性が高いか）、第二に追加で売れた一単位あたりの利幅（マージン）がどれだけ大きいか、で決まる。両方が大きいほど、最適な広告比率は高くなる。これは、価格づけにおける「マークアップは需要の価格弾力性の逆数で決まる」というラーナーの規則と、美しく対をなす。定理の含意として、マージンが厚く広告に反応しやすい財（化粧品やブランド品）は広告を多く打ち、コモディティはほとんど広告しない、という産業ごとの広告集約度の違いが説明できる。ドーフマン＝スタイナー条件は、広告の経済学・産業組織論における広告投資の実証研究の出発点となった、基礎的な命題である。

## 提唱者・初出
ロバート・ドーフマン ／ ピーター・スタイナー ／ アバ・ラーナー ／ カイル・バグウェル

## 出典
- [Wikipedia: Dorfman–Steiner theorem — advertising/sales ratio = advertising elasticity ÷ price elasticity (= margin × advertising elasticity), Dorfman & Steiner (1954)](https://en.wikipedia.org/wiki/Dorfman%E2%80%93Steiner_theorem) `リファレンス`
- [Dorfman & Steiner (1954) Optimal Advertising and Optimal Quality — American Economic Review, 44(5), 826–836 (Dorfman–Steiner Condition, Wiley Encyclopedia of Management reference)](https://onlinelibrary.wiley.com/doi/abs/10.1002/9781118785317.weom080141) `学術`

## 関連概念
- [[econ-lerner-index|ラーナーの独占度（ラーナー指数）]] — 同じ思想家
- [[econ-elasticity-pessimism-thesis|弾力性ペシミズム——為替レート調整による貿易収支改善への懐疑論]] — 同じ思想家
- [[econ-job-guarantee-functional-finance-lerner|機能的財政論と雇用保証——ラーナーのMMT先駆け理論と完全雇用財政政策の根拠]] — 同じ思想家
- [[econ-lerner-index-market-power|ラーナー指数（市場支配力の測定）]] — 同じ思想家
- [[econ-lerner-symmetry-theorem-trade|ラーナーの対称性定理——輸入関税と輸出税の等価性]] — 同じ思想家
- [[econ-marginal-efficiency-capital|資本の限界効率——期待収益に基づく投資決定とアニマル・スピリッツ]] — 同じ思想家
- [[econ-marshall-lerner-condition|マーシャル＝ラーナー条件——通貨減価が貿易収支を改善する弾力性条件]] — 同じ思想家
- [[econ-rotemberg-saloner-price-wars-booms|ロテンバーグ＝サローナー・モデル——好況期に協調的価格が崩れ価格戦争が生じる反循環的マークアップ理論]] — 同じ思想家
- [[econ-second-welfare-theorem|厚生経済学第二基本定理——効率と公平の分離と再分配の可能性]] — 同じ思想家
- [[econ-price-elasticity-demand|需要の価格弾力性]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
