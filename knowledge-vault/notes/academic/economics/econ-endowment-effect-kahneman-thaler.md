---
collection: academic
id: econ-endowment-effect-kahneman-thaler
category: "economics"
category_ja: "経済学"
title: "授かり効果——カーネマンとセイラーの所有バイアスと損失回避の経済的含意"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/economics
  - knowledge/verified
aliases:
  - "授かり効果——カーネマンとセイラーの所有バイアスと損失回避の経済的含意"
---

# 授かり効果——カーネマンとセイラーの所有バイアスと損失回避の経済的含意

> [!info] コレクション: [[学術概念]] ・ 区分: 経済学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
リチャード・セイラーは1980年の論文「Toward a Positive Theory of Consumer Choice」において，人間が自己の所有物に対して市場価格よりも高い価値を付与する傾向を「授かり効果（endowment effect）」として概念化した。ダニエル・カーネマンとジャック・クネッチとセイラーの1990年の実験的研究は，コーヒーマグカップを与えられた被験者の受容最小価格（WTA）が，持っていない被験者の支払意思額（WTP）の約2倍であることを実証し，授かり効果の堅固な実験的証拠を提供した。この効果はカーネマンとトベルスキーのプロスペクト理論における損失回避（loss aversion）——同等の利得よりも損失がより大きな心理的インパクトを持つ——から理論的に導出される。授かり効果はコースの定理（取引費用が無視できれば初期の権利配分は最終的な資源配分に影響しない）の実践的妥当性に疑問を投じ，消費者行動（返品保証の効果），交渉（BATNA評価の歪み），政策設計（デフォルト・オプションの影響力）などの広範な領域に含意を持つ。

## 提唱者・初出
リチャード・セイラー ／ ダニエル・カーネマン ／ ジャック・クネッチ ／ エイモス・トベルスキー

## 出典
- [Kahneman, D., Knetsch, J. L. & Thaler, R. H. (1990) Experimental Tests of the Endowment Effect — Journal of Political Economy](https://doi.org/10.1086/261737) `学術`
- [Wikipedia: Endowment effect — WTA-WTP disparity, loss aversion, prospect theory](https://en.wikipedia.org/wiki/Endowment_effect) `リファレンス`

## 関連概念
- [[human-endowment-effect|保有効果（授かり効果）]] — 語彙が近い
- [[econ-endowment-effect-loss-aversion|保有効果と損失回避]] — 語彙が近い
- [[human-anchoring|アンカリング効果（係留と調整）]] — 同じ思想家
- [[human-framing-effect|フレーミング効果（framing effect）]] — 同じ思想家
- [[human-prospect-theory|プロスペクト理論（行動経済学）]] — 同じ思想家
- [[infosoc-persuasive-technology-fogg|説得的テクノロジー（フォッグ）]] — 同じ思想家
- [[econ-behavioral-economics-kahneman-thaler|行動経済学——カーネマンとセイラーのヒューリスティクス・バイアス・ナッジ理論]] — 同じ思想家
- [[bizlaw-anchoring-litigation|法的判断におけるアンカリング効果：数値情報による判断の歪み]] — 同じ思想家
- [[econ-behavioral-biases-heuristics|行動経済学における認知バイアスとヒューリスティクス：カーネマンとトヴェルスキーの研究]] — 同じ思想家
- [[econ-nudge-theory|ナッジ理論（nudge theory）]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
