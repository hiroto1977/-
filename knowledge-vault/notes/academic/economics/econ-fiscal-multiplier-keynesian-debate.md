---
collection: academic
id: econ-fiscal-multiplier-keynesian-debate
category: "economics"
category_ja: "経済学"
title: "財政乗数論争——ケインズ的乗数効果の実証的評価と政策インプリケーション"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/economics
  - knowledge/verified
aliases:
  - "財政乗数論争——ケインズ的乗数効果の実証的評価と政策インプリケーション"
---

# 財政乗数論争——ケインズ的乗数効果の実証的評価と政策インプリケーション

> [!info] コレクション: [[学術概念]] ・ 区分: 経済学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
財政乗数とは政府支出の1単位増加が国民所得をどれだけ増加させるかを示す概念であり、ケインズ経済学の中核的メカニズムである。理論的にはIS-LMモデルにおいて限界消費性向・貨幣需要・価格硬直性等に依存するが、実証的な乗数の大きさについては激しい論争が続いてきた。ロバート・バローは合理的期待とリカーディアン等価定理に基づき乗数は1未満と主張し、一方クリスティーナ・ローマーとジャレド・バーンスタインは2009年のアメリカ復興・再投資法の分析において1.5前後の乗数を推定した。ランバートニ=ラマーズとヴァレーはゼロ金利制約下では乗数が有意に大きくなることを示し、オーエルバッハとゴロドニチェンコは景気後退期の乗数が拡張期より大きいという状態依存性を実証した。財政乗数の大きさは緊縮財政の是非を巡る政策論争の核心をなす。

## 提唱者・初出
ジョン・メイナード・ケインズ ／ ロバート・バロー ／ クリスティーナ・ローマー ／ アラン・オーエルバッハ

## 出典
- [Ramey, V. (2011) Can Government Purchases Stimulate the Economy? — Journal of Economic Literature](https://doi.org/10.1257/jep.26.3.69) `学術`
- [Auerbach, A. & Gorodnichenko, Y. (2012) Measuring the Output Responses to Fiscal Policy — AEJ](https://doi.org/10.1257/aer.102.3.1446) `学術`
- [Wikipedia: Fiscal multiplier — 財政乗数の理論と実証概観](https://en.wikipedia.org/wiki/Fiscal_multiplier) `リファレンス`

## 関連概念
- [[bizlaw-invoice-system|インボイス制度（適格請求書等保存方式）]] — 語彙が近い
- [[econ-fiscal-multiplier-crowding-out-debate|財政乗数とクラウディングアウト論争——IS-LMモデルから実証的な乗数推定への展開]] — 語彙が近い
- [[econ-fiscal-multiplier-keynesian-analysis|財政乗数——政府支出と減税の国民所得波及効果]] — 語彙が近い
- [[econ-tax-smoothing|課税平準化（タックス・スムージング）]] — 同じ思想家
- [[econ-backwardation-contango|バックワーデーションとコンタンゴ（Backwardation & Contango）]] — 同じ思想家
- [[econ-barro-growth|バロー成長回帰（Barro Growth Regression）]] — 同じ思想家
- [[econ-fiscal-policy-automatic-stabilizers|財政政策と自動安定化装置——ビルトイン・スタビライザーとケインズ的裁量的財政出動]] — 同じ思想家
- [[econ-keynesian-beauty-contest|ケインズの美人投票]] — 同じ思想家
- [[econ-knightian-uncertainty|ナイトの不確実性（リスクと不確実性）]] — 同じ思想家
- [[econ-paradox-of-thrift|倹約のパラドックス]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
