---
collection: academic
id: econ-goodharts-law-lucas-critique
category: "economics"
category_ja: "経済学"
title: "グッドハートの法則とルーカス批判——政策目標化された統計的関係の崩壊"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/economics
  - knowledge/verified
aliases:
  - "グッドハートの法則とルーカス批判——政策目標化された統計的関係の崩壊"
---

# グッドハートの法則とルーカス批判——政策目標化された統計的関係の崩壊

> [!info] コレクション: [[学術概念]] ・ 区分: 経済学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
グッドハートの法則は、チャールズ・グッドハートが1975年に定式化した「ある統計的規則性が政策目的のために利用されると、その規則性自体が崩壊する傾向がある」という経験則であり、経済政策の設計における根本的な問題を指摘する。グッドハートは英国のマネーサプライ目標政策の文脈でこの法則を提唱したが、その適用範囲は金融政策にとどまらず、教育・医療・行政評価等のあらゆるパフォーマンス指標に及ぶ。ロバート・ルーカスの「ルーカス批判」（1976年）は同様の洞察を一般均衡理論の文脈で厳密に定式化し、過去の統計的関係に基づく政策評価はその政策自体が経済主体の行動を変化させるため無効であると主張した。両概念はともに「メタ合理性」——政策変更を予期して行動を変える経済主体——の問題を扱い、現代マクロ経済学の方法論的基盤を形成している。

## 提唱者・初出
チャールズ・グッドハート ／ ロバート・ルーカス ／ トーマス・サージェント ／ クリストファー・シムズ

## 出典
- [Lucas, R. (1976) Econometric Policy Evaluation: A Critique — Carnegie-Rochester Conference Series](https://doi.org/10.1016/S0167-2231(76)80003-6) `学術`
- [Goodhart, C. (1975) Problems of Monetary Management — Papers in Monetary Economics](https://doi.org/10.1111/j.1468-0297.1975.tb00196.x) `学術`
- [Wikipedia: Goodhart's law — グッドハートの法則の概要](https://en.wikipedia.org/wiki/Goodhart%27s_law) `リファレンス`

## 関連概念
- [[econ-goodharts-law-monetary|グッドハートの法則——指標が目標となるとき有効性を失う統計的規則性]] — 語彙が近い
- [[econ-lucas-paradox|ルーカスのパラドックス]] — 同じ思想家
- [[econ-rational-expectations-muth|合理的期待——主観的予想が利用可能情報のもとでの客観的期待値に一致する仮説]] — 同じ思想家
- [[econ-ak-model|AKモデル（内生的成長の基本モデル）]] — 同じ思想家
- [[econ-expectations-augmented-phillips|期待修正フィリップス曲線——インフレ期待と失業のトレードオフの動態分析]] — 同じ思想家
- [[econ-fiscal-dominance|財政支配（Fiscal Dominance）]] — 同じ思想家
- [[econ-fiscal-theory-price|物価水準の財政理論（Fiscal Theory of the Price Level）]] — 同じ思想家
- [[econ-learning-by-doing|学習効果（ラーニング・バイ・ドゥーイング）]] — 同じ思想家
- [[econ-neutrality-of-money|貨幣の中立性（Neutrality of Money）]] — 同じ思想家
- [[econ-policy-ineffectiveness|政策無効命題]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
