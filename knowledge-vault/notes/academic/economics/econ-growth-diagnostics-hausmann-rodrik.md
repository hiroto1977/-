---
collection: academic
id: econ-growth-diagnostics-hausmann-rodrik
category: "economics"
category_ja: "経済学"
title: "成長診断——成長を最も縛る制約を見つけて改革を絞り込む"
as_of: "2026-06-27"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/economics
  - knowledge/verified
aliases:
  - "成長診断——成長を最も縛る制約を見つけて改革を絞り込む"
---

# 成長診断——成長を最も縛る制約を見つけて改革を絞り込む

> [!info] コレクション: [[学術概念]] ・ 区分: 経済学 ・ asOf: 2026-06-27 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
成長診断（growth diagnostics）は、リカルド・ハウスマン、ダニ・ロドリック、アンドレス・ベラスコが2005年前後に提示した、ある国の経済成長を最も強く縛っている「決定的制約（binding constraints）」を診断し、改革の優先順位を定めるための枠組みである。これは、すべての国に同じ改革の一覧（ワシントン・コンセンサスのような網羅的なリスト）を一律に適用しようとする「画一的な」アプローチへの批判から生まれた。成長診断は、あれもこれもと一度にすべてを行うのではなく、最も拘束力の強い単一の（あるいはごく少数の）制約——それを緩めることが成長に最大の効果をもたらす制約——を特定し、そこに改革を集中すべきだと論じる。この方法は「決定木（ディシジョン・ツリー）」を用いる。成長を妨げているのは、(A)経済活動の「収益が低い」か、(B)「資金調達の費用が高い」か、のいずれかである。収益の低さは、さらに、(A1)「社会的収益」が低い場合（地理的条件の悪さ、貧弱なインフラ、低い人的資本）と、(A2)収益の「専有可能性」が低い場合に分かれる。後者はさらに、「政府の失敗」（マクロのリスク：不安定、インフレ、財政・金融の問題／ミクロのリスク：腐敗、税、財産権、契約の執行）と、「市場の失敗」（情報の外部性すなわち「自己発見」、調整の失敗）に分かれる。資金調達費用の高さは、国際的な金融の問題と、国内の金融の問題（低い国内貯蓄、貧弱な金融仲介）に分かれる。分析者は、決定的制約の「兆候（シグネチャー）」を探す。たとえば、もし金融が制約なら、金利が高く、起業家は資金不足を訴えるはずである。逆に、それが制約でなければ、それを取り除いても成長は改善しない。これは、決定的制約の影の価格（シャドー・プライス）が高いという論理にもとづく。どの枝が決定的制約かを特定することで、政策立案者は、最も効果のあるところに改革を絞り込む。成長診断は、開発経済学と政策の実務において大きな影響を持ち（世界銀行や米州開発銀行などで用いられた）、網羅的な青写真ではなく、文脈に即した、順序立てた改革を重視する。成長診断は、成長を最も縛る決定的制約を見つけ出し、そこに改革を集中させることを説いた、開発経済学の重要な枠組みである。

## 提唱者・初出
リカルド・ハウスマン ／ ダニ・ロドリック ／ アンドレス・ベラスコ ／ ジョン・ウィリアムソン

## 出典
- [Hausmann, R., Rodrik, D. & Velasco, A. (2008) Growth diagnostics — in The Washington Consensus Reconsidered](https://www.worldcat.org/oclc/181368015) `リファレンス`
- [Wikipedia: Growth diagnostics — binding constraints, decision tree, development policy](https://en.wikipedia.org/wiki/Growth_diagnostics) `リファレンス`

## 関連概念
- [[econ-washington-consensus|ワシントン・コンセンサス]] — 同じ思想家
- [[econ-balassa-index-revealed-comparative-advantage|顕示比較優位指数——バラッサ指数による貿易特化の測定]] — 同じ思想家
- [[econ-currency-crisis-models-krugman-obstfeld|通貨危機の世代モデル——固定相場制崩壊の理論的枠組み]] — 同じ思想家
- [[econ-demographic-dividend-bloom-williamson|人口ボーナスと人口転換——ブルーム＝ウィリアムソンの経済成長への人口学的寄与の分析]] — 同じ思想家
- [[econ-economic-complexity|経済的複雑性指数（ECI）]] — 同じ思想家
- [[econ-globalization-backlash-rodrik-trilemma|グローバリゼーションのトリレンマ——ロドリックの超グローバル化・民主主義・国家主権の不可能三角形]] — 同じ思想家
- [[econ-hot-money-flows-calvo|ホットマネー・フローと急停止——新興国資本移動の不安定性]] — 同じ思想家
- [[econ-hot-money-flows-kindleberger|ホットマネーと国際資本移動——投機的資本フローの不安定化効果]] — 同じ思想家
- [[econ-middle-income-trap|中所得国の罠——中位の所得で成長が止まる現象]] — 同じ思想家
- [[econ-original-sin-eichengreen|原罪仮説——新興国の自国通貨建て対外借入の構造的不能]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
