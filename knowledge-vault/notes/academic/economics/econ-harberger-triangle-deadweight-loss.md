---
collection: academic
id: econ-harberger-triangle-deadweight-loss
category: "economics"
category_ja: "経済学"
title: "ハーバーガーの三角形——独占・課税による死荷重損失と余剰分析"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/economics
  - knowledge/verified
aliases:
  - "ハーバーガーの三角形——独占・課税による死荷重損失と余剰分析"
---

# ハーバーガーの三角形——独占・課税による死荷重損失と余剰分析

> [!info] コレクション: [[学術概念]] ・ 区分: 経済学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
アーノルド・ハーバーガーは1954年の論文「Monopoly and Resource Allocation」において，米国の製造業における独占の社会的コスト——完全競争と比較した場合の消費者余剰と生産者余剰の純損失——を需要曲線と限界費用曲線で囲まれた三角形の面積（「ハーバーガーの三角形」）として測定し，その規模がGDPの約0.1%と比較的小さいことを示した。この余剰分析の枠組みは，課税の死荷重損失（deadweight loss of taxation）の測定にも応用され，財政学における最適課税理論の実証的基盤となった。ゴードン・タロックはハーバーガーの分析が独占利潤の獲得・維持に費やされるレントシーキング（rent-seeking）コストを過小評価していると批判し，社会的損失は「ハーバーガーの三角形」だけでなくレント全体の大きさ（「タロックの四角形」）を含むべきだと主張した。マーティン・フェルドシュタインは所得税の限界税率が高いほど労働供給の弾力性を通じて死荷重損失が二次的に増大することを示し，税制改革の経済学的根拠を提供した。

## 提唱者・初出
アーノルド・ハーバーガー ／ ゴードン・タロック ／ マーティン・フェルドシュタイン ／ アルフレッド・マーシャル

## 出典
- [Harberger, A. C. (1954) Monopoly and Resource Allocation — American Economic Review](https://doi.org/10.1257/aer.44.2.77) `学術`
- [Wikipedia: Deadweight loss — Harberger triangle, monopoly, taxation, consumer surplus](https://en.wikipedia.org/wiki/Deadweight_loss) `リファレンス`

## 関連概念
- [[econ-deadweight-loss|死荷重（死重損失・超過負担）]] — 語彙が近い
- [[econ-deadweight-loss-harberger|死荷重損失とハーバーガーの三角形——歪みが生む厚生の純損失の計測]] — 語彙が近い
- [[econ-cross-price-income-elasticity|クロス価格弾力性と所得弾力性]] — 同じ思想家
- [[econ-agglomeration-economies|集積の経済]] — 同じ思想家
- [[econ-feldstein-horioka|フェルドシュタイン＝ホリオカのパズル]] — 同じ思想家
- [[econ-logrolling|ログローリング（票の取引）]] — 同じ思想家
- [[econ-rent-seeking|レントシーキング]] — 同じ思想家
- [[econ-supply-demand-equilibrium|需要と供給の均衡（価格メカニズム）]] — 同じ思想家
- [[econ-directly-unproductive-bhagwati|直接的に非生産的な利潤追求活動（DUP）——資源を浪費する利得の追求]] — 同じ思想家
- [[econ-agglomeration-economies-urban-economics|集積経済と都市経済学——マーシャルの外部経済からグレーザーの都市生産性まで]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
