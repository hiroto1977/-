---
collection: academic
id: econ-heterodox-post-keynesian-kalecki
category: "economics"
category_ja: "経済学"
title: "ポスト・ケインズ派経済学とカレツキ——有効需要・所得分配・マクロ投資決定の代替的枠組み"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/economics
  - knowledge/verified
aliases:
  - "ポスト・ケインズ派経済学とカレツキ——有効需要・所得分配・マクロ投資決定の代替的枠組み"
---

# ポスト・ケインズ派経済学とカレツキ——有効需要・所得分配・マクロ投資決定の代替的枠組み

> [!info] コレクション: [[学術概念]] ・ 区分: 経済学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
ポスト・ケインズ派経済学は、ケインズ・カレツキ・ミンスキーらの遺産を継承し、新古典派均衡パラダイムへの対抗軸として発展した非主流派マクロ経済学の潮流である。ミハウ・カレツキはケインズとほぼ同時期に独立して有効需要論を定式化し（1933年）、企業の利潤と投資の相互規定関係を中心に、所得分配（賃金・利潤）が有効需要に与える影響を分析した。「カレツキの原理（profits equal investment）」はGDPとの恒等式関係として定式化され、投資が利潤を生み出すという反直観的な論理を持つ。ポスト・ケインズ派は①歴史的時間と経路依存性の重視、②根本的不確実性（確率的ではない未知）、③有効需要の中心性、④貨幣の内生的供給論を特徴とし、金融不安定性仮説（ミンスキー）・ストックフロー整合的（SFC）マクロモデルへと発展している。

## 提唱者・初出
ミハウ・カレツキ ／ ジョーン・ロビンソン ／ ポール・デビッドソン ／ マルク・ラボワ

## 出典
- [Kalecki (1933/1971) Selected Essays on the Dynamics of the Capitalist Economy — Cambridge](https://doi.org/10.2307/2231821) `学術`
- [Lavoie (2014) Post-Keynesian Economics: New Foundations — Edward Elgar](https://doi.org/10.4337/9781785364280) `学術`
- [Wikipedia: Post-Keynesian economics — ポスト・ケインズ派の理論と系譜](https://en.wikipedia.org/wiki/Post-Keynesian_economics) `リファレンス`

## 関連概念
- [[econ-kalecki-profit|カレツキの利潤理論]] — 同じ思想家
- [[econ-monopolistic-competition|独占的競争]] — 同じ思想家
- [[econ-capital-controversy|ケンブリッジ資本論争（ロビンソン／スラッファ対サミュエルソン）——異質な資本財を一つの量に集計できるかをめぐり、再スイッチングと資本逆行が新古典派の限界生産力理論に根本的な疑義を投じた]] — 同じ思想家
- [[econ-kaldor-nonlinear-cycle|カルドアの非線形景気循環モデル——投資と貯蓄の非線形性が生む内生的循環]] — 同じ思想家
- [[econ-marginal-efficiency-capital|資本の限界効率——期待収益に基づく投資決定とアニマル・スピリッツ]] — 同じ思想家
- [[econ-marshall-lerner-condition|マーシャル＝ラーナー条件——通貨減価が貿易収支を改善する弾力性条件]] — 同じ思想家
- [[econ-monopsony-robinson|買い手独占（モノプソニー）——単一の買い手が価格と取引量を抑制する市場]] — 同じ思想家
- [[econ-new-institutional-economics-north|新制度経済学と制度変化（ノース）——制度は人間が考案した「ゲームのルール」であり、取引費用と経路依存性を通じて経済パフォーマンスの長期的な差を生む]] — 同じ思想家
- [[econ-political-business-cycle-nordhaus|政治的景気循環——選挙を見据えた政策操作が生む景気の周期的変動]] — 同じ思想家
- [[econ-poverty-trap|貧困の罠——低所得が貯蓄と投資を妨げて貧困を自己強化的に再生産する複数均衡と、その存在をめぐる実証]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
