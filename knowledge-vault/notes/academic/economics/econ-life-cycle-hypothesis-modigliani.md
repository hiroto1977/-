---
collection: academic
id: econ-life-cycle-hypothesis-modigliani
category: "economics"
category_ja: "経済学"
title: "ライフサイクル仮説——生涯を通じた消費平準化と貯蓄の動学"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/economics
  - knowledge/verified
aliases:
  - "ライフサイクル仮説——生涯を通じた消費平準化と貯蓄の動学"
---

# ライフサイクル仮説——生涯を通じた消費平準化と貯蓄の動学

> [!info] コレクション: [[学術概念]] ・ 区分: 経済学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
ライフサイクル仮説（life-cycle hypothesis）は、フランコ・モディリアーニとリチャード・ブランバーグが1954年に提唱した消費・貯蓄理論で、個人が現在の所得ではなく生涯にわたる資源の総量を見据えて消費計画を立て、効用を最大化するために消費水準を平準化しようとすると説く。所得は若年期に低く、壮年期に高く、退職後に再び低下するという典型的な山型を描くのに対し、人は安定した消費水準を望むため、所得が高い勤労期に貯蓄を積み上げ、所得が乏しい退職後にその貯蓄を取り崩して消費を支える。この見方では、消費は当期の所得ではなく生涯所得と保有資産に依存し、ケインズ的消費関数が示唆する所得と消費の単純な比例関係を退ける。仮説の含意は重要で、社会全体の貯蓄率は所得水準そのものではなく、人口構成（勤労世代と退職世代の比率）や経済成長率に依存することになる。成長率が高ければ若い勤労世代の貯蓄が高齢世代の取り崩しを上回り、総貯蓄が増える。理論は年金制度の設計、高齢化社会の貯蓄動向、世代間の資産移転の分析に広く応用され、ミルトン・フリードマンの恒常所得仮説と並んで現代の消費理論の礎をなす。

## 提唱者・初出
フランコ・モディリアーニ ／ リチャード・ブランバーグ ／ アルバート・アンドー ／ ミルトン・フリードマン

## 出典
- [Modigliani, F. & Brumberg, R. (1954) Utility Analysis and the Consumption Function — Post-Keynesian Economics (WorldCat)](https://www.worldcat.org/oclc/651838929) `学術`
- [Wikipedia: Life-cycle hypothesis — Modigliani, consumption smoothing, saving and demographics](https://en.wikipedia.org/wiki/Life-cycle_hypothesis) `リファレンス`

## 関連概念
- [[econ-permanent-income-hypothesis-friedman|恒常所得仮説——フリードマンの消費理論と生涯所得に基づく消費決定メカニズム]] — 語彙が近い
- [[mgmt-csr|企業の社会的責任（CSR）]] — 同じ思想家
- [[econ-consumption-smoothing-permanent-income|消費の平滑化——恒常所得仮説とライフサイクル仮説の統合的枠組み]] — 同じ思想家
- [[econ-friedman-rule|フリードマン・ルール（最適貨幣量）]] — 同じ思想家
- [[econ-helicopter-money|ヘリコプターマネー]] — 同じ思想家
- [[econ-natural-rate-friedman-phelps|自然失業率仮説——インフレと失業の長期トレードオフの不在]] — 同じ思想家
- [[econ-natural-rate-unemployment|自然失業率（NAIRU）]] — 同じ思想家
- [[econ-expectations-augmented-phillips|期待修正フィリップス曲線——インフレ期待と失業のトレードオフの動態分析]] — 同じ思想家
- [[econ-modigliani-miller|モディリアーニ＝ミラーの定理（MM理論）]] — 同じ思想家
- [[econ-limit-pricing-bain|参入阻止価格——潜在的参入者を思いとどまらせる戦略的な低価格設定]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
