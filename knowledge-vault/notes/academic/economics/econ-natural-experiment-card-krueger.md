---
collection: academic
id: econ-natural-experiment-card-krueger
category: "economics"
category_ja: "経済学"
title: "自然実験と最低賃金論争——カード＝クルーガーの差の差分析が覆した通説"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/economics
  - knowledge/verified
aliases:
  - "自然実験と最低賃金論争——カード＝クルーガーの差の差分析が覆した通説"
---

# 自然実験と最低賃金論争——カード＝クルーガーの差の差分析が覆した通説

> [!info] コレクション: [[学術概念]] ・ 区分: 経済学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
自然実験（natural experiment）は、研究者が意図的に介入せず、政策変更や制度差など外生的な要因が対象を処置群と対照群に擬似的に振り分ける状況を利用して因果効果を推定する実証手法である。その金字塔的研究が、デヴィッド・カードとアラン・クルーガーが1994年に発表した最低賃金の雇用効果の分析である。彼らは、1992年にニュージャージー州が最低賃金を引き上げ、隣接するペンシルベニア州が据え置いたことを自然実験として捉え、両州境界付近のファストフード店の雇用を引き上げ前後で比較する「差の差（difference-in-differences）」推定を行った。標準的な完全競争モデルは、最低賃金の引き上げが雇用を減らすと予測する。ところがカードとクルーガーは、最低賃金を上げたニュージャージー州で雇用が減少せず、むしろわずかに増加したことを見いだし、教科書的通説に実証的な疑問を突きつけた。この結果は、労働市場が買い手独占的な要素を含むという解釈と整合し、激しい論争を呼んだ。方法論的には、観察データから信頼できる因果推論を引き出す「信頼性革命（credibility revolution）」の象徴的事例となり、自然実験・差の差・回帰不連続・操作変数といった準実験的手法を経済学の標準的道具へと押し上げた。カードはこの実証労働経済学への貢献により2021年にノーベル経済学賞を受賞した。

## 提唱者・初出
デヴィッド・カード ／ アラン・クルーガー ／ ジョシュア・アングリスト ／ グイド・インベンス

## 出典
- [Card, D. & Krueger, A.B. (1994) Minimum Wages and Employment: New Jersey and Pennsylvania — AER 84(4)](https://www.jstor.org/stable/2118030) `学術`
- [Wikipedia: Natural experiment — Card-Krueger minimum wage, difference-in-differences, credibility revolution](https://en.wikipedia.org/wiki/Natural_experiment) `リファレンス`

## 関連概念
- [[econ-difference-in-differences|差の差分析（DID）]] — 語彙が近い
- [[econ-natural-experiments-rdd|自然実験と回帰不連続デザインによる因果推論：外生的変動を用いた処置効果の識別と政策評価]] — 同じ思想家
- [[econ-natural-experiment-angrist|自然実験と操作変数法による因果識別戦略（アングリスト）]] — 同じ思想家
- [[econ-environmental-kuznets-curve|環境クズネッツ曲線]] — 同じ思想家
- [[econ-minimum-wage-card-krueger|最低賃金の雇用効果（カード＆クルーガー）]] — 同じ思想家
- [[econ-monopsony-robinson|買い手独占（モノプソニー）——単一の買い手が価格と取引量を抑制する市場]] — 同じ思想家
- [[econ-rent-seeking|レントシーキング]] — 同じ思想家
- [[econ-directly-unproductive-bhagwati|直接的に非生産的な利潤追求活動（DUP）——資源を浪費する利得の追求]] — 同じ思想家
- [[bizlaw-non-compete-agreements|競業避止義務契約（ノン・コンピート条項）]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
