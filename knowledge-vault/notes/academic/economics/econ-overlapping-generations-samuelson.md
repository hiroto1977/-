---
collection: academic
id: econ-overlapping-generations-samuelson
category: "economics"
category_ja: "経済学"
title: "世代重複モデル——有限の生涯を生きる世代が共存する動学的一般均衡"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/economics
  - knowledge/verified
aliases:
  - "世代重複モデル——有限の生涯を生きる世代が共存する動学的一般均衡"
---

# 世代重複モデル——有限の生涯を生きる世代が共存する動学的一般均衡

> [!info] コレクション: [[学術概念]] ・ 区分: 経済学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
世代重複モデル（overlapping generations model、OLG）は、ポール・サミュエルソンが1958年の論文「利子の正確な消費＝貸付モデル」で定式化した動学的一般均衡の枠組みで、モーリス・アレが1947年に先駆的に提示していた着想を厳密化したものである。このモデルは、各個人が有限の期間（典型的には若年期と老年期の二期）だけ生き、どの時点においても異なる世代——働いて貯蓄する若年世代と、貯蓄を取り崩して消費する老年世代——が共存し、世代間で資源をやり取りする経済を描く。若年期に労働して所得を得る人々は、老年期の消費のために貯蓄したいが、自分が老いたときに生産はできない。そこで世代を超えた資源移転の仕組みが必要となる。サミュエルソンは、このモデルが、無限に生きる代表的個人を仮定する標準的モデルとは質的に異なる帰結を生むことを示した。とりわけ重要なのは、競争均衡が必ずしもパレート効率的とならず、資本が過剰に蓄積される「動学的非効率性」が生じうる点である。この非効率な経済では、内在的価値を持たない貨幣や、賦課方式（pay-as-you-go）の社会保障による世代間移転が、すべての世代の厚生を改善しうる。ピーター・ダイアモンドが1965年に資本と政府債務を組み込んでモデルを拡張し、OLGは年金・公的債務・世代間負担・貯蓄の分析に不可欠な道具となった。

## 提唱者・初出
ポール・サミュエルソン ／ モーリス・アレ ／ ピーター・ダイアモンド ／ イヴ・バラスコ

## 出典
- [Samuelson, P.A. (1958) An Exact Consumption-Loan Model of Interest — Journal of Political Economy 66(6)](https://doi.org/10.1086/258100) `学術`
- [Wikipedia: Overlapping generations model — Samuelson, Allais, Diamond, dynamic inefficiency](https://en.wikipedia.org/wiki/Overlapping_generations_model) `リファレンス`

## 関連概念
- [[econ-golden-rule-saving-phelps|貯蓄の黄金律——定常状態における消費最大化の最適貯蓄率]] — 語彙が近い
- [[econ-burdett-mortensen-wage-dispersion|バーデット＝モーテンセン・モデル——同一労働者でも賃金が分散する均衡]] — 同じ思想家
- [[econ-golden-rule-capital-phelps|資本蓄積の黄金律——定常状態の消費を最大化する貯蓄水準]] — 同じ思想家
- [[econ-harberger-excess-burden-taxation|ハーバーガーの超過負担——課税の厚生損失と最適税制設計]] — 同じ思想家
- [[econ-labor-market-search-diamond|労働市場の探索・マッチング理論と摩擦的失業の分析（ダイアモンド・モーテンセン・ピサリデス）]] — 同じ思想家
- [[econ-optimal-income-taxation-mirrlees|最適所得税理論（ミラーリーズ）]] — 同じ思想家
- [[econ-optimal-tax-theory-diamond-mirrlees|最適税制論（ダイアモンド＆マーリース）]] — 同じ思想家
- [[econ-optimal-taxation-mirrlees|最適課税理論——ミルリーズの情報の非対称性と効率性・公平性のトレードオフ]] — 同じ思想家
- [[econ-optimal-taxation-mirrlees-diamond|最適課税論——効率性と公平性のトレードオフにおける税制設計]] — 同じ思想家
- [[econ-ramsey-optimal-taxation|ラムゼイの最適課税理論——効率的税制設計と超過負担最小化の原理]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
