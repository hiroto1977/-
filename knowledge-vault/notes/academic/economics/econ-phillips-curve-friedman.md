---
collection: academic
id: econ-phillips-curve-friedman
category: "economics"
category_ja: "経済学"
title: "フィリップス曲線とフリードマン・ペルプス批判"
as_of: "2026-06-26"
source_count: 4
authoritative: true
tags:
  - collection/academic
  - academic/economics
  - knowledge/verified
aliases:
  - "フィリップス曲線とフリードマン・ペルプス批判"
---

# フィリップス曲線とフリードマン・ペルプス批判

> [!info] コレクション: [[学術概念]] ・ 区分: 経済学 ・ asOf: 2026-06-26 ・ 出典: 4件（うち権威ある出典 ✓）

## 概要
フィリップス曲線（Phillips curve）は、フィリップス（1958）がイギリス1861〜1957年のデータから導いた、失業率と賃金上昇率（後に物価上昇率）の間の安定的な逆相関関係であり、ケインズ政策の理論的支柱として1960年代に広く採用された。しかしフリードマン（1968）とペルプス（1967）は独立に、「期待インフレ率を考慮しない」短期フィリップス曲線は長期的には垂直（自然失業率仮説）であると論じた。インフレ期待が適応的（adaptive）に修正されることで、政府が失業を下げようとする拡張政策は当初は実質賃金を低下させるが、やがて期待が上方修正されて失業は自然率に戻り、より高いインフレだけが残るという「スタグフレーション（stagflation）」が予測された。1970年代の石油危機後の実績がこれを実証し、ケインズ需要管理政策への信頼を大きく失墜させた。ルーカス批判（1976）はさらに、政策ルールの変化が民間の期待形成自体を変えるため、歴史データに基づくフィリップス曲線は政策評価に使えないと論じ、合理的期待・DSGEモデルへの転換を加速した。

## 提唱者・初出
ミルトン・フリードマン ／ エドマンド・フェルプス ／ ウィリアム・フィリップス ／ ロバート・ルーカス

## 出典
- [Friedman, M. (1968) The Role of Monetary Policy — American Economic Review, 58(1), 1–17（自然失業率仮説の原典）](https://doi.org/10.2307/1830652) `学術`
- [Phillips, A. W. (1958) The Relation Between Unemployment and the Rate of Change of Money Wage Rates in the United Kingdom — Economica, 25(100), 283–299（フィリップス曲線原典）](https://doi.org/10.2307/1830199) `学術`
- [Lucas, R. E. (1976) Econometric Policy Evaluation: A Critique — Carnegie-Rochester Conference Series on Public Policy, 1, 19–46（ルーカス批判原典）](https://doi.org/10.1016/0304-3932(76)90002-1) `学術`
- [Wikipedia: Phillips curve — Friedman-Phelps natural rate, stagflation, Lucas critique, NAIRU](https://en.wikipedia.org/wiki/Phillips_curve) `リファレンス`

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
