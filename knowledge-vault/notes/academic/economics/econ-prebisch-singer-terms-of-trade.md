---
collection: academic
id: econ-prebisch-singer-terms-of-trade
category: "economics"
category_ja: "経済学"
title: "プレビッシュ＝シンガー仮説——一次産品の交易条件悪化と開発経済学"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/economics
  - knowledge/verified
aliases:
  - "プレビッシュ＝シンガー仮説——一次産品の交易条件悪化と開発経済学"
---

# プレビッシュ＝シンガー仮説——一次産品の交易条件悪化と開発経済学

> [!info] コレクション: [[学術概念]] ・ 区分: 経済学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
ラウル・プレビッシュとハンス・シンガーは1950年に独立して，一次産品（農産物・鉱物資源）の製造業製品に対する交易条件（terms of trade）が長期的に悪化する傾向があるという仮説を提示した。プレビッシュはUNCTAD（国連貿易開発会議）初代事務局長として，この仮説を途上国の工業化政策——輸入代替工業化（ISI）——の理論的根拠とした。交易条件悪化のメカニズムとして，一次産品の所得弾力性の低さ（エンゲルの法則の国際的拡張），技術進歩の果実が一次産品の価格低下として先進国に移転されること，一次産品市場の競争的構造と製造業市場の寡占的構造の非対称性が挙げられる。この仮説は新古典派経済学者から実証的妥当性を批判されてきたが，ホセ・アントニオ・オカンポの研究は20世紀を通じた交易条件データの再検証を行い，「スーパーサイクル」の存在を含む長期的トレンドの複雑性を明らかにした。資源呪い（resource curse）論や構造主義開発経済学の理論的基盤として現代にも影響を与えている。

## 提唱者・初出
ラウル・プレビッシュ ／ ハンス・シンガー ／ ホセ・アントニオ・オカンポ ／ セルソ・フルタード

## 出典
- [Singer, H. (1950) The Distribution of Gains between Investing and Borrowing Countries — American Economic Review](https://doi.org/10.2307/1882508) `学術`
- [Wikipedia: Prebisch-Singer hypothesis — terms of trade, primary commodities, ISI](https://en.wikipedia.org/wiki/Prebisch%E2%80%93Singer_hypothesis) `リファレンス`

## 関連概念
- [[econ-prebisch-singer-hypothesis|プレビッシュ＝シンガー仮説]] — 同じ思想家
- [[econ-dependency-theory-prebisch|従属理論——中心＝周辺構造が低開発を再生産する世界経済の不平等]] — 同じ思想家
- [[econ-cobweb-theorem-agricultural-cycles|蜘蛛の巣理論（農業価格循環）]] — 同分野の近傍
- [[econ-prebisch-singer-thesis|プレビッシュ＝シンガー命題]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
