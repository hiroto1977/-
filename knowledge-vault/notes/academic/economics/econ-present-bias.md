---
collection: academic
id: econ-present-bias
category: "economics"
category_ja: "経済学"
title: "現在バイアス：先延ばし行動と自己制御の行動経済学的分析"
as_of: "2026-06-25"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/economics
  - knowledge/verified
aliases:
  - "現在バイアス：先延ばし行動と自己制御の行動経済学的分析"
---

# 現在バイアス：先延ばし行動と自己制御の行動経済学的分析

> [!info] コレクション: [[学術概念]] ・ 区分: 経済学 ・ asOf: 2026-06-25 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
現在バイアス（present bias）とは、将来の二時点間では忍耐強い選択ができるにもかかわらず選択時点が「現在」に近づくと即時的な満足を過度に重視して時間的に非整合的な行動をとる傾向を指す。テッド・オドノヒューとマシュー・ラビンが1999年にAmerican Economic Review誌で「Doing It Now or Later」を発表し、β-δ準双曲割引モデルを基盤に現在バイアスを持つ個人が自身のバイアスを認識する「洗練された」タイプと認識しない「ナイーブな」タイプに分類し両者の行動帰結の違いを理論的に解明した。ナイーブな個人は先延ばしを繰り返し洗練された個人は過早着手に陥りうることを示した。この理論は貯蓄不足・ダイエットの失敗・依存症などの自己制御問題の分析に広く応用されコミットメント装置の設計やナッジ政策の理論的基盤となっている。

## 提唱者・初出
テッド・オドノヒュー ／ マシュー・ラビン ／ デイヴィッド・レイブソン

## 出典
- [O'Donoghue & Rabin 1999 Doing It Now or Later. American Economic Review 89(1) 103-124](https://doi.org/10.1257/aer.89.1.103) `学術`
- [Laibson 1997 Golden Eggs and Hyperbolic Discounting. Quarterly Journal of Economics 112(2) 443-478](https://doi.org/10.1162/003355397555253) `学術`
- [O'Donoghue & Rabin 2001 Choice and Procrastination. Quarterly Journal of Economics 116(1) 121-160](https://doi.org/10.1257/0002828053828482) `学術`

## 関連概念
- [[econ-disappointment-aversion-gul|失望回避——期待を下回る結果を重く評価する]] — 同じ思想家
- [[econ-temptation-self-control-gul-pesendorfer|誘惑と自制の選好——選択肢の集合そのものを評価する]] — 同じ思想家
- [[human-temporal-discounting-ainslie|時間割引と双曲割引——即時的報酬への選好と衝動性の行動経済学]] — 同じ思想家
- [[econ-learning-by-doing|学習効果（ラーニング・バイ・ドゥーイング）]] — 同分野の近傍
- [[econ-time-preference|時間選好]] — 同分野の近傍
- [[econ-logrolling|ログローリング（票の取引）]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
