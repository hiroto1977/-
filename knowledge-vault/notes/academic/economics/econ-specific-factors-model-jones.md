---
collection: academic
id: econ-specific-factors-model-jones
category: "economics"
category_ja: "経済学"
title: "特殊要素モデル——部門間要素移動の不完全性と所得分配"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/economics
  - knowledge/verified
aliases:
  - "特殊要素モデル——部門間要素移動の不完全性と所得分配"
---

# 特殊要素モデル——部門間要素移動の不完全性と所得分配

> [!info] コレクション: [[学術概念]] ・ 区分: 経済学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
ロナルド・ジョーンズが1971年に、ポール・サミュエルソンが同年にそれぞれ独立に定式化した特殊要素モデル（Ricardo-Viner モデル）は、ヘクシャー＝オリーンの2×2モデルが全要素の完全な部門間移動を仮定するのに対し、各産業に固有の特殊要素（特殊的資本・土地等）と全産業間で移動可能な一般要素（労働）を区別する。この部分的要素移動性の仮定により、貿易自由化の所得分配効果はHOモデルとは異なるパターンを示す：輸出産業の特殊要素保有者が利益を得、輸入競合産業の特殊要素保有者が損失を被り、一般要素（労働者）への効果は消費パターンに依存して不確定となる。マーヤーの政治経済学への応用は、特殊要素の産業特殊性が貿易政策に関する利益団体形成のパターン（産業ベース vs. 要素ベース）を説明すると論じ、グロスマンとヘルプマンの保護販売モデルの基礎を提供した。

## 提唱者・初出
ロナルド・ジョーンズ ／ ポール・サミュエルソン ／ ジェイコブ・ヴァイナー ／ ウォルフガング・マーヤー ／ ジーン・グロスマン

## 出典
- [Jones, R. W. (1971) "A Three-Factor Model in Theory, Trade, and History" — Trade, Balance of Payments, and Growth](https://doi.org/10.2307/2228259) `学術`
- [Samuelson, P. A. (1971) "Ohlin Was Right" — Swedish Journal of Economics](https://doi.org/10.2307/2296907) `学術`
- [Wikipedia: Specific factors model — 特殊要素モデルの理論と応用](https://en.wikipedia.org/wiki/Specific_factors_model) `リファレンス`

## 関連概念
- [[econ-stolper-samuelson-trade-wages|ストルパー＝サミュエルソン定理——貿易自由化と要素価格の変動]] — 語彙が近い
- [[econ-factor-price-equalization|要素価格均等化定理]] — 同じ思想家
- [[econ-environmental-kuznets-curve|環境クズネッツ曲線]] — 同じ思想家
- [[econ-transfer-problem-keynes|移転問題——ケインズ＝オリーン論争と国際資本移動の交易条件効果]] — 同じ思想家
- [[econ-rybczynski-theorem-factor-growth|リプチンスキー定理——要素賦存変化と産出構成の変動]] — 同じ思想家
- [[econ-customs-union-theory-viner|関税同盟の理論——貿易創出効果と貿易転換効果の厚生分析]] — 同じ思想家
- [[econ-lerner-symmetry-theorem-trade|ラーナーの対称性定理——輸入関税と輸出税の等価性]] — 同じ思想家
- [[econ-metzler-paradox-tariff|メッツラー・パラドックス——関税が国内価格を引き下げる逆説]] — 同じ思想家
- [[econ-second-best-lipsey-lancaster|次善の理論——最適条件の一部不達成時における政策評価の困難]] — 同じ思想家
- [[econ-grossman-helpman-protection-for-sale|保護の販売モデル（グロスマン＝ヘルプマン「Protection for Sale」）——貿易保護の政治経済学]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
