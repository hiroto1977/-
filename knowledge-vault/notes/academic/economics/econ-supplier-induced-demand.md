---
collection: academic
id: econ-supplier-induced-demand
category: "economics"
category_ja: "経済学"
title: "供給者誘発需要——医師は自らの需要を生み出すのか"
as_of: "2026-06-27"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/economics
  - knowledge/verified
aliases:
  - "供給者誘発需要——医師は自らの需要を生み出すのか"
---

# 供給者誘発需要——医師は自らの需要を生み出すのか

> [!info] コレクション: [[学術概念]] ・ 区分: 経済学 ・ asOf: 2026-06-27 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
供給者誘発需要（supplier-induced demand、SID）は、医療経済学の中心的な概念で、サービスの供給者（とりわけ医師）が、自らの専門的な情報の優位を利用して、患者が十分な情報をもっていれば選ばなかったはずの需要を生み出しうる、という現象を指す。通常の市場では、買い手（需要者）が自分の必要と選好にもとづいて需要を決め、売り手（供給者）はそれに応じて供給する。需要と供給を別々の主体が独立に決めるからこそ、市場は機能する。しかし医療では、この前提が崩れる。患者は、自分にどんな検査・治療がどれだけ必要かを判断する専門知識をもたず、その判断を医師に委ねる。すなわち医師は、患者（依頼人）の利益のために意思決定を代行する「代理人（エージェント）」であると同時に、その医療サービスを提供して報酬を得る「供給者」でもある。この二重の立場と、医師と患者のあいだの著しい「情報の非対称性」が、問題を生む。医師は、自分の収入や稼働を増やしたいという誘因をもつとき、医学的に必要な水準を超えて、検査や手術、再診を勧めることができてしまう。患者は専門知識がないため、その勧めを疑いにくく、需要が供給側によって「誘発」される。この概念を象徴的に示すのが、ある地域で医師（外科医）の数が増えると、本来は需要が一定のはずなのに、手術の件数がかえって増える、という観察である（ヴィクター・フュックスらの研究）。供給が需要を生むこの関係は、医師数を増やせば競争で医療費が下がるという通常の市場の論理が、医療には素直に当てはまらないことを意味する。供給者誘発需要は、出来高払い（行為ごとの報酬）が過剰診療を招きやすいこと、医療費の抑制、診療報酬制度の設計、医師数の政策をめぐる議論の核心にあり、情報の非対称性と代理人関係が市場を歪める典型例として、医療経済学・情報の経済学の重要な概念となっている。

## 提唱者・初出
ヴィクター・フュックス ／ ロバート・エヴァンス ／ ケネス・アロー ／ トマス・マグワイア

## 出典
- [How Does Market Competition Affect Supplier-Induced Demand? (Frontiers in Public Health, 2023) — physician agency, information asymmetry (on Evans 1974, Fuchs 1978)](https://www.frontiersin.org/journals/public-health/articles/10.3389/fpubh.2023.1024337/full) `学術`
- [Wikipedia: Supplier-induced demand — physician as agent, information asymmetry, fee-for-service, Evans (1974), Fuchs (1978)](https://en.wikipedia.org/wiki/Supplier-induced_demand) `リファレンス`

## 関連概念
- [[econ-welfare-theorems|厚生経済学の基本定理]] — 同じ思想家
- [[econ-experience-curve-henderson|経験曲線——累積生産量の倍増ごとに単位費用が一定率で低下する]] — 同じ思想家
- [[econ-learning-by-doing|学習効果（ラーニング・バイ・ドゥーイング）]] — 同じ思想家
- [[econ-welfare-economics|厚生経済学：社会的厚生と資源配分効率性の理論的分析]] — 同じ思想家
- [[econ-ces-production|CES生産関数]] — 同じ思想家
- [[econ-condorcet-paradox-cycling|コンドルセのパラドックス——多数決における選好の循環と社会的選択の不安定性]] — 同じ思想家
- [[econ-contingent-valuation|仮想評価法——表明選好により非市場財の価値を計測する手法]] — 同じ思想家
- [[econ-discrimination-becker|差別の経済学——「差別への嗜好」と競争による差別の侵食]] — 同じ思想家
- [[econ-edgeworth-box-exchange-efficiency|エッジワース・ボックスと契約曲線——純粋交換経済における効率的配分の幾何学]] — 同じ思想家
- [[econ-elasticity-substitution-allen-uzawa|代替の弾力性——アレン＝ウザワの生産要素間代替可能性の測定]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
