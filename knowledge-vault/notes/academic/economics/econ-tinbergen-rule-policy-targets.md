---
collection: academic
id: econ-tinbergen-rule-policy-targets
category: "economics"
category_ja: "経済学"
title: "ティンバーゲンの定理——政策目標と政策手段の対応関係"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/economics
  - knowledge/verified
aliases:
  - "ティンバーゲンの定理——政策目標と政策手段の対応関係"
---

# ティンバーゲンの定理——政策目標と政策手段の対応関係

> [!info] コレクション: [[学術概念]] ・ 区分: 経済学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
ティンバーゲンの定理（Tinbergen rule）は、ヤン・ティンバーゲンが1952年の著書で定式化したマクロ経済政策の基本命題であり、N個の独立した政策目標を同時に達成するためには、少なくともN個の独立した政策手段が必要であることを示す。例えば、完全雇用と物価安定という二つの目標を達成するためには、財政政策と金融政策の二つの独立した手段が必要となる。この定理はマンデルの政策割当原則（principle of effective market classification）によって拡張され、各政策手段は最も比較優位を持つ目標に割り当てられるべきであるとされた。ティンバーゲンの定理は、一つの政策手段で複数の目標を同時に達成しようとする政策の限界を理論的に明確にし、政策手段の独立性と十分性が政策設計において不可欠であることを示した。現代のマクロプルーデンス政策においても、金融安定と物価安定の複数目標に対する手段の割当問題として応用されている。

## 提唱者・初出
ヤン・ティンバーゲン ／ ロバート・マンデル ／ ジェームズ・ミード ／ ラグナル・フリッシュ

## 出典
- [Tinbergen, J. (1952) On the Theory of Economic Policy — North-Holland Publishing](https://doi.org/10.2307/2226322) `学術`
- [Mundell, R. A. (1962) The Appropriate Use of Monetary and Fiscal Policy — IMF Staff Papers](https://doi.org/10.2307/2228447) `学術`
- [Wikipedia: Tinbergen rule — policy instruments and targets assignment overview](https://en.wikipedia.org/wiki/Tinbergen_rule) `リファレンス`

## 関連概念
- [[econ-expenditure-switching|支出転換政策と支出削減政策：内外均衡の同時達成]] — 同じ思想家
- [[econ-meade-conflict-internal-external|ミードの内外均衡葛藤——開放経済における政策目標の両立問題]] — 同じ思想家
- [[econ-mundell-fleming-model|マンデル＝フレミング・モデル（Mundell-Fleming Model）]] — 同じ思想家
- [[econ-mundell-tobin-effect|マンデル＝トービン効果]] — 同じ思想家
- [[econ-swan-diagram-internal-external|スワン・ダイアグラム——支出転換・支出増減政策の図式的分析]] — 同じ思想家
- [[econ-trade-balance-current-account-absorption|経常収支と吸収アプローチ——アレキサンダーの国民所得恒等式と為替政策の効果]] — 同じ思想家
- [[econ-absorption-approach-balance-payments|アブソープション・アプローチ——所得と総支出の差として捉える国際収支]] — 同じ思想家
- [[econ-customs-union-theory-viner|関税同盟の理論——貿易創出効果と貿易転換効果の厚生分析]] — 同じ思想家
- [[econ-hicks-trade-cycle|ヒックスの景気循環論——天井と床に挟まれた持続的な循環]] — 同じ思想家
- [[econ-laffer-curve-supply-side|ラッファー曲線——供給側経済学と税率・税収の非線形関係]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
