---
collection: academic
id: econ-trade-adjustment-assistance-kaldor-hicks
category: "economics"
category_ja: "経済学"
title: "カルドア＝ヒックス基準と貿易調整支援——自由貿易の恩恵と損失者への再分配政策"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/economics
  - knowledge/verified
aliases:
  - "カルドア＝ヒックス基準と貿易調整支援——自由貿易の恩恵と損失者への再分配政策"
---

# カルドア＝ヒックス基準と貿易調整支援——自由貿易の恩恵と損失者への再分配政策

> [!info] コレクション: [[学術概念]] ・ 区分: 経済学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
カルドア＝ヒックス効率性基準は、政策変更により勝者の利得が敗者の損失を上回る場合（たとえ実際の補償が行われなくても）に効率改善とみなすもので、自由貿易政策の評価に広く用いられる。しかし貿易自由化で製造業雇用を失った労働者への補償が実際に行われなければ、分配上の問題が深刻化する。米国では1962年の貿易拡大法によって設立された「貿易調整支援（TAA）プログラム」が、輸入競争により失業した労働者への所得補助・職業訓練・就職支援を提供するが、認定条件の厳格さ・給付水準の低さ・執行の複雑さから実効性の評価は分かれる。ダニ・ロドリックはグローバル化の「政治的トリレンマ」を論じ、グローバル経済統合・民主主義・国家主権の三者は同時に完全実現できず、貿易政策は常に国内分配との緊張関係にあることを指摘した。

## 提唱者・初出
ニコラス・カルドア ／ ジョン・ヒックス ／ ダニ・ロドリック ／ ポール・クルーグマン

## 出典
- [Autor, Dorn & Hanson (2016) The China Shock — Annual Review of Economics](https://doi.org/10.1146/annurev-economics-080315-015041) `学術`
- [Rodrik (2000) How Far Will International Economic Integration Go? — Journal of Economic Perspectives](https://doi.org/10.3386/w7864) `学術`
- [Wikipedia: Kaldor–Hicks efficiency — カルドア＝ヒックス効率性基準](https://en.wikipedia.org/wiki/Kaldor%E2%80%93Hicks_efficiency) `リファレンス`

## 関連概念
- [[econ-compensating-equivalent-variation|補償変分と等価変分（ヒックスの厚生変化測度）]] — 同じ思想家
- [[econ-verdoorn-law|フェルドーンの法則（カルドア＝フェルドーンの法則）]] — 同じ思想家
- [[econ-liquidity-trap|流動性のわな]] — 同じ思想家
- [[econ-agglomeration-economies|集積の経済]] — 同じ思想家
- [[econ-backwardation-contango|バックワーデーションとコンタンゴ（Backwardation & Contango）]] — 同じ思想家
- [[econ-backwash-spread-myrdal|逆流効果と波及効果——ミュルダールの循環的累積的因果関係論]] — 同じ思想家
- [[econ-cost-benefit-analysis-kaldor-hicks|カルドア＝ヒックス基準——補償可能性に基づく潜在的パレート改善]] — 同じ思想家
- [[econ-endogenous-money|内生的貨幣供給理論（Endogenous Money Theory）]] — 同じ思想家
- [[econ-globalization-backlash-rodrik-trilemma|グローバリゼーションのトリレンマ——ロドリックの超グローバル化・民主主義・国家主権の不可能三角形]] — 同じ思想家
- [[econ-is-lm|IS-LMモデル]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
