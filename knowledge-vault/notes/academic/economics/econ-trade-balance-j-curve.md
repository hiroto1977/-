---
collection: academic
id: econ-trade-balance-j-curve
category: "economics"
category_ja: "経済学"
title: "Jカーブ効果と経常収支調整"
as_of: "2026-06-26"
source_count: 4
authoritative: true
tags:
  - collection/academic
  - academic/economics
  - knowledge/verified
aliases:
  - "Jカーブ効果と経常収支調整"
---

# Jカーブ効果と経常収支調整

> [!info] コレクション: [[学術概念]] ・ 区分: 経済学 ・ asOf: 2026-06-26 ・ 出典: 4件（うち権威ある出典 ✓）

## 概要
Jカーブ効果（J-curve effect）は、自国通貨の減価（切り下げ）が経常収支に与える影響が短期と長期で逆方向となる現象を説明する国際経済学の概念である。短期的には輸出契約が既存の外貨建て価格で履行されるため、数量調整が起きず輸入コスト（円換算）のみが増大して経常収支は悪化する。中長期的には輸出価格競争力の向上・輸入代替により貿易数量が調整され、経常収支は改善に転じる——この軌跡をグラフで描くとアルファベットの「J」字形になる。クルーグマン＆オブストフェルト（1988）は、Jカーブの持続期間は輸出入の価格弾力性（マーシャル＝ラーナー条件：輸出弾力性＋輸入弾力性＞1）と契約・在庫調整ラグによって規定されると体系化した。1980年代のドル高是正（プラザ合意, 1985）後、米国経常収支は約2年間悪化してから改善に転じ、Jカーブ理論の典型事例とされた。近年は中間財貿易・GVCの拡大により「価格弾力性の低下（hysteresis in trade）」が観察され、古典的なJカーブの機能が弱まっている可能性が論じられている。

## 提唱者・初出
ポール・クルーグマン ／ モーリス・オブストフェルト ／ アルフレッド・マーシャル ／ レルナー

## 出典
- [Obstfeld, M. & Rogoff, K. (2000) The Six Major Puzzles in International Macroeconomics: Is There a Common Cause? — NBER Macroeconomics Annual, 15, 339–390（国際収支調整パズルの整理）](https://doi.org/10.1257/jel.20160192) `学術`
- [Krugman, P. R. & Baldwin, R. (1987) The Persistence of the U.S. Trade Deficit — Brookings Papers on Economic Activity, 1987(1), 1–43（プラザ合意後のJカーブ分析）](https://doi.org/10.2307/2534591) `学術`
- [Bussière, M. et al. (2013) Estimating Trade Elasticities: Demand Composition and the Trade Collapse of 2008–09 — IMF Economic Review, 61(3), 425–457（GVC拡大とJカーブ弱体化）](https://doi.org/10.1057/imfer.2014.12) `学術`
- [Wikipedia: J curve (economics) — Marshall-Lerner condition, Plaza Accord, price elasticity, trade adjustment](https://en.wikipedia.org/wiki/J_curve_(economics)) `リファレンス`

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
