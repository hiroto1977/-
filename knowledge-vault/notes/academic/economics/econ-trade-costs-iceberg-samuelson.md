---
collection: academic
id: econ-trade-costs-iceberg-samuelson
category: "economics"
category_ja: "経済学"
title: "アイスバーグ型貿易費用——サミュエルソンの溶解モデルと貿易障壁の定量化"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/economics
  - knowledge/verified
aliases:
  - "アイスバーグ型貿易費用——サミュエルソンの溶解モデルと貿易障壁の定量化"
---

# アイスバーグ型貿易費用——サミュエルソンの溶解モデルと貿易障壁の定量化

> [!info] コレクション: [[学術概念]] ・ 区分: 経済学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
ポール・サミュエルソンが1954年に導入した「アイスバーグ型貿易費用」（iceberg trade costs）は、財の輸送において一定割合が氷山の溶解のように途中で「溶けて」消失するというモデリング手法であり、関税・輸送費・非関税障壁等の貿易費用を技術的に簡潔に表現する。1単位の財を出荷してτ > 1単位を発送する必要がある場合、τ − 1が貿易費用率を表す。この定式化はCES選好との相性の良さから、クルーグマンの新貿易理論・新経済地理学、メリッツの異質的企業モデル、アンダーソン＝ヴァン・ウィンクープの構造的重力モデルの全てで標準的な貿易費用の表現として採用された。アンダーソンとヴァン・ウィンクープの推定では、先進国間の「代表的な」貿易費用は従価換算で約74%（関税相当）に達し、その内訳として運輸費用21%・国境効果44%・通貨障壁14%等が特定された。ネイヴィとニクイタの包括的データベースは二国間貿易費用の時系列推定を提供している。

## 提唱者・初出
ポール・サミュエルソン ／ ジェームズ・アンダーソン ／ エリック・ヴァン・ウィンクープ ／ デイヴィッド・ネイヴィ ／ デニス・ニクイタ

## 出典
- [Samuelson, P. A. (1954) The Transfer Problem and Transport Costs, II: Analysis of Effects of Trade Impediments — The Economic Journal 64(254), 264–289](https://doi.org/10.2307/2226834) `学術`
- [Anderson, J. E. & van Wincoop, E. (2004) "Trade Costs" — Journal of Economic Literature](https://doi.org/10.1257/jel.42.3.691) `学術`
- [Wikipedia: Iceberg transport costs — サミュエルソンの貿易費用モデル](https://en.wikipedia.org/wiki/Iceberg_transport_costs) `リファレンス`

## 関連概念
- [[econ-gravity-equation-structural-estimation|構造的重力モデル——貿易フローの理論的基盤と推定]] — 語彙が近い
- [[human-dual-coding-paivio|二重符号化理論——言語と心像の二つの系統による情報処理]] — 同じ思想家
- [[econ-prebisch-singer-thesis|プレビッシュ＝シンガー命題]] — 同分野の近傍
- [[econ-eaton-kortum-trade|イートン＝コータム・モデル——確率的リカード・モデルと貿易の定量分析]] — 同分野の近傍
- [[econ-ellsberg-paradox|エルズバーグのパラドックス（曖昧性回避）]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
