---
collection: academic
id: econ-transfer-problem-keynes
category: "economics"
category_ja: "経済学"
title: "移転問題——ケインズ＝オリーン論争と国際資本移動の交易条件効果"
as_of: "2026-06-26"
source_count: 4
authoritative: true
tags:
  - collection/academic
  - academic/economics
  - knowledge/verified
aliases:
  - "移転問題——ケインズ＝オリーン論争と国際資本移動の交易条件効果"
---

# 移転問題——ケインズ＝オリーン論争と国際資本移動の交易条件効果

> [!info] コレクション: [[学術概念]] ・ 区分: 経済学 ・ asOf: 2026-06-26 ・ 出典: 4件（うち権威ある出典 ✓）

## 概要
移転問題（transfer problem）は、国際的な一方的資本移動（戦争賠償・援助・債務返済）が交易条件に及ぼす影響をめぐる古典的論争である。第一次大戦後のドイツ賠償問題を契機に、ケインズは賠償支払国が実質的に二重の負担を被ると主張した。すなわち、賠償金の財政移転に加え、輸出増加のために交易条件が悪化する「二次的負担」が生じるとした。これに対しオリーンは、所得効果を通じて受取国の需要増と支払国の需要減が自動的に調整されるため、交易条件の追加的悪化は必ずしも生じないと反論した。サミュエルソンは後にこの論争を一般均衡モデルで再定式化し、限界消費性向と貿易財の支出弾力性に依存して結果が決まることを示した。移転が支払国の交易条件を悪化させる場合を orthodox case、改善させる場合を anti-orthodox case と呼び、いずれになるかは両国の輸入財に対する限界支出性向に依存する。バグワティらはこの枠組みを厚生分析へ拡張した。この移転問題は現代でもIMFの構造調整融資、ユーロ圏内の財政移転、対外援助の効果分析、および資源収入流入が交易条件を通じて製造業を圧迫するオランダ病の分析において重要な分析枠組みであり続けている。

## 提唱者・初出
ジョン・メイナード・ケインズ ／ ベルティル・オリーン ／ ポール・サミュエルソン ／ ジャック・ヴァイナー

## 出典
- [Keynes, J.M. (1929) The German Transfer Problem — Economic Journal 39(153)](https://doi.org/10.2307/2224211) `学術`
- [Wikipedia: Transfer Problem — Keynes–Ohlin debate overview](https://en.wikipedia.org/wiki/Transfer_problem) `リファレンス`
- [Samuelson, P. A. (1952) The Transfer Problem and Transport Costs: The Terms of Trade When Impediments Are Absent — Economic Journal 62(246), 278-304](https://doi.org/10.2307/2227005) `学術`
- [Jones, R. W. & Neary, J. P. (1984) The Positive Theory of International Trade — Handbook of International Economics](https://doi.org/10.1016/S1573-4404(84)01006-4) `学術`

## 関連概念
- [[econ-backwardation-contango|バックワーデーションとコンタンゴ（Backwardation & Contango）]] — 同じ思想家
- [[econ-fallacy-composition-samuelson|合成の誤謬——個別的合理性と集合的帰結の乖離]] — 同じ思想家
- [[econ-fiscal-policy-automatic-stabilizers|財政政策と自動安定化装置——ビルトイン・スタビライザーとケインズ的裁量的財政出動]] — 同じ思想家
- [[econ-heckscher-ohlin|ヘクシャー＝オリーン理論]] — 同じ思想家
- [[econ-keynesian-beauty-contest|ケインズの美人投票]] — 同じ思想家
- [[econ-keynesian-cross|ケインジアンの45度線分析——有効需要が均衡国民所得を決める]] — 同じ思想家
- [[econ-keynesian-multiplier-fiscal|ケインズ乗数と財政政策の波及効果——有効需要管理の理論的基盤]] — 同じ思想家
- [[econ-knightian-uncertainty|ナイトの不確実性（リスクと不確実性）]] — 同じ思想家
- [[econ-paradox-of-thrift|倹約のパラドックス]] — 同じ思想家
- [[econ-specific-factors-model-jones|特殊要素モデル——部門間要素移動の不完全性と所得分配]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
