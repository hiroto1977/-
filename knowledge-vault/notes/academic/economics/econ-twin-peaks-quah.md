---
collection: academic
id: econ-twin-peaks-quah
category: "economics"
category_ja: "経済学"
title: "双峰分布仮説——世界所得分布の二極化と収束クラブの形成"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/economics
  - knowledge/verified
aliases:
  - "双峰分布仮説——世界所得分布の二極化と収束クラブの形成"
---

# 双峰分布仮説——世界所得分布の二極化と収束クラブの形成

> [!info] コレクション: [[学術概念]] ・ 区分: 経済学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
ダニー・クアーが1996年に提示した「双峰分布仮説（twin peaks hypothesis）」は、世界の国別一人当たり所得の分布が時間とともに一つの峰（単峰分布）から二つの峰（双峰分布）へと変容する傾向を実証的に示した研究である。新古典派成長理論が予測する絶対的収束（貧困国が富裕国に追いつく）に反し、クアーはカーネル密度推定法を用いて、所得分布が中間層の消失と低所得・高所得の二極への集中を示す「消えゆく中間」パターンを発見した。この分析は「収束クラブ」の概念と関連し、初期条件や制度的特徴が類似した国々は同一の定常状態に収束するが、異なるクラブに属する国々の間では格差が持続・拡大するという見方を支持した。ガロアとツィドンの統一成長理論、アセモグルらの制度決定論とも接続し、なぜ一部の国が「貧困の罠」から脱出できないのかという開発経済学の中心的問題に実証的基盤を提供した。

## 提唱者・初出
ダニー・クアー ／ ロバート・バロー ／ ダロン・アセモグル ／ オーデッド・ガロア

## 出典
- [Quah, D.T. (1996) Twin Peaks: Growth and Convergence in Models of Distribution Dynamics — Economic Journal 106(437)](https://doi.org/10.2307/2235381) `学術`
- [Wikipedia: Convergence (economics) — conditional convergence and convergence clubs](https://en.wikipedia.org/wiki/Convergence_(economics)) `リファレンス`
- [Durlauf, S.N., Johnson, P.A. & Temple, J.R.W. (2005) Growth Econometrics — Handbook of Economic Growth vol.1A](https://doi.org/10.1257/jel.40.3.869) `学術`

## 関連概念
- [[econ-tax-smoothing|課税平準化（タックス・スムージング）]] — 同じ思想家
- [[econ-barro-growth|バロー成長回帰（Barro Growth Regression）]] — 同じ思想家
- [[econ-inclusive-institutions|包摂的制度と収奪的制度]] — 同じ思想家
- [[econ-ricardian-equivalence|リカードの等価定理（リカード=バローの中立命題）]] — 同じ思想家
- [[econ-becker-quantity-quality|子どもの数と質のトレードオフ（ベッカー）——なぜ豊かになると出生率が下がるのか]] — 同じ思想家
- [[econ-convergence-hypothesis|収束仮説——資本の収穫逓減が貧しい経済の追い上げを駆動する]] — 同じ思想家
- [[econ-disequilibrium-economics-clower|不均衡経済学（クラウワー）]] — 同じ思想家
- [[econ-equity-premium-puzzle-mehra-prescott|株式プレミアム・パズル——標準理論で説明できない株式の超過収益]] — 同じ思想家
- [[econ-fiscal-multiplier-auerbach-gorodnichenko|財政乗数——景気局面依存性とアウアーバッハ＝ゴロドニチェンコの状態依存推定]] — 同じ思想家
- [[econ-fiscal-multiplier-crowding-out-debate|財政乗数とクラウディングアウト論争——IS-LMモデルから実証的な乗数推定への展開]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
