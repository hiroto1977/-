---
collection: academic
id: human-balance-theory-heider
category: "human-science"
category_ja: "人間科学"
title: "認知的バランス理論（ハイダー）——P-O-X三者関係の均衡"
as_of: "2026-06-27"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/human-science
  - knowledge/verified
aliases:
  - "認知的バランス理論（ハイダー）——P-O-X三者関係の均衡"
---

# 認知的バランス理論（ハイダー）——P-O-X三者関係の均衡

> [!info] コレクション: [[学術概念]] ・ 区分: 人間科学 ・ asOf: 2026-06-27 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
認知的バランス理論（balance theory）は、心理学者フリッツ・ハイダーが1946年・1958年に提唱した社会心理学・認知の一貫性の理論で、人は自分の態度や対人関係のあいだに「つじつまの合った（バランスのとれた）」状態を保とうとする動機を持つ、と論じる。中心となるのが「P-O-X三角形」のモデルである。Pは当人（観察者）、Oは他者、Xは何らかの対象（人・もの・考え・出来事）を表す。これら三者のあいだには、それぞれ好意・同意を表す「正（＋）」か、反感・不同意を表す「負（−）」の関係が結ばれる。ハイダーは、三つの関係の符号を掛け合わせた積が「正」になるとき、その三者関係は「バランスがとれている」と定義した。たとえば、PがOを好きで（P–O＝＋）、Oがある対象Xを好きで（O–X＝＋）、PもXを好き（P–X＝＋）なら、積は正でバランスがとれている。「友の友は友」「敵の敵は味方」といった関係はバランスがとれており、心理的に安定して快い。逆に、積が「負」になる関係（たとえば、好きな友人Oが、自分の嫌いなものXを好んでいる場合）は「アンバランス（不均衡）」であり、緊張・不快・違和感を生む。人はこの不快な不均衡を解消しようとして、いずれかの関係を変える——Xへの態度を変える、Oへの評価を改める、あるいはO–Xの関係についての認識を変える——よう動機づけられる。バランス理論は、態度変化、対人魅力、説得（好きな人が薦めるものを好きになる）、認知的一貫性の研究の礎となり、フェスティンガーの認知的不協和理論に先立つ、認知的整合性理論の源流の一つとなった。カートライトとハラリーは、この均衡の概念をグラフ理論を用いて任意の人数・対象へと一般化し（構造的バランス理論）、後の社会ネットワーク分析や符号つきネットワークの研究へとつながった。なお本理論は、ハイダーのもう一つの貢献である帰属理論（原因の推論）とは別個の、態度の一貫性をめぐる理論である。

## 提唱者・初出
フリッツ・ハイダー ／ ドーウィン・カートライト ／ フランク・ハラリー ／ レオン・フェスティンガー

## 出典
- [Heider, F. (1946) Attitudes and Cognitive Organization — The Journal of Psychology, 21(1), 107–112](https://doi.org/10.1080/00223980.1946.9917275) `学術`
- [Wikipedia: Balance theory — Heider, P-O-X triad, cognitive consistency, Cartwright & Harary graph formalization](https://en.wikipedia.org/wiki/Balance_theory) `リファレンス`

## 関連概念
- [[human-cognitive-miser|認知的倹約家（cognitive miser）]] — 同じ思想家
- [[human-cognitive-dissonance|認知的不協和（cognitive dissonance）]] — 同じ思想家
- [[human-social-comparison-theory-festinger|社会比較理論と自己評価プロセスの社会心理学（フェスティンガー）]] — 同じ思想家
- [[human-attribution-theory|帰属理論]] — 同じ思想家
- [[infosoc-selective-exposure|選択的接触理論]] — 同じ思想家
- [[human-autokinetic-effect|自動運動効果——暗闇の光点が動いて見える現象と社会規範の形成]] — 同じ思想家
- [[human-cognitive-dissonance-festinger|認知的不協和理論（フェスティンガー）]] — 同じ思想家
- [[human-cognitive-dissonance-revision|認知的不協和理論の現代的改訂——行為ベースモデルとアクション同定理論]] — 同じ思想家
- [[human-deindividuation-zimbardo|没個性化——集団状況における自己意識の低下と脱抑制的行動]] — 同じ思想家
- [[human-self-perception-bem|自己知覚理論——内的手がかりが曖昧なとき人は自らの行動から態度を推論する]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
