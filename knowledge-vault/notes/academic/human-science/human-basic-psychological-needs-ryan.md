---
collection: academic
id: human-basic-psychological-needs-ryan
category: "human-science"
category_ja: "人間科学"
title: "基本的心理欲求理論（ライアン＆デシ）"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/human-science
  - knowledge/verified
aliases:
  - "基本的心理欲求理論（ライアン＆デシ）"
---

# 基本的心理欲求理論（ライアン＆デシ）

> [!info] コレクション: [[学術概念]] ・ 区分: 人間科学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
基本的心理欲求理論（Basic Psychological Needs Theory: BPNT）は、自己決定理論（SDT）のサブ理論であり、人間の最適な発達と幸福のために普遍的に必要な三つの心理的欲求――自律性（autonomy）・有能感（competence）・関係性（relatedness）――を提唱する。リチャード・ライアンとエドワード・デシは、これらの欲求が満たされると内発的動機付けと心理的健康が促進され、欲求が阻害されると苦悩・不安・疎外感が生じることを文化横断的な実証研究により示した。職場・教育・医療・スポーツなど多様な文脈における応用研究が蓄積されており、「欲求満足度」と「欲求阻害」は独立した次元として測定される。この理論は人間の普遍的な欲求を想定する点でマズローの欲求階層説と共鳴しつつも、欲求の充足メカニズムと動機付けの質的差異をより精緻に分析する点で異なる。

## 提唱者・初出
リチャード・ライアン ／ エドワード・デシ ／ マーティン・セリグマン ／ キャロル・ドウェック

## 出典
- [Ryan, R.M. & Deci, E.L. (2000). Self-determination theory and intrinsic motivation — Psychological Review](https://doi.org/10.1037/0033-295X.107.1.1) `学術`
- [Wikipedia: Basic psychological needs — self-determination theory](https://en.wikipedia.org/wiki/Basic_psychological_needs) `リファレンス`

## 関連概念
- [[human-learned-optimism|学習性楽観主義]] — 同じ思想家
- [[human-flow-theory-csikszentmihalyi|フロー理論——チクセントミハイの最適経験と内発的動機づけ]] — 同じ思想家
- [[econ-fine-is-a-price-gneezy-rustichini|「罰金は価格である」——金銭的インセンティブが内発的・社会的動機を締め出し関係を取引に変える現象]] — 同じ思想家
- [[human-learned-helplessness|学習性無力感（learned helplessness）]] — 同じ思想家
- [[human-self-determination-intrinsic|自己決定理論における内発的動機づけ——自律性・有能感・関係性の基本的心理欲求]] — 同じ思想家
- [[human-broaden-and-build-fredrickson|拡張＆構築理論（フレドリクソン）]] — 同じ思想家
- [[human-explanatory-style-attribution|説明スタイルと帰属理論（ペターソン＆セリグマン）]] — 同じ思想家
- [[human-explanatory-style-seligman|説明スタイル——出来事の原因をどう説明するかが楽観と悲観を分ける]] — 同じ思想家
- [[human-flow-optimal-experience-csikszentmihalyi|フロー理論と最適経験の積極心理学（チクセントミハイ）]] — 同じ思想家
- [[human-flow-psychology-csikszentmihalyi|フロー理論と最適経験——チクセントミハイによる没入状態と人間の幸福の心理学]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
