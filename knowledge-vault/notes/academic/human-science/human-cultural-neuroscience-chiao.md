---
collection: academic
id: human-cultural-neuroscience-chiao
category: "human-science"
category_ja: "人間科学"
title: "文化神経科学——文化と脳機能の双方向的相互作用"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/human-science
  - knowledge/verified
aliases:
  - "文化神経科学——文化と脳機能の双方向的相互作用"
---

# 文化神経科学——文化と脳機能の双方向的相互作用

> [!info] コレクション: [[学術概念]] ・ 区分: 人間科学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
ジョアン・チャオとアンベディカルが2010年に体系化した文化神経科学（cultural neuroscience）は、文化的経験が脳の構造・機能にいかに影響し、逆に神経生物学的制約が文化的行動をいかに条件づけるかを、fMRI・EEG等の神経画像法と文化心理学の理論を統合して研究する学際領域である。チャオとアンベディカルの枠組みは、文化（価値観・信念・実践）→遺伝子発現（エピジェネティクス）→神経回路→行動→文化の循環的因果モデルを提示し、文化を「外部変数」としてではなく脳発達の構成的要因として位置づけた。ハンとノースオフの異文化比較fMRI研究は、自己参照処理における内側前頭前野の活動パターンが個人主義文化と集団主義文化で異なることを示し、キタヤマとウシクボは文化的自己構成理論と神経科学的知見の統合を試みている。

## 提唱者・初出
ジョアン・チャオ ／ シヘイ・ハン ／ ゲオルグ・ノースオフ ／ 北山忍 ／ ナリニ・アンベディカル

## 出典
- [Chiao, J. Y. & Ambady, N. (2007) "Cultural Neuroscience" — Handbook of Cultural Psychology](https://doi.org/10.1016/S0079-6123(09)17818-4) `学術`
- [Han, S. & Northoff, G. (2008) "Culture-Sensitive Neural Substrates of Human Cognition" — Nature Reviews Neuroscience](https://doi.org/10.1038/nrn2646) `学術`
- [Wikipedia: Cultural neuroscience — 文化と脳機能の相互作用研究](https://en.wikipedia.org/wiki/Cultural_neuroscience) `リファレンス`

## 関連概念
- [[human-fundamental-attribution-error|根本的な帰属の誤り]] — 同分野の近傍
- [[human-theory-of-constructed-emotion|情動の構成理論]] — 同分野の近傍
- [[human-critical-period-hubel-wiesel|臨界期——発達初期の経験依存的な神経可塑性の窓]] — 同分野の近傍
- [[human-developmental-psychopathology-cicchetti|発達精神病理学——正常と異常の発達的連続性と多重決定性]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
