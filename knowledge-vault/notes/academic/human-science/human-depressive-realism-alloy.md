---
collection: academic
id: human-depressive-realism-alloy
category: "human-science"
category_ja: "人間科学"
title: "抑うつリアリズム——軽度抑うつ者における判断の正確性仮説"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/human-science
  - knowledge/verified
aliases:
  - "抑うつリアリズム——軽度抑うつ者における判断の正確性仮説"
---

# 抑うつリアリズム——軽度抑うつ者における判断の正確性仮説

> [!info] コレクション: [[学術概念]] ・ 区分: 人間科学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
抑うつリアリズム（Depressive Realism）とは、軽度の抑うつ状態にある個人が、非抑うつ者よりも現実をより正確に知覚・判断するという仮説である。アロイとエイブラムソンは1979年の画期的実験で、抑うつ傾向の学生と非抑うつの学生にランダムな結果（光の点灯）に対する統制感を評価させたところ、非抑うつ者が自己の統制力を過大評価する「統制の幻想」を示したのに対し、抑うつ傾向者はより正確な推定を行うことを示した。この知見は「悲しいけれど賢い」（Sadder but Wiser）効果として広く知られ、正常な認知機能にはポジティブな幻想（positive illusions）が含まれるというテイラーとブラウンの理論を補完する。ただし後続の研究では、抑うつリアリズムの効果が課題の種類や文脈に依存すること、中度以上の臨床的うつ病では判断がネガティブに歪むことが示され、仮説の適用範囲に関する議論が続いている。

## 提唱者・初出
ローレン・アロイ ／ リン・エイブラムソン ／ シェリー・テイラー ／ ジョナサン・ブラウン

## 出典
- [Alloy & Abramson (1979) Judgment of Contingency in Depressed and Nondepressed Students: Sadder but Wiser? — Journal of Experimental Psychology: General](https://doi.org/10.1037/0021-843X.88.4.441) `学術`
- [Taylor & Brown (1988) Illusion and Well-Being: A Social Psychological Perspective on Mental Health — Psychological Bulletin](https://doi.org/10.1037/0033-2909.103.2.193) `学術`
- [Wikipedia: Depressive realism — 抑うつリアリズム仮説の実験的証拠と批判的検討](https://en.wikipedia.org/wiki/Depressive_realism) `リファレンス`

## 関連概念
- [[human-positive-illusions-taylor|積極的錯覚（テイラー＆ブラウン）]] — 同じ思想家
- [[human-tend-and-befriend|思いやり・絆形成反応（テンド・アンド・ビフレンド）]] — 同じ思想家
- [[human-cognitive-apprenticeship|認知的徒弟制]] — 同じ思想家
- [[human-explanatory-style-attribution|説明スタイルと帰属理論（ペターソン＆セリグマン）]] — 同じ思想家
- [[human-learned-helplessness-seligman-abramson|学習性無力感——セリグマンとエイブラムソンの統制不可能性体験と抑うつの帰属理論]] — 同じ思想家
- [[human-self-verification-swann|自己確証理論（スワン）——自己像を裏づける評価を求める動機]] — 同じ思想家
- [[human-tend-and-befriend-taylor|思いやり・絆反応——闘争逃走に並ぶストレス対処と養護・連帯の様式]] — 同じ思想家
- [[infosoc-internet-of-things-weiser-ubicomp|IoTとユビキタスコンピューティング——ワイザーの「コンピュータは消える」とスマートデバイス社会]] — 同じ思想家
- [[human-compensatory-control-kay|補償的制御理論（コンペンサトリー・コントロール理論）]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
