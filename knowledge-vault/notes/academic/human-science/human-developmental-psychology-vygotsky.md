---
collection: academic
id: human-developmental-psychology-vygotsky
category: "human-science"
category_ja: "人間科学"
title: "文化歴史的発達理論——ヴィゴツキーの最近接発達領域と社会的学習の認知科学"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/human-science
  - knowledge/verified
aliases:
  - "文化歴史的発達理論——ヴィゴツキーの最近接発達領域と社会的学習の認知科学"
---

# 文化歴史的発達理論——ヴィゴツキーの最近接発達領域と社会的学習の認知科学

> [!info] コレクション: [[学術概念]] ・ 区分: 人間科学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
レフ・ヴィゴツキーはソビエト時代の心理学者として、ピアジェの認知発達論への批判的応答として「文化歴史的発達理論（Cultural-Historical Activity Theory）」を構築した。核心概念の「最近接発達領域（Zone of Proximal Development, ZPD）」は「現在独力でできること」と「有能な他者の助けを借りてできること」の差異として定義され、学習はZPD内で他者との社会的相互作用を通じて内面化（内化）されると主張した。「スキャフォールディング（Scaffolding）」——ウッド・ブルーナー・ロスが1976年に命名——は支援者が子どもの学習過程で段階的な足場を提供し徐々に撤退する教育的関与の形式である。ヴィゴツキーは言語を認知発達の主要媒介と位置づけ、独り言（私語）が外言から内言への移行過程にあることを指摘した。現代の「活動理論（Activity Theory）」——アレクセイ・レオンチェフ→ユリア・エングェストロム——はこれをHCI・職場学習・組織学習研究に応用した。日本では協調学習・協働型授業設計・アクティブラーニングの理論的基盤として活用されている。

## 提唱者・初出
レフ・ヴィゴツキー ／ アレクセイ・レオンチェフ ／ ジェローム・ブルーナー ／ ユリア・エングェストロム

## 出典
- [Vygotsky, L.S. (1978) Mind in Society: The Development of Higher Psychological Processes — Harvard University Press](https://www.cambridge.org/core/books/mind-in-society/84B22D53A2E7E628000B22EDB9E7FE5A) `学術`
- [文部科学省 アクティブラーニングの理論的背景 — 文部科学省](https://www.mext.go.jp/b_menu/shingi/chukyo/chukyo3/siryo/attach/1325939.htm) `公的`
- [Wikipedia: ヴィゴツキー — 最近接発達領域と文化歴史的発達理論の解説](https://ja.wikipedia.org/wiki/%E3%83%B4%E3%82%A3%E3%82%B4%E3%83%84%E3%82%AD%E3%83%BC) `リファレンス`

## 関連概念
- [[infosoc-activity-theory|活動理論（Activity Theory）：ヴィゴツキー＆レオンチェフの道具媒介活動システム論]] — 同じ思想家
- [[human-cultural-historical-activity-theory-engestrom|文化歴史的活動理論——エンゲストロームの拡張的学習と活動システム分析]] — 同じ思想家
- [[human-narrative-psychology|ナラティブ心理学：物語を通じたアイデンティティと意味構築の理論]] — 同じ思想家
- [[human-zone-of-proximal-development|発達の最近接領域（ZPD）]] — 同じ思想家
- [[human-ecological-systems-theory-bronfenbrenner|生態学的システム理論——ブロンフェンブレンナーの入れ子モデルと人間発達の環境的文脈]] — 同じ思想家
- [[tax-donation-deduction|法人の寄附金の損金算入限度]] — 出典を共有
- [[legal-work-for-hire|職務著作（法人著作・著作権法15条）]] — 出典を共有
- [[subsidy-highschool-tuition|高等学校等就学支援金]] — 出典を共有

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
