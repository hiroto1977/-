---
collection: academic
id: human-door-in-the-face-cialdini
category: "human-science"
category_ja: "人間科学"
title: "ドア・イン・ザ・フェイス——過大な要請の拒否後に本命の要請を通す互恵的譲歩"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/human-science
  - knowledge/verified
aliases:
  - "ドア・イン・ザ・フェイス——過大な要請の拒否後に本命の要請を通す互恵的譲歩"
---

# ドア・イン・ザ・フェイス——過大な要請の拒否後に本命の要請を通す互恵的譲歩

> [!info] コレクション: [[学術概念]] ・ 区分: 人間科学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
ドア・イン・ザ・フェイス（door-in-the-face）技法は、ロバート・チャルディーニらが1975年の研究で実証した承諾誘導の方略で、まず相手がほぼ確実に断るであろう過大な要請を持ちかけ、それが拒否された後に、本来の目的である小さく穏当な要請を出すと、最初からその小さな要請だけをする場合よりも応諾率が高まる現象を指す。名称は、訪問者が玄関先で最初の大きな要請を「門前払い（ドアを顔の前で閉められる）」されることに由来し、最初に小さな承諾から入る「フット・イン・ザ・ドア」とは逆向きの段階的方略である。チャルディーニの実験では、青少年を二年間ボランティアで指導するという過大な要請をまず断られた後に、「一度だけ動物園への遠足に付き添ってほしい」と頼むと、いきなり遠足の依頼をした群よりはるかに高い割合で承諾が得られた。この効果を説明する中心的メカニズムが「互恵的譲歩（reciprocal concessions）」である。要請者が大きな要請から小さな要請へと「譲歩」して見せると、受け手は互恵性の規範に従って自分も譲歩で応えなければならないという社会的圧力を感じ、小さな要請を受け入れる。加えて、過大な要請を基準点として後の要請が相対的に小さく見える「知覚的対比」も寄与する。ドア・イン・ザ・フェイスは、交渉・募金・マーケティングの実務に広く用いられ、互恵性と譲歩の心理を理解する説得研究の古典的知見となっている。

## 提唱者・初出
ロバート・チャルディーニ ／ ヴァンサン・ジュール ／ ダニエル・ハワード ／ ジェリー・バーガー

## 出典
- [Cialdini, R.B. et al. (1975) Reciprocal Concessions Procedure for Inducing Compliance: The Door-in-the-Face — JPSP 31(2)](https://doi.org/10.1037/h0076284) `学術`
- [Wikipedia: Door-in-the-face technique — Cialdini, reciprocal concessions, perceptual contrast](https://en.wikipedia.org/wiki/Door-in-the-face_technique) `リファレンス`

## 関連概念
- [[human-cialdini-influence|チャルディーニの影響力の6原理]] — 同じ思想家
- [[human-reciprocity|返報性の原理（reciprocity）]] — 同じ思想家
- [[human-foot-in-the-door-freedman|フット・イン・ザ・ドア——小さな承諾が大きな要請への応諾を高める]] — 同じ思想家
- [[human-norm-reciprocity-gouldner|返報性の規範（ゴールドナー）——「お返し」を求める普遍的な規範]] — 同じ思想家
- [[human-psychological-reactance-brehm|心理的リアクタンス理論——自由の脅威に対する動機づけ的反発]] — 同じ思想家
- [[human-reactance-brehm|心理的リアクタンス——自由の脅威が回復への動機づけと反発を喚起する]] — 同じ思想家
- [[human-status-characteristics|地位特性理論（期待状態理論）]] — 同じ思想家
- [[human-door-in-the-face|ドア・イン・ザ・フェイス・テクニック（譲歩的要請法）]] — 同分野の近傍
- [[human-change-blindness-simons|変化盲——視覚環境の大規模変化に気づかない知覚的失敗]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
