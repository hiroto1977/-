---
collection: academic
id: human-embodied-simulation-rizzolatti
category: "human-science"
category_ja: "人間科学"
title: "身体的シミュレーションと鏡像ニューロン（リゾラッティ）"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/human-science
  - knowledge/verified
aliases:
  - "身体的シミュレーションと鏡像ニューロン（リゾラッティ）"
---

# 身体的シミュレーションと鏡像ニューロン（リゾラッティ）

> [!info] コレクション: [[学術概念]] ・ 区分: 人間科学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
鏡像ニューロン（mirror neurons）は、1990年代にジャコモ・リゾラッティのパルマ大学チームがサルの腹側前運動皮質（F5野）で発見した神経細胞群であり、自分が特定の行動をする際だけでなく他者が同じ行動をするのを観察する際にも活動する。この発見は「意図理解・模倣・共感・言語進化」に革命的示唆をもたらすとして「世紀の神経科学的発見」と称されたが、その後の人間における間接的証拠（fMRI・TMS）の解釈をめぐる過大評価批判も根強い（Gregory Hickok等）。ヴィットリオ・ガレーゼは「身体的シミュレーション仮説」として、感情・行動の他者理解が「感覚運動シミュレーション」によることを論じ、他者の痛み・意図・感情の神経的共鳴メカニズムを解明しようとした。自閉症スペクトラムとの関連仮説（壊れた鏡像ニューロン説）は提唱後に否定的な実証が積み重なっている。

## 提唱者・初出
ジャコモ・リゾラッティ ／ ヴィットリオ・ガレーゼ ／ マルコ・イアコボーニ ／ グレゴリー・ヒコック

## 出典
- [Rizzolatti, G. et al. (1996). Premotor cortex and the recognition of motor actions — Cognitive Brain Research](https://doi.org/10.1016/0926-6410(96)00039-2) `学術`
- [Wikipedia: Mirror neuron — embodied simulation](https://en.wikipedia.org/wiki/Mirror_neuron) `リファレンス`

## 関連概念
- [[human-embodied-simulation|身体化されたシミュレーション理論と鏡像ニューロン系：他者理解・共感・社会的認知の神経基盤]] — 語彙が近い
- [[human-mirror-neuron-controversy|ミラーニューロン論争（Mirror Neuron Controversy）]] — 語彙が近い
- [[human-mirror-neuron-system|ミラーニューロン・システム（リゾラッティ／ガレーゼ）——行為の実行時と観察時の双方で発火するニューロン群が行為理解・模倣・共感の神経基盤とされる仮説と、その証拠をめぐる論争]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
