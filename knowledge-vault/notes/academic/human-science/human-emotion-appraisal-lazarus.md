---
collection: academic
id: human-emotion-appraisal-lazarus
category: "human-science"
category_ja: "人間科学"
title: "感情の評価理論と一次・二次評価プロセス（ラザルス）"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/human-science
  - knowledge/verified
aliases:
  - "感情の評価理論と一次・二次評価プロセス（ラザルス）"
---

# 感情の評価理論と一次・二次評価プロセス（ラザルス）

> [!info] コレクション: [[学術概念]] ・ 区分: 人間科学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
感情の評価理論（cognitive appraisal theory of emotion）は、リチャード・ラザルスが「心理的ストレスと対処プロセス」（1966年）・「感情と適応」（1991年）で体系化した理論で、感情はストレッサーの客観的特性ではなく個人による認知的評価（appraisal）の結果として生じると主張する。ラザルスは評価を一次評価（primary appraisal：状況が無関連・恵み・ストレスのいずれかを判断）と二次評価（secondary appraisal：対処資源・選択肢の評価）に分け、その組み合わせが怒り・不安・罪悪感・誇りなど異なる感情を生むと論じた。シェラーの「コンポーネント・プロセス・モデル」・スミスとエルズワースの「次元評価モデル」などの競合理論が評価次元をめぐって議論を続け、文化普遍的な評価次元（新奇性・楽しさ・目標一致性・コントロール可能性）と文化依存的変動が研究されている。評価理論は認知行動療法（CBT）の理論的基盤と重なり、認知的再評価（reappraisal）が感情調節の最も有効な戦略の一つとして位置づけられる。

## 提唱者・初出
リチャード・ラザルス ／ バーニス・フォルクマン ／ クラウス・シェラー ／ ポーラ・ニーデンタール

## 出典
- [Lazarus et al. (1970) Emotions: Their Parameters and Measurement — Raven Press](https://doi.org/10.1037/h0033158) `学術`
- [Wikipedia: Appraisal theory — Lazarus, primary/secondary, Scherer, CBT, reappraisal, emotion regulation](https://en.wikipedia.org/wiki/Appraisal_theory) `リファレンス`

## 関連概念
- [[human-cognitive-appraisal|認知的評価理論（感情）]] — 同じ思想家
- [[human-general-adaptation-syndrome-selye|汎適応症候群——警告反応期・抵抗期・疲憊期を経る非特異的ストレス反応]] — 同じ思想家
- [[human-schachter-singer-two-factor|シャクター＝シンガー二要因説——生理的喚起の認知的解釈が情動を決める]] — 同じ思想家
- [[human-stress-coping|ストレスとコーピング（ラザルス）]] — 同じ思想家
- [[human-appraisal-theory-of-emotion|感情の評価理論]] — 同分野の近傍
- [[human-differential-emotions|分化感情理論（ディファレンシャル・エモーションズ理論）]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
