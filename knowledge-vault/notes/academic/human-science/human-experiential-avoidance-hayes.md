---
collection: academic
id: human-experiential-avoidance-hayes
category: "human-science"
category_ja: "人間科学"
title: "体験の回避と心理的柔軟性——ヘイズのACTモデルにおける中核的病理プロセス"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/human-science
  - knowledge/verified
aliases:
  - "体験の回避と心理的柔軟性——ヘイズのACTモデルにおける中核的病理プロセス"
---

# 体験の回避と心理的柔軟性——ヘイズのACTモデルにおける中核的病理プロセス

> [!info] コレクション: [[学術概念]] ・ 区分: 人間科学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
体験の回避（experiential avoidance）は、スティーブン・ヘイズが受容コミットメント・セラピー（ACT）の文脈で理論化した、不快な私的体験（思考・感情・身体感覚・記憶）の形態・頻度・文脈を変えようとする試みであり、心理的苦痛の維持・悪化の中核的機能的プロセスとされる。ヘイズは関係フレーム理論（RFT）に基づき、人間の言語・認知の関係的枠付け能力が皮肉にも苦痛の回避を強化するメカニズムを分析した。体験の回避は短期的には苦痛を軽減するが、長期的には行動レパートリーを狭め、価値に沿った行動を阻害する。ACTは体験の回避に代わる「心理的柔軟性」（今この瞬間への注意・脱フュージョン・受容・文脈としての自己・価値の明確化・コミットされた行動）を治療目標とする。ボンドらのメタ分析は体験の回避が広範な精神病理の横断的リスク因子であることを実証している。

## 提唱者・初出
スティーブン・ヘイズ ／ カーク・ストローサル ／ ケリー・ウィルソン ／ フランク・ボンド

## 出典
- [Hayes, S. et al. (1996) Experiential Avoidance and Behavioral Disorders — Behaviour Research and Therapy](https://doi.org/10.1016/S0005-7967(96)00023-8) `学術`
- [Bond, F. et al. (2011) Preliminary Psychometric Properties of the AAQ-II — Behavior Therapy](https://doi.org/10.1016/j.brat.2006.06.003) `学術`
- [Wikipedia: Experiential avoidance — 体験の回避の概要](https://en.wikipedia.org/wiki/Experiential_avoidance) `リファレンス`

## 関連概念
- [[human-acceptance-commitment-therapy-hayes|ACT（受容コミットメント療法）——ヘイズの心理的柔軟性と六つのコアプロセス]] — 語彙が近い
- [[human-acceptance-commitment-therapy|アクセプタンス＆コミットメント療法（ACT）と心理的柔軟性]] — 語彙が近い
- [[human-gestalt-therapy-perls-here-now|ゲシュタルト療法——パールズの「今ここ」の気づきと接触・統合・自己責任の実存的アプローチ]] — 同分野の近傍
- [[human-affective-neuroscience-panksepp|情動神経科学——基本情動システムの神経生物学的基盤]] — 同分野の近傍
- [[human-relational-cultural-theory|関係文化理論（RCT）]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
