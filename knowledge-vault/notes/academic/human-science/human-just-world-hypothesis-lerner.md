---
collection: academic
id: human-just-world-hypothesis-lerner
category: "human-science"
category_ja: "人間科学"
title: "公正世界仮説——人は相応しい結果を得るという信念とその社会的帰結"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/human-science
  - knowledge/verified
aliases:
  - "公正世界仮説——人は相応しい結果を得るという信念とその社会的帰結"
---

# 公正世界仮説——人は相応しい結果を得るという信念とその社会的帰結

> [!info] コレクション: [[学術概念]] ・ 区分: 人間科学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
公正世界仮説（Just-World Hypothesis）とは、メルヴィン・ラーナーが1965年以降の一連の研究で体系化した社会心理学の概念であり、人々が「この世界は基本的に公正であり、人は自分にふさわしい結果を受ける」という暗黙の信念を持つ傾向を指す。ラーナーは実験において、被験者が無作為に選ばれた「被害者」に対して、その人の性格や行動に問題があったと推論し被害者を非難する傾向を示すことを観察した。公正世界信念は、予測不可能で制御不能な世界に直面した際の認知的対処メカニズムとして機能し、個人の安全感と将来への投資意欲を支える適応的側面を持つ。しかし同時に、貧困者、犯罪被害者、差別の対象者に対する「被害者非難」（victim blaming）を正当化するイデオロギー的機能も果たす。リプカスとダルバートの研究は、公正世界信念が精神的健康と正の相関を示す一方で、社会的弱者への共感を阻害しうるという二面性を明らかにしている。

## 提唱者・初出
メルヴィン・ラーナー ／ ゾック・リプカス ／ クラウディア・ダルバート ／ エイドリアン・ファーナム

## 出典
- [Lerner (1980) The Belief in a Just World: A Fundamental Delusion — Plenum Press](https://doi.org/10.1007/978-1-4757-6418-5) `学術`
- [Hafer & Begue (2005) Experimental Research on Just-World Theory — Psychological Bulletin](https://doi.org/10.1037/0033-2909.131.1.128) `学術`
- [Wikipedia: Just-world hypothesis — 公正世界仮説の理論と社会的含意の概説](https://en.wikipedia.org/wiki/Just-world_hypothesis) `リファレンス`

## 関連概念
- [[human-attribution-theory|帰属理論]] — 同分野の近傍
- [[human-identifiable-victim|特定可能な被害者効果（Identifiable Victim Effect）]] — 同分野の近傍
- [[human-polyvagal-porges|ポリヴェーガル理論（ポージェス）]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
