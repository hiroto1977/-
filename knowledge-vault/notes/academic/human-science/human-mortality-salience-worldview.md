---
collection: academic
id: human-mortality-salience-worldview
category: "human-science"
category_ja: "人間科学"
title: "死の顕著性と世界観防衛（恐怖管理理論）"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/human-science
  - knowledge/verified
aliases:
  - "死の顕著性と世界観防衛（恐怖管理理論）"
---

# 死の顕著性と世界観防衛（恐怖管理理論）

> [!info] コレクション: [[学術概念]] ・ 区分: 人間科学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
恐怖管理理論（Terror Management Theory: TMT）は、アーネスト・ベッカーの思想的遺産を実験的に検証した社会心理学理論であり、人間の死の意識（mortality salience）が自己尊重感の追求と文化的世界観への固執を動機付けるとする。ベッカーは著書『死の拒絶』において、人間の文明・宗教・国家・イデオロギーが死への不安を管理するための「不死化プロジェクト」として機能すると論じた。グリーンバーグ、ソロモン、ピシンスキーらはこれを精緻化し、死の想起（MS操作）が外集団への偏見・権威主義・宗教的原理主義・愛国主義を強化することを数百の実験で示した。排外主義やポピュリズムの心理的基盤を説明する理論として政治心理学にも応用されるとともに、実存主義的心理療法との接点も論じられている。

## 提唱者・初出
ジェフ・グリーンバーグ ／ シェルドン・ソロモン ／ トム・ピシンスキー ／ アーネスト・ベッカー

## 出典
- [Greenberg, J. et al. (1986). Evidence for Terror Management Theory — JPSP](https://doi.org/10.1016/0022-1031(86)90070-3) `学術`
- [Wikipedia: Terror management theory — social psychology](https://en.wikipedia.org/wiki/Terror_management_theory) `リファレンス`

## 関連概念
- [[human-terror-management-greenberg|恐怖管理理論（グリーンバーグ＆ピジシンスキー）]] — 語彙が近い
- [[human-terror-management|存在脅威管理理論（恐怖管理理論）]] — 同じ思想家
- [[human-terror-management-theory-becker|テラー管理理論——ベッカーの死の否定とグリーンバーグらの死の顕現化実験]] — 同じ思想家
- [[mgmt-equity-theory-adams|公平理論——投入と報酬の比率の社会的比較が動機づけを左右する]] — 同じ思想家
- [[mgmt-illegitimate-tasks-semmer|不当な課業——役割や自尊心にそぐわない仕事がストレスを生む]] — 同じ思想家
- [[mgmt-organizational-justice-greenberg|組織的公正——分配・手続き・相互作用の三次元からなる職場の公正知覚]] — 同じ思想家
- [[mgmt-workplace-deviance-robinson|職場逸脱行動——組織規範に反する自発的行為の分類と規定要因]] — 同じ思想家
- [[human-social-identity-threat|社会的アイデンティティ脅威]] — 同分野の近傍
- [[human-existential-therapy-yalom|実存療法——ヤーロムの四つの究極的関心と治療的対峙]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
