---
collection: academic
id: human-place-attachment-theory-scannell
category: "human-science"
category_ja: "人間科学"
title: "場所愛着理論——人間-場所間の情動的絆の三次元モデル"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/human-science
  - knowledge/verified
aliases:
  - "場所愛着理論——人間-場所間の情動的絆の三次元モデル"
---

# 場所愛着理論——人間-場所間の情動的絆の三次元モデル

> [!info] コレクション: [[学術概念]] ・ 区分: 人間科学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
リア・スキャネルとロバート・ギフォードが2010年に体系化した場所愛着（place attachment）の三次元PPP枠組みは、人（person: 個人的記憶・文化的アイデンティティ）、過程（process: 認知・感情・行動の各側面）、場所（place: 社会的・物理的特性）の三次元から場所への情動的絆を分析する統合モデルである。場所愛着研究は環境心理学のルーツを持ち、タンの「トポフィリア」（場所愛）やレルフの「場所の喪失」に思想的淵源を持つが、スキャネルとギフォードは散在する研究知見を統合的な概念枠組みに整理し、変数間関係の体系的検証を可能にした。リーウィッカの「場所アイデンティティ」概念との関連では、場所愛着がより包括的な上位概念として場所アイデンティティを含み、環境保全行動・災害後回復・移住適応等の応用領域で予測的妥当性を示している。

## 提唱者・初出
リア・スキャネル ／ ロバート・ギフォード ／ ハロルド・プロシャンスキー ／ イーフー・タン ／ エドワード・レルフ

## 出典
- [Scannell, L. & Gifford, R. (2010) "Defining Place Attachment" — Journal of Environmental Psychology](https://doi.org/10.1016/j.jenvp.2009.09.006) `学術`
- [Lewicka, M. (2011) "Place Attachment: How Far Have We Come?" — JEP](https://doi.org/10.1006/jevp.2001.0226) `学術`
- [Wikipedia: Place attachment — 場所愛着の理論と測定法](https://en.wikipedia.org/wiki/Place_attachment) `リファレンス`

## 関連概念
- [[human-place-attachment-theory|場所愛着理論]] — 語彙が近い
- [[human-place-identity|場所アイデンティティ（Place Identity）]] — 語彙が近い
- [[human-illusory-correlation|錯誤相関]] — 同じ思想家
- [[human-place-attachment|プレイス・アタッチメント（場所愛着）]] — 同じ思想家
- [[human-place-attachment-low-altman|場所愛着理論——人間と場所の情動的絆と環境心理学]] — 同じ思想家
- [[human-social-identity-complexity|社会的アイデンティティ複雑性理論]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
