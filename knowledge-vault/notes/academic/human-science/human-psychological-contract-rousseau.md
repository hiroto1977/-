---
collection: academic
id: human-psychological-contract-rousseau
category: "human-science"
category_ja: "人間科学"
title: "心理的契約理論——ルソーの暗黙的雇用関係と契約違反の帰結"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/human-science
  - knowledge/verified
aliases:
  - "心理的契約理論——ルソーの暗黙的雇用関係と契約違反の帰結"
---

# 心理的契約理論——ルソーの暗黙的雇用関係と契約違反の帰結

> [!info] コレクション: [[学術概念]] ・ 区分: 人間科学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
心理的契約（psychological contract）理論は、デニス・ルソーが1989年に体系化した、雇用者と被雇用者の間の明示的な法的契約に含まれない暗黙的な相互期待・義務の知覚を分析する概念枠組みである。ルソーは心理的契約を「交換関係における相互義務の条件に関する個人の信念」と定義し、取引的契約（transactional：短期的・金銭的交換）と関係的契約（relational：長期的・社会情動的交換）の二類型を区別した。心理的契約の「違反」（perceived breach）と「裏切り」（violation）は概念的に区別され、違反の認知的評価が感情的裏切り反応を媒介して、職務満足・組織コミットメント・組織市民行動の低下、離職意図の増大等の否定的帰結をもたらす。モリソンとロビンソンの研究は契約違反の先行要因（過度な約束・業績不振・不一致）と調整要因（信頼・公正感）を精緻化した。

## 提唱者・初出
デニス・ルソー ／ エリザベス・モリソン ／ サンドラ・ロビンソン ／ ニール・コンウェイ

## 出典
- [Rousseau, D. (1995) Psychological Contracts in Organizations — SAGE](https://doi.org/10.1177/014920639501500306) `学術`
- [Morrison, E. & Robinson, S. (1997) When Employees Feel Betrayed — AMR](https://doi.org/10.5465/amr.1997.9707180265) `学術`
- [Wikipedia: Psychological contract — 心理的契約の概要](https://en.wikipedia.org/wiki/Psychological_contract) `リファレンス`

## 関連概念
- [[mgmt-psychological-contract-rousseau|心理的契約とその違反——暗黙の雇用期待と組織コミットメントの動態]] — 語彙が近い
- [[mgmt-psychological-contract|心理的契約]] — 分野横断
- [[mgmt-evidence-based-management-pfeffer|エビデンスに基づく経営（ファフェー＆サットン）と管理的迷信の批判]] — 同じ思想家
- [[mgmt-evidence-based-management-rousseau|エビデンス・ベースド・マネジメント——科学的根拠に基づく経営判断と「管理職の神話」の打破]] — 同じ思想家
- [[mgmt-mum-effect-bad-news|MUM効果——悪い知らせを伝えたがらない心理]] — 同じ思想家
- [[mgmt-workplace-deviance-robinson|職場逸脱行動——組織規範に反する自発的行為の分類と規定要因]] — 同じ思想家
- [[human-rumination|反芻思考]] — 同分野の近傍
- [[human-affordance-theory|アフォーダンス理論]] — 同分野の近傍
- [[human-hostile-attribution-bias|敵意的帰属バイアス]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
