---
collection: academic
id: human-resilience-adversity-luthar
category: "human-science"
category_ja: "人間科学"
title: "レジリエンスと逆境適応の発達心理学——リスク・保護因子の動態モデル（ルサール）"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/human-science
  - knowledge/verified
aliases:
  - "レジリエンスと逆境適応の発達心理学——リスク・保護因子の動態モデル（ルサール）"
---

# レジリエンスと逆境適応の発達心理学——リスク・保護因子の動態モデル（ルサール）

> [!info] コレクション: [[学術概念]] ・ 区分: 人間科学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
レジリエンス（resilience）とは、発達心理学において重大な逆境（adverse experiences）に直面しながらも良好な適応・発達を示す動態的過程を指し、サンドラ・ルサールとフェリシア・シリはこれを固定的な特性ではなく文脈依存的なプロセスとして再定義した。ルサールは特権的環境（高社会経済地位・エリート私立校）の青年において物質乱用・不安・抑うつが高率で生じる「繁栄の下の苦悩」を発見し、達成圧力・孤立感・保護的人間関係の欠如が脆弱性要因となることを実証した。オプトゥ・マスタン（Ann Masten）は「普通の魔法（ordinary magic）」概念でレジリエンスが特殊な能力ではなく基本的な人間システム——愛着・自己調節・認知・社会的制度——が機能するとき自然に発現すると論じた。現代のレジリエンス研究はエピジェネティクス（逆境の生物学的刻印）・コミュニティレジリエンス・文化的レジリエンスへと拡張し、トラウマインフォームドケアの臨床的基盤を形成している。

## 提唱者・初出
サンドラ・ルサール ／ アン・マスタン ／ エミー・ワーナー ／ マイケル・ラター

## 出典
- [Luthar (2006) Resilience in Development: A Synthesis of Research across Five Decades — Developmental Psychopathology](https://doi.org/10.1111/j.1469-7610.2006.01682.x) `学術`
- [Wikipedia: Psychological resilience — adversity, protective factors, ordinary magic, Masten, trauma-informed](https://en.wikipedia.org/wiki/Psychological_resilience) `リファレンス`

## 関連概念
- [[human-developmental-psychopathology-cicchetti|発達精神病理学——正常と異常の発達的連続性と多重決定性]] — 同じ思想家
- [[human-resilience|レジリエンス（心理的回復力）]] — 同じ思想家
- [[human-resilience-theory-masten|レジリエンス理論——逆境からの回復力と「普通の魔法」]] — 同じ思想家
- [[human-resilience-bonanno-adversity|レジリエンスと逆境後の軌跡——ボナンノの回復力類型と精神的健康の動態モデル]] — 同分野の近傍
- [[human-undermining-effect|アンダーマイニング効果（過正当化効果）]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
