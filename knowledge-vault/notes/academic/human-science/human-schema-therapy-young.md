---
collection: academic
id: human-schema-therapy-young
category: "human-science"
category_ja: "人間科学"
title: "スキーマ療法（ヤング）と早期不適応スキーマの臨床的応用"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/human-science
  - knowledge/verified
aliases:
  - "スキーマ療法（ヤング）と早期不適応スキーマの臨床的応用"
---

# スキーマ療法（ヤング）と早期不適応スキーマの臨床的応用

> [!info] コレクション: [[学術概念]] ・ 区分: 人間科学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
スキーマ療法（Schema Therapy）は、ジェフリー・ヤングが開発した統合的認知行動療法の拡張版であり、通常のCBTで効果が限定的な慢性うつ・境界性パーソナリティ障害・ナルシシズム的障害への応用を主な動機として発展した。「早期不適応スキーマ（Early Maladaptive Schema: EMS）」とは、幼少期の基本的感情的欲求が満たされなかった際に形成される広汎かつ侵入的な主題であり、18種類のEMS（例：見捨てられ・欠陥/恥・依存など）が分類される。スキーマ・モード（現在活性化している感情状態・対処スタイル）と「ヘルシーアダルト」モードの強化が治療目標となる。制限的再養育（limited reparenting）・共感的直面化・イメージ書き直し・ダイアドモードワークなどの技法を用い、個人・集団・カップル形式で実施される。

## 提唱者・初出
ジェフリー・ヤング ／ ジャネット・クロスコ ／ マービン・ワイシャー ／ アーノウド・アーナウツ

## 出典
- [Young, J.E. et al. (2003). Schema Therapy — Guilford Press](https://doi.org/10.1521/jscp.2004.23.2.173) `学術`
- [Wikipedia: Schema therapy — early maladaptive schemas](https://en.wikipedia.org/wiki/Schema_therapy) `リファレンス`

## 関連概念
- [[human-schema-therapy-young-maladaptive|スキーマ療法——ヤングの早期不適応スキーマ理論と人格障害・慢性うつへの統合的アプローチ]] — 語彙が近い
- [[infosoc-media-materialism-kittler|キットラーのメディア唯物論——技術メディアが我々の状況を規定する]] — 同じ思想家
- [[human-hyperbolic-discounting|双曲割引]] — 同分野の近傍
- [[human-body-schema|身体図式（ボディスキーマ）]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
