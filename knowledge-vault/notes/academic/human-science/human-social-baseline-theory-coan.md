---
collection: academic
id: human-social-baseline-theory-coan
category: "human-science"
category_ja: "人間科学"
title: "社会的ベースライン理論——社会的近接性と神経生理的効率"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/human-science
  - knowledge/verified
aliases:
  - "社会的ベースライン理論——社会的近接性と神経生理的効率"
---

# 社会的ベースライン理論——社会的近接性と神経生理的効率

> [!info] コレクション: [[学術概念]] ・ 区分: 人間科学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
社会的ベースライン理論（SBT）はジェームズ・コーアンとデイヴィッド・スベッカにより提唱された神経科学的枠組みであり、人間の脳はデフォルトとして社会的近接性——信頼できる他者との物理的・心理的近さ——を前提に設計されていると主張する。従来の心理学が個体の自律的調整を基準（ベースライン）とし社会的文脈をその修飾因と位置づけたのに対し、SBTは社会的埋め込みこそが神経代謝の基準状態であり、孤立がコストのかかる逸脱であると逆転させた。fMRI研究では、手つなぎ条件（特にパートナー）で脅威刺激に対する前帯状皮質・島皮質の活動が有意に減弱し、neural threat responseの社会的緩衝が示された。ベッケスとコーアンのエコロジー的枠組みは、この社会的調整をエネルギー効率の進化論的最適化として位置づけ、「社会的資源のload sharing」概念を導入した。

## 提唱者・初出
ジェームズ・コーアン ／ デイヴィッド・スベッカ ／ レイン・ベッケス ／ ヒラリー・シャエファー ／ レーン・バーケット

## 出典
- [Coan, J. A. & Sbarra, D. A. (2015) "Social Baseline Theory" — Current Directions in Psychological Science](https://doi.org/10.1177/0963721411414560) `学術`
- [Coan, J. A. et al. (2006) "Lending a Hand: Social Regulation of the Neural Response to Threat" — Psychological Science](https://doi.org/10.1037/a0014546) `学術`
- [Wikipedia: Social baseline theory — 社会的近接性の神経生理的基盤](https://en.wikipedia.org/wiki/Social_baseline_theory) `リファレンス`

## 関連概念
- [[human-positive-psychology-flourishing|ポジティブ心理学における繁栄とPERMAモデル（セリグマン）]] — 同分野の近傍
- [[human-actor-observer-bias|行為者-観察者バイアス]] — 同分野の近傍
- [[human-self-evaluation-maintenance|自己評価維持モデル（SEM）]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
