---
collection: academic
id: human-somatic-experiencing-levine
category: "human-science"
category_ja: "人間科学"
title: "ソマティック・エクスペリエンシング（レバイン）とトラウマの身体的解放"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/human-science
  - knowledge/verified
aliases:
  - "ソマティック・エクスペリエンシング（レバイン）とトラウマの身体的解放"
---

# ソマティック・エクスペリエンシング（レバイン）とトラウマの身体的解放

> [!info] コレクション: [[学術概念]] ・ 区分: 人間科学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
ソマティック・エクスペリエンシング（Somatic Experiencing: SE）は、ピーター・レバインが40年以上の研究を経て開発した身体指向のトラウマ療法であり、野生動物の行動観察から着想を得ている。レバインは、野生動物が高度な脅威に晒されても慢性的トラウマを残さない理由として「神経系の自然な自己調整（放電）プロセス」を提示し、人間においてそれが文化的抑制によって完了できないことがPTSDの根本原因と論じた。SEは「フェルトセンス（felt sense）」「タイトレーション（少量ずつの曝露）」「リソース（安全基地感覚）」「ペンデュレーション（安全と活性の間の振り子運動）」「放電（trembling/shaking）」をコア概念とし、過覚醒・低覚醒を「耐性の窓」内に維持することで凍り付き反応の完了を促す。神経科学的基盤については研究が積み重なっているが、RCTによる有効性研究はEMDRや認知処理療法と比べて少ない。

## 提唱者・初出
ピーター・レバイン ／ アン・フレデリック ／ パット・オグデン ／ ジャネット・ポウルタ

## 出典
- [Levine, P.A. (1997). Waking the Tiger — North Atlantic Books](https://doi.org/10.1007/978-1-58394-451-9) `学術`
- [Wikipedia: Somatic experiencing — trauma therapy](https://en.wikipedia.org/wiki/Somatic_experiencing) `リファレンス`

## 関連概念
- [[human-sensorimotor-psychotherapy-ogden|センサリモーター・サイコセラピー——オグデンの身体志向トラウマ治療と階層的情報処理]] — 同じ思想家
- [[human-somatic-experiencing-levine-trauma|ソマティック・エクスペリエンシング——レヴィンの身体的トラウマ解放と神経系の自己調整]] — 同じ思想家
- [[human-interpersonal-neurobiology-siegel|対人神経生物学——シーゲルの統合マインド理論と関係性が脳発達に与える神経科学的基盤]] — 同分野の近傍
- [[human-self-perception-bem|自己知覚理論——内的手がかりが曖昧なとき人は自らの行動から態度を推論する]] — 同分野の近傍
- [[human-circumplex-affect-russell|ラッセルの感情円環モデル（Circumplex Model of Affect／Russell 1980）——あらゆる感情は「快－不快」と「覚醒」の二次元の円環上に配置される]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
