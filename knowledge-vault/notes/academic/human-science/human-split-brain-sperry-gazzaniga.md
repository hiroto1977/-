---
collection: academic
id: human-split-brain-sperry-gazzaniga
category: "human-science"
category_ja: "人間科学"
title: "分離脳研究"
as_of: "2026-06"
source_count: 4
authoritative: true
tags:
  - collection/academic
  - academic/human-science
  - knowledge/verified
aliases:
  - "分離脳研究"
---

# 分離脳研究

> [!info] コレクション: [[学術概念]] ・ 区分: 人間科学 ・ asOf: 2026-06 ・ 出典: 4件（うち権威ある出典 ✓）

## 概要
脳梁切断患者を対象にSperryとGazzanigaが左右半球の独立した意識・機能を実証した。左脳は言語・論理を担い経験を物語化する「インタープリター」として機能し、右脳は空間認知に優れることが示された。

## 提唱者・初出
Roger W. Sperry ／ Michael S. Gazzaniga ／ Joseph E. Bogen

## 出典
- [Nobel Prize — Roger W. Sperry Facts (1981)](https://www.nobelprize.org/prizes/medicine/1981/sperry/facts/) `リファレンス`
- [Gazzaniga (2002) The split brain revisited — Brain](https://doi.org/10.1093/brain/awg012) `学術`
- [Sperry (1961) Cerebral Organization & Behavior — Science](https://doi.org/10.1126/science.133.3466.1749) `学術`
- [Psychology Today — Split Brain Overview](https://www.psychologytoday.com/us/basics/split-brain) `リファレンス`

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
