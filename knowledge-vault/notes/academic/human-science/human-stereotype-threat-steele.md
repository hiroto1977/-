---
collection: academic
id: human-stereotype-threat-steele
category: "human-science"
category_ja: "人間科学"
title: "ステレオタイプ脅威——否定的固定観念の顕在化が当事者の遂行を損なう"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/human-science
  - knowledge/verified
aliases:
  - "ステレオタイプ脅威——否定的固定観念の顕在化が当事者の遂行を損なう"
---

# ステレオタイプ脅威——否定的固定観念の顕在化が当事者の遂行を損なう

> [!info] コレクション: [[学術概念]] ・ 区分: 人間科学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
ステレオタイプ脅威（stereotype threat）は、クロード・スティールとジョシュア・アロンソンが1995年の研究で実証した社会心理学的現象で、ある集団に対する能力についての否定的ステレオタイプが状況的に顕在化したとき、その集団の成員が「ステレオタイプを自ら裏づけてしまうのではないか」という懸念や不安を抱き、それが遂行を妨げて、皮肉にもステレオタイプ通りの結果を生んでしまうことを指す。スティールらは、知的能力を測定すると教示された難しいテストにおいて、アフリカ系アメリカ人学生が、人種を意識させられない統制条件に比べて成績を下げることを示した。同様の効果は、数学課題における女性、加齢を意識した高齢者など、多様な集団と領域で観察されている。脅威が遂行を損なう機序としては、不安や生理的喚起、結果の絶えざる自己監視、ネガティブな思考の抑制に作業記憶資源が奪われることなどが指摘される。重要なのは、これが当事者の実際の能力や努力の不足ではなく、ステレオタイプが流通する状況そのものが生む負荷だという点であり、テスト前にステレオタイプの妥当性を否定したり、自己肯定を促したりする介入で効果を緩和できることも示された。教育格差や雇用の不平等を、本質的能力差ではなく状況的要因から捉え直す視座を提供した。

## 提唱者・初出
クロード・スティール ／ ジョシュア・アロンソン ／ スティーブン・スペンサー ／ トニ・シュメイダー

## 出典
- [Steele, C.M. & Aronson, J. (1995) Stereotype Threat and the Intellectual Test Performance of African Americans — JPSP 69(5)](https://doi.org/10.1037/0022-3514.69.5.797) `学術`
- [Wikipedia: Stereotype threat — Steele, Aronson, performance, working memory, interventions](https://en.wikipedia.org/wiki/Stereotype_threat) `リファレンス`

## 関連概念
- [[human-stereotype-threat|ステレオタイプ脅威]] — 語彙が近い
- [[human-self-affirmation-steele|自己肯定化理論——別領域の価値の確認が自己統合性を回復し防衛を緩める]] — 同じ思想家
- [[human-defensive-pessimism-norem|防衛的悲観主義——低い期待と最悪の予行演習が不安を制御し成果を高める]] — 同じ思想家
- [[human-social-identity-threat|社会的アイデンティティ脅威]] — 同分野の近傍
- [[human-cognitive-reflection-test|認知反映テスト（CRT）]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
