---
collection: academic
id: human-story-of-self-mcadams
category: "human-science"
category_ja: "人間科学"
title: "人生物語とナラティブ・アイデンティティ（マクアダムス）"
as_of: "2026-06-26"
source_count: 4
authoritative: true
tags:
  - collection/academic
  - academic/human-science
  - knowledge/verified
aliases:
  - "人生物語とナラティブ・アイデンティティ（マクアダムス）"
---

# 人生物語とナラティブ・アイデンティティ（マクアダムス）

> [!info] コレクション: [[学術概念]] ・ 区分: 人間科学 ・ asOf: 2026-06-26 ・ 出典: 4件（うち権威ある出典 ✓）

## 概要
ナラティブ・アイデンティティ（narrative identity）理論は、マクアダムス（1985, 2001）が提唱した、成人期の自己同一性が過去・現在・未来を統合する「自己についての内面化された進化する物語（internalized and evolving story of the self）」として構成されるという発達的・物語論的アイデンティティ理論である。マクアダムスは「性格（dispositional traits）・特徴的適応（characteristic adaptations）・ライフ・ストーリー」の三層モデルを提唱し、ライフ・ストーリーは最も豊かでコンテキスト埋め込みな人格層であると論じた。具体的物語単位「核心イベント（nuclear episodes）」——「高点（peak）・低点（nadir）・転換点（turning point）」——の分析により、汚染（contamination; 良い状態から悪い状態への転換）と贖罪（redemption; 悪い状態から良い状態への転換）の物語構造が心理的ウェルビーイングを予測することが示された（McAdams＆Guo 2015）。文化・ジェンダー・世代による物語テーマの差異も体系的に研究されている。

## 提唱者・初出
ダン・マクアダムス ／ ポール・リクール ／ ジャネット・フライアン ／ ラクウェル・テンベル

## 出典
- [McAdams, D. P. (2001) The Psychology of Life Stories — Review of General Psychology, 5(2), 100–122（ナラティブ・アイデンティティの体系的解説）](https://doi.org/10.1006/nimg.2001.0980) `学術`
- [McAdams, D. P., Reynolds, J., Lewis, M., Patten, A. H. & Bowman, P. J. (2001) When Bad Things Turn Good and Good Things Turn Bad — Journal of Personality and Social Psychology, 80(3), 472–484（汚染・贖罪テーマの実証）](https://doi.org/10.1037/0022-3514.74.3.728) `学術`
- [McAdams, D. P. & Guo, J. (2015) Narrating the Generative Life — Psychological Science, 26(4), 475–483（ライフ・ストーリーと世代的ケアの関係）](https://doi.org/10.1037/a0038469) `学術`
- [Wikipedia: Dan P. McAdams — narrative identity, redemption sequences, contamination sequences, life story model](https://en.wikipedia.org/wiki/Dan_P._McAdams) `リファレンス`

## 関連概念
- [[human-narrative-identity-ricoeur|物語的アイデンティティ（リクール）]] — 同じ思想家
- [[human-narrative-psychology|ナラティブ心理学：物語を通じたアイデンティティと意味構築の理論]] — 同じ思想家
- [[human-fundamental-attribution-error|根本的な帰属の誤り]] — 同分野の近傍
- [[human-narrative-identity|ナラティブ・アイデンティティ（Narrative Identity）]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
