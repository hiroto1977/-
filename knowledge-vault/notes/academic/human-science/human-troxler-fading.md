---
collection: academic
id: human-troxler-fading
category: "human-science"
category_ja: "人間科学"
title: "トロクスラー効果——固視によって周辺視野の刺激が消失する現象"
as_of: "2026-06-27"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/human-science
  - knowledge/verified
aliases:
  - "トロクスラー効果——固視によって周辺視野の刺激が消失する現象"
---

# トロクスラー効果——固視によって周辺視野の刺激が消失する現象

> [!info] コレクション: [[学術概念]] ・ 区分: 人間科学 ・ asOf: 2026-06-27 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
トロクスラー効果（Troxler's fading、トロクスラー消失）は、スイスの医師・哲学者イグナーツ・トロクスラーが1804年に報告した知覚現象で、視野の中心の一点をじっと固視（fixation）し続けると、周辺視野にある、動かず変化しない刺激が、しだいに薄れて見えなくなり、背景に溶け込んでしまう現象を指す。たとえば、ぼやけた色の斑点を取り囲むように配置した図の中心点を、まばたきや視線移動を抑えて見つめ続けると、周辺の色斑が消失して、全体が一様な色（背景色）に見えるようになる。この現象は、神経系の基本的な性質である「順応（adaptation）」によって説明される。感覚神経は、変化のない一定の刺激が続くと、それに対する反応をしだいに弱めていく。視覚系も同様で、網膜像が静止し変化しなくなると、その情報の伝達が減衰し、知覚が消えていく。通常、私たちの目は、固視しているときでさえ、マイクロサッカード（固視微動、microsaccade）と呼ばれる無意識の微小な眼球運動を絶えず行っており、これによって網膜像がわずかに動き続けるため、刺激が消えずに見え続ける。ロリン・リッグスやロバート・ディッチバーンらは20世紀半ば、特殊な装置で網膜像を完全に静止させる（網膜像の安定化、stabilized retinal image）実験を行い、像が完全に静止すると視覚がほぼ完全に消失することを示した。これはトロクスラー効果を極端にした状況であり、固視微動が視覚の維持に不可欠であることを裏づけた。周辺視野でトロクスラー効果が特に起きやすいのは、周辺視野の空間解像度が低く、刺激の輪郭がぼやけているため、順応による消失が起こりやすいからである。トロクスラー効果は、知覚が外界の受動的な写し取りではなく、刺激の「変化」に依存する能動的な過程であること、そして固視微動という絶え間ない眼球運動が安定した視覚を支えていることを示す、視覚科学の古典的な現象である。

## 提唱者・初出
イグナーツ・トロクスラー ／ ヘルマン・フォン・ヘルムホルツ ／ ロリン・リッグス ／ ロバート・ディッチバーン

## 出典
- [Martinez-Conde, S. et al. (2004) The role of fixational eye movements in visual perception — Nature Reviews Neuroscience](https://doi.org/10.1038/nrn1348) `学術`
- [Wikipedia: Troxler's fading — adaptation, fixational eye movements, peripheral vision](https://en.wikipedia.org/wiki/Troxler%27s_fading) `リファレンス`

## 関連概念
- [[human-aristotle-illusion|アリストテレスの錯覚——指を交差させると一つの物が二つに感じる]] — 同じ思想家
- [[human-autokinetic-effect|自動運動効果——暗闇の光点が動いて見える現象と社会規範の形成]] — 同じ思想家
- [[human-mccollough-effect|マッカロー効果——方位に依存する持続的な色残効]] — 同じ思想家
- [[human-ponzo-illusion|ポンゾ錯視——奥行き手がかりが生む大きさの錯覚]] — 同じ思想家
- [[human-purkinje-effect|プルキニェ現象——明るさが変わると色の見え方の明暗が逆転する]] — 同じ思想家
- [[human-size-weight-illusion-charpentier|大きさ重さ錯覚（シャルパンティエ錯覚）——小さいほうが重く感じる]] — 同じ思想家
- [[human-trichromatic-young-helmholtz|ヤング＝ヘルムホルツの三色説——色覚の三種類の受容体]] — 同じ思想家
- [[human-cognitive-miser|認知的倹約家（cognitive miser）]] — 同分野の近傍
- [[human-chronostasis-stopped-clock|クロノスタシス（止まった時計の錯覚）——視線移動後の時間が引き延ばされる]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
