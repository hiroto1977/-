---
collection: academic
id: human-weapon-focus-effect
category: "human-science"
category_ja: "人間科学"
title: "凶器注目効果——武器の存在が目撃者の顔識別を低下させる注意バイアス"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/human-science
  - knowledge/verified
aliases:
  - "凶器注目効果——武器の存在が目撃者の顔識別を低下させる注意バイアス"
---

# 凶器注目効果——武器の存在が目撃者の顔識別を低下させる注意バイアス

> [!info] コレクション: [[学術概念]] ・ 区分: 人間科学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
凶器注目効果（weapon focus effect）は、犯罪場面において武器が存在する場合、目撃者の注意が凶器に集中し、犯人の顔や身体的特徴の記憶が有意に低下する現象である。エリザベス・ロフタスらが1987年に実験的に確認したこの効果では、ナイフを持った人物の目撃条件と、中性的な物（ペンなど）を持った人物の目撃条件を比較し、凶器条件で顔の識別精度が著しく低下することが示された。アイ・トラッキング研究では、凶器条件において参加者の視線が武器に長時間固定され、顔への視線滞留時間が短縮されることが確認されている。メカニズムについては、凶器の脅威性による感情的覚醒が注意を捕捉する「脅威仮説」と、凶器が文脈に不適切な異常物として注意を引く「異常性仮説（不一致仮説）」が競合している。ステビレーとダイスタヒューのメタ分析では、凶器注目効果の頑健性が確認され、特に記述同定（描写）より顔識別に大きな影響を及ぼすことが示された。

## 提唱者・初出
エリザベス・ロフタス ／ ナンシー・ステビレー ／ ケネス・デフェンバッハー ／ ゲイリー・ウェルズ

## 出典
- [Loftus, E.F., Loftus, G.R. & Messo, J. (1987) Some Facts About "Weapon Focus" — Law and Human Behavior 11(1)](https://doi.org/10.1037/0022-3514.42.4.609) `学術`
- [Wikipedia: Weapon Focus — attentional bias reducing eyewitness identification accuracy](https://en.wikipedia.org/wiki/Weapon_focus) `リファレンス`
- [Steblay, N.M. (1992) A Meta-Analytic Review of the Weapon Focus Effect — Law and Human Behavior 16(4)](https://doi.org/10.1037/1076-8971.12.1.1) `学術`

## 関連概念
- [[human-misinformation-effect-loftus|誤情報効果——事後情報による記憶の歪曲と変容メカニズム]] — 同じ思想家
- [[human-spreading-activation|活性化拡散]] — 同じ思想家
- [[human-false-memory|虚記憶（フォールス・メモリ）]] — 同じ思想家
- [[human-own-race-bias-meissner|自人種バイアス——同人種の顔認識優位性と異人種顔識別の困難]] — 同じ思想家
- [[human-reconstructive-memory-bartlett|再構成的記憶——スキーマに基づき能動的に再構成される記憶]] — 同じ思想家
- [[human-base-rate-fallacy|基準率の無視]] — 同分野の近傍
- [[human-rubber-hand-illusion|ラバーハンド錯覚（ゴムの手錯覚）]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
