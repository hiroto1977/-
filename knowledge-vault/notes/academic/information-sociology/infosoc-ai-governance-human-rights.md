---
collection: academic
id: infosoc-ai-governance-human-rights
category: "information-sociology"
category_ja: "情報社会学"
title: "AIガバナンスと人権基盤アプローチの国際政治社会学"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "AIガバナンスと人権基盤アプローチの国際政治社会学"
---

# AIガバナンスと人権基盤アプローチの国際政治社会学

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
AI（人工知能）ガバナンスとは、AIシステムの開発・展開・使用を規律する法的・倫理的・技術的枠組みであり、「AI倫理」から「AI規制」への移行期において国際的制度整備が進んでいる。EUのAI法（2024年）は世界初の包括的AI規制であり、リスクに基づくアプローチで「容認不可（禁止）」「高リスク」「透明性義務」「汎用AI基盤モデル」の四層を規律する。国連人権理事会・ユネスコ・欧州評議会はAIと人権の接合を「人権基盤アプローチ（HRBA）」として推進し、UNESCOのAI倫理勧告（2021年）が採択された。中国の「次世代AI発展計画」・米国のNISF AIリスク管理フレームワーク・英国のセクター別規制アプローチなどが対比的に論じられる。「ガバナンスの断片化」と「規制の地政学化」がグローバル・デジタル秩序形成の鍵課題となっている。

## 提唱者・初出
ヴィルジニー・エラール ／ ジャスミン・マルク ／ ヴラダン・ヨルダノフ ／ フリオ・ファラニョーリ

## 出典
- [Marchetti, J. (2022). AI Governance and Human Rights — Oxford UP](https://doi.org/10.1093/oso/9780197559536.001.0001) `学術`
- [Wikipedia: AI governance — human rights based approach](https://en.wikipedia.org/wiki/AI_governance) `リファレンス`

## 関連概念
- [[infosoc-post-truth-epistemology|ポスト・トゥルース時代の認識論と情報環境の政治社会学]] — 同分野の近傍
- [[infosoc-platform-society-vandijck|プラットフォーム社会——データ化・商品化・選別による社会の再編]] — 同分野の近傍
- [[infosoc-critical-discourse-analysis|批判的談話分析（CDA）]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
