---
collection: academic
id: infosoc-commons-based-peer-production-benkler
category: "information-sociology"
category_ja: "情報社会学"
title: "コモンズ型ピア生産（ベンクラー）"
as_of: "2026-06-26"
source_count: 4
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "コモンズ型ピア生産（ベンクラー）"
---

# コモンズ型ピア生産（ベンクラー）

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06-26 ・ 出典: 4件（うち権威ある出典 ✓）

## 概要
コモンズ型ピア生産（commons-based peer production; CBPP）は、ヨハイ・ベンクラー（2002/2006）が提唱した、オープンソースソフトウェア・Wikipedia・科学論文プレプリント等に見られる、分散した多数の個人が所有・指揮系統なしに協調的に知識・情報財を生産・共有する新しい生産形態を指す概念である。ベンクラー（2006）の『ネットワークの富（The Wealth of Networks）』は、デジタルネットワーク化によって「市場（価格システム）」と「企業（権威的指揮）」に次ぐ第三の調整メカニズムとして「社会的生産（social production）」が台頭していると論じた。CBPPの成立条件として①タスクの細粒度化（granularity）——個人の小片貢献が統合可能、②低い統合コスト——プラットフォーム（GitHub・MediaWiki）が貢献管理、③エラー訂正機構——コミュニティによるピアレビュー——の三条件を提示した。Linuxカーネル・Wikipediaの規模拡大・OpenStreetMap・Human Protein Atlasがこの生産形態の典型事例である。法的枠組みとしてCreative Commons・GPLがCBPPのコモンズ維持を支える「著作権の逆転利用（copyleft）」として機能する。

## 提唱者・初出
ヨハイ・ベンクラー ／ リナス・トーバルズ ／ ジミー・ウェールズ ／ ローレンス・レッシグ

## 出典
- [Benkler, Y. (2002) Coase's Penguin, or, Linux and "The Nature of the Firm" — Yale Law Journal, 112(3), 369–446（CBPPの原典）](https://doi.org/10.2307/1229511) `学術`
- [Benkler, Y. (2006) The Wealth of Networks: How Social Production Transforms Markets and Freedom — Yale University Press（社会的生産の体系書）](https://doi.org/10.4159/9780674977075) `学術`
- [Crowston, K. & Howison, J. (2005) The Social Structure of Free and Open Source Software Development — First Monday, 10(2)（オープンソース開発のコミュニティ構造）](https://doi.org/10.1145/1134307.1134387) `学術`
- [Wikipedia: Commons-based peer production — Benkler, Linux, Wikipedia, copyleft, Creative Commons](https://en.wikipedia.org/wiki/Commons-based_peer_production) `リファレンス`

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
