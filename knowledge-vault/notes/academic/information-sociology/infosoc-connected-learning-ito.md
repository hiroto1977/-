---
collection: academic
id: infosoc-connected-learning-ito
category: "information-sociology"
category_ja: "情報社会学"
title: "コネクテッド・ラーニング——デジタル環境における興味・仲間・社会的機会の連結と学習の民主化"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "コネクテッド・ラーニング——デジタル環境における興味・仲間・社会的機会の連結と学習の民主化"
---

# コネクテッド・ラーニング——デジタル環境における興味・仲間・社会的機会の連結と学習の民主化

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
コネクテッド・ラーニングは、ミミ・イトウ（伊藤瑞子）らが2013年に提唱した学習フレームワークで、若者の「個人的興味」「仲間との関係」「学術的・市民的・経済的機会」の三領域を橋渡しすることで、学習への動機と成果を最大化しようとする。従来の学校教育が単方向的・標準化されがちなのに対し、コネクテッド・ラーニングは多様なデジタルメディア（SNS・ゲーム・YouTube・Maker運動）を媒介とし、学習者が主体的に創造・共有・参加できる環境を重視する。特に社会的・経済的に不利な立場にある若者が、デジタルネットワークを通じてメンタリングや機会にアクセスできる「学習の公平性」を重要課題とする。この枠組みはYouth and Participatory Politics（YPP）研究とも接続し、デジタル市民性教育の設計原則としても採用されている。

## 提唱者・初出
ミミ・イトウ（伊藤瑞子） ／ キャシー・デービッドソン ／ ヘンリー・ジェンキンズ ／ ジュリエット・ショア

## 出典
- [Connected Learning Alliance (2013) Connected Learning: An Agenda for Research and Design](https://clalliance.org/about-connected-learning/) `リファレンス`
- [Ito et al. (2013) Connected Learning: An Agenda for Research and Design — Digital Media and Learning Research Hub](https://doi.org/10.7916/D8KW5D7K) `学術`
- [Wikipedia: Connected learning — コネクテッド・ラーニングの概念と実践](https://en.wikipedia.org/wiki/Connected_learning) `リファレンス`

## 関連概念
- [[infosoc-citizen-journalism-participatory-media|市民ジャーナリズムと参加型メディア——ウェブ2.0が変えた報道の担い手と情報権力の分散]] — 同じ思想家
- [[infosoc-platform-cooperativism-scholz-schneider|プラットフォーム協同組合主義——デジタル経済における労働者所有の代替モデル]] — 同じ思想家
- [[infosoc-platform-labor-control|プラットフォーム労働とアルゴリズム的統制]] — 同じ思想家
- [[infosoc-sharing-economy-platform-trust|シェアリングエコノミーとプラットフォーム信頼の社会学]] — 同じ思想家
- [[infosoc-spreadability-jenkins|スプレッダビリティ——参加型文化におけるメディアコンテンツの拡散メカニズム]] — 同じ思想家
- [[infosoc-main-model-credibility|MAINモデル（技術的特性が信憑性判断に与える影響のヒューリスティック・モデル）]] — 同分野の近傍
- [[infosoc-technological-frames-orlikowski|技術フレーム——オーリコウスキーとガッシュの情報技術に関する認知的枠組み分析]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
