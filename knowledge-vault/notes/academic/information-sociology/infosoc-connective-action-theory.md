---
collection: academic
id: infosoc-connective-action-theory
category: "information-sociology"
category_ja: "情報社会学"
title: "接続的行為理論"
as_of: "2026-06"
source_count: 4
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "接続的行為理論"
---

# 接続的行為理論

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06 ・ 出典: 4件（うち権威ある出典 ✓）

## 概要
Bennett & Segerbergは集合的行為（Olson）と対比し、デジタルメディアによる個人化されたコンテンツ共有が、強固な組織や集合的アイデンティティなしに大規模な政治的動員を可能にする「接続的行為」の論理を提唱した。

## 提唱者・初出
W. Lance Bennett ／ Alexandra Segerberg

## 出典
- [Bennett & Segerberg (2012) Information, Communication & Society](https://doi.org/10.1080/1369118X.2012.670661) `学術`
- [Bennett & Segerberg (2013) The Logic of Connective Action](https://doi.org/10.1017/CBO9781139198608) `学術`
- [Bimber, Flanagin & Stohl (2005) collective action space](https://doi.org/10.1111/j.1460-2466.2005.tb02685.x) `学術`
- [Bennett & Segerberg (2013) response & extension](https://doi.org/10.1080/1369118X.2013.808364) `学術`

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
