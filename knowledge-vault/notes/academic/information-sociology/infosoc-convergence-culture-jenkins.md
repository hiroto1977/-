---
collection: academic
id: infosoc-convergence-culture-jenkins
category: "information-sociology"
category_ja: "情報社会学"
title: "コンバージェンス・カルチャー——旧来メディアと新興メディアの衝突と参加文化"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "コンバージェンス・カルチャー——旧来メディアと新興メディアの衝突と参加文化"
---

# コンバージェンス・カルチャー——旧来メディアと新興メディアの衝突と参加文化

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
コンバージェンス・カルチャー（convergence culture）は、メディア研究者ヘンリー・ジェンキンスが2006年の著作で展開した概念で、メディアの収斂（convergence）を単なる複数機能を一台に集約する技術的現象としてではなく、文化的・社会的な変容として捉える視座を提示した。ジェンキンスは収斂を三つの相互に絡む過程として定義する。第一に、同一のコンテンツが映画・テレビ・書籍・ゲーム・ウェブなど複数のメディア・プラットフォームを横断して流通すること。第二に、それを可能にする複数のメディア産業間の協働。第三に、求める娯楽体験を得るためにメディアの間を能動的に渡り歩く受け手の「移動的（migratory）」な行動である。この枠組みの核心は、受け手をもはや受動的な消費者ではなく、コンテンツの解釈・流通・創造に積極的に関与する生産者＝消費者（プロシューマー）として捉える「参加文化（participatory culture）」の概念にある。ファン・コミュニティが番組の謎を集団で解き明かし、二次創作を生み出し、作品世界を拡張する活動はその典型で、ジェンキンスはピエール・レヴィの「集合知（collective intelligence）」概念を援用してこれを論じた。また、一つの物語世界を複数のメディアにまたがって展開する「トランスメディア・ストーリーテリング」も中心的な論点である。コンバージェンス・カルチャー論は、デジタル時代のメディア産業と受け手の関係の根本的な再編成を捉え、現代のソーシャルメディアやファン経済の理解の基礎となった。

## 提唱者・初出
ヘンリー・ジェンキンス ／ ピエール・レヴィ ／ アクセル・ブランス ／ ジョン・フィスク

## 出典
- [Jenkins, H. (2006) Convergence Culture: Where Old and New Media Collide — NYU Press (WorldCat)](https://www.worldcat.org/oclc/64594290) `リファレンス`
- [Wikipedia: Convergence culture — Jenkins, participatory culture, collective intelligence, transmedia](https://en.wikipedia.org/wiki/Convergence_culture) `リファレンス`

## 関連概念
- [[infosoc-participatory-culture|参加型文化]] — 同じ思想家
- [[infosoc-transmedia-storytelling|トランスメディア・ストーリーテリング]] — 同じ思想家
- [[infosoc-produsage-bruns|プロデュセージ——生産と利用が融合する協働的なコンテンツ創造]] — 同じ思想家
- [[infosoc-spreadable-media-jenkins|スプレッダブル・メディア——能動的な受け手の共有が価値を循環させる]] — 同じ思想家
- [[infosoc-textual-poaching-jenkins|テクストの密猟——ファンによる能動的なメディアの流用と作り替え]] — 同じ思想家
- [[infosoc-meme-culture|インターネット・ミーム文化：デジタル模倣子の伝播とオンライン文化形成の社会学的分析]] — 同じ思想家
- [[infosoc-remix-culture-lessig-jenkins|リミックス文化（レシッグ／ジェンキンス）——既存の文化的素材を再構成して創作するデジタル時代の実践と、それを抑圧しうる著作権制度との緊張]] — 同じ思想家
- [[infosoc-collective-intelligence-levy|集合知——レヴィの「知識の宇宙」とウェブ2.0・ウィキペディア・クラウドソーシングへの展開]] — 同じ思想家
- [[infosoc-citizen-journalism|市民ジャーナリズム]] — 同分野の近傍
- [[infosoc-second-self-turkle|セカンド・セルフ——コンピュータが映し出す自己と精神への問い]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
