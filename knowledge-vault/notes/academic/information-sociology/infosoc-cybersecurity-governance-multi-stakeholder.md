---
collection: academic
id: infosoc-cybersecurity-governance-multi-stakeholder
category: "information-sociology"
category_ja: "情報社会学"
title: "サイバーセキュリティ・ガバナンスとマルチステークホルダー・モデル"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "サイバーセキュリティ・ガバナンスとマルチステークホルダー・モデル"
---

# サイバーセキュリティ・ガバナンスとマルチステークホルダー・モデル

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
サイバーセキュリティ・ガバナンスとは、国家・民間企業・国際機関・市民社会が協調してサイバー空間の安全性と信頼性を維持するための制度的枠組みを指す。インターネットの分散的アーキテクチャとグローバルな相互接続性ゆえに、単一の国家規制では対応不能であり、ICANNに体現されるマルチステークホルダー・アプローチが国際的に発展してきた。ISOの27000シリーズ・NIST CSF（サイバーセキュリティ・フレームワーク）・EU NIS指令・日本の産業サイバーセキュリティ基本法などが規制の多層的体系を構成する。国家間の「サイバー兵器」開発とその輸出規制（ワッセナー体制の適用）・重要インフラ保護・サプライチェーン・セキュリティ・民間脅威情報共有の制度化が現代的課題として浮上している。シンガポールの「サイバースペース安定性規範」提案やタリン・マニュアル（NATO）は国際法適用の先駆的試みである。

## 提唱者・初出
ミルトン・ミューラー ／ ジョセフ・ナイ ／ ブルース・シュナイアー ／ ジャン・ランセロット

## 出典
- [Nye, J.S. (2017). Deterrence and Dissuasion in Cyberspace — International Security](https://doi.org/10.1093/oxfordhb/9780198719960.013.46) `学術`
- [Wikipedia: Cybersecurity governance — multi-stakeholder model](https://en.wikipedia.org/wiki/Cybersecurity_governance) `リファレンス`

## 関連概念
- [[infosoc-cybersecurity-threat-landscape|サイバーセキュリティの社会的地平——国家・犯罪・ハクティビズムの脅威と安全保障ガバナンス]] — 同じ思想家
- [[infosoc-noopolitik-arquilla-ronfeldt|ヌーポリティーク——理念とソフトパワーにもとづく情報時代の外交]] — 同じ思想家
- [[infosoc-techno-nationalism-digital-sovereignty|テクノナショナリズムとデジタル主権戦略の比較政治経済学]] — 同じ思想家
- [[infosoc-internet-governance|インターネット・ガバナンス]] — 同分野の近傍
- [[infosoc-eu-ai-act|EU AI規制法（AI Act）]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
