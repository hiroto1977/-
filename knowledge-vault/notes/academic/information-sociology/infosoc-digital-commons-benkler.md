---
collection: academic
id: infosoc-digital-commons-benkler
category: "information-sociology"
category_ja: "情報社会学"
title: "デジタル・コモンズとコモンズ・ベースド・ピア・プロダクション——ベンクラーの協働的知識生産"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "デジタル・コモンズとコモンズ・ベースド・ピア・プロダクション——ベンクラーの協働的知識生産"
---

# デジタル・コモンズとコモンズ・ベースド・ピア・プロダクション——ベンクラーの協働的知識生産

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
デジタル・コモンズとは、インターネット上で非排他的・非競合的に共有されるデジタル知識資源（オープンソースソフトウェア・Wikipedia・Creative Commonsコンテンツ等）の総体を指す。ヤーチャイ・ベンクラーは2006年の著作『ネットワークの富』において「コモンズ・ベースド・ピア・プロダクション（CBPP）」の概念を提唱し、市場でも企業組織でもない第三の生産様式として、自発的・分散的・非金銭的動機に基づく大規模協働を理論化した。ベンクラーはコースの取引費用理論を拡張し、デジタル技術が協調コストを劇的に低下させたことで、モジュール化された課題に対するピア・プロダクションが市場・企業に対して比較優位を持つ条件を分析した。エリノア・オストロムのコモンズ・ガバナンス理論やローレンス・レシッグのフリーカルチャー論とも理論的に接合し、デジタル時代の知識ガバナンスの規範的基盤を提供している。

## 提唱者・初出
ヤーチャイ・ベンクラー ／ ローレンス・レシッグ ／ エリノア・オストロム ／ エリック・フォン・ヒッペル

## 出典
- [Benkler, Y. (2006) The Wealth of Networks — Yale University Press](https://doi.org/10.2307/j.ctt1njknw) `学術`
- [Benkler, Y. (2002) Coase's Penguin, or Linux and the Nature of the Firm — Yale Law Journal](https://doi.org/10.1177/0002764202045011004) `学術`
- [Wikipedia: Commons-based peer production — CBPPの概要](https://en.wikipedia.org/wiki/Commons-based_peer_production) `リファレンス`

## 関連概念
- [[econ-tragedy-of-commons|共有地の悲劇（コモンズの悲劇）]] — 同じ思想家
- [[infosoc-open-source-governance-ostrom|オープンソースガバナンスとコモンズ——オストロムの共有資源論とデジタルコモンズ]] — 同じ思想家
- [[infosoc-remix-culture-lessig|リミックス文化とクリエイティブ・コモンズ（レシッグ）]] — 同じ思想家
- [[mgmt-lead-user|リードユーザー法]] — 同じ思想家
- [[infosoc-remix-culture-lessig-jenkins|リミックス文化（レシッグ／ジェンキンス）——既存の文化的素材を再構成して創作するデジタル時代の実践と、それを抑圧しうる著作権制度との緊張]] — 同じ思想家
- [[infosoc-wikinomics-tapscott|ウィキノミクス——マス・コラボレーションが変える生産と経済]] — 同じ思想家
- [[econ-commons-tragedy-hardin|コモンズの悲劇とオストロムの集合行為論]] — 同じ思想家
- [[econ-commons-tragedy-hardin-ostrom|共有地の悲劇とオストロムの制度分析——コモンズ管理のガバナンス設計]] — 同じ思想家
- [[econ-public-goods-samuelson|公共財（サミュエルソン）——非競合性と非排除性を持つ財はフリーライダー問題により市場で過少供給され、政府供給や集合的解決の根拠になる]] — 同じ思想家
- [[infosoc-digital-commons|デジタル・コモンズ（Digital Commons）]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
