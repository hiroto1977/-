---
collection: academic
id: infosoc-gatekeeping-white
category: "information-sociology"
category_ja: "情報社会学"
title: "ゲートキーピング——膨大な情報を選別し公衆に届く記事を決める門番の作用"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "ゲートキーピング——膨大な情報を選別し公衆に届く記事を決める門番の作用"
---

# ゲートキーピング——膨大な情報を選別し公衆に届く記事を決める門番の作用

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
ゲートキーピング（gatekeeping、門番作用）は、利用可能な膨大な情報が、選別・取捨・加工を経て、実際に公衆に届くごく限られた数のメッセージへと絞り込まれていく過程を指す理論で、その意思決定を担う者を「ゲートキーパー（門番）」と呼ぶ。概念の起源は、社会心理学者クルト・レヴィンが1947年に、家庭の食卓に何が並ぶかが、情報が通る「チャンネル」とその要所に位置する「門番」（主婦や購買担当者）の決定に左右されると論じたことにさかのぼる。デヴィッド・マニング・ホワイトは1950年に、この概念をニュース報道の分析に応用した先駆的研究を発表した。彼は、ある地方紙で通信社配信記事の取捨選択を担う電信編集者（「ミスター・ゲイツ」と仮名された人物）が、どの記事を紙面に載せ、どれを没にするかを一週間にわたり記録・分析した。その結果、何が「ニュース」として読者に届くかは、客観的な重要度だけでなく、この編集者個人の好み・価値観・先入観に強く左右される、きわめて主観的な選択であることが明らかになった。ある記事は「宣伝くさい」「面白くない」といった編集者の主観的理由で排除されていた。この研究は、ニュースが現実をそのまま映す鏡ではなく、門番の判断を通過した選択的構成物であることを実証的に示した。後にパメラ・シューメーカーは、ゲートキーピングを個人の編集者のレベルにとどめず、報道の慣行（ルーティン）、メディア組織、社会制度、社会システムという複数の階層で作用する多層的な過程として体系化した。ゲートキーピング理論は、議題設定研究やニュース生産の社会学とともに、メディアがいかにして現実を選別し構成するかを理解する基礎的な枠組みとなっている。

## 提唱者・初出
クルト・レヴィン ／ デヴィッド・マニング・ホワイト ／ パメラ・シューメーカー ／ ウォルター・ギーバー

## 出典
- [White, D.M. (1950) The Gate Keeper: A Case Study in the Selection of News — Journalism Quarterly 27(4)](https://doi.org/10.1177/107769905002700403) `学術`
- [Wikipedia: Gatekeeping (communication) — Lewin, White, Mr. Gates, Shoemaker multilevel theory](https://en.wikipedia.org/wiki/Gatekeeping_(communication)) `リファレンス`

## 関連概念
- [[mgmt-kotter-8-steps|コッターの変革8段階モデル]] — 同じ思想家
- [[mgmt-lewin-change|レヴィンの変革モデル（解凍・変革・再凍結）]] — 同じ思想家
- [[mgmt-three-horizons|3つの地平（スリー・ホライズン）]] — 同じ思想家
- [[human-approach-avoidance-elliot|接近回避動機——望ましいものへの接近と望ましくないものの回避の二系統]] — 同じ思想家
- [[human-ecological-systems-theory-bronfenbrenner|生態学的システム理論——ブロンフェンブレンナーの入れ子モデルと人間発達の環境的文脈]] — 同じ思想家
- [[mgmt-action-centred-leadership-adair|アクション・センタード・リーダーシップ——課題・チーム・個人の三つの輪]] — 同じ思想家
- [[mgmt-bridges-transition-model-ending-neutral-zone-beginning|ブリッジズのトランジション・モデル——「終わり」「ニュートラルゾーン」「新たな始まり」の3段階で変化への心理的移行を捉える枠組み]] — 同じ思想家
- [[infosoc-media-literacy|メディア・リテラシー]] — 同分野の近傍
- [[infosoc-dataveillance-clarke|データ監視（デイタヴェイランス）——個人データを通じた体系的な監視]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
