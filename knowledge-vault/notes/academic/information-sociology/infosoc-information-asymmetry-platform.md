---
collection: academic
id: infosoc-information-asymmetry-platform
category: "information-sociology"
category_ja: "情報社会学"
title: "プラットフォーム情報非対称性理論"
as_of: "2026-06"
source_count: 5
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "プラットフォーム情報非対称性理論"
---

# プラットフォーム情報非対称性理論

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06 ・ 出典: 5件（うち権威ある出典 ✓）

## 概要
デジタルプラットフォームは、運営者が利用者の行動データを大規模に収集・分析できる一方、利用者はアルゴリズムの意思決定過程にアクセスできないという新たな情報非対称性を生み出している。Zuboffは「監視資本主義」概念において、プラットフォーム企業が行動余剰データを予測商品として商品化し、利用者の知らないうちに経済的価値へと転換する構造を批判的に分析した。Pasqualeは「ブラックボックス社会」の比喩を用いて、検索・信用・医療・金融の各アルゴリズムが透明性を欠いたまま個人の生活機会を決定する危険性を指摘した。Resnick らはプラットフォーム媒介型評判システムの研究において、評判情報の非対称的配分が取引信頼を形成する一方で、プラットフォームによる評判データの独占が利用者の交渉力を構造的に弱める可能性を示した。さらにプラットフォームは価格設定・マッチング・広告ターゲティングにおいて情報優位を戦略的に利用し、利用者の選択肢を可視化されない形で限定することで、非対称的な権力関係を強化している。

## 提唱者・初出
Shoshana Zuboff ／ Frank Pasquale ／ Paul Resnick ／ Jean Tirole ／ Nick Srnicek

## 出典
- [Zuboff, S. (2019). Surveillance capitalism and the challenge of collective action. New Labor Forum.](https://doi.org/10.1177/2053951719841843) `学術`
- [Pasquale, F. (2015). The Black Box Society: The Secret Algorithms That Control Money and Information. Harvard University Press.](https://doi.org/10.2307/j.ctt1287kz5) `学術`
- [Resnick, P., Kuwabara, K., Zeckhauser, R., & Friedman, E. (2000). Reputation systems. Communications of the ACM, 43(12), 45-48.](https://doi.org/10.1145/355019.355026) `学術`
- [Tirole, J. (2014). Market failures and public policy. Nobel Prize Lecture / American Economic Review, 104(6), 1609-1652.](https://doi.org/10.1257/aer.104.6.1609) `学術`
- [Srnicek, N. (2017). Platform Capitalism. Polity Press.](https://doi.org/10.1111/1467-954X.12408) `学術`

## 関連概念
- [[infosoc-behavioral-surplus-zuboff|行動余剰と監視資本主義（ズボフ）]] — 語彙が近い
- [[infosoc-information-asymmetry-in-digital-markets|デジタル市場の情報非対称性]] — 同じ思想家
- [[infosoc-algorithmic-curation-theory|アルゴリズムキュレーション理論]] — 同じ思想家
- [[infosoc-algorithmic-governance-theory|アルゴリズム・ガバナンス理論]] — 同じ思想家
- [[infosoc-attention-economy-theory|アテンション・エコノミー理論]] — 同じ思想家
- [[infosoc-digital-sovereignty-theory|デジタル主権論]] — 同じ思想家
- [[infosoc-platform-labor-theory|プラットフォーム労働論]] — 同じ思想家
- [[infosoc-platform-urbanism-barns|プラットフォーム都市主義——デジタルプラットフォームによる都市空間の再編]] — 同分野の近傍
- [[infosoc-connective-action|つながりの行為論理（コネクティブ・アクション）]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
