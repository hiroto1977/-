---
collection: academic
id: infosoc-information-behavior-wilson-model
category: "information-sociology"
category_ja: "情報社会学"
title: "情報行動モデル——ウィルソンの情報ニーズ・探索・利用の統合的枠組み"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "情報行動モデル——ウィルソンの情報ニーズ・探索・利用の統合的枠組み"
---

# 情報行動モデル——ウィルソンの情報ニーズ・探索・利用の統合的枠組み

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
情報行動（information behavior）モデルは、トーマス・D・ウィルソンが1981年に提唱し、1996年・1999年に改訂した、人間の情報ニーズの発生から情報探索・情報利用に至る一連の過程を包括的に分析する理論的枠組みである。ウィルソンの1996年モデルは、情報ニーズを生じさせるコンテクスト（個人的・役割関連・環境的要因）、探索行動を活性化するメカニズム（ストレス対処理論・リスク報酬理論・社会的学習理論・自己効力感理論）、情報探索の具体的モード（受動的注意・受動的探索・能動的探索・進行中の探索）、探索を阻害する介在変数（心理的・人口統計的・役割関連・環境的・情報源の特性）を統合的に位置づけた。クルトゥーのセンスメイキング・メソドロジー、デルヴィンのギャップ概念、クールソーの情報探索プロセスモデルとともに情報行動研究の四大理論枠組みの一つとされる。

## 提唱者・初出
トーマス・D・ウィルソン ／ キャロル・クールソー ／ ブレンダ・デルヴィン ／ リーデル・サヴォライネン

## 出典
- [Wilson, T.D. (1999) Models in Information Behaviour Research — Journal of Documentation](https://doi.org/10.1108/eb026702) `学術`
- [Wilson, T.D. (2000) Human Information Behavior — Informing Science](https://doi.org/10.1016/j.lisr.2006.06.005) `学術`
- [Wikipedia: Information behavior — 情報行動の概要](https://en.wikipedia.org/wiki/Information_behavior) `リファレンス`

## 関連概念
- [[infosoc-information-horizons-sonnenwald|情報の地平——文脈のなかで認識される情報資源の広がり]] — 同じ思想家
- [[infosoc-way-of-life-savolainen|日常生活情報探索——「生き方」と「生活の統御」のなかの情報]] — 同じ思想家
- [[infosoc-critical-information-literacy-elmborg|批判的情報リテラシー——エルムボーグのフレイレ的図書館教育論と権力分析]] — 同分野の近傍
- [[infosoc-imagined-communities-anderson-digital|想像の共同体とデジタル・ナショナリズム——アンダーソンのメディア論の情報社会への拡張]] — 同分野の近傍
- [[infosoc-information-bank|情報銀行（情報利用信用銀行）]] — 同分野の近傍
- [[infosoc-information-seeking-wilson|ウィルソンの情報行動モデル——文脈・欲求・障壁から情報探索を捉える]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
