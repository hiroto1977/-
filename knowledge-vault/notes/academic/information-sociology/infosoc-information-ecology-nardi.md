---
collection: academic
id: infosoc-information-ecology-nardi
category: "information-sociology"
category_ja: "情報社会学"
title: "情報エコロジー——ナルディとオデイの技術・人間・実践の相互作用分析"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "情報エコロジー——ナルディとオデイの技術・人間・実践の相互作用分析"
---

# 情報エコロジー——ナルディとオデイの技術・人間・実践の相互作用分析

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
情報エコロジーは、ボニー・ナルディとヴィッキー・オデイが1999年の著作で提唱した、特定のローカル環境における人間・技術・実践・価値観の相互依存的なシステムを分析する概念枠組みである。生物学的エコロジーからの類推により、情報技術を孤立した道具としてではなく、図書館・病院・オフィス等の具体的な場において多様なアクター（技術者・利用者・管理者）の相互作用のなかで意味を獲得するものとして理解する。ナルディらは「キーストーン種」の概念を導入し、組織内で技術と人間の橋渡し役を果たす個人（パワーユーザー・ローカル専門家）の重要性を強調した。この枠組みは技術決定論と社会構成主義の双方を回避し、「健全な情報エコロジー」の育成という規範的ビジョンを提示する点で、CSCWや参加型デザインの研究にも影響を与えた。

## 提唱者・初出
ボニー・ナルディ ／ ヴィッキー・オデイ ／ スーザン・リー・スター ／ ルーシー・サッチマン

## 出典
- [Nardi, B. & O'Day, V. (1999) Information Ecologies — MIT Press](https://doi.org/10.7551/mitpress/3880.001.0001) `学術`
- [Nardi, B. (2010) My Life as a Night Elf Priest — University of Michigan Press](https://doi.org/10.1177/1461444809342738) `学術`
- [Wikipedia: Information ecology — 情報エコロジーの概要](https://en.wikipedia.org/wiki/Information_ecology) `リファレンス`

## 関連概念
- [[infosoc-information-ecology-nardi-oday|情報生態学——技術・人・実践・価値からなる局所的システムとしての技術利用]] — 語彙が近い
- [[infosoc-infrastructure-studies|インフラストラクチャー研究（Infrastructure Studies）]] — 同じ思想家
- [[infosoc-infrastructural-inversion|インフラストラクチャの逆転（ボウカー＆スター）——背景に退いて不可視になった分類・標準・データ基盤を前景化し、その政治性を可視化する方法論]] — 同じ思想家
- [[infosoc-platform-capitalism|プラットフォーム資本主義]] — 同分野の近傍
- [[infosoc-participatory-design-information-systems|情報システムにおける参加型デザイン]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
