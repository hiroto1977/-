---
collection: academic
id: infosoc-information-pollution
category: "information-sociology"
category_ja: "情報社会学"
title: "情報汚染：情報エコシステムにおける誤情報・偽情報の拡散メカニズム"
as_of: "2026-06-25"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "情報汚染：情報エコシステムにおける誤情報・偽情報の拡散メカニズム"
---

# 情報汚染：情報エコシステムにおける誤情報・偽情報の拡散メカニズム

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06-25 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
情報汚染（Information Pollution）とは、誤情報（misinformation）・偽情報（disinformation）・悪意ある情報（malinformation）が情報エコシステムを汚染し、公共的知識基盤の質を劣化させる現象である。Floridi（2011）は情報の哲学的基盤を体系化し、意味的情報の真理性条件と情報倫理の枠組みを提示した。Wardle & Derakhshan（2017）は欧州評議会報告書で「情報障害」の三類型（mis/dis/malinformation）と、生成・制作・流通の三段階モデルを提唱し、政策立案の学際的枠組みを確立した。Vosoughi, Roy & Aral（2018）はTwitter上の12万6千件のニュース拡散を実証分析し、虚偽情報が真実より速く・広く・深く伝播することを示した。これらの研究は、デジタルプラットフォーム時代における情報品質の制度的保障と市民のメディアリテラシー向上の必要性を提起している。

## 提唱者・初出
ルチアーノ・フロリディ ／ クレア・ウォードル ／ ホセイン・デラクシャン ／ ソルーシュ・ヴォソウギ ／ デブ・ロイ ／ シナン・アラル

## 出典
- [Floridi, L. (2011) The Philosophy of Information — Oxford University Press](https://doi.org/10.1093/acprof:oso/9780199232383.001.0001) `学術`
- [Wardle, C. & Derakhshan, H. (2017) Information Disorder: Toward an Interdisciplinary Framework for Research and Policy Making — Council of Europe](https://rm.coe.int/information-disorder-report-version-august-2018/16808c9c77) `公的`
- [Vosoughi, S., Roy, D. & Aral, S. (2018) The Spread of True and False News Online — Science, 359, 1146-1151](https://doi.org/10.1126/science.aap9559) `学術`

## 関連概念
- [[infosoc-information-disorder|情報障害（Information Disorder）]] — 語彙が近い
- [[infosoc-data-sovereignty|データ主権]] — 同じ思想家
- [[infosoc-disinformation|誤情報・偽情報（情報の無秩序）]] — 同じ思想家
- [[infosoc-digital-divide-age-grey|高齢者のデジタル格差——第三のデジタルデバイドと情報社会の包摂課題]] — 同じ思想家
- [[infosoc-information-disorder-taxonomy|情報障害の分類体系（ワードル＆デラクシャン）]] — 同じ思想家
- [[infosoc-onlife|オンライフ]] — 同じ思想家
- [[infosoc-data-ethics|データ倫理とアルゴリズム公正性の学際的枠組み]] — 同じ思想家
- [[infosoc-misinformation-disinformation|誤情報・偽情報：インフォメーション・ディスオーダーの分類体系とデジタルメディアにおける虚偽情報拡散の実証研究]] — 同じ思想家
- [[infosoc-digital-sovereignty-floridi|デジタル主権——国家・市民のデータ統制権とテクノロジー地政学]] — 同じ思想家
- [[infosoc-fake-news|フェイクニュース（情報の無秩序）]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
