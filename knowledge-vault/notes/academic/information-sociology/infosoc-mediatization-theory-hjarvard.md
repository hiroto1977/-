---
collection: academic
id: infosoc-mediatization-theory-hjarvard
category: "information-sociology"
category_ja: "情報社会学"
title: "メディア化理論（ヒャーバード）"
as_of: "2026-06-26"
source_count: 4
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "メディア化理論（ヒャーバード）"
---

# メディア化理論（ヒャーバード）

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06-26 ・ 出典: 4件（うち権威ある出典 ✓）

## 概要
メディア化（mediatization）理論は、ヒャーバード（2008）が定式化した、現代社会においてメディア機関・メディアロジックが政治・宗教・家族・科学など社会の諸領域の自律的ロジックを変容・再編成するマクロ社会プロセスの概念である。メディア論的転回（mediated turn）とは区別され、コミュニケーションの媒介化（mediation）の量的増大だけでなく、社会制度がメディアの論理（速度・可視性・劇的化・断片化）に従属・適応するという質的変化を指す。クロッツ＆シュルツ（2014）は「制度的メディア化」と「社会的メディア化」の二層を提唱し、政治家の行動がメディア論理に適応する「政治のメディア化」を代表例として分析した。ハイップ＆クリング（2016）はデジタルメディア化として、アルゴリズムとデータ化が追加の論理層を導入しメディア化を深化させると論じた。批判としてメディアロジックの単一性仮定・西洋中心性・エージェンシーの軽視が挙げられ、エンゲジメント・アフォーダンスのアプローチとの統合が進んでいる。

## 提唱者・初出
スティグ・ヒャーバード ／ フリードリヒ・クロッツ ／ アンドレアス・ヘップ ／ ウィン・シュルツ

## 出典
- [Hjarvard, S. (2008) The Mediatization of Society: A Theory of the Media as Agents of Social and Cultural Change — Nordicom Review, 29(2), 105–134（メディア化理論の原典）](https://doi.org/10.1177/0163443708098201) `学術`
- [Couldry, N. & Hepp, A. (2013) Conceptualizing Mediatization: Contexts, Traditions, Arguments — Communication Theory, 23(3), 191–202（概念整理と批判）](https://doi.org/10.1177/0267323114521426) `学術`
- [Hepp, A. & Krotz, F. (eds.) (2014) Mediatized Worlds: Culture and Society in a Media Age — Palgrave Macmillan（社会・文化への応用）](https://doi.org/10.1057/9781137300355) `学術`
- [Wikipedia: Mediatization (media) — Hjarvard, media logic, institutional vs. social mediatization, critique](https://en.wikipedia.org/wiki/Mediatization_(media)) `リファレンス`

## 関連概念
- [[infosoc-mediatization|メディア化（メディアティゼーション）]] — 同じ思想家
- [[infosoc-mediatization-theory|メディアタイゼーション理論（Mediatization Theory）]] — 同じ思想家
- [[human-dopamine-reward-prediction-error|ドーパミン報酬予測誤差仮説：強化学習の神経基盤と意思決定の計算論的モデル]] — 同じ思想家
- [[infosoc-communicative-figurations|コミュニカティブ・フィギュレーション（Communicative Figurations）]] — 同じ思想家
- [[infosoc-deep-mediatization|深層メディアタイゼーション（Deep Mediatization）]] — 同じ思想家
- [[infosoc-network-effects|ネットワーク効果（Network Effects）]] — 同分野の近傍
- [[infosoc-granovetter-threshold|グラノヴェッターの閾値モデル（集合行動と社会変動）]] — 同分野の近傍
- [[infosoc-news-values|ニュース・バリュー（ニュースの価値）]] — 同分野の近傍
- [[infosoc-technological-momentum|技術的モメンタム]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
