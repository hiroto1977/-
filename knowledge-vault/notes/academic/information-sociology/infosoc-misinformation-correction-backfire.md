---
collection: academic
id: infosoc-misinformation-correction-backfire
category: "information-sociology"
category_ja: "情報社会学"
title: "誤情報訂正とバックファイア効果の実証研究"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "誤情報訂正とバックファイア効果の実証研究"
---

# 誤情報訂正とバックファイア効果の実証研究

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
誤情報（misinformation）の訂正研究は、訂正が時に信念をより強化する「バックファイア効果（backfire effect）」の存在を2010年代の研究（Nyhan & Reifler）が指摘したことから大きな注目を集めた。しかし2019年以降の大規模再現研究（Pasek et al., Wood & Porter）は、バックファイア効果は一般的現象でなく特定の条件下のみに生じると反論し、訂正は平均的に誤信念を低減する効果を持つことを示した。「前置き訂正（prebunking / inoculation theory）」は誤情報に先立って操作手法を開示することで抵抗力を高めるアプローチとして注目される。Youtubeとの提携による大規模prebunking実験（Jigsaw）が効果を示す一方、社会的アイデンティティや政治的動機付けが訂正受容に与える調整効果の理解が深まっている。

## 提唱者・初出
ブレンダン・ニアン ／ ジェイソン・ライフラー ／ サンダー・ファン・デル・リンデン ／ ジョン・グレッコ

## 出典
- [Nyhan, B. & Reifler, J. (2010). When corrections fail — Political Behavior](https://doi.org/10.1017/S0007123417000047) `学術`
- [Wikipedia: Backfire effect — misinformation research](https://en.wikipedia.org/wiki/Backfire_effect) `リファレンス`

## 関連概念
- [[infosoc-misinformation-cognitive-science|誤情報の認知科学と信念訂正研究の情報行動論]] — 語彙が近い
- [[infosoc-inoculation-theory|接種理論（inoculation theory）]] — 同じ思想家
- [[infosoc-misinformation-prebunking|ミスインフォメーションとプリバンキング——接種理論とファクトチェックの情報社会論]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
