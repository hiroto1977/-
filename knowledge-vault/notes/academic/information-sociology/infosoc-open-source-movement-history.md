---
collection: academic
id: infosoc-open-source-movement-history
category: "information-sociology"
category_ja: "情報社会学"
title: "オープンソース運動の歴史と知識コモンズの思想的系譜"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "オープンソース運動の歴史と知識コモンズの思想的系譜"
---

# オープンソース運動の歴史と知識コモンズの思想的系譜

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
オープンソース運動（open source movement）は、ソフトウェアのソースコードを公開し誰でも閲覧・改変・再配布できる開発モデルを推進する社会的・技術的運動であり、リチャード・ストールマンによるGNUプロジェクト（1983年）とフリーソフトウェア財団（FSF、1985年）がその思想的起点となる。1998年にエリック・レイモンドらが「オープンソース」という実用主義的用語を商業的文脈で普及させ、Linuxカーネル・Apache・MySQLなどのプロジェクトが企業ITインフラの中核を担うに至った。スティーブン・ウェーバーは「オープンソースの成功」（2004年）でオープンソースを「財産を私有化する権利」ではなく「配布する義務」に基づくコモンズ的所有形態として分析した。オープンソースの経済学的説明にはカスタマイズのニーズ・シグナリング・公共財的性格が挙げられ、コントリビュータの動機・ガバナンス構造・コミュニティ持続性がソフトウェア社会学の重要研究テーマとなっている。

## 提唱者・初出
リチャード・ストールマン ／ エリック・レイモンド ／ リーナス・トーバルズ ／ スティーブン・ウェーバー

## 出典
- [Raymond (1999) The Cathedral and the Bazaar — Knowledge, Technology & Policy 12(3)](https://doi.org/10.1007/BF02512351) `学術`
- [Wikipedia: Open-source movement — Stallman, FSF, Linux, commons-based peer production, governance](https://en.wikipedia.org/wiki/Open-source_movement) `リファレンス`

## 関連概念
- [[infosoc-copyleft-free-software|コピーレフトとフリーソフトウェア運動（ストールマン）]] — 語彙が近い
- [[infosoc-open-source-software-movement|オープンソースソフトウェア運動——フリーソフトウェア倫理・バザール開発モデルと情報コモンズ]] — 同じ思想家
- [[infosoc-hacker-culture-levy|ハッカー文化とハッカー倫理（レヴィ）]] — 同じ思想家
- [[infosoc-commons-based-peer-production-benkler|コモンズ型ピア生産（ベンクラー）]] — 同じ思想家
- [[infosoc-open-source-governance-ostrom|オープンソースガバナンスとコモンズ——オストロムの共有資源論とデジタルコモンズ]] — 同じ思想家
- [[infosoc-knowledge-commons-theory|知識コモンズ理論]] — 同分野の近傍
- [[infosoc-ambient-awareness|アンビエント・アウェアネス]] — 同分野の近傍
- [[infosoc-information-goods|情報財の経済的特性（高固定費・ゼロ限界費用・非競合性・経験財）]] — 同分野の近傍
- [[infosoc-smart-city-urban-informatics|スマートシティと都市情報学の批判的・社会技術的分析]] — 同分野の近傍
- [[infosoc-media-system-dependency-ball-rokeach|メディア・システム依存理論——メディアへの依存が効果の強さを規定する]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
