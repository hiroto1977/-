---
collection: academic
id: infosoc-platform-economics
category: "information-sociology"
category_ja: "情報社会学"
title: "プラットフォーム経済学・二面市場理論：デジタル市場の仲介構造と価格戦略"
as_of: "2026-06-24"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "プラットフォーム経済学・二面市場理論：デジタル市場の仲介構造と価格戦略"
---

# プラットフォーム経済学・二面市場理論：デジタル市場の仲介構造と価格戦略

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06-24 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
プラットフォーム経済学は、異なるユーザー群の間を仲介するデジタル基盤の経済構造を分析する。ロシェとティロル（2003年）およびパーカーとヴァン・アルスタイン（2005年）が二面市場理論を確立し、ネットワーク外部性・価格設定・独占規制を体系化した。ティロルは2014年にノーベル経済学賞を受賞し、Google・Amazon・Facebookなどの巨大プラットフォームの規制論議に理論的基盤を提供した。

## 提唱者・初出
ジャン・ティロル ／ ジャン＝シャルル・ロシェ ／ マーシャル・ヴァン・アルスタイン

## 出典
- [Rochet & Tirole 2003 — Platform Competition in Two-Sided Markets, Journal of the European Economic Association 1(4):990-1029](https://doi.org/10.1111/1467-937X.00160) `学術`
- [Parker & Van Alstyne 2005 — Two-Sided Network Effects: A Theory of Information Product Design, Management Science 51(10):1494-1504](https://doi.org/10.1287/mnsc.1040.0250) `学術`
- [Nobel Prize 2014 — Economic Sciences: Jean Tirole — The Sveriges Riksbank Prize in Economic Sciences](https://www.nobelprize.org/prizes/economic-sciences/2014/summary/) `リファレンス`

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
