---
collection: academic
id: infosoc-platform-labor-control
category: "information-sociology"
category_ja: "情報社会学"
title: "プラットフォーム労働とアルゴリズム的統制"
as_of: "2026-06-25"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "プラットフォーム労働とアルゴリズム的統制"
---

# プラットフォーム労働とアルゴリズム的統制

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06-25 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
UberやAmazon Mechanical Turk等のデジタルプラットフォームは、従来の管理職階層を介さずアルゴリズムによって労働者を管理・監視・規律化する新たな労働統制形態を生み出した。ローゼンブラットは著書『Uberland』でUberのアルゴリズム的管理の実態を民族誌的に記録し、情報の非対称性と「ソフトな統制」の構造を明らかにした。ヴァラス＆ショアはプラットフォーム経済を労働組織の新形態として分析し、労働者の自律性と従属性の矛盾を指摘した。メールマン＆ザルマンソンはアルゴリズム管理が「手を離しつつも遍在的」な統制メカニズムとして機能する過程を解明した。これらの研究は労働過程論と監視研究の交差点に位置し、デジタル時代の労働者の主体性と権力構造を問い直す重要な理論的基盤を提供している。

## 提唱者・初出
アレックス・ローゼンブラット ／ スティーヴン・ヴァラス ／ ジュリエット・ショア ／ マリーン・メールマン ／ ルーマ・ザルマンソン

## 出典
- [Rosenblat, A. (2018) Uberland: How Algorithms Are Rewriting the Rules of Work — University of California Press](https://doi.org/10.1525/luminos.51) `学術`
- [Vallas, S. & Schor, J. (2020) What Do Platforms Do? Understanding the Gig Economy — Annual Review of Sociology, 46, 273-294](https://doi.org/10.1146/annurev-soc-090820-023943) `学術`
- [Mohlmann, M. & Zalmanson, L. (2017) Hands on the Wheel: Navigating Algorithmic Management — Academy of Management Proceedings](https://doi.org/10.5465/AMBPP.2017.58) `学術`

## 関連概念
- [[infosoc-platform-labor-gig-economy|プラットフォーム労働とギグエコノミー——デジタル仲介による労働の再編と社会的保護]] — 語彙が近い
- [[infosoc-connected-learning-ito|コネクテッド・ラーニング——デジタル環境における興味・仲間・社会的機会の連結と学習の民主化]] — 同じ思想家
- [[infosoc-platform-labor-regulation|プラットフォーム労働規制論——ギグワーカーの法的地位と社会的保護]] — 同じ思想家
- [[infosoc-sharing-economy-platform-trust|シェアリングエコノミーとプラットフォーム信頼の社会学]] — 同じ思想家
- [[infosoc-platform-cooperativism-scholz-schneider|プラットフォーム協同組合主義——デジタル経済における労働者所有の代替モデル]] — 同じ思想家
- [[infosoc-digital-labor-scholz-platform-cooperativism|デジタル・レイバーとプラットフォーム協同組合主義——ショルツの労働者所有型プラットフォーム構想]] — 同分野の近傍
- [[infosoc-critical-algorithm-studies-seaver|批判的アルゴリズム研究——シーヴァーの文化的実践としてのアルゴリズム分析]] — 同分野の近傍
- [[infosoc-platform-labor-politics|プラットフォーム労働の政治学]] — 同分野の近傍
- [[infosoc-digital-memory-collective|デジタル集合記憶とオンライン追悼実践の情報社会学]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
