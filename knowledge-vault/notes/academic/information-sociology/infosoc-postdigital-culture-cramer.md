---
collection: academic
id: infosoc-postdigital-culture-cramer
category: "information-sociology"
category_ja: "情報社会学"
title: "ポストデジタル文化（クレーマー）"
as_of: "2026-06-26"
source_count: 4
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "ポストデジタル文化（クレーマー）"
---

# ポストデジタル文化（クレーマー）

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06-26 ・ 出典: 4件（うち権威ある出典 ✓）

## 概要
ポストデジタル（post-digital）は、クレーマー（2015）が定義した、「デジタルが当然の前提となった文化的状況」を指す概念であり、デジタル技術への興奮・新奇性が失われ、デジタルとアナログの境界が自明に溶解した時代の文化的・美学的態度を表す。批判的メディア研究の文脈では「デジタルに何が来るか」でなく「デジタルが遍在化した後の世界をどう生きるか」という問いとして位置づけられる。具体的現象としてはアナログ・レコード復活・ジン（同人誌）・手書き・フィルムカメラ人気——デジタルが圧倒的主流になった後に現れる意図的アナログ回帰——が含まれる。デジタルアーツ・教育学では「ポストデジタル・ペダゴジー」として、技術の批判的・倫理的使用を教授内容の中心に置く実践論に発展した。クレーマーはまた「計算的メディア（computational media）」の大量採用後に生まれるデジタル疲弊・アルゴリズム不信・熟練手工芸再評価を分析した。近年はAI生成コンテンツの氾濫に対するヒューマン・タッチへの需要増がポストデジタル文化の新局面として論じられている。

## 提唱者・初出
フロリアン・クレーマー ／ キム・カスコーン ／ ウィム・クルーウェン ／ テリー・ロウリンソン

## 出典
- [Cramer, F. (2015) What is "Post-digital"? — A Peer-reviewed Journal About Post-Digital Research, 3(1)（ポストデジタルの定義論文）](https://doi.org/10.12801/1947-5403.2015.07.03.01) `学術`
- [Jandrić, P. et al. (2018) Postdigital Science and Education — Educational Philosophy and Theory, 50(10), 893–899（教育学への応用）](https://doi.org/10.1080/00131857.2018.1454000) `学術`
- [Cascone, K. (2000) The Aesthetics of Failure: "Post-Digital" Tendencies in Contemporary Computer Music — Computer Music Journal, 24(4), 12–18（美学的起源）](https://doi.org/10.1162/014892600559489) `学術`
- [Wikipedia: Post-digital — Cramer, analog revival, post-digital pedagogy, AI content backlash](https://en.wikipedia.org/wiki/Post-digital) `リファレンス`

## 関連概念
- [[infosoc-post-digital-aesthetics-cramer|ポスト・デジタル美学——デジタルの日常化と新たなメディア意識]] — 同分野の近傍
- [[infosoc-algorithmic-opacity-burrell|アルゴリズム不透明性の三類型——企業秘密・技術的リテラシー・機械学習の規模に起因する不透明性]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
