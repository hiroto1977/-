---
collection: academic
id: infosoc-privacy-by-design-cavoukian
category: "information-sociology"
category_ja: "情報社会学"
title: "プライバシー・バイ・デザインと個人情報保護の設計原則（カブキアン）"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "プライバシー・バイ・デザインと個人情報保護の設計原則（カブキアン）"
---

# プライバシー・バイ・デザインと個人情報保護の設計原則（カブキアン）

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
プライバシー・バイ・デザイン（Privacy by Design: PbD）は、アン・カブキアン（カナダ・オンタリオ州プライバシーコミッショナー）が1990年代に提唱した概念であり、システム・製品・サービスの設計段階からプライバシー保護を組み込む原則を指す。七つの基礎原則として「事前予防」「プライバシーをデフォルトに」「設計への組み込み」「完全な機能性」「エンドツーエンドのセキュリティ」「可視性と透明性」「ユーザーへの尊重」が示される。EUのGDPR第25条が「データ保護バイデザイン及びバイデフォルト」を法的義務として規定したことで国際的に制度化された。日本の個人情報保護法・プライバシーマーク制度・JIS Q 15001とも接合される。「プライバシー強化技術（PETs）」「差分プライバシー」「合成データ」「ゼロ知識証明」など技術的実装手段の研究が進んでいる。

## 提唱者・初出
アン・カブキアン ／ ジョン・フォン・ヘルン ／ マーティン・ランゲバック ／ ルドガー・ヴァイス

## 出典
- [Cavoukian, A. (2012) Privacy by Design: Origins, Meaning, and Prospects for Assuring Privacy and Trust in the Information Era — IGI Global（Privacy Protection Measures and Technologies in Business Organizations 所収, pp.170-208）](https://doi.org/10.4018/978-1-61350-501-4.ch007) `学術`
- [Wikipedia: Privacy by design — GDPR Article 25](https://en.wikipedia.org/wiki/Privacy_by_design) `リファレンス`

## 関連概念
- [[infosoc-privacy-by-design|プライバシー・バイ・デザイン]] — 語彙が近い
- [[infosoc-digital-identity-self-sovereign|デジタルアイデンティティと自己主権——SSIの分散管理と個人情報支配の再設計]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
