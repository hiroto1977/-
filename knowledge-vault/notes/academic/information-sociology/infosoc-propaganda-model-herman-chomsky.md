---
collection: academic
id: infosoc-propaganda-model-herman-chomsky
category: "information-sociology"
category_ja: "情報社会学"
title: "プロパガンダ・モデル——五つのフィルターによる合意の製造"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "プロパガンダ・モデル——五つのフィルターによる合意の製造"
---

# プロパガンダ・モデル——五つのフィルターによる合意の製造

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
プロパガンダ・モデル（propaganda model）は、エドワード・ハーマンとノーム・チョムスキーが1988年の著作『合意の製造（Manufacturing Consent）』で提示したマスメディア分析の枠組みで、市場経済を基盤とする民主主義社会において、報道機関が陰謀的にではなく構造的に、支配的なエリート層の利益に奉仕し、人々の「合意を製造（manufacture consent）」する仕組みを説明する。モデルの核心は、ニュースが社会に届くまでに通過する五つの「フィルター」である。第一に、巨大メディア企業の「集中した企業所有」——営利企業としての性格が報道の方向性を規定する。第二に、収入の大半を「広告」に依存すること——広告主である企業の意向に反する報道は抑制されやすい。第三に、ニュースの「情報源」を政府・企業・専門家といった権威ある供給源に依存すること——効率的な取材のためにエリート的情報源に組み込まれる。第四に、「フラック（flak）」——メディアの報道に対して組織的に向けられる否定的反応や圧力が、報道を規律する。第五に、共通の敵を設定するイデオロギー的フィルター——冷戦期の反共産主義、後には「対テロ」や市場原理主義がこれにあたる。これらのフィルターを通過する過程で、エリートの利益に反する情報は体系的に排除・周縁化され、結果として報道は権力構造を補強する。ウォルター・リップマンが用いた「合意の製造」という語を批判的に継承したこのモデルは、メディアの客観性や第四権力としての独立性という通念に根本的な疑問を投げかけ、政治経済学的なメディア批判の代表的理論となった。

## 提唱者・初出
エドワード・ハーマン ／ ノーム・チョムスキー ／ ウォルター・リップマン ／ ロバート・マクチェスニー

## 出典
- [Herman, E.S. & Chomsky, N. (1988) Manufacturing Consent: The Political Economy of the Mass Media — Pantheon (WorldCat)](https://www.worldcat.org/oclc/17441737) `学術`
- [Wikipedia: Propaganda model — Herman & Chomsky, five filters, ownership/advertising/sourcing/flak/ideology](https://en.wikipedia.org/wiki/Propaganda_model) `リファレンス`

## 関連概念
- [[infosoc-lippmann-pseudo-environment|擬似環境とステレオタイプ（リップマン『世論』）]] — 同じ思想家
- [[infosoc-media-ownership-concentration|メディア所有の集中（メディア独占論）]] — 同じ思想家
- [[infosoc-indexing-hypothesis-bennett|インデックス仮説——報道は公的エリートの論争の幅に従う]] — 同じ思想家
- [[human-language-acquisition-chomsky|言語獲得装置と普遍文法——言語習得の生得的基盤をめぐる生成文法理論]] — 同じ思想家
- [[infosoc-public-opinion-lippmann|リップマンの世論論——擬似環境とステレオタイプが現実認識を媒介する]] — 同じ思想家
- [[infosoc-troll-farms-howard|トロール・ファームと計算論的プロパガンダ（ハワード）]] — 同分野の近傍
- [[infosoc-network-agenda-setting|ネットワーク議題設定モデル（Network Agenda Setting、議題設定理論の第三レベル）]] — 同分野の近傍
- [[infosoc-algorithmic-culture|アルゴリズム文化論]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
