---
collection: academic
id: infosoc-sharing-economy-platform-trust
category: "information-sociology"
category_ja: "情報社会学"
title: "シェアリングエコノミーとプラットフォーム信頼の社会学"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "シェアリングエコノミーとプラットフォーム信頼の社会学"
---

# シェアリングエコノミーとプラットフォーム信頼の社会学

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
シェアリングエコノミー（sharing economy）とは、デジタルプラットフォームを通じて個人が保有する資産・スキル・時間を他者と共有・交換することで経済的価値を生み出すモデルの総称であり、Airbnb（住居）・Uber（移動）・TaskRabbit（労働）・Lending Club（金融）などが典型例である。レイチェル・ボッツマンとルー・ロジャーズはその著書『シェア』でこの概念を広め、「コラボレーティブ消費」と称した。信頼は非市場的評価システム（評価・レビュー・評判スコア）によって媒介される「アルゴリズム的信頼」として機能するが、差別・詐欺・プラットフォームへの依存という問題が指摘されている。税務・労働・宿泊・交通分野での既存規制との衝突および規制の空白も論点であり、「シェア」という用語の商業的転用に対して真のコモンズ・エコノミーとの差異を強調する批判的研究も存在する。

## 提唱者・初出
レイチェル・ボッツマン ／ ニック・スルニセク ／ アルン・サンドララジャン ／ ジュリエット・ショア

## 出典
- [Schor, J.B. (2017). Does the sharing economy increase inequality? — Cambridge Journal of Regions](https://doi.org/10.1177/0263276417732554) `学術`
- [Wikipedia: Sharing economy — platform trust sociology](https://en.wikipedia.org/wiki/Sharing_economy) `リファレンス`

## 関連概念
- [[infosoc-gig-economy-platform-labor|ギグエコノミーとプラットフォーム労働の法的・社会学的課題]] — 同じ思想家
- [[infosoc-connected-learning-ito|コネクテッド・ラーニング——デジタル環境における興味・仲間・社会的機会の連結と学習の民主化]] — 同じ思想家
- [[infosoc-platform-cooperativism-scholz-schneider|プラットフォーム協同組合主義——デジタル経済における労働者所有の代替モデル]] — 同じ思想家
- [[infosoc-platform-labor-control|プラットフォーム労働とアルゴリズム的統制]] — 同じ思想家
- [[infosoc-digital-labour|デジタル労働（digital labour）]] — 同分野の近傍
- [[infosoc-context-collapse|コンテキストの崩壊（文脈崩壊）]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
