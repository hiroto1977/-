---
collection: academic
id: infosoc-smart-city-governance-kitchin
category: "information-sociology"
category_ja: "情報社会学"
title: "スマートシティ・ガバナンス——データ駆動型都市管理の批判的検討"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "スマートシティ・ガバナンス——データ駆動型都市管理の批判的検討"
---

# スマートシティ・ガバナンス——データ駆動型都市管理の批判的検討

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
ロブ・キッチンは2014年の著作『データ革命（The Data Revolution）』および一連の論文において，「スマートシティ」——都市のインフラ・交通・エネルギー・行政サービスをIoTセンサー，ビッグデータ分析，AIによってリアルタイムに最適化する構想——の技術決定論的な語りに対して批判的分析を展開した。キッチンはスマートシティの推進がIBM，Cisco，Googleなどのテクノロジー企業の商業的利益と結びつき，都市を「効率性」と「最適化」の対象として脱政治化する危険性を指摘した。シャノン・マッターンはスマートシティの「ダッシュボード的知（dashboard epistemology）」——複雑な都市の社会的現実を定量的指標に還元する認識論的偏向——を批判した。アダム・グリーンフィールドは著作『ラディカル・テクノロジーズ（Radical Technologies）』でスマートシティ構想の理論的前提（都市問題は技術的に解決可能であるという仮定）を根本的に疑問視した。バルセロナの「デジタル主権（digital sovereignty）」戦略やアムステルダムの「責任あるセンシング（Responsible Sensing）」ガイドラインは，市民参加と民主的統制を組み込んだオルタナティブなスマートシティ・モデルとして国際的に注目されている。

## 提唱者・初出
ロブ・キッチン ／ シャノン・マッターン ／ アダム・グリーンフィールド ／ フランチェスカ・ビアンキーニ

## 出典
- [Kitchin, R. (2014) The Real-Time City? Big Data and Smart Urbanism — GeoJournal](https://doi.org/10.1007/s10708-014-9580-x) `学術`
- [Wikipedia: Smart city — IoT, urban analytics, governance, data-driven, citizen participation](https://en.wikipedia.org/wiki/Smart_city) `リファレンス`

## 関連概念
- [[infosoc-smart-city-critique-greenfield|スマートシティ批判——技術的ソリューショニズムと都市ガバナンス]] — 語彙が近い
- [[infosoc-smart-city-governance|スマートシティのガバナンス]] — 同じ思想家
- [[infosoc-ambient-computing-ubicomp-weiser|ユビキタスコンピューティングとアンビエント情報環境——マーク・ワイザーの「消えるコンピュータ」ビジョン]] — 同じ思想家
- [[infosoc-critical-data-studies-kitchin-dalton|批判的データ研究——データの政治性と知識生産の権力構造]] — 同じ思想家
- [[infosoc-datafication-social-life|ソーシャルライフのデータ化とアルゴリズム的統治の情報社会論]] — 同じ思想家
- [[infosoc-smart-city-urban-informatics|スマートシティと都市情報学の批判的・社会技術的分析]] — 同じ思想家
- [[infosoc-communicative-capitalism|コミュニカティブ・キャピタリズム（Communicative Capitalism）]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
