---
collection: academic
id: infosoc-social-media-affordances-boyd
category: "information-sociology"
category_ja: "情報社会学"
title: "ネットワーク化された公衆とソーシャルメディアのアフォーダンス（ボイド）"
as_of: "2026-06-26"
source_count: 4
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "ネットワーク化された公衆とソーシャルメディアのアフォーダンス（ボイド）"
---

# ネットワーク化された公衆とソーシャルメディアのアフォーダンス（ボイド）

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06-26 ・ 出典: 4件（うち権威ある出典 ✓）

## 概要
danah boyd（2010，2014）はネットワーク化された公衆（networked publics）を特徴付ける四つの構造的アフォーダンスを提示した：①永続性（persistence）――発言が半永久的に保存・検索可能になること、②複製可能性（replicability）――コンテンツが容易にコピー・拡散されること、③スケーラビリティ（scalability）――メッセージが潜在的に膨大な聴衆に届くこと、④検索可能性（searchability）――人物・コンテンツが検索で発見されること。ジェームズ・ギブソンの生態的アフォーダンス概念を社会・技術的文脈に拡張したこの枠組みは、ティーンエイジャーのアイデンティティ演技・文脈の崩壊・プライバシー管理を分析する理論的基盤となった。ボイド＆エリソン（2007）はSNSの定義・歴史・学術的系譜を整理し、エリソン（2013）はアフォーダンスが自己呈示・ネットワーク形成・情報拡散に与える差異を明らかにした。

## 提唱者・初出
danah boyd ／ ニコール・エリソン ／ ジェームズ・ギブソン ／ ヤン・ファン・ダイク

## 出典
- [Boyd, d. (2010) Social Network Sites as Networked Publics — In A Networked Self (Papacharissi ed.), Routledge](https://www.danah.org/papers/2010/SNSasNetworkedPublics.pdf) `学術`
- [Boyd, d. & Ellison, N. B. (2007) Social Network Sites: Definition, History, and Scholarship — Journal of Computer-Mediated Communication, 13(1), 210–230](https://doi.org/10.1111/j.1083-6101.2007.00393.x) `学術`
- [Boyd, d. (2014) It's Complicated: The Social Lives of Networked Teens — Yale University Press](https://yalebooks.yale.edu/book/9780300166316/its-complicated/) `学術`
- [Wikipedia: Affordance — Gibson's original ecological concept and its extension to social media design](https://en.wikipedia.org/wiki/Affordance) `リファレンス`

## 関連概念
- [[infosoc-networked-publics-boyd|ネットワーク化された公共圏：デジタル技術が再構成する公衆と文脈の崩壊]] — 語彙が近い
- [[infosoc-networked-publics|ネットワーク化された公衆]] — 語彙が近い
- [[infosoc-digital-inequality|デジタル格差・デジタル不平等：情報技術へのアクセスと活用における社会的不均衡]] — 同じ思想家
- [[human-tilt-aftereffect|傾き残効——傾いた線を見続けると垂直が逆に傾いて見える]] — 同じ思想家
- [[infosoc-affordance-theory-gibson|アフォーダンス理論——環境が提供する行為可能性と技術の社会的利用への展開]] — 同じ思想家
- [[infosoc-aspirational-labor-duffy|アスピレーショナル労働（aspirational labour／夢追い型デジタル労働）——「好きを仕事に」の約束に支えられた、ほぼ無償のセルフブランディング労働]] — 出典を共有
- [[econ-behavioral-nudge-thaler-sunstein|行動経済学のナッジと自由主義的温情主義——選択アーキテクチャによる政策介入]] — 出典を共有
- [[infosoc-cognitive-surplus|認知的余剰（Cognitive Surplus）]] — 出典を共有
- [[infosoc-content-moderation-theory|大規模コンテンツ・モデレーション：プラットフォームにおけるユーザー生成コンテンツ統治の理論と実践]] — 出典を共有
- [[infosoc-affective-publics-papacharissi|感情的公衆——デジタルメディアにおける情動的ネットワーク形成]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
