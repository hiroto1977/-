---
collection: academic
id: infosoc-structural-holes-burt
category: "information-sociology"
category_ja: "情報社会学"
title: "構造的空隙——分断された集団を橋渡しする仲介者が得る情報と統制の優位"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "構造的空隙——分断された集団を橋渡しする仲介者が得る情報と統制の優位"
---

# 構造的空隙——分断された集団を橋渡しする仲介者が得る情報と統制の優位

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
構造的空隙（structural holes）は、社会学者ロナルド・バートが1992年の著作『構造的空隙——競争の社会構造』で提唱した社会ネットワーク理論の概念で、社会ネットワークの中で、互いに直接つながっていない二つの個人や集団の間に存在する「隙間（空隙）」を指す。これらの二者は、互いに接触がないために、重複しない補完的な情報を持っている。構造的空隙の理論の核心は、この隙間を橋渡しする位置にいる行為者——本来は分断されている複数のクラスターを結びつける「仲介者（broker）」あるいはゲオルク・ジンメルのいう「漁夫の利を得る第三者（tertius gaudens）」——が、ネットワーク上の構造的な優位を獲得するという点にある。バートは、この仲介者が得る便益を二つに区別した。第一は「情報の便益（information benefits）」である。互いにつながっていない複数の集団に橋を架けることで、仲介者はそれぞれの集団から多様で重複しない情報に、より早く、より広くアクセスできる。密に結びついた集団の内部では同じ情報が反復して流れるだけだが、空隙を越えてつながれば新規の情報がもたらされる。第二は「統制の便益（control benefits）」である。分断された二者の間に立つ仲介者は、両者の間の情報や資源の流れを統制し、互いを競わせ、あるいは仲介の条件を有利に操作する立場に立てる。これらの便益ゆえに、構造的空隙を橋渡しする位置は社会関係資本の重要な源泉となり、個人にとっては昇進やアイデアの優位、企業にとっては革新や競争優位につながる。バートの理論は、緊密で閉じたネットワークの内部結束（クロージャー）が信頼と規範を生むとするジェームズ・コールマンの社会関係資本論と対比され、ネットワーク上の「位置」が情報と権力をいかに配分するかを分析する有力な枠組みとして、組織研究、イノベーション、キャリア研究に広く応用されている。

## 提唱者・初出
ロナルド・バート ／ マーク・グラノヴェッター ／ ジェームズ・コールマン ／ ゲオルク・ジンメル

## 出典
- [Burt, R.S. (1992) Structural Holes: The Social Structure of Competition — Harvard University Press (WorldCat)](https://www.worldcat.org/oclc/25008425) `リファレンス`
- [Wikipedia: Structural holes — Burt, brokerage, tertius gaudens, information/control benefits, social capital](https://en.wikipedia.org/wiki/Structural_holes) `リファレンス`

## 関連概念
- [[infosoc-social-network-analysis|社会ネットワーク分析（SNA）]] — 同じ思想家
- [[infosoc-granovetter-threshold|グラノヴェッターの閾値モデル（集合行動と社会変動）]] — 同じ思想家
- [[infosoc-strength-weak-ties-granovetter|弱い紐帯の強さ——希薄なつながりが新規情報の橋渡しをする]] — 同じ思想家
- [[mgmt-network-structural-holes-burt|構造的空隙理論とネットワーク仲介優位の組織社会学（バート）]] — 同じ思想家
- [[mgmt-social-capital-intellectual-capital-nahapiet-ghoshal|ナハピエト＝ゴシャールの社会関係資本論——構造的・関係的・認知的の三次元による知的資本創造と組織の優位性]] — 同じ思想家
- [[econ-social-capital-putnam|ソーシャル・キャピタル（パットナム）]] — 同じ思想家
- [[infosoc-polanyi-great-transformation|ポランニーの大転換（市場の埋め込みと脱埋め込み）]] — 同じ思想家
- [[econ-economic-sociology-granovetter|経済行動の埋め込み（グラノヴェッター）]] — 同じ思想家
- [[infosoc-complex-contagion|複雑伝染（complex contagion／コンプレックス・コンテイジョン）——採用に複数の社会的補強を要する行動の拡散]] — 同じ思想家
- [[infosoc-media-multiplexity-haythornthwaite|メディア・マルチプレクシティ理論（ヘイソーンスウェイト）——絆の強さと使うメディアの数]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
