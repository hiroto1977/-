---
collection: academic
id: infosoc-tam-healthcare
category: "information-sociology"
category_ja: "情報社会学"
title: "医療情報技術の受容モデル：TAM・UTAUTのヘルスケア領域への適用と拡張"
as_of: "2026-06-25"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "医療情報技術の受容モデル：TAM・UTAUTのヘルスケア領域への適用と拡張"
---

# 医療情報技術の受容モデル：TAM・UTAUTのヘルスケア領域への適用と拡張

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06-25 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
技術受容モデル（TAM）の医療分野への適用は、臨床情報システム・電子カルテ・遠隔医療技術などの導入成否を「知覚された有用性」と「知覚された使いやすさ」で説明・予測する研究群として発展した。ホールデンとカーシュが2010年に20件以上の実証研究をレビューし、TAMが医療ITの利用・受容の相当部分を予測できるが医療固有の文脈変数の追加が必要と結論づけた。チャウとフーが2002年に400名超の医師を対象にTAMと計画的行動理論を比較し、医師の遠隔医療採用では知覚された有用性が使いやすさより強い予測因子であることを示した。ヴェンカテシュらが2003年にTAMを含む8モデルを統合しUTAUT（技術受容統一理論）を提唱した。

## 提唱者・初出
リチャード・ホールデン ／ ヴィスワナート・ヴェンカテシュ ／ パトリック・チャウ

## 出典
- [Holden, R.J. & Karsh, B.-T. 2010 The Technology Acceptance Model: Its past and its future in health care. Journal of Biomedical Informatics 43(1) 159-172](https://doi.org/10.1016/j.jbi.2009.07.002) `学術`
- [Venkatesh, V. et al. 2003 User Acceptance of Information Technology: Toward a Unified View. MIS Quarterly 27(3) 425-478](https://doi.org/10.2307/30036540) `学術`
- [Chau, P.Y.K. & Hu, P.J.-H. 2002 Investigating healthcare professionals' decisions to accept telemedicine technology. Information & Management 39(4) 297-311](https://doi.org/10.1016/S0378-7206(01)00098-2) `学術`

## 関連概念
- [[mgmt-technology-acceptance-model|技術受容モデル（TAM）——情報システム採用の知覚有用性と知覚容易性]] — 分野横断
- [[infosoc-technological-affordance|テクノロジカル・アフォーダンス（Technological Affordance）]] — 同分野の近傍
- [[infosoc-platform-affordance|プラットフォーム・アフォーダンス（Platform Affordances）]] — 同分野の近傍
- [[infosoc-telemedicine|遠隔医療（テレメディシン）：エビデンスと政策的枠組みの体系化]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
