---
collection: academic
id: infosoc-techno-feminism-wajcman
category: "information-sociology"
category_ja: "情報社会学"
title: "テクノフェミニズム——技術とジェンダーの相互構成的関係"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "テクノフェミニズム——技術とジェンダーの相互構成的関係"
---

# テクノフェミニズム——技術とジェンダーの相互構成的関係

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
テクノフェミニズム（TechnoFeminism）は、ワイクマン（2004）が提唱した、技術とジェンダーが相互に構成し合う関係を分析するフェミニスト技術論の枠組みである。技術決定論（技術が社会を一方的に変革する）とジェンダー本質主義（技術は本来的に男性的）の双方を退け、技術の設計・開発・使用のあらゆる段階でジェンダー関係が埋め込まれ、再生産されるプロセスを解明する。コックバーン（1983, 1985）は印刷技術の労働過程分析を通じ、技術的スキルの定義自体がジェンダー化されていることを示した。ハラウェイ（1985）の「サイボーグ宣言」は、自然/文化、人間/機械の二元論を解体し、テクノサイエンスにおけるフェミニスト的介入の可能性を開いた。バルカモとサブセイ（2016）はデジタル技術のデザインにおけるジェンダー・バイアスの再生産を分析し、アルゴリズムによる差別の問題と接続した。ハインズ（2019）はソーシャルメディア・AI・プラットフォーム経済におけるジェンダー不平等を包括的に検討し、テクノフェミニズムの現代的展開を示した。

## 提唱者・初出
ジュディ・ワイクマン ／ ドナ・ハラウェイ ／ シンシア・コックバーン ／ ウェンディ・チャン ／ サフィヤ・ノーブル

## 出典
- [Wajcman (2004) TechnoFeminism — Polity Press](https://doi.org/10.1017/CBO9780511816208) `学術`
- [Haraway (1991) Simians, Cyborgs, and Women: The Reinvention of Nature — Routledge](https://doi.org/10.1215/9780822373049) `学術`
- [Wikipedia: Technofeminism — テクノフェミニズムの理論的枠組みと展開の概説](https://en.wikipedia.org/wiki/Technofeminism) `リファレンス`

## 関連概念
- [[infosoc-techno-feminist-theory|テクノフェミニスト理論]] — 語彙が近い
- [[infosoc-algorithmic-bias|アルゴリズムバイアス]] — 同じ思想家
- [[infosoc-co-production-jasanoff|共産出——科学的知識と社会秩序はともに生み出される]] — 同じ思想家
- [[infosoc-critical-algorithm-studies-seaver|批判的アルゴリズム研究——シーヴァーの文化的実践としてのアルゴリズム分析]] — 同じ思想家
- [[infosoc-epistemic-injustice|認識的不正義（epistemic injustice）]] — 同じ思想家
- [[infosoc-information-asymmetry-social|情報の非対称性の社会学——知識格差と権力関係の情報社会論的分析]] — 同じ思想家
- [[infosoc-platform-temporality|プラットフォーム時間性：デジタルプラットフォームによる時間の構造化と加速]] — 同じ思想家
- [[infosoc-posthumanism-hayles|ポストヒューマニズム（ヘイルズ）]] — 同分野の近傍
- [[infosoc-feminist-technoscience-haraway|フェミニスト・テクノサイエンス論]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
