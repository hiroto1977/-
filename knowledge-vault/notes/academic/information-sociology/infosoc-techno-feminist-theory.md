---
collection: academic
id: infosoc-techno-feminist-theory
category: "information-sociology"
category_ja: "情報社会学"
title: "テクノフェミニスト理論"
as_of: "2026-06"
source_count: 4
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "テクノフェミニスト理論"
---

# テクノフェミニスト理論

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06 ・ 出典: 4件（うち権威ある出典 ✓）

## 概要
テクノフェミニスト理論は、ジェンダーと技術が相互に形成し合うという視点から、技術を中立的なものとして捉える見方を批判する理論的枠組みである。Wajcmanは技術嫌悪（テクノフォビア）と無批判なテクノユートピア主義の双方を退け、技術の設計・普及・利用においてジェンダー関係が埋め込まれていることを示した。Harawayのサイボーグ・フェミニズムは先駆的な貢献として位置づけられ、自然と文化・人間と機械の境界を解体し、技術を通じた解放の可能性を提示した。Cockburnは職場における技術的スキルのジェンダー化を分析し、男性優位の技術文化がいかに女性の排除を再生産するかを明らかにした。科学技術社会論（STS）との交差により、テクノフェミニスト理論はAIやアルゴリズムにおける性差別・人種差別バイアスの批判的分析へと展開し、データ駆動型社会における権力構造の問い直しを促している。

## 提唱者・初出
Judy Wajcman ／ Donna Haraway ／ Cynthia Cockburn ／ Anne Balsamo ／ Rosi Braidotti

## 出典
- [Wajcman, J. (2004). TechnoFeminism. Polity Press.](https://doi.org/10.1093/0199247269.001.0001) `学術`
- [Haraway, D. (1985). A Cyborg Manifesto: Science, Technology, and Socialist-Feminism. Socialist Review, 80, 65-108.](https://doi.org/10.2307/3178066) `学術`
- [Cockburn, C. (1985). Machinery of Dominance: Women, Men and Technical Know-How. Pluto Press.](https://doi.org/10.1093/sf/70.4.1101) `学術`
- [Wajcman, J. (1998). Feminism Confronts Technology. Penn State University Press.](https://doi.org/10.1177/0306312798028003004) `学術`

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
