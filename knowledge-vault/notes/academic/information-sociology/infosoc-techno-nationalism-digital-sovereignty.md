---
collection: academic
id: infosoc-techno-nationalism-digital-sovereignty
category: "information-sociology"
category_ja: "情報社会学"
title: "テクノナショナリズムとデジタル主権戦略の比較政治経済学"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "テクノナショナリズムとデジタル主権戦略の比較政治経済学"
---

# テクノナショナリズムとデジタル主権戦略の比較政治経済学

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
テクノナショナリズム（techno-nationalism）とは、技術・データ・デジタルインフラを国家安全保障・経済競争力・文化的自律性の観点から国家主権下に置こうとする政治戦略および政策イデオロギーである。米中技術覇権競争において顕在化した「デカップリング」政策（Huawei排除・TikTok規制など）はその典型例であり、インターネットの「スプリンターネット化」という構造変容をもたらす。EUは「デジタル主権」を掲げてGAFAM規制（GDPR・DSA・DMA）を強化し、自律的なクラウドインフラ（GAIA-X）の構築を推進する。その一方で、デジタル貿易自由化を求めるTPP・RCEP・WTO電子商取引交渉との緊張関係が生じている。エイドリアン・ジョーンズらは、テクノナショナリズムが技術標準の国際的分断と発展途上国の技術格差拡大をもたらすリスクを論じる。

## 提唱者・初出
ミルトン・ミューラー ／ エイドリアン・ジョーンズ ／ ヘンリー・ファレル ／ アイブラハム・ニューマン

## 出典
- [Farrell, H. & Newman, A. (2019). Weaponized interdependence — International Security](https://doi.org/10.1093/isr/viab048) `学術`
- [Wikipedia: Splinternet — digital sovereignty and techno-nationalism](https://en.wikipedia.org/wiki/Splinternet) `リファレンス`

## 関連概念
- [[human-self-handicapping-jones-berglas|セルフ・ハンディキャッピング——失敗の言い訳を先取りし自尊心を防衛する方略]] — 同じ思想家
- [[human-self-perception-bem|自己知覚理論——内的手がかりが曖昧なとき人は自らの行動から態度を推論する]] — 同じ思想家
- [[infosoc-cybersecurity-governance-multi-stakeholder|サイバーセキュリティ・ガバナンスとマルチステークホルダー・モデル]] — 同じ思想家
- [[mgmt-impression-management-goffman|印象管理——ゴフマンの演劇的アプローチと組織における自己呈示]] — 同じ思想家
- [[infosoc-digital-sovereignty-theory|デジタル主権論]] — 同分野の近傍
- [[infosoc-attention-economy-theory|アテンション・エコノミー理論]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
