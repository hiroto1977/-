---
collection: academic
id: infosoc-technological-somnambulism-winner
category: "information-sociology"
category_ja: "情報社会学"
title: "技術的夢遊病——社会が技術変化を無意識に受容する問題構造"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/information-sociology
  - knowledge/verified
aliases:
  - "技術的夢遊病——社会が技術変化を無意識に受容する問題構造"
---

# 技術的夢遊病——社会が技術変化を無意識に受容する問題構造

> [!info] コレクション: [[学術概念]] ・ 区分: 情報社会学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
ラングドン・ウィナーが1986年の著書『鯨と原子炉』で提唱した「技術的夢遊病（technological somnambulism）」は、現代社会が技術的変化の社会的・政治的意味を十分に吟味することなく、無批判に新技術を受け入れ、生活様式を変容させていく傾向を批判的に描写した概念である。ウィナーによれば、人々は新技術を単なる「道具」として捉え、その使用がもたらす社会構造・権力関係・生活世界の再編成に対して驚くほど無関心である。彼は「人工物には政治がある（artifacts have politics）」という有名なテーゼを展開し、技術設計に内在する価値選択と権力配分の問題を可視化した。ロバート・モーゼスのロングアイランド高架橋の設計（低い橋桁がバスの通行を阻み、自動車を持たない低所得層・マイノリティの海浜公園へのアクセスを制限した）は、技術に政治が埋め込まれた典型例として広く引用される。この視座はSTS（科学技術社会論）の発展、価値敏感設計、参加型技術評価の理論的基盤となった。

## 提唱者・初出
ラングドン・ウィナー ／ ルイス・マンフォード ／ ジャック・エリュール ／ アンドリュー・フィーンバーグ

## 出典
- [Winner, L. (1986) The Whale and the Reactor — University of Chicago Press ch.1](https://doi.org/10.1162/DAED_a_00611) `学術`
- [Wikipedia: Langdon Winner — technological somnambulism and politics of artifacts](https://en.wikipedia.org/wiki/Langdon_Winner) `リファレンス`
- [Winner, L. (1980) Do Artifacts Have Politics? — Daedalus 109(1)](https://doi.org/10.1162/DAED_a_00142) `学術`

## 関連概念
- [[infosoc-solutionism|テクノロジカル・ソリューショニズム（技術的解決主義）]] — 同じ思想家
- [[infosoc-technical-code|技術的コード——技術設計に埋め込まれ中立を装う支配的社会利害]] — 同じ思想家
- [[infosoc-technoculture|テクノカルチャー（技術文化研究）：技術と文化の相互構成論]] — 同じ思想家
- [[infosoc-script-akrich|技術の「スクリプト」（アクリッシュ）——設計者が物に書き込む利用者像]] — 同じ思想家
- [[infosoc-technological-determinism-mcluhan|技術決定論——技術とりわけメディアが社会と意識を規定するという見方]] — 同じ思想家
- [[infosoc-algorithmic-governance-danaher|アルゴリズミック・ガバナンス——ダナハーの自動化された意思決定と民主的正統性]] — 同分野の近傍
- [[infosoc-grobalization-ritzer|グロウバリゼーション——画一的な「無」が世界を覆う]] — 同分野の近傍
- [[infosoc-complex-contagion|複雑伝染（complex contagion／コンプレックス・コンテイジョン）——採用に複数の社会的補強を要する行動の拡散]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
