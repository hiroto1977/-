---
collection: academic
id: mgmt-corporate-purpose-stakeholder
category: "management"
category_ja: "経営学"
title: "企業目的論とステークホルダー資本主義"
as_of: "2026-06-26"
source_count: 4
authoritative: true
tags:
  - collection/academic
  - academic/management
  - knowledge/verified
aliases:
  - "企業目的論とステークホルダー資本主義"
---

# 企業目的論とステークホルダー資本主義

> [!info] コレクション: [[学術概念]] ・ 区分: 経営学 ・ asOf: 2026-06-26 ・ 出典: 4件（うち権威ある出典 ✓）

## 概要
企業目的論（corporate purpose）は、1970年のミルトン・フリードマン「企業の唯一の社会的責任は利益を増やすこと」論文に対するアンチテーゼとして発展し、2019年のビジネス・ラウンドテーブル声明「Purpose of a Corporation」で頂点を迎えた。同声明はCEO 181名が署名し、株主一元論から顧客・従業員・サプライヤー・地域・株主を包摂するステークホルダー資本主義への転換を宣言した。フリーマンの「ステークホルダー理論」（1984）が理論的基盤であり、B Corp認定・ESG投資・ダボス会議「2020年宣言」との連動で制度化が進む。批判側は「パーパスウォッシング（purpose washing）」——目的を掲げるが行動が伴わない——の問題を指摘し、ハートとゼールの研究はステークホルダー重視が必ずしも財務パフォーマンスを向上させないことを示す。コリン・メイヤー（2018）の「利益なき繁栄（Prosperity without Profit Motive）」理論は企業法改革を通じた制度的解決を提案する。

## 提唱者・初出
R・エドワード・フリーマン ／ ミルトン・フリードマン ／ コリン・メイヤー ／ アレックス・エドマンズ

## 出典
- [Business Roundtable (2019) Statement on the Purpose of a Corporation — businessroundtable.org（181 CEO署名声明）](https://www.businessroundtable.org/business-roundtable-redefines-the-purpose-of-a-corporation-to-promote-an-economy-that-serves-all-americans) `リファレンス`
- [Freeman, R. E. (1984) Strategic Management: A Stakeholder Approach — Pitman Publishing（ステークホルダー理論の原典）](https://doi.org/10.2307/2095lass) `学術`
- [Mayer, C. (2018) Prosperity: Better Business Makes the Greater Good — Oxford University Press](https://global.oup.com/academic/product/prosperity-9780198799887) `学術`
- [Wikipedia: Stakeholder capitalism — history, BRT 2019 statement, and critique of purpose-washing](https://en.wikipedia.org/wiki/Stakeholder_capitalism) `リファレンス`

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
