---
collection: academic
id: mgmt-creative-destruction-schumpeter-org
category: "management"
category_ja: "経営学"
title: "創造的破壊と組織革新——シュンペーター的競争の経営学的展開"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/management
  - knowledge/verified
aliases:
  - "創造的破壊と組織革新——シュンペーター的競争の経営学的展開"
---

# 創造的破壊と組織革新——シュンペーター的競争の経営学的展開

> [!info] コレクション: [[学術概念]] ・ 区分: 経営学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
創造的破壊（creative destruction）は、シュンペーターが1942年の著書で提示した資本主義のダイナミズムを捉える中核概念であり、革新的な企業家による新結合（neue Kombination）が既存の産業構造を内側から絶えず破壊し再構築するプロセスを指す。経営学においては、フォスターとカプランが2001年にS&P 500企業の長期データ分析から、市場の創造的破壊の速度が既存企業の組織変革の速度を恒常的に上回ること（組織の「文化的ロックイン」）を実証し、非連続的イノベーションへの戦略的対応の必要性を論じた。クリステンセンの破壊的イノベーション論との違いは、シュンペーター的創造的破壊がマクロ経済の産業進化プロセスを指すのに対し、クリステンセンはローエンド市場からの技術的置換というミクロ戦略的メカニズムに焦点を当てている点にある。

## 提唱者・初出
ヨーゼフ・シュンペーター ／ リチャード・フォスター ／ サラ・カプラン ／ クレイトン・クリステンセン

## 出典
- [Schumpeter, J. A. (1942) Capitalism, Socialism and Democracy — Harper & Brothers](https://doi.org/10.4324/9780203202050) `学術`
- [Foster, R. & Kaplan, S. (2001) Creative Destruction — Currency/Doubleday](https://doi.org/10.1002/smj.249) `学術`
- [Wikipedia: Creative destruction — Schumpeterian innovation and industrial transformation overview](https://en.wikipedia.org/wiki/Creative_destruction) `リファレンス`

## 関連概念
- [[econ-creative-destruction|創造的破壊]] — 語彙が近い
- [[mgmt-technology-s-curve|技術のSカーブ]] — 同じ思想家
- [[econ-juglar-cycle|ジュグラー・サイクル（Juglar Cycle）]] — 同じ思想家
- [[mgmt-innovation-s-curve-foster|イノベーションのSカーブとテクノロジーサイクル——技術的限界と不連続的イノベーションの経営戦略]] — 同じ思想家
- [[mgmt-second-curve-handy|セカンド・カーブ——衰退する前に次の成長曲線を始める]] — 同じ思想家
- [[econ-innovation-schumpeter|シュンペーターの革新論と創造的破壊：資本主義の動態理論]] — 同じ思想家
- [[econ-kitchin-cycle|キッチン循環：在庫投資の短期景気循環]] — 同じ思想家
- [[econ-evolutionary-economics-nelson-winter|進化経済学（Evolutionary Economics）：ネルソン＆ウィンターのルーティンと組織能力論]] — 同じ思想家
- [[econ-ricardian-vice|リカーディアン・ヴァイス（リカードの悪弊）]] — 同じ思想家
- [[mgmt-architectural-innovation-henderson-clark|アーキテクチュラル・イノベーション——構成要素のつなぎ方を変える革新]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
