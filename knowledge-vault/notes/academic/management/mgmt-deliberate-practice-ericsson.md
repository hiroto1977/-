---
collection: academic
id: mgmt-deliberate-practice-ericsson
category: "management"
category_ja: "経営学"
title: "意図的練習理論（エリクソン）"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/management
  - knowledge/verified
aliases:
  - "意図的練習理論（エリクソン）"
---

# 意図的練習理論（エリクソン）

> [!info] コレクション: [[学術概念]] ・ 区分: 経営学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
エリクソン（K. Anders Ericsson）は1993年の論文で，チェスチャンピオン・音楽家・スポーツ選手などの専門家は生来の才能よりも長期にわたる「意図的練習（deliberate practice）」——即時フィードバックを受けながら現在の能力の限界付近で集中的に取り組む活動——の累積によって卓越したパフォーマンスが形成されると論じた。「1万時間ルール」（グラッドウェルが普及させた解釈）の理論的基盤であるが，エリクソン自身は単なる時間量よりも練習の質（意図性・フィードバック・目標設定）を重視した。組織学習・人材育成・エキスパート育成研究に広く応用されており，無意識の習慣的反復（naive practice）との区別が実践的含意の核心となっている。

## 提唱者・初出
カール・アンダース・エリクソン ／ ロバート・クランブ ／ マルコム・グラッドウェル ／ デビッド・エプスタイン

## 出典
- [Ericsson, K.A., Krampe, R.T. & Tesch-Römer, C. (1993) The Role of Deliberate Practice in the Acquisition of Expert Performance — Psychological Review 100(3), 363–406（意図的練習理論の原論文）](https://doi.org/10.1037/0033-295X.100.3.363) `学術`
- [Wikipedia: Deliberate practice — 意図的練習の概説](https://en.wikipedia.org/wiki/Deliberate_practice) `リファレンス`

## 関連概念
- [[mgmt-emergent-strategy|創発戦略]] — 同分野の近傍
- [[mgmt-value-disciplines-treacy-wiersema|価値基準（バリュー・ディシプリン）——市場リーダーが選ぶ三つの卓越]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
