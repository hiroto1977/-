---
collection: academic
id: mgmt-family-business-governance-gersick
category: "management"
category_ja: "経営学"
title: "ファミリービジネスの三円モデル（ガーシック）"
as_of: "2026-06-26"
source_count: 4
authoritative: true
tags:
  - collection/academic
  - academic/management
  - knowledge/verified
aliases:
  - "ファミリービジネスの三円モデル（ガーシック）"
---

# ファミリービジネスの三円モデル（ガーシック）

> [!info] コレクション: [[学術概念]] ・ 区分: 経営学 ・ asOf: 2026-06-26 ・ 出典: 4件（うち権威ある出典 ✓）

## 概要
ファミリービジネスの三円モデル（Three-Circle Model of Family Business）は、ガーシック他（1997）が体系化した、家族企業の構造的複雑性を「家族（Family）」「所有（Ownership）」「事業（Business）」の三つのサブシステムの重なりとして表現するフレームワークである。三円が重なることで七つのステークホルダー領域が生まれ、それぞれ異なる目標・利害・感情が共存する。オーナーシップ発展段階（所有者支配→兄弟によるパートナーシップ→従兄弟連合）・ファミリー発展段階（創業者期→若い事業承継世代→一緒に働くカップル）・事業発展段階（スタートアップ→成長→成熟）の各軸上の位置が経営課題を規定する。事業承継（succession）・ファミリーカウンシル・役員報酬制度・株式承継スキームの設計に応用される。近年は同族企業のESG・長期志向・SEW（感情的資産価値; socioemotional wealth）の高さが競争優位の源泉として再評価されており、ベルナル他（2012）のSEW理論と三円モデルの統合が進んでいる。

## 提唱者・初出
ケリン・ガーシック ／ ジョン・デービス ／ マリオン・マクコール ／ イバン・レンゾン

## 出典
- [Gersick, K. E. et al. (1997) Generation to Generation: Life Cycles of the Family Business — Harvard Business School Press（三円モデルの体系書）](https://www.hbs.edu/faculty/Pages/item.aspx?num=8827) `リファレンス`
- [Gomez-Mejia, L. R. et al. (2007) Socioemotional Wealth and Business Risks in Family-Controlled Firms — Administrative Science Quarterly, 52(1), 106–137（SEW理論の原典）](https://doi.org/10.2189/asqu.52.1.106) `学術`
- [Miller, D. & Le Breton-Miller, I. (2005) Managing for the Long Run: Lessons in Competitive Advantage from Great Family Businesses — Harvard Business Review Press（ファミリービジネスの長期志向）](https://doi.org/10.1111/j.1540-6520.2010.00407.x) `学術`
- [Wikipedia: Family business — three-circle model, Gersick, succession, SEW, governance structures](https://en.wikipedia.org/wiki/Family_business) `リファレンス`

## 関連概念
- [[econ-financialization-lazonick-shareholder|金融化とシェアホルダー・バリュー——ラゾニックの「保持・再投資」から「縮小・分配」への転換]] — 同じ思想家
- [[mgmt-corporate-governance-stewardship|コーポレートガバナンスとスチュワードシップ理論——エージェンシー論との対比]] — 同じ思想家
- [[mgmt-family-business-succession|ファミリービジネスの事業承継と経営継続性の組織論]] — 同じ思想家
- [[mgmt-ge-mckinsey-matrix|GE・マッキンゼー・マトリクス（ビジネス・スクリーン）]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
