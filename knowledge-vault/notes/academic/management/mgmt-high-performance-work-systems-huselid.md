---
collection: academic
id: mgmt-high-performance-work-systems-huselid
category: "management"
category_ja: "経営学"
title: "高業績労働システム——ヒューセリッドの人的資源管理バンドルと企業業績"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/management
  - knowledge/verified
aliases:
  - "高業績労働システム——ヒューセリッドの人的資源管理バンドルと企業業績"
---

# 高業績労働システム——ヒューセリッドの人的資源管理バンドルと企業業績

> [!info] コレクション: [[学術概念]] ・ 区分: 経営学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
マーク・ヒューセリッドは1995年の論文「The Impact of Human Resource Management Practices on Turnover, Productivity, and Corporate Financial Performance」において，個別の人的資源管理（HRM）施策ではなく，複数の施策を「バンドル」として統合的に導入する「高業績労働システム（High-Performance Work Systems: HPWS）」が従業員の離職率低下，労働生産性向上，企業の財務業績向上に有意な正の効果を持つことを大規模調査データで実証した。HPWSの中核要素には，厳格な採用選考，広範な従業員教育訓練，成果連動型報酬，情報共有，従業員参加，職務設計の柔軟性が含まれる。ジェフリー・フェファーは「人材を通じた競争優位（competitive advantage through people）」を提唱し，ジョン・ポール・マクダフィーは自動車産業の国際比較を通じて「柔軟な生産システム」とHPWSの補完性を示した。エイミー・エドモンドソンは心理的安全性がHPWSの効果を促進する媒介変数として機能することを論じ，単なる制度導入ではなく組織文化との適合性の重要性を指摘した。

## 提唱者・初出
マーク・ヒューセリッド ／ ジェフリー・フェファー ／ ジョン・ポール・マクダフィー ／ エイミー・エドモンドソン

## 出典
- [Huselid, M. A. (1995) The Impact of Human Resource Management Practices — Academy of Management Journal](https://doi.org/10.2307/256741) `学術`
- [Wikipedia: High-performance work system — HPWS, HRM bundles, Huselid, Pfeffer](https://en.wikipedia.org/wiki/High-performance_work_system) `リファレンス`

## 関連概念
- [[mgmt-evidence-based-management|証拠に基づく経営（エビデンスに基づく経営）]] — 同じ思想家
- [[mgmt-abilene-paradox-harvey|アビリーンのパラドックス（ハーヴェイ）——「合意」のマネジメントの失敗]] — 同じ思想家
- [[mgmt-radical-transparency-dalio|ラジカル・トランスパレンシー（ダリオ）]] — 同じ思想家
- [[mgmt-resource-dependence|資源依存理論]] — 同じ思想家
- [[mgmt-amo-model|AMOモデル（能力・動機づけ・機会）]] — 同じ思想家
- [[mgmt-boundary-spanning-roles-aldrich|境界連結役割——アルドリッチの組織間関係論と環境スキャン・資源獲得の橋渡し機能]] — 同じ思想家
- [[mgmt-cooptation-selznick|コオプテーション——脅威を回避するための外部要素の指導構造への取り込み]] — 同じ思想家
- [[mgmt-employee-engagement-kahn|従業員エンゲージメント理論と心理的安全性の組織行動論（カーン）]] — 同じ思想家
- [[mgmt-employee-silence-pinder-harlos|従業員サイレンス——黙従的沈黙と諦観的沈黙]] — 同じ思想家
- [[mgmt-evidence-based-management-rousseau|エビデンス・ベースド・マネジメント——科学的根拠に基づく経営判断と「管理職の神話」の打破]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
