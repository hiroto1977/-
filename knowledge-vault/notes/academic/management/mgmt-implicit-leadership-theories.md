---
collection: academic
id: mgmt-implicit-leadership-theories
category: "management"
category_ja: "経営学"
title: "暗黙のリーダーシップ理論"
as_of: "2026-06-25"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/management
  - knowledge/verified
aliases:
  - "暗黙のリーダーシップ理論"
---

# 暗黙のリーダーシップ理論

> [!info] コレクション: [[学術概念]] ・ 区分: 経営学 ・ asOf: 2026-06-25 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
暗黙のリーダーシップ理論（Implicit Leadership Theories: ILT）とは、人々がリーダーとはどのような人物であるべきかについて暗黙の認知プロトタイプ（スキーマ）を保持しており、これがリーダーの知覚・評価・出現に体系的な影響を及ぼすという理論である。ロバート・ロードらは1984年の研究で、個人が持つリーダー・プロトタイプと対象人物の特性との一致度がリーダーシップ認知を規定することを実証した。エデンとレヴィアタンは1975年に、実在のリーダーが存在しない条件でもリーダーシップ因子が質問紙から抽出されることを示し、回答者の暗黙理論の存在を証明した。エピトロパキとマーティンは2004年にILT測定尺度を開発し、プロトタイプの構造と安定性を明らかにした。

## 提唱者・初出
ロバート・ロード ／ ロザンヌ・フォティ ／ クリスティ・デ・ヴェイダー ／ ドヴ・エデン ／ ウリエル・レヴィアタン ／ オルガ・エピトロパキ ／ ロビン・マーティン

## 出典
- [Lord, R.G., Foti, R.J. & De Vader, C.L. (1984) A Test of Leadership Categorization Theory — Organizational Behavior and Human Performance, 34(3), 343-378](https://doi.org/10.1016/0030-5073(84)90043-6) `学術`
- [Eden, D. & Leviatan, U. (1975) Implicit Leadership Theory as a Determinant of the Factor Structure Underlying Supervisory Behavior Scales — Journal of Applied Psychology, 60(6), 736-741](https://doi.org/10.1037/h0077731) `学術`
- [Epitropaki, O. & Martin, R. (2004) Implicit Leadership Theories in Applied Settings — Journal of Applied Psychology, 89(2), 293-310](https://doi.org/10.1037/0021-9010.89.2.293) `学術`

## 関連概念
- [[human-golem-effect|ゴーレム効果]] — 同じ思想家
- [[mgmt-playing-to-win-lafley-martin|プレイング・トゥ・ウィン——五つの選択からなる戦略のカスケード]] — 同じ思想家
- [[mgmt-design-thinking-brown-ideo|デザイン思考——ブラウンとIDEOの共感・定義・発想・プロトタイプ・テストの革新プロセス]] — 同じ思想家
- [[mgmt-stakeholder-capitalism-freeman|ステークホルダー資本主義と企業の目的（フリーマン）]] — 同分野の近傍
- [[mgmt-ansoff-matrix|アンゾフの成長マトリクス]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
