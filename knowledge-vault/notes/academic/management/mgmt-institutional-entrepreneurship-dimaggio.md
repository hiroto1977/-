---
collection: academic
id: mgmt-institutional-entrepreneurship-dimaggio
category: "management"
category_ja: "経営学"
title: "制度的起業家精神（ディマジオ）"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/management
  - knowledge/verified
aliases:
  - "制度的起業家精神（ディマジオ）"
---

# 制度的起業家精神（ディマジオ）

> [!info] コレクション: [[学術概念]] ・ 区分: 経営学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
ディマジオ（Paul DiMaggio）は1988年に，制度理論における「エージェンシーのパラドクス」——制度によって形成されたアクターが当の制度を変革できるか——を解決する概念として「制度的起業家（institutional entrepreneur）」を提唱した。制度的起業家とは，既存の制度的配置に利害関係を持ちながら新たな制度の形成を主体的にリードするアクターであり，組織フィールドを動員・政治的支持を構築・代替的制度論理を正当化することで制度変革を実現する。後にガウド（Raghu Garud）・ハーディ（Cynthia Hardy）・マグアイア（Steve Maguire）らによる詳細な事例研究を通じて理論が精緻化され，WHO・EU・NGOなどのアクターが規制・標準・分類システムを制度化するプロセスを分析するフレームワークとなっている。

## 提唱者・初出
ポール・ディマジオ ／ ラグー・ガウド ／ シンシア・ハーディ ／ スティーブ・マグアイア

## 出典
- [DiMaggio, P.J. (1988) Interest and Agency in Institutional Theory — Institutional Patterns and Organizations（制度的起業家精神の原論文）](https://doi.org/10.1086/228499) `学術`
- [Wikipedia: Institutional entrepreneurship — 制度的起業家精神の概説](https://en.wikipedia.org/wiki/Institutional_entrepreneurship) `リファレンス`

## 関連概念
- [[mgmt-institutional-isomorphism|制度的同型化]] — 同じ思想家
- [[mgmt-decoupling-meyer-rowan|デカップリング——正統性のための公式構造と実際の業務活動の分離]] — 同じ思想家
- [[mgmt-identity-work-pratt|アイデンティティ・ワーク（プラット＆フォアマン）]] — 同じ思想家
- [[mgmt-institutional-theory|制度理論（制度的同型化）]] — 同じ思想家
- [[mgmt-paradox-embedded-agency-seo|埋め込まれたエージェンシーのパラドクス——制度変革者の制度的条件付け]] — 同じ思想家
- [[mgmt-institutional-entrepreneurship|制度的アントレプレナーシップ（Institutional Entrepreneurship）]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
