---
collection: academic
id: mgmt-institutional-logics-thornton
category: "management"
category_ja: "経営学"
title: "制度的ロジック理論：制度的多元性と組織的合理性の再解釈"
as_of: "2026-06-25"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/management
  - knowledge/verified
aliases:
  - "制度的ロジック理論：制度的多元性と組織的合理性の再解釈"
---

# 制度的ロジック理論：制度的多元性と組織的合理性の再解釈

> [!info] コレクション: [[学術概念]] ・ 区分: 経営学 ・ asOf: 2026-06-25 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
制度的ロジック理論は、制度が複数の競合するロジック（象徴的・物質的な実践体系）を通じて機能し、組織や個人の行動を方向づけると論じる理論。フリードランドとアルフォード（1991）が資本主義・国家・民主主義・家族・宗教といった制度領域ごとに固有のロジックが存在すると提唱し、ソーントンとオカシオ（1999）がAmerican Journal of Sociologyで企業出版業における制度ロジックの転換と権力構造の変動を実証した。ソーントン・オカシオ・ラウンズベリー（2012）は『The Institutional Logics Perspective』（Oxford UP）で理論を体系化し、家族・共同体・宗教・国家・市場・専門職・企業の7つの制度領域からなる「制度間システム」を定式化した。組織はこれら複数のロジックが併存・競合する「ロジック多元性」の中で戦略的に対応する。

## 提唱者・初出
パトリシア・ソーントン ／ ウィリアム・オカシオ ／ マイケル・ラウンズベリー

## 出典
- [Thornton, P.H. & Ocasio, W. 1999. Institutional Logics and the Historical Contingency of Power in Organizations. American Journal of Sociology, 105(3), 801-843.](https://www.jstor.org/stable/2657300) `学術`
- [Thornton, P.H., Ocasio, W. & Lounsbury, M. 2012. The Institutional Logics Perspective: A New Approach to Culture, Structure, and Process. Oxford University Press.](https://global.oup.com/academic/product/the-institutional-logics-perspective-9780199601936) `学術`
- [Wikipedia — Institutional logic](https://en.wikipedia.org/wiki/Institutional_logic) `リファレンス`

## 関連概念
- [[mgmt-institutional-logics|制度ロジック：信念体系が組織行動を形成するメカニズム]] — 語彙が近い
- [[mgmt-attention-based-view|注意ベース理論（アテンション・ベースト・ビュー）]] — 同じ思想家
- [[mgmt-strategic-issue-diagnosis-dutton|戦略的イシュー診断——経営者の注意配分と問題カテゴリー化の認知プロセス]] — 同じ思想家
- [[mgmt-workplace-spirituality|職場のスピリチュアリティ（意味・目的・共同体意識）]] — 同分野の近傍
- [[mgmt-portfolio-matrix-bcg-growth-share|BCGポートフォリオマトリクス——市場成長率と相対シェアによる事業資源配分フレームワーク]] — 同分野の近傍
- [[mgmt-creative-destruction-schumpeter-org|創造的破壊と組織革新——シュンペーター的競争の経営学的展開]] — 同分野の近傍
- [[mgmt-institutional-logic-friedland|制度的ロジック（フリードランド＆アルフォード）]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
