---
collection: academic
id: mgmt-institutional-work-lawrence
category: "management"
category_ja: "経営学"
title: "制度的ワーク論——アクターによる制度の創造・維持・破壊の実践"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/management
  - knowledge/verified
aliases:
  - "制度的ワーク論——アクターによる制度の創造・維持・破壊の実践"
---

# 制度的ワーク論——アクターによる制度の創造・維持・破壊の実践

> [!info] コレクション: [[学術概念]] ・ 区分: 経営学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
制度的ワーク（Institutional Work）論は、ローレンスとサダビーが2006年に提唱した理論的枠組みであり、組織フィールドにおいてアクター（個人、組織、集団）が制度を意図的に創造（creating）、維持（maintaining）、破壊（disrupting）するために行う目的的な行動を分析する。従来の新制度論が制度の同型化圧力と構造的決定論を強調したのに対し、制度的ワーク論はアクターのエージェンシー（能動的行為主体性）を前面に据え、制度変化の微視的メカニズムを解明する。ローレンスらは制度的ワークを9つの類型に分類し、創造のワーク（唱導、理論化、教育等）、維持のワーク（規範の強化、日常的再生産等）、破壊のワーク（既存規範の切り崩し、正統性の否定等）のそれぞれについて具体的な実践を特定した。この枠組みはCSR実践の制度化、専門職の境界変動、新興市場における制度構築等の分析に広く応用されている。

## 提唱者・初出
トーマス・ローレンス ／ ロイ・サダビー ／ バーナード・ラモ ／ シンシア・ハーディ

## 出典
- [Lawrence & Suddaby (2006) Institutions and Institutional Work — in Handbook of Organization Studies (2nd ed.)](https://doi.org/10.5465/amr.2006.19379628) `学術`
- [Lawrence, Suddaby & Leca (2009) Institutional Work: Actors and Agency in Institutional Studies of Organizations — Cambridge University Press](https://doi.org/10.1017/CBO9780511596605) `学術`
- [Wikipedia: Institutional work — 制度的ワーク論の理論と応用の概説](https://en.wikipedia.org/wiki/Institutional_work) `リファレンス`

## 関連概念
- [[mgmt-institutional-work|制度的ワーク]] — 語彙が近い
- [[mgmt-identity-work-pratt|アイデンティティ・ワーク（プラット＆フォアマン）]] — 同じ思想家
- [[mgmt-institutional-entrepreneurship-dimaggio|制度的起業家精神（ディマジオ）]] — 同じ思想家
- [[mgmt-paradox-embedded-agency-seo|埋め込まれたエージェンシーのパラドクス——制度変革者の制度的条件付け]] — 同じ思想家
- [[mgmt-dynamic-capabilities-sensing-seizing|動的ケイパビリティ：感知・捕捉・変容]] — 同分野の近傍
- [[mgmt-shared-value|共有価値の創造（CSV）]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
