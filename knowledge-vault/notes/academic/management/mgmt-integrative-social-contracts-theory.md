---
collection: academic
id: mgmt-integrative-social-contracts-theory
category: "management"
category_ja: "経営学"
title: "統合的社会契約理論（ISCT）——ビジネス倫理の普遍的規範と地域的規範の二層構造"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/management
  - knowledge/verified
aliases:
  - "統合的社会契約理論（ISCT）——ビジネス倫理の普遍的規範と地域的規範の二層構造"
---

# 統合的社会契約理論（ISCT）——ビジネス倫理の普遍的規範と地域的規範の二層構造

> [!info] コレクション: [[学術概念]] ・ 区分: 経営学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
統合的社会契約理論（Integrative Social Contracts Theory, ISCT）は、トーマス・ドナルドソンとトーマス・ダンフィが1994年・1999年に提唱したビジネス倫理の規範的フレームワークである。理論は二層構造を持ち、「超社会的契約（Hypernorm）」——文化・宗教・伝統の差異を超えて普遍的に認められる倫理的原則（殺人・拷問の禁止・基本的人権尊重）——と、「微社会的契約（Microsocial Contract）」——特定の産業・地域・コミュニティが同意する地域的・慣習的規範——から成る。両層の規範が抵触する場合は超社会的契約が優先される「優先規則」を設ける。多国籍企業が異なる文化・法制度の国で事業展開する際の「文化的相対主義 vs. 道徳的帝国主義」のジレンマ解決に有効とされる。サプライチェーン倫理・腐敗防止・労働基準・環境規範のグローバル水準設定においてISCTが援用され、国連グローバル・コンパクトの十原則設計にも理論的影響を与えている。

## 提唱者・初出
トーマス・ドナルドソン ／ トーマス・ダンフィ ／ リン・シャープ・ペイン ／ デニス・モーバーグ

## 出典
- [Donaldson, T. & Dunfee, T.W. (1994) Toward a Unified Conception of Business Ethics — Academy of Management Review](https://doi.org/10.5465/amr.1994.9410210749) `学術`
- [UN Global Compact Ten Principles — Universal Ethical Standards for Business](https://www.unglobalcompact.org/what-is-gc/mission/principles) `公的`
- [Wikipedia: ビジネス倫理 — 社会契約理論と企業倫理の解説](https://ja.wikipedia.org/wiki/%E3%83%93%E3%82%B8%E3%83%8D%E3%82%B9%E5%80%AB%E7%90%86) `リファレンス`

## 関連概念
- [[mgmt-360-feedback|360度フィードバック（多面評価）]] — 同分野の近傍
- [[mgmt-3c-model-ohmae|大前の3Cモデル——顧客・自社・競合の戦略的三角形]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
