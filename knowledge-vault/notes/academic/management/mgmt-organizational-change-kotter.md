---
collection: academic
id: mgmt-organizational-change-kotter
category: "management"
category_ja: "経営学"
title: "組織変革の8ステップ——コッターの変革管理フレームワークと抵抗の克服"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/management
  - knowledge/verified
aliases:
  - "組織変革の8ステップ——コッターの変革管理フレームワークと抵抗の克服"
---

# 組織変革の8ステップ——コッターの変革管理フレームワークと抵抗の克服

> [!info] コレクション: [[学術概念]] ・ 区分: 経営学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
ジョン・コッターはハーバード・ビジネス・スクール教授として、1995年のHBR論文「なぜ変革の試みは失敗するのか」と著書『企業変革力』（1996年）において、組織変革を成功させる「8段階プロセス」を提示した。①「危機感の醸成」②「変革推進チームの結成」③「ビジョンと戦略の策定」④「変革ビジョンの周知徹底」⑤「自発的行動の促進（障害の除去）」⑥「短期的成果の達成」⑦「成果を生かした継続的変革」⑧「新しいアプローチの企業文化への定着」である。各ステップを順守せずに進めると変革プロジェクトが停滞・失敗するとされる。コッターは2014年に「デュアル・オペレーティング・システム（Dual Operating System）」を提唱し、効率運用のための「ヒエラルキー」と変革のための「ネットワーク」が同時並行で機能する組織構造を推奨した。一方、デービッド・ネーデラーらの批判は段階的プロセスが現実の変革の反復性・非線形性を過度に単純化すると指摘する。DX・M&Aポスト統合・コロナ禍の組織再設計での実践的活用が広い。

## 提唱者・初出
ジョン・コッター ／ クルト・ルーウィン ／ エドガー・シャイン ／ デービッド・ネーデラー

## 出典
- [Kotter, J.P. (1995) Leading Change: Why Transformation Efforts Fail — Harvard Business Review](https://hbr.org/1995/05/leading-change-why-transformation-efforts-fail) `学術`
- [経済産業省 DX推進指標 — 組織変革と経営改革の枠組み](https://www.meti.go.jp/policy/it_policy/dx/dx.html) `公的`
- [Wikipedia: 組織変革 — コッターの8ステップと変革管理の解説](https://ja.wikipedia.org/wiki/%E7%B5%84%E7%B9%94%E5%A4%89%E9%9D%A9) `リファレンス`

## 関連概念
- [[mgmt-kotter-8-steps|コッターの変革8段階モデル]] — 語彙が近い
- [[mgmt-change-management-kotter-8-step|コッターの8段階変革モデル——組織変革の失敗要因分析と変革リーダーシップの実践フレームワーク]] — 語彙が近い
- [[mgmt-kotter-change|コッターの変革の8段階プロセス]] — 同じ思想家
- [[mgmt-radical-transparency-dalio|ラジカル・トランスパレンシー（ダリオ）]] — 同じ思想家
- [[mgmt-bridges-transition-model-ending-neutral-zone-beginning|ブリッジズのトランジション・モデル——「終わり」「ニュートラルゾーン」「新たな始まり」の3段階で変化への心理的移行を捉える枠組み]] — 同じ思想家
- [[mgmt-corporate-culture-schein-artifacts|組織文化論——シャインの三層モデル（人工物・信念・基本的仮定）と文化変革]] — 同じ思想家
- [[mgmt-defensive-routines-argyris|組織の防衛的ルーチン——脅威を避けて学習を妨げる習慣]] — 同じ思想家
- [[mgmt-humble-leadership-owens|謙虚なリーダーシップ——自己認識・他者尊重・学習志向による影響過程]] — 同じ思想家
- [[mgmt-mintzberg-managerial-roles|ミンツバーグの管理者の役割——観察に基づく管理職の実際の仕事の十類型]] — 同じ思想家
- [[mgmt-psychological-contract|心理的契約]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
