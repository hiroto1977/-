---
collection: academic
id: mgmt-red-queen-barnett
category: "management"
category_ja: "経営学"
title: "レッドクイーン競争——競争を通じた相互適応が淘汰圧を絶えず再生産する"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/management
  - knowledge/verified
aliases:
  - "レッドクイーン競争——競争を通じた相互適応が淘汰圧を絶えず再生産する"
---

# レッドクイーン競争——競争を通じた相互適応が淘汰圧を絶えず再生産する

> [!info] コレクション: [[学術概念]] ・ 区分: 経営学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
レッドクイーン競争（Red Queen competition）は、ウィリアム・バーネットとモートン・ハンセンが1996年の論文で組織進化に導入した概念で、ルイス・キャロル『鏡の国のアリス』の赤の女王が「同じ場所に留まるためには全力で走り続けねばならない」と語る場面に由来する。生物学者リー・ヴァン・ヴェイレンが1973年に提唱した共進化的軍拡競争の仮説を組織に応用したもので、競争にさらされた企業は対抗するために能力を高め学習するが、その適応がライバルにとっての競争圧力を強め、ライバルもまた適応する、という自己強化的な共進化の循環を描く。この過程で各企業は強靭になるが、競争相手も同時に強くなるため、優位は相対的なものにとどまり、走り続けなければ脱落する。バーネットらは銀行業の実証研究で、激しい競争を生き延びた組織ほど生存力が高い一方、特定の競争環境への過剰適応が環境変化時に「能力の罠（competency trap）」を生む危険も指摘した。

## 提唱者・初出
ウィリアム・バーネット ／ モートン・ハンセン ／ リー・ヴァン・ヴェイレン ／ ダニエル・レヴィンソール

## 出典
- [Barnett, W.P. & Hansen, M.T. (1996) The Red Queen in Organizational Evolution — Strategic Management Journal 17(S1)](https://www.jstor.org/stable/2486816) `学術`
- [Wikipedia: Red Queen hypothesis — Van Valen, coevolution, organizational evolution](https://en.wikipedia.org/wiki/Red_Queen_hypothesis) `リファレンス`

## 関連概念
- [[mgmt-red-queen-effect|赤の女王効果（Red Queen Effect）]] — 語彙が近い
- [[mgmt-absorptive-capacity-cohen|吸収能力（コーエン＆レヴィンソール）]] — 同じ思想家
- [[mgmt-transactive-memory-wegner|トランザクティブ記憶システム（ウェグナー）]] — 同じ思想家
- [[mgmt-competency-trap|コンピテンシー・トラップと成功の罠（レヴィット＝マーチ／レヴィンタール＝マーチ）——過去の成功への習熟が学習を深化に偏らせ、より優れた選択肢への転換を妨げる]] — 同分野の近傍
- [[mgmt-supply-chain-resilience|サプライチェーン強靭化（supply chain resilience）]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
