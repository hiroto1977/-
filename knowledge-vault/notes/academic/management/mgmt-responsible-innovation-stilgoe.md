---
collection: academic
id: mgmt-responsible-innovation-stilgoe
category: "management"
category_ja: "経営学"
title: "責任ある研究・イノベーション（RRI）と科学技術ガバナンス"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/management
  - knowledge/verified
aliases:
  - "責任ある研究・イノベーション（RRI）と科学技術ガバナンス"
---

# 責任ある研究・イノベーション（RRI）と科学技術ガバナンス

> [!info] コレクション: [[学術概念]] ・ 区分: 経営学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
責任ある研究・イノベーション（Responsible Research and Innovation: RRI）は、研究・開発・イノベーションのプロセスを倫理的・社会的・環境的考慮と連携させ、持続可能で公正な成果を生み出そうとする政策・研究枠組みである。ジャック・スティルゲイほかは、RRIを「予測（anticipate）・省察（reflect）・関与（engage）・行動（act）」の四次元で構造化し、科学技術の「意図せぬ結果」を事前に社会的対話を通じて管理しようとした。EUのホライズン2020・ホライズン・ヨーロッパはRRIを主要ガバナンス原則として組み込み、科学教育・オープンアクセス・ジェンダー平等・倫理・社会参加を柱とする。AIのアライメント問題・合成生物学・気候工学・ナノテクノロジーなど新興技術の分野でRRIフレームワークの応用が進んでいる。

## 提唱者・初出
ジャック・スティルゲイ ／ ルネ・フォン・シュームベルク ／ エイドリアン・スミス ／ アレックス・ヴォス

## 出典
- [Stilgoe, J., Owen, R. & Macnaghten, P. (2013). Developing a framework for responsible innovation — Research Policy 42(9), 1568–1580（旧 DOI は OUP 書籍のものだったため Elsevier 論文ページへ差し替え）](https://www.sciencedirect.com/science/article/pii/S0048733313000930) `学術`
- [Wikipedia: Responsible Research and Innovation — EU science policy](https://en.wikipedia.org/wiki/Responsible_Research_and_Innovation) `リファレンス`

## 関連概念
- [[mgmt-cross-cultural-leadership-house-globe|GLOBEプロジェクト——文化次元とリーダーシップの国際比較]] — 同分野の近傍
- [[mgmt-lawrence-lorsch|分化と統合（ローレンス＆ローシュ）]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
