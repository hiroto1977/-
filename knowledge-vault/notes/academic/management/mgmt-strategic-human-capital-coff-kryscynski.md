---
collection: academic
id: mgmt-strategic-human-capital-coff-kryscynski
category: "management"
category_ja: "経営学"
title: "戦略的人的資本——知識労働者の獲得・保持と持続的競争優位"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/management
  - knowledge/verified
aliases:
  - "戦略的人的資本——知識労働者の獲得・保持と持続的競争優位"
---

# 戦略的人的資本——知識労働者の獲得・保持と持続的競争優位

> [!info] コレクション: [[学術概念]] ・ 区分: 経営学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
戦略的人的資本論は、リソース・ベースト・ビュー（RBV）の枠組みの中で、人的資本が企業の持続的競争優位の源泉となるメカニズムを解明する研究領域である。コフ（1997）は、人的資本が価値あり・稀少で・模倣困難な資源であっても、知識労働者の高い移動性ゆえに企業がその価値を占有（appropriate）できない可能性を指摘し、「人的資本は誰のものか」という根本的問いを提起した。コフとクリシンスキー（2011）は、隔離メカニズム（isolating mechanisms）の概念を人的資本文脈に拡張し、企業が人的資本からレントを獲得するために労働市場の不完全性（情報の非対称性、移動コスト、企業特殊的スキル）に依存していることを理論化した。キャンベルら（2012）は、マイクロファウンデーションの観点から個人レベルの人的資本が組織レベルの能力にどう集約されるかを検討し、補完性と共特殊化の役割を強調した。ラザーとオイヤー（2012）は内部労働市場と外部労働市場の相互作用から人的資本投資の効率性を分析している。

## 提唱者・初出
ラッセル・コフ ／ デイヴィッド・クリシンスキー ／ ベンジャミン・キャンベル ／ エドワード・ラザー ／ ポール・オイヤー

## 出典
- [Coff & Kryscynski (2011) Drilling for Micro-Foundations of Human Capital-Based Competitive Advantages — Journal of Management](https://doi.org/10.1177/0149206310397772) `学術`
- [Campbell et al. (2012) Resource Aggregation and Its Implications for Strategic Management — Strategic Management Journal](https://doi.org/10.1002/smj.2116) `学術`
- [Wikipedia: Human capital — 人的資本理論の経済学・経営学横断的概説](https://en.wikipedia.org/wiki/Human_capital) `リファレンス`

## 関連概念
- [[mgmt-causal-ambiguity-lippman-rumelt|因果的曖昧性——成功の原因が分からないことが模倣を防ぐ]] — 同分野の近傍
- [[mgmt-organizational-resilience-theory|組織レジリエンス理論]] — 同分野の近傍
- [[mgmt-dynamic-capabilities-sensing-seizing|動的ケイパビリティ：感知・捕捉・変容]] — 同分野の近傍
- [[mgmt-executive-hubris|ヒューブリス仮説（経営者の過信）]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
