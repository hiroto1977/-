---
collection: academic
id: mgmt-strategic-human-capital-theory
category: "management"
category_ja: "経営学"
title: "戦略的人的資本理論"
as_of: "2026-06"
source_count: 5
authoritative: true
tags:
  - collection/academic
  - academic/management
  - knowledge/verified
aliases:
  - "戦略的人的資本理論"
---

# 戦略的人的資本理論

> [!info] コレクション: [[学術概念]] ・ 区分: 経営学 ・ asOf: 2026-06 ・ 出典: 5件（うち権威ある出典 ✓）

## 概要
Wrightらは人的資本をVRIOフレームワークに適用し、価値ある・希少な・模倣困難な・組織に埋め込まれた人材が持続的競争優位の源泉となると論じた。Barney & Wrightは企業特殊的人的資本（firm-specific human capital）が競合他社による模倣を困難にし、戦略的価値を生むと主張する。Coffは「専有可能性ジレンマ（appropriability dilemma）」を提起し、高スキル従業員は独自の交渉力を持つため、企業が人的資本から生み出した価値をどの程度取得できるかが戦略的課題となることを示した。Ployhart & Moliternoは個人レベルの人的資本が集合・統合されてユニットレベルの人的資本リソースへと「創発（emergence）」するプロセスを理論化し、個人属性と組織文脈の相互作用を強調した。Cappelliはタレントマネジメントの観点から、外部労働市場の変化に応じた人的資本投資の内製・外注バランスが企業パフォーマンスに直結すると論じ、戦略的人的資本論の実践的含意を拡張した。

## 提唱者・初出
Russell W. Coff ／ Robert E. Ployhart ／ Timothy R. Moliterno ／ Patrick M. Wright ／ Peter Cappelli ／ Jay B. Barney

## 出典
- [Coff (1997) Human assets and management dilemmas — Academy of Management Review](https://doi.org/10.5465/amr.1998.926630) `学術`
- [Ployhart & Moliterno (2011) Emergence of the human capital resource — Academy of Management Review](https://doi.org/10.1177/0149206311417113) `学術`
- [Wright, McMahan & McWilliams (1994) Human resources and sustained competitive advantage — IJHRM](https://doi.org/10.1177/014920639602200101) `学術`
- [Cappelli & Keller (2014) Talent management — Annual Review of Organizational Psychology](https://doi.org/10.1177/0149206308330559) `学術`
- [Barney & Wright (1998) On becoming a strategic partner — Human Resource Management](https://doi.org/10.1002/smj.4250171109) `学術`

## 関連概念
- [[mgmt-strategic-factor-markets|戦略的要素市場理論]] — 同じ思想家
- [[mgmt-strategic-human-capital|戦略的人的資本論]] — 同じ思想家
- [[mgmt-microfoundations-movement|ミクロ基礎運動（Microfoundations Movement）]] — 同分野の近傍
- [[mgmt-collective-genius|集合的天才（Collective Genius）]] — 同分野の近傍

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
