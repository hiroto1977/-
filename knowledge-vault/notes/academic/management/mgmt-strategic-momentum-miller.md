---
collection: academic
id: mgmt-strategic-momentum-miller
category: "management"
category_ja: "経営学"
title: "戦略的モメンタム——成功パターンの固定化が組織衰退を招くメカニズム"
as_of: "2026-06-26"
source_count: 3
authoritative: true
tags:
  - collection/academic
  - academic/management
  - knowledge/verified
aliases:
  - "戦略的モメンタム——成功パターンの固定化が組織衰退を招くメカニズム"
---

# 戦略的モメンタム——成功パターンの固定化が組織衰退を招くメカニズム

> [!info] コレクション: [[学術概念]] ・ 区分: 経営学 ・ asOf: 2026-06-26 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
ダニー・ミラーが1990年の著書「イカロスの逆説」で理論化した戦略的モメンタム（strategic momentum）は、過去の成功を支えた戦略パターン・構造・文化が組織内で自己強化され、環境変化への適応を妨げる逆説的プロセスを分析する概念である。ミラーは多数の企業の縦断的研究に基づき、成功企業が典型的に四つの衰退軌道を辿ることを同定した。「集中化」（技術的卓越性の追求が技術的自己満足に陥る）、「冒険」（成長志向が無謀な多角化に変質する）、「発明」（イノベーション重視が実用性を無視した発明マニアに堕落する）、「脱凝集」（官僚制の回避が組織的混沌を招く）。イカロスが太陽に近づきすぎて墜落したギリシャ神話になぞらえ、各企業の「翼」（最大の強み）が最終的に「墜落」の原因となるパラドックスを描いた。コンピテンシー・トラップやサクセス・トラップの概念とも関連し、組織学習論における「搾取と探索のバランス」議論の基盤となった。

## 提唱者・初出
ダニー・ミラー ／ クレイトン・クリステンセン ／ ジェームズ・マーチ ／ ドロシー・レオナルド＝バートン

## 出典
- [Miller, D. (1993) The Architecture of Simplicity — Academy of Management Review 18(1)](https://doi.org/10.1016/0149-2063(93)90053-P) `学術`
- [Wikipedia: Icarus Paradox — Miller success-driven organizational decline](https://en.wikipedia.org/wiki/Icarus_paradox) `リファレンス`
- [Miller, D. (1992) The Icarus Paradox: How Exceptional Companies Bring About Their Own Downfall — Business Horizons 35(1)](https://doi.org/10.1002/smj.4250130307) `学術`

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
