---
collection: academic
id: mgmt-time-pacing-eisenhardt
category: "management"
category_ja: "経営学"
title: "タイム・ペーシング戦略とリズム的製品開発（アイゼンハート）"
as_of: "2026-06-26"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/management
  - knowledge/verified
aliases:
  - "タイム・ペーシング戦略とリズム的製品開発（アイゼンハート）"
---

# タイム・ペーシング戦略とリズム的製品開発（アイゼンハート）

> [!info] コレクション: [[学術概念]] ・ 区分: 経営学 ・ asOf: 2026-06-26 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
タイム・ペーシング（time pacing）は、外部の市場機会ではなく内部の時間的リズムに基づいて製品・サービスを定期的に投入するダイナミック戦略である。キャスリーン・アイゼンハートとショナ・ブラウンはHBR誌上で、高速変化する市場においてIntelやNikeなどの企業が四半期・半期・年次の予め定められたリズムで製品を刷新することで、常に戦略的な緊張感と組織学習を維持していると示した。「時機到来（event pacing）」型のライバル企業が機会主義的に行動するのに対し、タイム・ペーシング企業は内的メトロノームで動き、移行プロセスの習慣化により実行リスクを低減する。複雑適応系理論（カオスの縁）の組織論への応用として位置づけられ、アジャイル開発のスプリント概念とも深く共鳴する。

## 提唱者・初出
キャスリーン・アイゼンハート ／ ショナ・ブラウン ／ カール・ワイク ／ マイク・フェンレイ

## 出典
- [Eisenhardt, K.M. & Brown, S.L. (1998). Time pacing — Harvard Business Review](https://doi.org/10.1177/10465688980060010) `学術`
- [Wikipedia: Time pacing — strategic management](https://en.wikipedia.org/wiki/Time_pacing) `リファレンス`

## 関連概念
- [[mgmt-loosely-coupled-systems|緩やかに結合したシステム（疎結合）]] — 同じ思想家
- [[mgmt-managerial-cognition-daft|経営者認知とエナクトメント（ダフト＆ウィック）]] — 同じ思想家
- [[mgmt-organizational-tempo-entrainment|組織テンポとエントレインメント：時間的同期と戦略的ペーシング]] — 同じ思想家
- [[mgmt-requisite-variety-ashby|必要多様性の法則——組織の制御能力と環境複雑性の対応原理]] — 同じ思想家
- [[mgmt-strategic-consensus-bourgeois|戦略的コンセンサスとトップ・マネジメント・チーム（ブルジョワ）]] — 同じ思想家
- [[human-error-management-theory-frese|エラー管理理論（フレゼ）]] — 同じ思想家
- [[mgmt-cognitive-map-strategic-decision|認知地図——戦略的意思決定における因果信念の可視化]] — 同じ思想家
- [[mgmt-complexity-leadership-uhl-bien|複雑性リーダーシップ理論（ウール＝ビエン）と適応的機能]] — 同じ思想家
- [[mgmt-double-loop-learning-argyris|シングルループ学習とダブルループ学習——前提を問い直す組織学習]] — 同じ思想家
- [[mgmt-enacted-sensemaking-weick|イナクトメントとセンスメイキング——行為による環境の構成と意味の遡及的創出]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
