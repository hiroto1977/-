---
collection: academic
id: mgmt-value-disciplines-treacy-wiersema
category: "management"
category_ja: "経営学"
title: "価値基準（バリュー・ディシプリン）——市場リーダーが選ぶ三つの卓越"
as_of: "2026-06-27"
source_count: 2
authoritative: true
tags:
  - collection/academic
  - academic/management
  - knowledge/verified
aliases:
  - "価値基準（バリュー・ディシプリン）——市場リーダーが選ぶ三つの卓越"
---

# 価値基準（バリュー・ディシプリン）——市場リーダーが選ぶ三つの卓越

> [!info] コレクション: [[学術概念]] ・ 区分: 経営学 ・ asOf: 2026-06-27 ・ 出典: 2件（うち権威ある出典 ✓）

## 概要
価値基準（value disciplines、バリュー・ディシプリン）は、経営コンサルタントのマイケル・トレーシーとフレッド・ウィアセーマが1993年の論文および1995年の著書『市場リーダーの法則（The Discipline of Market Leaders）』で提示した競争戦略の枠組みで、市場をリードする企業は、顧客に提供する価値の三つの「型」のいずれか一つに絞って卓越することで成功している、と論じる。彼らの出発点は「すべての人にすべてを提供しようとする企業は成功できない」という洞察である。企業は、自社だけが特定の市場に届けられる独自の価値を見定め、それに資源と組織を集中させなければならない。トレーシーとウィアセーマは、その価値を三つの規律に分類した。第一は「オペレーショナル・エクセレンス（業務の卓越）」で、無駄を削ぎ、業務プロセスを徹底的に効率化することで、信頼できる製品・サービスを最も低い総コストと手間で提供する（例：ウォルマート、マクドナルド）。第二は「プロダクト・リーダーシップ（製品の卓越）」で、絶え間ない技術革新によって、市場で最も優れた最先端の製品を生み出し続ける（例：アップル、インテル）。第三は「カスタマー・インティマシー（顧客との親密さ）」で、特定の顧客を深く理解し、要望にきめ細かく応え、関係を築いて、その顧客にとって最適な総合的解決策を提供する（例：高級ホテル、専門コンサルティング）。彼らの中心的な主張は、企業はこの三つのうち一つを選んで業界の頂点を目指す一方、残る二つについても、顧客に見放されない最低限の水準（業界標準）は維持しなければならない、というものである。三つすべてで一流になろうとすれば、組織の焦点が分散し、どれも中途半端になる。価値基準は、ポーターの三つの基本戦略（コスト・リーダーシップ、差別化、集中）と並ぶ競争戦略の枠組みとして、企業が自社の戦略的アイデンティティと組織能力をどこに集中させるべきかを問う、戦略経営論の実務的なモデルとなっている。

## 提唱者・初出
マイケル・トレーシー ／ フレッド・ウィアセーマ ／ マイケル・ポーター ／ ピーター・ドラッカー

## 出典
- [Treacy, M. & Wiersema, F. (1993) Customer Intimacy and Other Value Disciplines — Harvard Business Review, 71(1), 84–93](https://hbr.org/1993/01/customer-intimacy-and-other-value-disciplines) `学術`
- [Toolshero: Value Disciplines Model (Treacy & Wiersema) — operational excellence, product leadership, customer intimacy; excel in one, threshold in others](https://www.toolshero.com/strategy/value-disciplines-treacy-wiersema/) `リファレンス`

## 関連概念
- [[mgmt-playing-to-win-lafley-martin|プレイング・トゥ・ウィン——五つの選択からなる戦略のカスケード]] — 同じ思想家
- [[infosoc-knowledge-society|知識社会（ナレッジ・ソサエティ）]] — 同じ思想家
- [[mgmt-3c-model-ohmae|大前の3Cモデル——顧客・自社・競合の戦略的三角形]] — 同じ思想家
- [[mgmt-portfolio-matrix-bcg-growth-share|BCGポートフォリオマトリクス——市場成長率と相対シェアによる事業資源配分フレームワーク]] — 同じ思想家
- [[mgmt-transient-advantage|一時的競争優位（トランジェント・アドバンテージ）]] — 同じ思想家
- [[econ-knowledge-economy-machlup|知識経済論と情報財の経済的特性——マクルップからドラッカーへ]] — 同じ思想家
- [[econ-porter-hypothesis|ポーター仮説——適切な環境規制がイノベーションを通じて競争力を高める]] — 同じ思想家
- [[infosoc-information-society-machlup|知識産業論——知識の生産と分配を経済の主要部門として計量する]] — 同じ思想家
- [[mgmt-diamond-model-porter|ポーターのダイヤモンドモデル——国の競争優位を生む四つの決定要因]] — 同じ思想家
- [[mgmt-generic-strategies-porter|ポーターの基本戦略——コスト・リーダーシップ・差別化・集中]] — 同じ思想家

## 関連
- コレクション: [[学術概念]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
