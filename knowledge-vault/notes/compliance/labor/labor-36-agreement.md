---
collection: compliance
id: labor-36-agreement
category: "labor"
category_ja: "労務"
title: "36協定（時間外・休日労働に関する労使協定）"
as_of: "2026-06"
source_count: 3
authoritative: true
tags:
  - collection/compliance
  - compliance/labor
  - knowledge/verified
aliases:
  - "36協定（時間外・休日労働に関する労使協定）"
---

# 36協定（時間外・休日労働に関する労使協定）

> [!info] コレクション: [[法務・税務・労務]] ・ 区分: 労務 ・ asOf: 2026-06 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
法定労働時間（原則1日8時間・週40時間）を超える時間外労働や法定休日労働を行わせるには、労働者の過半数代表等との書面による労使協定（36協定）を締結し、所轄労働基準監督署長に届け出ることが必要である（労基法36条）。36協定で延長できる時間外労働には上限規制があり、原則として月45時間・年360時間が限度とされる。

## 所管・根拠
所管: 厚生労働省（労働基準法）

## 出典
- [厚生労働省 労働基準法36条について](https://www.mhlw.go.jp/shinsai_jouhou/koyou_roudou/2r9852000001auw2.html) `公的`
- [厚生労働省 働き方改革 時間外労働の上限規制](https://hatarakikatakaikaku.mhlw.go.jp/overtime.html) `公的`
- [厚生労働省 スタートアップ労働条件 36協定](https://www.startup-roudou.mhlw.go.jp/36_pact.html) `公的`

## 関連概念
- [[labor-overtime-premium|割増賃金率（時間外・休日・深夜）]] — 語彙が近い
- [[labor-work-rules|就業規則の作成・届出義務]] — 語彙が近い
- [[labor-wage-deduction|賃金からの控除と労使協定（24協定）]] — 語彙が近い
- [[labor-safety-management|安全衛生管理体制（50人以上）]] — 語彙が近い
- [[labor-dispatch-period|労働者派遣の期間制限（3年ルール）]] — 語彙が近い
- [[labor-variable-working-hours|変形労働時間制]] — 語彙が近い
- [[bizlaw-labor-standards|労働基準法（労働時間・36協定）]] — 出典を共有
- [[labor-leave-allowance|休業手当（労働基準法26条）]] — 出典を共有
- [[labor-overtime-cap|時間外労働の上限規制（36協定）]] — 出典を共有
- [[labor-paid-leave-5days|年次有給休暇の年5日取得義務]] — 出典を共有

## 関連
- コレクション: [[法務・税務・労務]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
