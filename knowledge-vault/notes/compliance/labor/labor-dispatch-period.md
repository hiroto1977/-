---
collection: compliance
id: labor-dispatch-period
category: "labor"
category_ja: "労務"
title: "労働者派遣の期間制限（3年ルール）"
as_of: "2026-06"
source_count: 3
authoritative: true
tags:
  - collection/compliance
  - compliance/labor
  - knowledge/verified
aliases:
  - "労働者派遣の期間制限（3年ルール）"
---

# 労働者派遣の期間制限（3年ルール）

> [!info] コレクション: [[法務・税務・労務]] ・ 区分: 労務 ・ asOf: 2026-06 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
2015年改正により、派遣先の同一事業所での受入れは原則3年が上限（事業所単位）、同一組織単位で同一の派遣労働者を受け入れるのも原則3年が上限（個人単位）。事業所単位は過半数労働組合等への意見聴取で延長できる。

## 所管・根拠
所管: 厚生労働省（労働者派遣法）

## 出典
- [厚生労働省 愛知労働局 派遣の期間制限](https://jsite.mhlw.go.jp/aichi-roudoukyoku/hourei_seido_tetsuzuki/roudousha_haken/hourei_seido/hakensaki_00001.html) `公的`
- [派遣の抵触日 解説](https://www.manpowergroup.jp/client/manpowerclip/temporary/restriction_period.html) `メディア`
- [派遣法の3年ルール 解説](https://www.pasona.co.jp/clients/service/column/jhk/haken3years_rule/) `メディア`

## 関連概念
- [[labor-36-agreement|36協定（時間外・休日労働に関する労使協定）]] — 語彙が近い
- [[labor-dispatch-equal-treatment|派遣労働者の同一労働同一賃金（不合理な待遇差の禁止）]] — 語彙が近い
- [[subsidy-human-resource-development|人材開発支援助成金]] — 出典を共有
- [[labor-disguised-contract|偽装請負（労働者派遣と請負の区分）]] — 同分野の近傍

## 関連
- コレクション: [[法務・税務・労務]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
