---
collection: compliance
id: labor-mandatory-retirement-age
category: "labor"
category_ja: "労務"
title: "定年年齢の下限（60歳未満定年の禁止）"
as_of: "2026-06"
source_count: 3
authoritative: true
tags:
  - collection/compliance
  - compliance/labor
  - knowledge/verified
aliases:
  - "定年年齢の下限（60歳未満定年の禁止）"
---

# 定年年齢の下限（60歳未満定年の禁止）

> [!info] コレクション: [[法務・税務・労務]] ・ 区分: 労務 ・ asOf: 2026-06 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
事業主が定年を定める場合、その定年年齢は60歳を下回ることができない（高年齢者雇用安定法8条。坑内作業など厚生労働省令で定める一部業務を除く）。これは同法9条が定める「65歳までの高年齢者雇用確保措置」とは別の、定年年齢そのものに対する下限規制である。

## 所管・根拠
所管: 厚生労働省（高年齢者雇用安定法）

## 出典
- [厚生労働省 高年齢者雇用安定法の概要](https://www.mhlw.go.jp/stf/seisakunitsuite/bunya/koyou_roudou/koyou/koureisha/topics/tp120903-1_00001.html) `公的`
- [厚生労働省 高年齢者雇用安定法の概要（PDF）](https://www.mhlw.go.jp/content/11700000/001245647.pdf) `公的`
- [高年齢者雇用安定法 8条 解説](https://www.rodo.co.jp/laws/117605/) `メディア`

## 関連概念
- [[bizlaw-elderly-employment|高年齢者雇用安定法]] — 分野横断
- [[labor-dispatch-license|労働者派遣事業の許可制]] — 出典を共有
- [[labor-dispatch-prohibited|労働者派遣の適用除外業務（派遣禁止業務）]] — 出典を共有
- [[labor-pregnant-work-restriction|妊産婦の就業制限（労働基準法の母性保護規定）]] — 出典を共有
- [[labor-safety-education|雇入れ時等の安全衛生教育（労働安全衛生法59条）]] — 出典を共有
- [[labor-safety-health-committee|安全委員会・衛生委員会・安全衛生委員会の設置義務]] — 出典を共有
- [[labor-elderly-employment|高年齢者雇用確保措置]] — 同分野の近傍
- [[labor-high-professional|高度プロフェッショナル制度（労基法41条の2）]] — 同分野の近傍

## 関連
- コレクション: [[法務・税務・労務]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
