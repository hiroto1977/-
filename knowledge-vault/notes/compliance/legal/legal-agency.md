---
collection: compliance
id: legal-agency
category: "legal"
category_ja: "法務"
title: "民法の代理（代理権・無権代理・表見代理）"
as_of: "2026-06"
source_count: 3
authoritative: true
tags:
  - collection/compliance
  - compliance/legal
  - knowledge/verified
aliases:
  - "民法の代理（代理権・無権代理・表見代理）"
---

# 民法の代理（代理権・無権代理・表見代理）

> [!info] コレクション: [[法務・税務・労務]] ・ 区分: 法務 ・ asOf: 2026-06 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
代理人がその権限内において本人のためにすることを示して（顕名）した意思表示は、直接本人に対して効力を生じる（民法99条）。代理権を有しない者が本人の代理人としてした契約（無権代理）は、本人が追認しなければ本人に対して効力を生じない（113条）が、代理権授与の表示があった場合・権限を越えた場合・代理権消滅後の場合等で相手方に代理権を信ずべき正当な理由があるとき等は、本人が責任を負う表見代理（109条・110条・112条）が成立しうる。

## 所管・根拠
所管: 法務省（民法）

## 出典
- [e-Gov法令検索 民法（99条・109条・110条・112条・113条）](https://laws.e-gov.go.jp/law/129AC0000000089) `公的`
- [法務省 民法（債権関係）改正資料（表見代理等）](https://www.moj.go.jp/content/000118124.pdf) `公的`
- [民法113条（無権代理）条文](https://ja.wikibooks.org/wiki/民法第113条) `メディア`

## 関連概念
- [[bizlaw-apparent-agency|表見代理]] — 分野横断
- [[legal-coolingoff-electronic|電磁的記録によるクーリング・オフ通知（2022年6月施行）]] — 同分野の近傍
- [[legal-abuse-superior-bargaining-position|優越的地位の濫用（独占禁止法上の不公正な取引方法）]] — 同分野の近傍

## 関連
- コレクション: [[法務・税務・労務]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
