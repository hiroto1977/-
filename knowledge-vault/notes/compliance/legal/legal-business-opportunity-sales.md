---
collection: compliance
id: legal-business-opportunity-sales
category: "legal"
category_ja: "法務"
title: "業務提供誘引販売取引（内職商法・モニター商法）"
as_of: "2026-06"
source_count: 3
authoritative: true
tags:
  - collection/compliance
  - compliance/legal
  - knowledge/verified
aliases:
  - "業務提供誘引販売取引（内職商法・モニター商法）"
---

# 業務提供誘引販売取引（内職商法・モニター商法）

> [!info] コレクション: [[法務・税務・労務]] ・ 区分: 法務 ・ asOf: 2026-06 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
業務提供誘引販売取引とは、「提供する仕事で収入が得られる」と勧誘し、その仕事に必要だとして商品等を販売し金銭負担を負わせる取引で、特定商取引法の規制対象として概要書面・契約書面の交付義務がある。消費者は契約書面を受け取った日から20日間、書面または電磁的方法によりクーリング・オフができる。

## 所管・根拠
所管: 消費者庁（特定商取引法）

## 出典
- [消費者庁 特定商取引法ガイド 業務提供誘引販売取引](https://www.no-trouble.caa.go.jp/what/businessopportunity/) `公的`
- [東京くらしWEB 業務提供誘引販売取引](https://www.shouhiseikatu.metro.tokyo.lg.jp/torihiki/f_tori/tokushohou/t_gyomu.html) `自治体`
- [岡山県消費生活センター 業務提供誘引販売取引](https://www.pref.okayama.jp/site/syohi/mame-advice-gyoumuteikyou.html) `自治体`

## 関連概念
- [[legal-chain-sales|連鎖販売取引（マルチ商法）]] — 語彙が近い
- [[legal-cooling-off|クーリング・オフ（特定商取引法）]] — 語彙が近い
- [[bizlaw-specified-commercial-transactions|特定商取引法]] — 語彙が近い
- [[legal-document-electronic|特定商取引法の契約書面等の電子化（2023年6月施行）]] — 語彙が近い
- [[legal-email-ad-optin|特定商取引法における電子メール広告のオプトイン規制]] — 語彙が近い
- [[legal-individual-credit|個別信用購入あっせん（個別クレジット）に関する割賦販売法の規制]] — 出典を共有
- [[legal-mailorder-return|通信販売の返品ルール（特定商取引法）]] — 出典を共有
- [[subsidy-okayama-pref|中小企業等海外展開支援事業費補助金（海外出願支援／岡山県）]] — 出典を共有

## 関連
- コレクション: [[法務・税務・労務]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
