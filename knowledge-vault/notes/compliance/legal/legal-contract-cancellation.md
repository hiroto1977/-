---
collection: compliance
id: legal-contract-cancellation
category: "legal"
category_ja: "法務"
title: "契約の解除（催告解除・無催告解除／改正民法）"
as_of: "2026-06"
source_count: 3
authoritative: true
tags:
  - collection/compliance
  - compliance/legal
  - knowledge/verified
aliases:
  - "契約の解除（催告解除・無催告解除／改正民法）"
---

# 契約の解除（催告解除・無催告解除／改正民法）

> [!info] コレクション: [[法務・税務・労務]] ・ 区分: 法務 ・ asOf: 2026-06 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
2020年4月1日施行の改正民法により、当事者の一方が債務を履行しないときは、相手方が相当の期間を定めて履行を催告し期間内に履行がなければ契約を解除でき（催告解除・民法541条、ただし不履行が軽微なときを除く）、履行不能や明確な履行拒絶等の一定の場合には催告なしに直ちに解除できる（無催告解除・542条）。解除に債務者の帰責事由は不要となった一方、債務不履行が債権者の責めに帰すべき事由によるときは解除できない（543条）。

## 所管・根拠
所管: 法務省（民法）

## 出典
- [法務省 民法（債権法）改正について](https://www.moj.go.jp/MINJI/minji06_001070000.html) `公的`
- [e-Gov法令検索 民法（541条・542条・543条）](https://laws.e-gov.go.jp/law/129AC0000000089) `公的`
- [契約解除と帰責事由の関係 解説](https://www.businesslawyers.jp/practices/1175) `メディア`

## 関連
- コレクション: [[法務・税務・労務]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
