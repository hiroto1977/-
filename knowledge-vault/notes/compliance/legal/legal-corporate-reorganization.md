---
collection: compliance
id: legal-corporate-reorganization
category: "legal"
category_ja: "法務"
title: "会社法における組織再編（合併・会社分割・株式交換・株式移転・株式交付）"
as_of: "2026-06"
source_count: 3
authoritative: true
tags:
  - collection/compliance
  - compliance/legal
  - knowledge/verified
aliases:
  - "会社法における組織再編（合併・会社分割・株式交換・株式移転・株式交付）"
---

# 会社法における組織再編（合併・会社分割・株式交換・株式移転・株式交付）

> [!info] コレクション: [[法務・税務・労務]] ・ 区分: 法務 ・ asOf: 2026-06 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
会社法は組織再編行為として、合併（吸収合併・新設合併）、会社分割（吸収分割・新設分割）、株式交換、株式移転、及び令和元年改正で創設された株式交付を定めている。これらを行うには原則として組織再編契約・計画の作成、株主総会の特別決議による承認のほか、反対株主の株式買取請求や債権者異議手続（債権者保護手続）等の所定の手続を経る必要がある。

## 所管・根拠
所管: 法務省（会社法）

## 出典
- [e-Gov法令検索 会社法（第五編 組織再編）](https://laws.e-gov.go.jp/law/417AC0000000086) `公的`
- [法務省 会社法改正（株式交付制度の創設）](https://www.moj.go.jp/MINJI/minji07_00001.html) `公的`
- [組織再編の会社形態一覧 解説](https://shiodome.co.jp/js/blog/7478) `メディア`

## 関連
- コレクション: [[法務・税務・労務]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
