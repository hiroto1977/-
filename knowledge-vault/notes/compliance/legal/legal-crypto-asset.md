---
collection: compliance
id: legal-crypto-asset
category: "legal"
category_ja: "法務"
title: "暗号資産交換業の登録制（資金決済法）"
as_of: "2026-06"
source_count: 3
authoritative: true
tags:
  - collection/compliance
  - compliance/legal
  - knowledge/verified
aliases:
  - "暗号資産交換業の登録制（資金決済法）"
---

# 暗号資産交換業の登録制（資金決済法）

> [!info] コレクション: [[法務・税務・労務]] ・ 区分: 法務 ・ asOf: 2026-06 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
暗号資産の売買・交換やその媒介、利用者の金銭・暗号資産の管理等を業として行う「暗号資産交換業」を営むには、資金決済に関する法律に基づき内閣総理大臣（金融庁）の登録を受ける必要がある。登録業者には利用者財産の分別管理、情報の安全管理、利用者への情報提供等の義務が課される。

## 所管・根拠
所管: 金融庁（資金決済に関する法律）

## 出典
- [金融庁 暗号資産関連事業を行うみなさまへ](https://www.fsa.go.jp/policy/virtual_currency/index_2.html) `公的`
- [e-Gov法令検索 資金決済に関する法律](https://laws.e-gov.go.jp/law/421AC0000000059) `公的`
- [暗号資産交換業の登録 解説](https://www.businesslawyers.jp/articles/788) `メディア`

## 関連概念
- [[legal-money-lending|貸金業法（登録制と総量規制）]] — 語彙が近い
- [[legal-my-number|マイナンバー（特定個人情報）の取扱い（番号法）]] — 同分野の近傍

## 関連
- コレクション: [[法務・税務・労務]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
