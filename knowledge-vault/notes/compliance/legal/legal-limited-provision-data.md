---
collection: compliance
id: legal-limited-provision-data
category: "legal"
category_ja: "法務"
title: "不正競争防止法における「限定提供データ」の保護"
as_of: "2026-06"
source_count: 3
authoritative: true
tags:
  - collection/compliance
  - compliance/legal
  - knowledge/verified
aliases:
  - "不正競争防止法における「限定提供データ」の保護"
---

# 不正競争防止法における「限定提供データ」の保護

> [!info] コレクション: [[法務・税務・労務]] ・ 区分: 法務 ・ asOf: 2026-06 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
2018年改正不正競争防止法（2019年7月1日施行）により、「限定提供データ」（業として特定の者に提供する情報として電磁的方法により相当量蓄積され、かつ電磁的方法により管理されている技術上又は営業上の情報。秘密として管理される営業秘密を除く）の不正な取得・使用・開示等が不正競争として規制対象に追加され、ビッグデータ等の事業者間で共有・取引されるデータの保護を目的とする。これらに対しては差止請求・損害賠償請求が認められる。

## 所管・根拠
所管: 経済産業省（不正競争防止法）

## 出典
- [経済産業省 限定提供データと利活用](https://www.meti.go.jp/policy/economy/chizai/chiteki/data.html) `公的`
- [経済産業省 限定提供データに関する指針](https://www.meti.go.jp/policy/economy/chizai/chiteki/guideline/h31pd.pdf) `公的`
- [限定提供データ 改正不正競争防止法 解説](https://xtrend.nikkei.com/atcl/contents/skillup/00009/00041/) `メディア`

## 関連
- コレクション: [[法務・税務・労務]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
