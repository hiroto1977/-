---
collection: compliance
id: legal-statutory-interest
category: "legal"
category_ja: "法務"
title: "民法の法定利率（変動制）"
as_of: "2026-06"
source_count: 3
authoritative: true
tags:
  - collection/compliance
  - compliance/legal
  - knowledge/verified
aliases:
  - "民法の法定利率（変動制）"
---

# 民法の法定利率（変動制）

> [!info] コレクション: [[法務・税務・労務]] ・ 区分: 法務 ・ asOf: 2026-06 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
2020年4月1日施行の改正民法により、法定利率は当初年3%とされ、その後3年ごとに市場金利の動向に応じて自動的に見直される変動制が導入された（民法404条）。改正前は年5%の固定であり、約定利率の定めがない場合の遅延損害金等の計算に用いられる（具体的な利率は3年ごとに変動し得る）。

## 所管・根拠
所管: 法務省（民法）

## 出典
- [法務省 民法（債権法）改正について](https://www.moj.go.jp/MINJI/minji06_001070000.html) `公的`
- [e-Gov法令検索 民法（404条 法定利率）](https://laws.e-gov.go.jp/law/129AC0000000089) `公的`
- [法務省 令和8年4月1日以降の法定利率について](https://www.moj.go.jp/MINJI/minji07_00366.html) `公的`

## 関連概念
- [[legal-risk-bearing|危険負担（改正民法536条）]] — 同分野の近傍
- [[legal-prescription|債権の消滅時効（改正民法・5年/10年ルール）]] — 同分野の近傍
- [[legal-interest-limit|利息制限法の上限金利]] — 同分野の近傍

## 関連
- コレクション: [[法務・税務・労務]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
