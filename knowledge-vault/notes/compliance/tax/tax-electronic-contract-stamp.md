---
collection: compliance
id: tax-electronic-contract-stamp
category: "tax"
category_ja: "税務"
title: "電子契約（電磁的記録）と印紙税の非課税"
as_of: "2026-06"
source_count: 3
authoritative: true
tags:
  - collection/compliance
  - compliance/tax
  - knowledge/verified
aliases:
  - "電子契約（電磁的記録）と印紙税の非課税"
---

# 電子契約（電磁的記録）と印紙税の非課税

> [!info] コレクション: [[法務・税務・労務]] ・ 区分: 税務 ・ asOf: 2026-06 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
印紙税は課税物件表に掲げる「文書」の作成に対して課される税であり、課税文書を電磁的記録（電子契約・PDF等）で作成し電子的に交付・保存する場合は、文書の「作成」に当たらないため印紙税は課されないと解されている（国税庁の質疑応答事例および平成17年の政府答弁書による）。

## 所管・根拠
所管: 国税庁（印紙税法）

## 出典
- [国税庁 質疑応答事例 電磁的記録と印紙税](https://www.nta.go.jp/law/shitsugi/inshi/02/10.htm) `公的`
- [参議院 印紙税に関する答弁書（平成17年）](https://www.sangiin.go.jp/japanese/joho1/kousei/syuisyo/162/touh/t162009.htm) `公的`
- [福岡国税局 注文請書を電子メール送信した場合の印紙税](https://www.nta.go.jp/about/organization/fukuoka/bunshokaito/inshi_sonota/081024/01.htm) `公的`

## 関連
- コレクション: [[法務・税務・労務]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
