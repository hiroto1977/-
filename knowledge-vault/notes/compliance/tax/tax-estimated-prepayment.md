---
collection: compliance
id: tax-estimated-prepayment
category: "tax"
category_ja: "税務"
title: "所得税の予定納税"
as_of: "2026-06"
source_count: 3
authoritative: true
tags:
  - collection/compliance
  - compliance/tax
  - knowledge/verified
aliases:
  - "所得税の予定納税"
---

# 所得税の予定納税

> [!info] コレクション: [[法務・税務・労務]] ・ 区分: 税務 ・ asOf: 2026-06 ・ 出典: 3件（うち権威ある出典 ✓）

## 概要
その年の前年分の所得金額や税額などを基に計算した「予定納税基準額」が15万円以上である場合、納税者はその年の所得税及び復興特別所得税の一部をあらかじめ納付する（予定納税）。原則として予定納税基準額の3分の1ずつを第1期分（7月）と第2期分（11月）に納付する。

## 所管・根拠
所管: 国税庁（所得税法）

## 出典
- [国税庁 No.2040 予定納税](https://www.nta.go.jp/taxes/shiraberu/taxanswer/shotoku/2040.htm) `公的`
- [国税庁 予定納税（第1期分）](https://www.nta.go.jp/publication/pamph/koho/campaign/r7/Jul/02.htm) `公的`
- [国税庁 予定納税（第2期分）](https://www.nta.go.jp/publication/pamph/koho/campaign/r7/Nov/02.htm) `公的`

## 関連概念
- [[tax-income-tax-progressive|所得税の超過累進税率]] — 語彙が近い
- [[tax-withholding-payment-special|源泉所得税の納期の特例]] — 語彙が近い
- [[tax-listed-securities|上場株式等の譲渡益・配当に対する課税]] — 語彙が近い
- [[tax-withholding-nonresident|非居住者・外国法人への源泉徴収]] — 語彙が近い
- [[tax-real-estate-capital-gains|土地・建物等の譲渡所得の課税]] — 語彙が近い
- [[tax-tax-audit|税務調査（質問検査権と事前通知）]] — 同分野の近傍
- [[tax-filing-deadline|確定申告の期限（所得税・個人事業者の消費税）]] — 同分野の近傍

## 関連
- コレクション: [[法務・税務・労務]]
- ヴォルト入口: [[Home]]
- オーケストレーション連携: [[AI_ORCHESTRATION_CONTEXT]]

---
*このノートはリポジトリの確証済み知識データ（`src/renderer/data/*Knowledge.ts` ほか）から `npm run vault:build` で自動生成されています。直接編集しないでください（編集は本体データ側に行い再生成する）。*
