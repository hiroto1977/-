 ...
 *
 * **入力した発話が端末から出ないことを、頁に書く。**
 *
 * この頁は自由入力を 1 つ持ち、placeholder は「発話を入力して合議にかける
 * (例: 全部壊したい)」である。突き合わせる `CRISIS_MARKERS` には
 * 「死にたい」「消えたい」等が並ぶ —— つまり**利用者が自分について本当のことを
 * 打ちうる面**である。
 *
 * 同じ性質の入力を持つ `counseling-demo` は「データは端末内のみ・送信なし」と
 * 書いているのに、**こちらには無かった** (2026-08-25)。実測では 3 つの demo とも
 * 読み込み時も操作後も外部通信 **0 件**なので、書いていないだけで挙動は同じ。
 * **黙っていると、書いてある頁との差が「こちらは送っている」と読める。**
 *
 * (打った人へ相談先を出すべきかは別の判断で、ここでは決めない ——
 *  この頁は検知の精度を見るためのもので、応答を返す頁ではない。)
 *
...
<header>
  ...
  <span class="sub">検知役/安全監査役/レビュー役/合議役 が実エンジンの判定を合議し、精度を測ります。安全に直結する検知ルールは自動変更せず、改善は人のレビュー(PR)を通します。<strong>入力した発話は端末内だけで判定し、送信しません。</strong></span>
</header>
<div id="feed"></div>
<footer>
