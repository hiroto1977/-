...
const TEMPLATES = [
  ...
  },
  {
    ...
    note: [
      ...
      '適用範囲は資本金基準に加えて従業員数基準（製造委託等300人以下・情報成果物/役務100人以下）が併用になり、対象が拡大しました。代金の支払期日は給付受領日から60日以内で、手形による支払は禁止（銀行振込等の現金払いに）。フリーランスへの発注もフリーランス新法により同様の60日ルール・条件明示義務が適用されます（2025年以降、勧告・一斉指導など執行が強化されています）。',
    ],
  },
  {
    id: 'invoice',
  ...
  },
  {
    ...
    note: [
      ...
      '「適格簡易請求書」（宛名省略可・税率か消費税額のいずれかで足りる）を交付できるのは小売業・飲食店業・タクシー業等の不特定多数向け事業に限られます。対象外の業種では、宛名（交付を受ける事業者名）と税率・消費税額を記載した通常の適格請求書の要件を満たす必要があります。',
    ],
  },
  {
    id: 'sokai',
