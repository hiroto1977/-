...
const STEPS = {
  kk: [
    ...
    ['④ 公証役場で電子認証', '株式会社の定款は公証人の認証が必要です（会社法30条）。登記・供託オンライン申請システムから認証を嘱託し、ウェブ会議（テレビ電話）での認証にも対応しています。認証手数料は資本金の額等に応じて定まるため、管轄の公証役場で確認してください。'],
    ['⑤ 出資の払込みと設立登記', '発起人が出資を払い込み、本店所在地を管轄する法務局へ設立登記を申請します。会社は設立登記により成立します（会社法49条）。'],
  ...
  gk: [
    ...
    ['③ 公証人の認証は不要', '合同会社の定款は公証人認証が不要です。電子定款なら印紙代 4 万円もかからず、定款費用は実質 0 円になります。'],
    ['④ 出資の払込みと設立登記', '出資を払い込み、法務局へ設立登記を申請します（登録免許税は最低 6 万円）。会社は設立登記により成立します（会社法49条）。'],
...
// 根拠（検証済み知識ベース由来・画面のみ）
const NOTES = [
  '電子定款が選ばれる最大の理由: 印紙税は「紙の課税文書」のみに課され、電磁的記録（電子文書）には課されません。紙の定款に必要な収入印紙 4 万円が、電子定款では不要になります。',
...
function buildKk() {
  var ms = members();
  var hokkininBody = ms.map(function (m) {
    return '発起人 ' + (m.name || '【発起人1 氏名】') + '（住所: ' + (m.addr || '【発起人1 住所】') + '）は、設立時発行株式 ' + (m.kabu || '【引受株数・出資額】') + ' を引き受ける。';
  });
  return [
