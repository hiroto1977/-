 ...
 *
 * 成果物を相手にする道具は 5 つある (`e2e` (= `e2e:lite`) / `e2e:ollama` / `perf` /
 * `smoke` / `smoke:app`)。**判定はここ 1 つ**に置く —— 5 か所へ書き写すと、比べているのが
 * 写しになる。母集団は `artifactFreshness.test.ts` が package.json の `e2e*` / `perf*` /
 * `smoke*` から導く (2026-09-17 パス 304 まで検査は 3 本を手で並べており、
 * `e2e:ollama` と `smoke:app` が鮮度を見ていないことに誰も気づかなかった)。
 *
