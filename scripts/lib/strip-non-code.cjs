 ...
 * - **`://` は行注記を始めない** (2026-09-25 · パス 464) —— この走査器は **JS の
 *   字句解析器**で、JSX の素のテキストは JS ではない。`<p>docs: http://x</p>{…}` の
 *   `//` を行注記として読み、**その行の後ろを丸ごと落としていた**。実測: 同じ
 *   禁止対象を素の行に置くと `lint:forbidden` は鳴り、同じ行の JSX テキストに
 *   URL を 1 つ足すと **1 件も鳴らない** —— 規約が URL 1 つで迂回できた。
 *   影響は `src` + `scripts` の 1,537 本のうち **3 本 (すべて `.tsx`)** だけ。
 *   ★ **JSX はまだ解析しない** —— JSX テキストの `'` や `<code>/api/x</code>` は
 *   今も読み違えうる (2026-09-25 の出荷 `src` では実測 0 行)。直すなら本当に
 *   JSX を解析することになり、読み違えれば**コードを飲む**側 —— 今より悪い向き ——
 *   へ倒れうるので、要る消費者が現れるまで広げない。
