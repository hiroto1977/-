...
const KNOWN_UNMEASURED = {
  ...
  'src/renderer/web-shim.ts':
    ...
    + ' **実測 (2026-09-01): 有効 1,497 / Killed 583 / 生存 625 / 未到達 289 ='
    + ' 総合 38.94% / 覆われた分 48.26%。** 10 日前 (総合 8.34% / 未到達 1,178) から'
    + '未到達は 83% → 19% へ減った (資格情報の門を 3 状態で留める検査を 25 本足した分を含む)。'
    + '**据え置く理由は「載せられない」ではなく、'
    + '生存 625 のまま載せると週次 CI が恒常的に赤になる、の 1 点である。**'
    ...
    + ' 未到達 149 = 総合 69.54% / 覆われた分 77.23%。** 同日の起点は 58.69% / 未到達 266。',
};
