...
const VERDICTS = {
  ...
  apiResponse:
    '対称 (実測・2026-09-26 パス 482 で数え直した) —— このモジュールが母集団に入ったのは、'
    + '`apiNumberOf` (第三者が文字列で返す数の読み手) を足して「否定で答えられる」'
    + '述語が増えたため (パス 416)。**両ビルドの食い違いは無い** —— ただし'
    + '**2026-09-23 に書いた理由は 2 か所で古びていた**: 「renderer がこのモジュールから直接読むのは '
    + '`parseJsonText` と `displayField` の 2 種類だけ」「`null` を返す読み手に両ビルドが届く道は '
    + '`shared/api/*.ts` ただ 1 つ」と書いたが、パス 442 / 443 から renderer は '
    + '**`finiteNumberOf` も直接読む** (保管した販売記録と KPI 実績の金額を読む漏斗)。'
    + '**それでも割れないのは、問うている値が別だから**である —— renderer の `finiteNumberOf` が読むのは '
    + '**renderer 自身の保管層** (IndexedDB の販売記録・KPI 実績。main はこの保管層を読まない) の値で、'
    + '「no」のあとの動作 (行を落として件数を述べる) は renderer にしか無い。'
    + 'main 側の読み手は `src/main/clients/` の第三者の応答を読む main 専用の経路で、'
    + '**ブラウザ版はそれらのクライアントを 1 行も読み込まない** (パス 262 / 412 で実測)。'
    + '**同じ値を両ビルドが問う形**は `shared/api/*.ts` (cursor ほか) を通る物だけで、'
    + 'そこは実装が 1 つなので「no のあとの動作」も 1 つしかない。'
    + '`parseJsonText` は `null` を返さず**投げる**・`displayField` は `null` を返さない '
    + '(非文字列は空文字・長すぎる値は天井 + `…`) ので、この 2 つは「no」と言う枝そのものを持たない。'
    + '★ **読み手はここに並べない —— 名前だけでなく種類も** —— 2026-09-23 の文は名前を並べるのをやめて'
    + '「2 種類だけ」と**種類**を並べたが、種類が 1 つ増えた日に同じく偽になった。'
    + '数は左の列が数え、この欄は**なぜ対称なのか** (誰の値を問うているか) だけを述べる。'
