...
// The sparkline is a purely visual artifact in the exported HTML —
// boundary mutations on `< 2`, `|| 1`, `>= 0` color flip etc change
// the color or skip rendering but don't affect any user-observable
// correctness contract (the HTML still validates and renders).
// Stryker disable ConditionalExpression,EqualityOperator,LogicalOperator,MethodExpression,UnaryOperator,ArrowFunction,BlockStatement
...
// Test seam: deps.fetchSnapshot / deps.writeFile / deps.now defaults
// are picked up by the production action wrapper. The `?? fetchStocksSnapshot`
// LogicalOperator mutant only matters when deps.fetchSnapshot is undefined
// — the production action takes that path. Tests inject the dep, so the
// fallback expression is exercised only by the production wrapper.
...
// Production wrapper: delegates to exportDashboardImpl with default
// deps (real fs.writeFile / Date.now / fetchStocksSnapshot). Tests
// exercise exportDashboardImpl directly with injected deps. This
// wrapper is a trivial pass-through; mutating the body to `{}` would
// only manifest when ACTIONS["export-dashboard"] is invoked with no
