 ...
 *
 * 母集団を数えた (2026-09-12): renderer の `.tsx` に `maxLength` は **59 件**、
 * そのうち**字面の数**を持つのは **12 件**。うち 8 件は検証側にも同じ数が別に在り、
 * **1 組は既にずれていた**:
 *
 * | 画面 | 字面 | 検証側 | |
 * | --- | --- | --- | --- |
...
describe('入力欄の天井は名前で持つ (パス 167)', () => {
  ...
  it('走査が死んでいない (maxLength そのものは多数在る)', () => {
    // 2026-09-12 実測 59 件。床は実測の半分より少し下 —— 減ったら気付く。
    expect(findSites(ANY_RE).length).toBeGreaterThanOrEqual(25);
  });
