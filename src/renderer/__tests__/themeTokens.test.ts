 ...
 *   ③ 画面の部品の規則 (紙の節より前) に直書きの色は無い。残るのは半透明の影と光輪だけで、
 *      その一覧を台帳として持つ (両方向)。紙の節 (書類スタジオ・銀行提出書式・印刷) は白い紙なので配色の外。
 *
...
/**
 * 部品の規則に残す半透明 (影・光輪・押した時の色)。どちらの配色でも成り立つので直書きのまま。
 * 数が動けば鳴る —— 新しい直書きは (a) トークンにするか (b) ここに理由を書いて足す。
 */
const REMAINING_UI_LITERALS = [
  '#ffffff', // .sidebar-item.active .icon の文字 (グラデーションの上の白)
  'rgba(120, 90, 150, 0.05)', // .data-list-item の影
  'rgba(120, 90, 150, 0.06)', // button の影
  'rgba(120, 90, 150, 0.18)', // モバイルのドロワーの影
  'rgba(238, 111, 168, 0.15)', // .topbar h1::before の光輪
  'rgba(238, 111, 168, 0.16)', // フォーカスリング ×3 (input / range / sidebar-search)
  'rgba(238, 111, 168, 0.16)',
  'rgba(238, 111, 168, 0.16)',
  'rgba(238, 111, 168, 0.28)', // button.primary の影
  'rgba(238, 111, 168, 0.4)', // range のつまみの影
].sort();
