...
describe('固定回数の待ちへの依存は、綴りではなく振る舞いで測る (パス 369)', () => {
  ...
  it('★ 針が実物の台帳に当たる (標本 — 空の台帳で通っていない)', () => {
    ...
    /*
     * **床を行数に置かない** (2026-09-21 · パス 382)。
     *
     * 台帳は消化するたびに短くなるので、実測の行数に張り付けた床は
     * **直した日に落ちる門**になり、「減らすのが正しい向き」と真逆の圧力を
     * かける。この注記は 2026-09-21 (パス 378) から**そう書いてあった**のに
     * 床は `>= 20` → `>= 10` と実測に張り付いたままで、**2 度**その通りに
     * 落ちた (パス 378 で 25 → 20、パス 382 で 10 → 8)。
     * 「減らすたびに門を書き換える」なら、門は何も守っていない。
     *
     * 床が本当に言いたいのは「**針が実物に当たっている**」で、それは
     * **別の綴りで数えた行数と一致すること**で言える —— こちらは台帳が
     * 何本になっても (0 本になっても) 動かず、`ledgerRows` の正規表現が
     * 実物の形に当たらなくなった日に鳴る。上限の側は
     * `fixedTickAssertionCensus.test.ts` が別に持つ。
     */
    const fileLines = (audit.match(/^\s*file: 'src\//gm) ?? []).length;
    expect(rows.length, '針が実物の台帳を読み落としている').toBe(fileLines);
  });
