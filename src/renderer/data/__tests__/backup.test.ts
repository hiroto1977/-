...
describe('バックアップ画面の文言 (誤った保証を書き戻せないように)', () => {
   ...
   *
   * 否定の先読み `(?!ではありません)` が在るので、この規則は
   * 「当たりすぎ」と「当たらなさすぎ」の両方に外れうる —— 今の文面
   * (`SHA-256 で破損検知（改ざん検知ではありません）`) に当たってはならず、
   * 戻ってきた偽の主張 (`SHA-256 で改ざん検知`) には当たらねばならない。
   * どちらも `it` で主張する (注記は落ちない)。
   */
  const FALSE_TAMPER_CLAIM = /SHA-?256\s*(で|による)?\s*改ざん検知(?!ではありません)/;

  it('★ この規則が空でない (偽の主張に当たり、今の文面には当たらない)', () => {
    expect('SHA-256 で改ざん検知します', '偽の主張を捕まえられない').toMatch(FALSE_TAMPER_CLAIM);
    expect('SHA256による改ざん検知', 'ハイフン無し・助詞違いも捕まえる').toMatch(FALSE_TAMPER_CLAIM);
    expect('SHA-256 で破損検知（改ざん検知ではありません）', '今の文面に当たってしまう').not.toMatch(
      FALSE_TAMPER_CLAIM,
    );
  });

  it('「SHA-256 で改ざん検知」と書いていない', () => {
    expect(PANEL).not.toMatch(FALSE_TAMPER_CLAIM);
  });
});
