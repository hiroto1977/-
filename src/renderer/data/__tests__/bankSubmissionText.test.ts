...
describe('金融機関等提出用の書面 — 全文', () => {
  it('入力が揃った書面 (販売記録・KPI・予算・貸借対照表・会計連携・返済予定)', () => {
    ...
    expect(m.sections).toEqual([
      ...
      // ★ 2026-09-22 (パス 395) まで `caption: null` だった —— 実績が 1 期だと
      //   4 行が `―` になるのに理由が 0 文で、**この全文の見本がその状態を留めていた**。
      { title: '7. 成長性', caption:
        '比べられる期がまだ揃っていないため、前期比売上高成長率・平均成長率（CAGR）・売上トレンドは算定していません（期を追加すると算定します）。'
        + '前年同月の実績が無いため、前年同月比は算定していません。', rows: [
  ...
  it('何も入力していない書面 — 行は残り、断り書きが付く', () => {
    ...
    expect(m.sections).toEqual([
      ...
      // ★ 2026-09-22 (パス 395) まで、この見本は `caption: null` と
      //   `売上高（販売記録）= '0'` / `受注件数 = '0件'` / `販売チャネル数 = '0'` を
      //   固定していた —— **「何も入力していない書面」が §2 で 3 つの数値を刷り、
      //   理由を 1 文も出さない**状態を、この全文の見本が留めていた。
      { title: '2. 販売の状況', caption: '販売記録が未入力のため算定していません。', rows: [
          { label: '売上高（販売記録）', value: '―', note: '販売記録の合計' },
          { label: '受注件数', value: '―', note: '' },
          // 注文 0 件 → **算定不能で ―** (2026-09-08 までこの見本が
          // 「何も入力していない書面」の平均受注単価を `'0円'` で固定していた)。
          { label: '平均受注単価', value: '―', note: '売上高 ÷ 受注件数' },
          { label: '販売チャネル数', value: '―', note: '' },
