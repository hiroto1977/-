...
describe('computeFinancialRatios — round 68 境界ガード (分母 0 / 負 / null)', () => {
  ...
  /**
   * 2026-09-14 (パス 224) に**期待を変えた**。以前は「負値もありうる」として −40% を
   * 留めていたが、これはコードの振る舞いを写した境界の検査で、要求ではなかった。
   *
   * 棚卸資産は流動資産の**内数**なので、棚卸 > 流動資産 の控えは内部矛盾している ——
   * 当座比率が小さい企業ではなく、**当座比率が定まらない控え**である (当座資産は
   * 現預金 + 売上債権 + 有価証券で、負にならない)。実測ではこの値が金融機関へ渡す
   * 書面 §4 に「当座比率 −800%」として刷られていた。算定不能は null (このファイルの
   * 他の指標と同じ約束)。書き手と復元の 2 つの門で断つようになったが、**門より前に
   * 保存された控えは残りうる**ので計算の側でも受ける。
   *
   * 隣の 現金比率 の 0 クランプはそのまま —— あちらは「概算現預金 = 流動資産 − 債権 −
   * 棚卸」という**導出した推定**で、台帳の欄ではない。
   */
  it('当座比率: 棚卸が流動資産を超える控えは算定不能 (内訳が親項目を超えている)', () => {
    const r2 = computeFinancialRatios({ ...SAMPLE, inventory: 6000 }); // 5000 < 6000
    expect(r2.quickRatioPct).toBeNull();
  });

  it('当座比率: 棚卸 == 流動資産 は算定する (境界の反対側・対照)', () => {
    const edge = computeFinancialRatios({ ...SAMPLE, inventory: 5000 }); // (5000 − 5000)/2500
    expect(edge.quickRatioPct).toBe(0);
  });
});
