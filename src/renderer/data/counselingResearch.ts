 ...
 * する**ことをテストで不変条件化する (全ペルソナを回して
 * `crisisReferrals === crisisSessions` を床つきで見る)。
 * 検知・応答ルールの変更は人のレビュー (PR) を通す。
 *
 * **`crisis` のラベルは自己申告なので、そこも検査する** (2026-09-06)。
 * `crisisReferred` は `persona.crisis ? referred : null` なので、危機語を含む発話を
 * 持つのに `crisis: false` と書いたペルソナは `crisisSessions` に数えられず、
 * **不変条件の対象から静かに外れる**。「全件が到達する」を確かめても、
 * **何が全件なのかを表が自分で決めている**限りそこが逃げ道になるため、
 * 「危機語を含むなら `crisis: true`」を検査で留めてある。ペルソナを足すときは、
 * 発話が `detectCrisis` に当たるかを見てラベルを付けること。
 *
