 ...
 *
 * ## 照合は**両方向**でなければ意味がない (2026-09-06 に片方向だったのを直した)
 *
 * ここが以前「テストで『全窓口が確証済み』を不変条件化する」と書いていたのに対し、
 * 実際の検査は**確証済みの各件が出荷一覧に在るか**しか見ていなかった。
 * 危ないのは逆で、危機応答で見せる `counseling.ts` の `SUPPORT_RESOURCES` に
 * **手打ちの窓口を 1 行足すと、出典が 1 件も無くても検査は全部緑のまま**通った。
 * 番号が古ければ、いま最も助けが要る人が誰にも繋がらない電話を掛ける。
 *
 * いまは `unverifiedSupportResources()` (`sourceVerification.ts`) が
 * **出荷する側から**照合する。窓口を足すなら、ここに出典つきで足すこと
 * (`kind: 'hotline'` は確証必須。`kind: 'emergency'` は 119 / 110 の案内だけで、
 * 自前のダイヤルイン番号を持てない)。
 *
