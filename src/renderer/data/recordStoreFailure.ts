/**
 * **業務レコードの読み書きが断られたことを、画面へ届ける 1 本の経路。**
 *
 ...
 * のと同じ形が、いちばん下の層に在った。
 *
 * ここは判定と文面だけを持つ。**入口 (`useCollection`) が失敗をここへ写し、
 * 画面 (`RecordStoreFailureBanner`) が最後の 1 件を出す** —— 13 か所へ
 * 同じ try/catch を書いて回ると、必ずどれか 1 つが漏れる
...
/** 断られた操作。文面の主語と「今どうなっているか」が変わる。 */
export type RecordStoreOp = 'read' | 'save' | 'delete';

/** 断られた理由。打ち手が変わる 3 通り。 */
export type RecordStoreFailureKind = 'quota' | 'blocked' | 'unknown';

export interface RecordStoreFailure {
  readonly op: RecordStoreOp;
  /** どのコレクションで起きたか (画面には出さない。診断と検査のため)。 */
  readonly collection: string;
  readonly message: string;
}

const HEADING: Record<RecordStoreOp, string> = {
...
const STATE: Record<RecordStoreOp, string> = {
...
const CAUSE: Record<Exclude<RecordStoreFailureKind, 'unknown'>, string> = {
  quota: 'この端末の保存領域が一杯です',
  blocked: 'ブラウザの設定 (プライベートモードなど) で端末への保存が禁じられています',
};

const REMEDY: Record<RecordStoreFailureKind, string> = {
 ...
 */
export function recordStoreFailureKind(err: unknown): RecordStoreFailureKind {
...
/** 画面に出す 1 行。 */
export function recordStoreFailureMessage(op: RecordStoreOp, err: unknown): string {
  const kind = recordStoreFailureKind(err);
  const cause = kind === 'unknown' ? describeStorageError(err) : CAUSE[kind];
  return `${HEADING[op]}（${cause}）。${STATE[op]}${REMEDY[kind]}`;
}
...
let latest: RecordStoreFailure | null = null;
const listeners = new Set<(f: RecordStoreFailure | null) => void>();

/** 最後に断られた 1 件 (画面が出すもの)。 */
export function currentRecordStoreFailure(): RecordStoreFailure | null {
  return latest;
}
 ...
 */
export function reportRecordStoreFailure(op: RecordStoreOp, collection: string, err: unknown): void {
  publish({ op, collection, message: recordStoreFailureMessage(op, err) });
}

/** 画面から閉じたとき。次に失敗すればまた出る。 */
export function clearRecordStoreFailure(): void {
  publish(null);
}

function publish(next: RecordStoreFailure | null): void {
  latest = next;
  for (const fn of listeners) fn(next);
}

/** 画面が購読する。戻り値を呼ぶと解除。 */
export function subscribeRecordStoreFailure(fn: (f: RecordStoreFailure | null) => void): () => void {
  listeners.add(fn);
  return () => {
    listeners.delete(fn);
  };
...
export function _resetRecordStoreFailureForTests(): void {
  latest = null;
  listeners.clear();
}
