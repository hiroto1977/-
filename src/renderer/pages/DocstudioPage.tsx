 ...
 * 利益が反映されていない書面が出来上がる。
 *
 * **「まとめて」は 4 点ちょうど** (2026-09-19・パス 328・依頼「これらを４つに分けて」)。
 * 決算公告の要旨は会社法 440 条の公告で 435 条 2 項の計算書類ではないので、まとめてには入れず
 * **貸借対照表を選んだときだけ**組で出す。パス 323 まで「計算書類（4点まとめて）」が 5 枚出しており、
 * タブの名乗りと実際の枚数が食い違っていた。
 *
...
function KessanSheets({ values, fields, sheet }: { values: Values; fields: readonly DocField[]; sheet: KessanSheet }) {
  ...
  if (sheet === 'bs') {
    /*
     * 決算公告の要旨は**貸借対照表を選んだときだけ**付ける (2026-09-19・パス 328)。
     *
     * これは会社法**440 条**の公告であって、同 435 条 2 項の計算書類 4 点ではない。
     * パス 323 まで `show('bs')` だったので「計算書類（**4点**まとめて）」を選ぶと
     * **5 枚**出ており、**タブの名乗りと出る枚数が食い違っていた** (このリポジトリが
     * 繰り返し直してきた「画面の主張と実態」の形)。まとめては 4 点ちょうどにし、
     * 公告の要旨は貸借対照表と組で出す (要旨の元は貸借対照表なので置き場所はそこが正しい)。
     */
    pages.push({
      id: 'notice',
      kind: 'statutory',
