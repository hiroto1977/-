...
const LEDGER: Readonly<Record<string, { kind: Kind; why: string }>> = {
  ...
  'src/shared/api/microsoft365.ts': { kind: 'measure-next', why: '**資格情報を使う書き込み口**。両ビルドが 1 つの実装を通る (パス 274) のに未測定。' },
  'src/shared/api/security.ts': { kind: 'measure-next', why: '**資格情報を使う書き込み口** (HIBP / VirusTotal)。両ビルドが同じ関数を通る (パス 321)。' },
