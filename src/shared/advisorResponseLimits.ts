...
/**
 * **リスク要因の「件数」は business と同じ 1 つを読む** (2026-09-22 · パス 404)。
 *
 * 上の business の表は 6 行あり、その 1 行が `riskFactors 1..3` である。
 * ところが**この 40 行下の株式の側には、その行が無かった** —— 検証器 (main /
 * ブラウザ版の双子) はどちらも `riskFactors` が空でないことしか見ておらず、
 * 実測 (2026-09-22 · 直す前) で**両ビルドとも 100,000 件を通した**。
 *
 * しかも**アプリ自身が上限を宣言していた** —— 両ビルドの prompt が
 * 「各 recommendation には必ず riskFactors (1-3 件) を含めること。」と書いており、
 * 検めていたのは相手 (LLM) の善意だけである。第三者の応答は
 * 「プロンプト注入・乗っ取られた proxy・悪意ある MCP サーバ」が誘導しうると
 * `assistantLimits.ts` が名指ししている当のものなので、**述べた上限は検める**。
 *
 * なぜ新しい数を作らず `MAX_ADVISOR_RISK_FACTORS` を読むのか —— 2 つの prompt の
 * 文が**同じ 1 文** (「riskFactors (1-3 件)」) で、同じ要求から来ているため。
 * 理由の長さ (600 / 400) が別なのは**表示欄の幅が違う**からで、そちらは別の数で
 * 正しい。**同じ要求は 1 つ、別の要求は別に**。件数を上の表から読むのは、
 * すぐ上の `MAX_ADVISOR_RECOMMENDATIONS` (5) と同じ扱いである。
 */
