 ...
 *
 * ## 4 つ目の呼び手の家系: **保管層から読んだ値** (パス 417)
 *
 * 名前は「API の応答」だが、理由は出どころではなく**宣言の型を誰も読みで確かめていない**
 * ことである。`store.list()` は `cur.value as StoredRecord<T>` で何も検めない
 * (`recordShapeAudit.ts` の設計 —— 読みで落とすと壊れた行が UI から触れなくなる) ので、
 * 復元・古い版・別の道具が入れた行は第三者の応答と**同じ強さの主張**しか持たない。
 * 実測 (2026-09-22 · パス 417): 200,000 字の販売記録のメモ 1 件で売上集計の画面が
 * **200,257 字**・金融機関等提出用の書面 §2 の但し書きが **200,056 字**になった。
 * だから呼び手は 3 家系ではなく **4 家系**である (第三者の応答 / 書き込みの結果 /
 * 利用者が設定したホストの応答 / **保管層から読んだ値**)。
 */
