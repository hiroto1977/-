...
export const LAWS: readonly Law[] = [
  ...
  },
  {
    ...
    provenance: ['パターン 0-a-7', 'パス 467 (読めない台帳が「空」として緑になり、37 ゲート全部が素通りした)', 'パス 468 (母集団を空にして測ると、33 ゲートのうち 5 本が exit 0 だった)', 'パス 469 (床は 0 件にしか当たらない —— .tsx が消えても 5 本が exit 0 だった)', 'パス 470 (git ls-files の 2 本は一部の死を 1 度も測られておらず、フォールバックが詰め直していた)', 'パス 471 (走査が 1% 死んでも木を歩く 6 本すべてが緑 —— 道具の keep が逆数しか表せず、半分より小さい損失は 1 度も測られていなかった)'],
    enforcedBy: [
  ...
  },
  {
    ...
    enforcedBy: [test(T.shared('bareFetchLedger')), test(T.shared('egressRedirectCensus')), test(T.shared('jsonBodyCensus')), test(T.renderer('downloadFilenameCensus')), test(T.renderer('numericInputReaderCensus')), test(T.shared('prereleaseVersionOrder'))],
  },
  {
