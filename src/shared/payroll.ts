...
/**
 * 雇用保険料率の会社負担 (一般の事業, 令和8年度)。
 *
 * 本人 0.5% に対し事業主 0.85% (失業等給付等 0.5% + 雇用保険二事業 0.35%)。
 * 二事業の分は事業主だけが負担するので、本人と同率にはならない。
 * 令和6年度 0.95% → 令和7年度 0.9% → 令和8年度 0.85% と 2 年連続で下がった。
 * 年度は `SOCIAL_INSURANCE_RATE_FISCAL_YEAR` が持つ。
 */
