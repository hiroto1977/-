 ...
 *
 * ブラウザ版は `web-shim.ts` が `tryGrabSvgFromPage()` で画面の SVG をそのまま
 * 出すので、**多角形については**外れていなかった —— デスクトップ版だけが 2 つ目の
 * 実装で外れていた。
 *
 * ### この段落は 2026-09-15 (パス 268) に訂正した
 *
 * ここには当初「ブラウザ版は…既に正しい」と書いていた。**それは誤りだった。**
 * 掻き取れるのは `<svg>` 要素**だけ**で、この節が言う「図の外で名指しする」⚠ の行は
 * その外側の `<div data-skill-radar-omitted>` に在る。標題・部署・評価時点・凡例も同じ。
 * つまりブラウザ版が渡していたのは**名前も日付も断りも無い裸の図**で、
 * 5 人のうち 2 人が未評価なら受け取った人には「3 人のチーム」に見えていた ——
 * 直したはずのパス 41 (断りが画面にだけ乗る) が、もう一方のビルドに残っていた。
 * 組み立ては `shared/teamRadarSvg.ts` へ移し、両ビルドが同じ関数を呼ぶ。
 * **「そのまま」は「全部」ではない** —— 掻き取る範囲を確かめずに正しさを書いた。
 *
 * ## 置き方
 *
