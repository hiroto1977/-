...
// 2 年度分取り残されていた (雇用保険は 2 年連続で引き下げられている)。
// 直したうえで、`lint:rate-freshness` が「宣言した年度から 2 年度以上
// 離れたら落ちる」ようにして、黙って古くならないようにした。

/**
 * ここに並ぶ料率がどの年度のものか。**更新したらここも上げること。**
 *
 * 年度は 4 月始まり。協会けんぽの健康保険・介護保険料率は 3 月分
 * (4 月納付分) から、雇用保険料率は 4 月 1 日以後最初の賃金締日から
 * 改定される。
 */
export const SOCIAL_INSURANCE_RATE_FISCAL_YEAR = 2026;

/** 厚生年金保険料率の本人負担 (18.3% の半分)。2017 年に上限で固定され、以後不変。 */
export const PENSION_RATE = 0.0915;
/** 健康保険料率の本人負担 (全国平均 約10% の半分、40歳未満)。都道府県別なので概算。 */
export const HEALTH_RATE = 0.05;
/** 介護保険料率の本人負担 (40歳以上65歳未満が健康保険に上乗せ)。令和8年度 1.62% の半分。 */
export const CARE_RATE = 0.0081;
/** 雇用保険料率の本人負担 (一般の事業, 令和8年度)。令和6年度 0.6% → 令和7年度 0.55% → 令和8年度 0.5%。 */
