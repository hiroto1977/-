 ...
 * 下限としてどちらの道も色は**入口で検証される** (属性から抜けられない) ——
 * ただし**同じ関数ではない**: main は `isHexColor` で断り (throw)、ブラウザ版と
 * 画面は `safeColor` で既定値へ落とす。2026-09-20 (パス 348) まで、この行は
 * 「どちらの道も `safeColor` を通る」と書いており、`safeColor` の呼び出しは
 * 実際には**緩い側の 2 か所だけ**だった (実測)。守りの強さは変わらないが、
 * 読んだ人は `safeColor` を消せば両方が鳴ると思うことになる。
 */
